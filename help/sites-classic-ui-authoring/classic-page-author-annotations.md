---
title: 编辑页面时的注释
description: 向网站页面添加内容时，通常会先进行讨论，然后再实际发布。 为帮助实现此目的，许多与内容直接相关的组件允许您添加注释。
...
您可以将注释移动到页面上的新位置。 单击顶部边框区域，然后按住，同时将注释拖到新位置。 这可以位于页面上的任何位置，不过以某种方式保持与段落的连接通常是有意义的。

注释（包括相关草图）也包含在对其所附加的段落执行的任何复制、剪切或删除操作中；对于复制或剪切操作，注释（和相关草图）的位置将保持其相对于原始段落的位置。

通过拖动右下角，还可以增加或减少注释的大小。

出于跟踪目的，页脚行将指示创建注释的用户和日期。 后续作者可以编辑相同的注释（将更新页脚），或为同一段落创建新注释。

当您选择删除注释时（删除注释也会删除附加到该注释的任何草图），将请求确认。
...
* 草图会叠加内容。 这意味着，在草绘的4个角内，您无法单击基础段落；例如，如果需要编辑或访问链接。 如果出现此问题（例如，您有一个草图覆盖了页面的大片区域），请最小化相应的注释，因为这样还可以最小化所有相关草图，从而使您有权访问基础区域。
