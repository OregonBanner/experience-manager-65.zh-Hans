...
>[!CAUTION]
>
>因为设计导入程序需要访问 `/apps`，则无法在集装箱云环境中使用， `/apps` 不可变。
...
**潜在客户表单** 添加此组件可向登陆页面添加潜在客户表单。 潜在客户表单会自动包含一个潜在客户表单的开头和潜在客户表单的结尾字段。其间，您还可以添加本节中介绍的潜在客户表单组件。

![chlimage_1-42](assets/chlimage_1-42.png)

潜在客户表单组件使用 **表单开始** 和 **表单结尾** 元素。 它们始终是成对的，以确保正确定义表单。
...
**操作配置** 根据“帖子选项”中的选择，“操作配置”会发生更改。 例如，如果您选择创建潜在客户，则可以配置要将潜在客户添加到哪个列表。
...
由于页面导入涉及 HTML 准备、验证和页面测试，因此导入登陆页面应该是一项管理员任务。作为管理员，执行导入的用户需要具有 `/apps`. 如果用户没有这些权限，则导入将失败。

>[!NOTE]
>
>由于设计导入器旨在作为管理工具，需要具有 `/apps`，则Adobe不建议在生产中使用设计导入程序。
...
如果导入的设计包存在某些问题，例如是关于某些包中并不存在的图像/脚本，或诸如此类的问题，则设计导入程序会以列表形式显示这些问题。在经典用户界面中，要查看问题列表，请单击登陆页面工具栏中的问题链接。在下图中，单击 **问题** 链接会打开“导入问题”窗口。
...
在清除登陆页面时，您可以删除内容更改。如果单击 **否**，则内容更改会保留，即下面的结构 `jcr:content/importer`将保留，并且仅保留导入器页面组件和 `etc/design` 删除。 然而，如果您单击 **是**, `jcr:content/importer` 也会被删除。
