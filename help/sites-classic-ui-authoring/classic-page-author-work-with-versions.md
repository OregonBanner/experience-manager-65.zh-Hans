...
1. 使用日期和时间选择器指定所需的日期/时间，然后单击 **开始**.
   ...
   >[!NOTE]
   >
   >如果浏览到的页面已从存储库删除或删除，则当该页面的旧版本仍然在存储库中时，该页面将正常呈现。
...
在以下示例中，红线表示页面在初始绿色版本的时间范围内进行编辑，黄线表示页面在红色版本期间进行编辑，依此类推。
...
1. 单击:
   ...
   1. 显示该内容时，请使用 **退出时间扭曲** 以退出并返回当前创作页面。
...
* **时间扭曲仅基于页面内容**  — 如果呈现网站的元素（如代码、css和图像资产）发生更改，则视图将与它原来的样子不同，因为这些项目不在存储库中进行版本控制。
