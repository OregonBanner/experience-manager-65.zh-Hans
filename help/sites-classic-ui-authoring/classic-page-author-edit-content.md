---
title: 编辑页面内容
description: 使用可拖动到页面上的组件添加内容。 然后，可以就地编辑、移动或删除这些内容。
...
内容添加方式 [组件](/help/sites-classic-ui-authoring/classic-page-author-default-components.md) （适用于内容类型）的任意页面加载项。 然后，可以就地编辑、移动或删除这些内容。
...
打开页面后，您可以开始添加内容。 可通过添加组件（也称为段落）来完成此操作。
...
1. 有多种方法可选择要插入的段落类型：
   ...
   * 从浮动工具栏（称为sidekick）拖动组件以插入新段落。
   * 右键单击现有段落并选择 **新建……**  — 将打开“插入新组件”工具栏。 选择元件并单击 **确定**.

   ![screen_shot_2012-02-15at115605am](assets/screen_shot_2012-02-15at115605am.png)

1. 在副手和 **插入新组件** 工具栏您将看到可用组件（段落类型）的列表。 它们可以分成多个部分（例如，“常规”、“列”等），这些部分可以根据需要展开。

   根据您的生产环境，这些选项可能有所不同。 有关组件的完整详细信息，请参阅 [默认组件](/help/sites-classic-ui-authoring/classic-page-author-default-components.md).
...
1. 将所需的资产拖动到所需位置。 此 [组件占位符](#componentplaceholder) 显示组件的放置位置。
...
* **双击** 打开该页面的段落。 您会看到与创建包含现有内容的段落时相同的窗口。 进行更改并单击 **确定**.

* **右键单击** 段落并单击 **编辑**.

* **单击** 在段落上单击两次（缓慢双击）以进入就地编辑模式。 您将能够直接编辑页面上的文本，而不是在对话框窗口中编辑。 在此模式下，您将在页面顶部获得一个工具栏。 只需进行更改，更改将自动保存。
...
1. 将段落拖动到新位置 — AEM表示可以将带有绿色复选标记的段落移动到何处。 将其拖放到所需位置。
1. 此时将移动您的段落：
...
1. 选择 **删除** 从菜单中。 AEM WCM要求确认您要删除段落，因为此操作无法撤消。
1. 单击&#x200B;**确定**。

>[!NOTE]
>
>如果您设置了 [用户属性以显示全局编辑工具栏](/help/sites-classic-ui-authoring/author-env-user-props.md) 您还可以使用对段落执行某些操作。 **复制**， **剪切**， **粘贴**， **删除** 按钮可用。
>
...
作为时间 [删除组件](#deleting-a-component) 可以使用上下文菜单复制、剪切和/或粘贴组件
...
* [多站点管理](/help/sites-administering/msm.md)；还将与 [基架](/help/sites-classic-ui-authoring/classic-feature-scaffolding.md#scaffolding-with-msm-inheritance).

* [启动次数](/help/sites-classic-ui-authoring/classic-launches.md) （当基于livecopy时）。
* 特定组件；例如Geometrixx中的继承段落系统。

您可以取消（随后也可以重新启用）继承。根据组件，这可以从以下位置获得：

1. **Live Copy**

   如果某个组件是livecopy或launch的一部分，则会以挂锁图标表示。 您可以单击挂锁取消继承。
...
要更大规模地编辑同一短语，请 **[查找和替换](/help/sites-classic-ui-authoring/author-env-search.md#find-and-replace)** 菜单选项允许您在网站的部分中搜索和替换字符串的多个实例。

## 锁定页面 {#locking-a-page}

AEM允许您锁定页面，这样其他人就无法修改其内容。 当您要对某个特定页面进行大量编辑，或者需要冻结页面一段时间时，此功能非常有用。

>[!CAUTION]
>
>锁定页面应谨慎使用，因为唯一可以解锁页面的人是锁定页面的人（或具有管理员权限的帐户）。

要锁定页面：

1. 在 **网站** 选项卡，选择要锁定的页面。
1. 双击该页面以将其打开以进行编辑。
1. 在 **页面** 按Sidekick选项卡，选择 **锁定页面**：

   ![screen_shot_2012-02-08at15750pm](assets/screen_shot_2012-02-08at15750pm.png)

   将显示一条消息，表明您的页面已被其他用户锁定。 此外，在 **网站** 控制台中，AEM WCM将页面显示为已锁定，并指示哪个用户已锁定该页面。
...
当您撤消或重做一个或多个段落的删除、添加或重新定位时，闪烁（默认行为）的突出显示指示受影响的段落。
...
AEM存储您执行的操作的历史记录以及执行操作的顺序。 因此，您可以按照执行顺序撤消多个操作。 然后，可以使用重做重新应用一个或多个操作。

如果选择了内容页面上的某个元素，则撤消和重做命令将应用于所选项目，例如文本组件。

撤消和重做命令的行为与其他软件程序中的类似。 在您对内容做出决策时，可使用命令恢复网页的最近状态。 例如，如果您将文本段落移至页面上的其他位置，可以使用撤消命令移回该段落。如果您随后再次决定移动段落，请使用“重做”命令。

>[!NOTE]
>
>您可以：
>
>* 只要自使用“撤消”操作后尚未对页面进行编辑，就可以重做操作。
>* 撤消最多20个编辑操作（默认设置）。
...
>[!NOTE]
>
>对文件和图像的更改执行撤消和重做操作需要特殊的权限。此外，还原文件和图像更改的历史记录至少会持续几小时。 然而，在这段时间之后，这些变化的撤销并非板上钉钉。 您的管理员可以提供权限并更改十小时的默认时间。
