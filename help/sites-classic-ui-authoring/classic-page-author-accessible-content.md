...
>[!CAUTION]
>
>由于经典UI在AEM 6.4中已弃用，因此尚未针对WCAG 2.1更新此页面上的内容。
>
...
这些指南根据三个一致性级别进行分级：A级（最低）、AA级和AAA级（最高）。 简言之，级别定义如下：

* **级别 A：**&#x200B;您的站点满足基本的、最低级别的无障碍性。要达到此级别，需满足所有级别 A 成功标准。
* **AA级：** 这是您努力实现的理想辅助功能级别，站点的辅助功能级别更高，因此多数情况下大部分人都可以使用大部分技术访问站点。 为达到此级别，所有 A 级和 AA 级成功标准均已满足。
...
>[!NOTE]
>
>由于某些类型的内容不可能满足所有AAA级成功标准，因此建议不要将此级符合作为常规策略来要求。
...
网页上的信息可以以许多不同的非文本格式提供，例如图片、视频、动画、图表和图形。失明或严重视力障碍的人无法看到非文本内容，但他们可以通过屏幕阅读器阅读文本内容或通过盲文显示设备以触觉形式呈现文本内容。 因此，通过以图形格式提供内容的替代文本，看不到图形内容的人可以访问内容提供的信息的等效版本。

另一个有用的好处是，文本替代项使非文本内容能够通过搜索引擎技术编制索引。

#### 如何达到标准 — 非文本内容(1.1.1) {#how-to-meet-non-text-content}

对于静态图形，基本的要求是为图形提供对等的替换文本。此方法可在 **替换文字** 字段：
...
此 **替换文字** 字段位于 **高级** 的图像属性选项卡 **图像** 组件对话框：

![经典UI中图像组件的“编辑”对话框；显示“替换文本”字段。](assets/chlimage_1-17a.png)

AEM添加 **替换文字** 默认添加到您的图像。 对于经典UI，创建默认属性的方式有两种不同的方案，但默认值可能不足以作为替代值，并且可能必须在中进行编辑 **高级** “图像属性”选项卡：

* 文件:

从用户的硬盘上传映像。 如果将图像组件添加到页面，然后从硬盘或其他源（的默认值）中选择图像 **替换文字** 是 `file`. 此值必须在 **高级** “图像属性”选项卡。 同样，此值不会显示在 **替换文字** 字段，但是当值更改时，新值显示在字段中。

* 资产:

从数字资产存储库中添加图像。 如果将图像从数字资产存储库拖到网页中，则 **标题** 和 **替换文字** 该图像的值获取自该图像的元数据。

>[!NOTE]
>
>在上述两种情况下，默认 **替换文字** 值在 **高级图像属性** 选项卡。 要更改默认值，只需在 **替换文字** 字段。

>[!NOTE]
>
>如果您的图像仅起装饰作用(请参阅 [创建有效的替换文本](#creating-good-text-alternatives))，则可在 **替换文字** 使用空格键的字段。 这样做会创建一个空的 `alt` 属性，提示屏幕阅读器忽略图像。

#### 创建有效的替换文本 {#creating-good-text-alternatives}

非文本内容有多种形式，因此替换文本的值取决于图形在网页中所扮演的角色。 一些一般经验规则包括：
...
此 [W3C草案：HTML5用于提供实用替代文本的技巧](https://html.spec.whatwg.org/multipage/images.html#alt) 提供了针对不同类型图像的适当替换文本设置的更多详细信息和示例。

以下特定类型的非文本内容可能需要替换文本：

* 说明性照片：

这些是人、对象或地点的图像。 想一想照片在页面中的作用；相应的对等文本可能为 *照片 [对象]*，但可能取决于周围的文本。

* 图标：

传递特定信息的小图形符号（图形）。 必须在页面和站点中始终如一地使用它们。 图标在页面或网站上的所有实例都应使用相同的简短替换文本，除非这样做会与相邻的文本产生不必要的重复。

* 图表和图形：

这些通常表示数值数据。 因此，提供替换文本的一个选项可能是包含图表或图形中显示的主要趋势的简短摘要。 如有必要，还可以在文本中使用 **描述** 中的字段 **高级** “图像属性”选项卡。 此外，还可以在页面或站点的其他位置以表形式提供源数据。
...
在AEM中，您可以使用 **替换文字** 和 **描述** 图像配置对话框中的字段 — 如所示 [如何达到标准 — 非文本内容(1.1.1)](#how-to-meet-non-text-content).

* 映射、图表、流程图：

用于提供空间数据的图形(例如， 要支持描述对象或进程之间的关系)，请确保以文本格式提供关键消息。对于映射，提供等效完整文本可能不太现实，但如果提供映射来作为帮助相关人员找到特定位置的一种方法，则映射图像的替换文本可以简短地指示为 *X 映射*，然后在页面其他位置的文本中或者通过&#x200B;**图像**&#x200B;组件&#x200B;**高级**&#x200B;选项卡中的&#x200B;**描述**&#x200B;字段提供该特定位置的说明。

* 验证码：

验证码是 *全自动公共图灵测试区分计算机和人类*. 它是一种在网页上使用的安全检查，用于区分人类和恶意软件，但可能会导致访问障碍。 这些图像要求用户描述他们看到的内容，以便通过安全测试。 为这些图像提供替代文本是不可能的，因此必须思考出非图形的替代解决方案。
...
>[!NOTE]
>
>背景和前景文本之间应具有适当级别的对比度。 此对比将在以下部分中详细讨论 [对比度（最小）(1.4.3)](#contrast-minimum).
...
* 纯音频和纯视频（预先录制）：对于预先录制的纯音频媒体和预先录制的纯视频媒体，均存在以下情况，除非音频或视频就是文本的替代媒体 ，且明确进行了如下标记：
   ...
   * 纯视频预录制：提供基于时间的媒体的替代方案或音轨，该音轨为纯视频预录内容提供等效信息。
...
* 如果内容是预先录制的不含视频的音频（如播客）：
   ...
   * 在紧靠内容之前或之后提供指向音频内容的文本转录的链接。
   ...
   成绩单应该是一个HTML页，其中包含所有口语和重要的非口语内容的等效文本。 它还应该标明讲话者、讲话背景、声音表情和任何其他重要音频的描述。
...
>[!NOTE]
>
>如果音频或视频内容是作为网页上以其他格式存在的内容的替代内容提供的，则无需遵循上述要求。 例如，如果视频演示了文本指令列表，则此视频不需要替代文本，因为文本指令已用作该视频的替代文本。

在AEM网页中插入多媒体(尤其是Flash内容)与插入图像类似。 但是，由于多媒体内容远不止是静态图像，因此对于控制如何播放多媒体，有各种不同的设置和选项。
...
耳聋或听力缺佳的人无法或很难访问音频内容。 字幕是口语和非口语音频的对等文本，在视频播放期间的适当时间显示在屏幕上。 它们允许听不到音频的人了解正在发生的事情。

>[!NOTE]
>
>如果视频或动画所在的页面上提供了合适的文本或非文本等效项（直接提供等效信息），则不需要使用字幕。
...
尽可能使用隐藏式字幕。 它让用户可以选择是否查看字幕。

对于隐藏式字幕，请创建并提供相应格式的同步字幕文件，例如 [SMIL](https://www.w3.org/AudioVideo/)，以及视频文件。

请参阅中的教程 [更多信息 — 字幕（预先录制）(1.2.2)](#more-information-captions-pre-recorded). 确保提供注释，让用户知道视频有字幕可用。

如果必须使用打开字幕，请将文本嵌入到视频字幕中。 此方法可使用视频编辑应用程序实现，该应用程序允许在视频上叠加标题。
...
1. 包括视频内容的附加音频描述。 您可以通过以下三种方式之一实现此方法：
   ...
   * 创建视频内容的第二版本以允许扩展的音频描述。 这样，通过在适当的时间暂时暂停音频和视频，减少了在现有对话之间的间隙中提供详细音频描述方面的困难。 因此，在操作再次开始之前，可以给出更长的音频描述。 与上一个示例一样，最好将其作为可选的额外音频轨道提供，以防止对不需要额外描述的用户造成中断。


1. 提供文本转录，该文本转录是视频或动画的音频和视觉元素的合适等效文本。 在适当情况下，应包括讲话者的指示、讲话背景、声音表述的说明。 根据文本长度的不同，既可以将记录放置在视频或动画所在的页面上，也可以将其放置在单独的页面上；如果选择后者，则需要在视频或动画旁边提供记录的链接。
...
遵循为以下各项提供的指南： [字幕（预先录制）](#captions-pre-recorded) 上面。但是，由于媒体的实时性质，必须尽快提供字幕，并对正在发生的情况做出回应。 因此，应当考虑使用实时字幕工具或语音转文本工具。
...
遵循为以下各项提供的指南： [音频描述或替代媒体（预先录制）](#audio-description-or-media-alternative-pre-recorded).
...
[准则1.3自适应：创建能够以不同方式（例如更简单的布局）呈现的内容且不会丢失信息或结构。](https://www.w3.org/TR/WCAG20/#content-structure-separation)

本指南涵盖支持以下人员所需的要求：

* 可能无法访问作者在中提供的信息 *标准* 二维、多列、彩色网页布局

* 可能使用纯音频或替代视觉显示（如大文本或高对比度）。
...
残障人士使用的许多辅助型技术都依赖结构性信息来有效地显示或输出内容。 此结构信息可以采用页面标题、表格行和列标题以及列表类型的形式。 例如，屏幕阅读器可允许用户在页面中从标题导航到标题。 但是，当页面内容似乎只有通过可视样式而非底层HTML的结构时，则辅助型技术将没有可用的结构信息，从而限制了其支持更轻松浏览的能力。

该成功标准旨在确保通过HTML提供此类结构性信息，以便浏览器和辅助型技术可以访问和利用这些信息。

#### 如何达到标准 – 信息和关系 (1.3.1) {#how-to-meet-info-and-relationships}

AEM 允许轻松地使用相应的 HTML 元素构建网页。在RTE（文本组件）中打开页面内容，并使用 **格式** 菜单来指定相应的结构元素（例如段落和标题）。

下图显示了已设置为段落文本样式的文本；正在使用的源代码视图显示它有正确的打开和关闭 &lt;p> 和 &lt;/p> 标记之间。

![在源编辑模式（经典UI）中显示的Paragraph元素示例。](assets/chlimage_1-18a.png)

确保通过以下方式为您的网页提供了适当的结构：

* **使用标题：**

只要您启用了RTE辅助功能(请参阅 [AEM和可访问性](/help/sites-administering/rte-accessible-content.md))，AEM提供了三个级别的页面标题。 您可以使用这些标题标识内容的章节和子章节。标题 1 是最高级别的标题，标题 3 是最低级别的标题。系统管理员可以配置系统以允许使用更多标题级别。
...
* &lt;i> 对象 &lt;em>
  >
它们实际上相同，但和更可取，因为它们在语义上是正确的HTML。 开发团队可以在开发项目实例时将RTE配置为使用和(而不是和)。
...
如果要将现有内容更改为特定列表类型，请突出显示相应的文本并选择相应的列表类型。 与前面显示如何输入段落文本的示例一样，相应的列表元素会自动添加到HTML中，但您可以在源编辑视图中查看它。
...
此外，辅助表会使用以下元素和属性：
    ...
    * &#39;&lt;caption>&#39;元素用于为表提供可见的标题。 默认情况下，字幕显示在表格上方，但可以使用CSS进行适当定位。 描述采用编程方式与表相关联，因此这是一种提供内容简介的有用方法。
...
添加 **表**，您可以配置 **表属性** 使用对话框。
    ...
    *合适的**字幕**。
    *理想情况下，请移除**宽度**、**高度**、**边框**、**单元格边距**、**单元格间距**的任何默认值。 因为这些属性可以在全局样式表中设置。

![表属性对话框。](assets/chlimage_1-20a.png)

然后，您可以使用 **单元格属性** 要选择单元格是数据还是标题单元格，如果是标题单元格，则选择它是否与行或列相关，或同时与这两者相关，请执行以下操作：

![调用属性对话框；将行（通常是第一行）设置为标头行。](assets/chlimage_1-21a.png)

* **复杂数据表：**

有时，如果存在具有两个或多个标题级别的复杂表，则基本的表属性可能不足以提供所有必要的结构信息。 对于这些类型的复杂表，必须在标题及其相关单元格之间使用 **标头** 和 **id** 属性。 例如，在下表中，标题和 ID 是相匹配的，以便为辅助型技术用户建立程序化关联。
...
>[!NOTE]
此功能在标准安装中不可立即使用。 它需要配置RTE、HTML规则和序列化程序。
...
设计者往往关注可视设计特征，如颜色、形状、文本样式，或者内容在展示信息时所在的绝对或相对位置。在传达信息方面，这些可能是强大的设计技术，但是失明或患有视觉障碍的人可能无法访问需要视觉识别属性（如位置、颜色或形状）的信息。
...
>[!NOTE]
如果用户在非可视上下文中理解描述性术语的含义，则可以使用描述性术语。 例如，使用 *以上* 和 *以下* 通常是可以接受的，因为它们分别表示特定内容项之前和之后的内容。 在朗读内容时，这仍然有意义。
...
颜色能够有效地增强网页的美感，而且还有助于传递信息。但是，由于各种视觉障碍（从失明到色盲）的限制，部分用户无法辨认某些颜色。此问题使得颜色编码成为提供信息的一种不可靠的方式。

例如，患有红绿色觉缺失的人无法区分绿色阴影和红色阴影。 这些用户可能会将这两种颜色看成第三种颜色（如棕色），在这种情况下，他们就无法辨认红色、绿色和棕色。
...
例如，确保通过颜色传递的信息也明确地提供在文本中。下图显示了颜色和文本如何指示性能的座位可用性：
...
某些用户可能会发现移动的内容会让人分心，并且难以专注于页面的其他部分。 此外，如果用户无法跟上移动的文本，他们可能就很难阅读此类内容。

#### 如何达到标准 - 暂停、停止、隐藏 (2.2.2) {#how-to-meet-pause-stop-hide}

如果创建的网页包含移动、闪光或闪烁的内容，则可以采纳以下一项或多项建议，具体视内容的性质而定。

* 提供一种暂停滚动内容的方法，以便用户有足够的时间来阅读它。 例如，新闻滚动条或自动更新的文本。
* 确保闪烁的内容会在 5 秒钟后停止闪烁。
...
* 如果以上任何一项均不可用，请提供指向包含所有内容但不带任何闪烁的页面的链接。
...
此成功标准可帮助每个人（无论是否有任何特定的缺陷）快速识别网页的内容，而无需阅读整个页面。 当多个网页在浏览器选项卡中打开时，此设计非常有用，因为页面标题显示在选项卡中，因此可以快速定位。
...
* 成功标准 2.4.4
* A 级
* 链接目的（在上下文中）：每个链接的目的都可以单独从链接文本中确定，也可以从链接文本及其以编程方式确定的链接上下文一起确定。 但通常情况下，链接目的对用户不明确的情况除外。

#### 用途 — 链接目的（在上下文中）(2.4.4) {#purpose-link-purpose-in-context}

对于所有用户来说，无论是否有任何缺陷，通过适当的链接文本明确指示链接的方向都至关重要。 此设计可帮助用户确定他们是否确实想要关注链接。 对于视力正常的用户而言，有意义的链接文本在页面上存在多个链接时非常有用（尤其是页面包含大量文本时），因为有意义的链接文本可以更清晰地指示目标页面的功能。 而使用辅助技术（可以在单个页面上生成所有链接的列表）的用户可以更轻松地脱离上下文理解链接文本。
...
但是，在编写本报告时，围绕标题的使用存在一些问题：

* 标题属性中包含的文本仅供鼠标用户作为工具提示弹出窗口使用，不能使用键盘访问。
...
* 当链接包含在数据表内，从而可以从相关标题中明确识别目的时。
* 当链接列表包含在一组标题中，且标题本身提供合适的上下文时。
* 其中链接列表包含在嵌套链接中，并且嵌套链接上方的父列表项提供了合适的上下文。

有时，一个页面上会有多个链接（其中每个链接都提供了复杂而又必要的链接方向详情），此时可以为该网页提供一个替代版本，使其显示完全相同的内容，只是其中的链接文本较为简洁。

或者，也可以使用脚本，以便在链接本身中提供最少的文本。但是，在激活位于页面顶部的适当控件时，链接文本为 *已扩展* 详细介绍。类似的方法是使用CSS来 *隐藏* 视力正常的用户提供的完整链接，但仍将其输出到全屏阅读器用户。这不属于本文档的范围，但有关如何实现此目标的更多信息，请参阅 [更多信息 — 链接目的（在上下文中）(2.4.4)](#more-information-link-purpose-in-context) 部分。
...
* 成功标准 3.1.2
* AA 级
* 部分语言：内容中每个段落或短语的人类语言可以通过编程确定，但专有名称、技术术语、不定语言单词以及成为紧邻文本白话一部分的单词或短语除外。
...
提供帮助人们完成表单的说明是界面可用性良好实践的基本部分。 对于有视觉或认知障碍的人，如果在其他情况下他们可能难以理解表单的布局以及在特定表单字段中要提供的数据类型，则此功能会很有帮助。

在AEM中，添加表单组件时会添加默认标签，例如 **文本字段**，以访问页面。 此默认标题取决于组件类型。您也可以在该字段编辑对话框的&#x200B;**标题与文本**&#x200B;选项卡中添加自己的标题。应务必确保标签能够帮助用户理解与每个表单组件相关的数据。

![标题和文本选项卡（“编辑”对话框）；已添加标题“描述”。](assets/chlimage_1-22a.png)

此 **标题** 字段必须用于字段元素，因为它提供了一个可用于辅助技术的标签。 仅在该字段旁边的文本中写入标签是不够的。

对于某些表单组件，还可以使用 **隐藏标题** 复选框。以这种方式隐藏的标签仍对辅助型技术可用，但不会显示在屏幕上。 虽然这在某些情况下可能是一个不错的方法，但最好尽可能加入可视标签。某些用户可能正在查看屏幕的一小部分（一次查看一个字段），并且需要标签才能正确识别该字段。
...
但是，组标题和单选按钮本身之间并没有编程关联。模板编辑器必须将标题包装在必需的内容中 `fieldset` 和 `legend` 标记创建此关联，且只能通过编辑页面源代码来完成。 或者，系统管理员也可以添加对这些元素的支持，以使它们显示在 **字段属性** 对话框(请参阅 [添加对其他HTML元素和属性的支持](/help/sites-administering/rte-accessible-content.md#add-support-for-more-html-elements-and-attributes))。
