...
文本和个性化（营销活动）组件允许您使用WYSIWYG编辑器输入文本块，该编辑器具有以下功能 [富文本编辑器](/help/sites-authoring/rich-text-editor.md). 此外，通过此组件，您还可以使用 Adobe Campaign 提供的上下文字段和个性化基块；另请参阅[插入个性化](/help/sites-classic-ui-authoring/classic-personalization-ac-campaign.md#inserting-personalization)。
...
* **约束 — 约束**  — 您可以选择 —  **无** 或 **日期** 添加日期限制或无限制。 如果您选择日期，则用户在字段中输入的回答必须采用某种日期格式。
...
* **约束 — 约束**  — 下拉列表 — 您可以选择 —  **无**， **电子邮件**， **名称** （无变音）以添加电子邮件地址、名称或无限制的约束。 如果您选择电子邮件，则用户在字段中输入的回答必须是电子邮件地址。如果您选择名称，则输入的回答必须是名称（不允许包含变音）。
