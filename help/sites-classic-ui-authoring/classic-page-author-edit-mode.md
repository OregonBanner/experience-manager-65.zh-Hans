...
以下组件适用于为标准网页创作内容时。 这些组件构成了可用于标准安装AEM的现成组件的子集。
...
无论选择哪种方法，包含在列表中的页面都应该已经有一个与页面关联的图像。 该图像显示在轮播中。 如果该页面的“页面属性”下没有页面的图像，则应该在开始之前将图像与页面关联。 否则，轮播会显示一个空白页面。 请参阅 [编辑页面属性](/help/sites-classic-ui-authoring/classic-page-author-edit-page-properties.md).
根据您选择的项目，将显示一个新面板：
...
>[!NOTE]
>
>您可以为Adobe Experience Manager创建一个自定义轮播组件，以在AEM DAM中显示数字资产。
...
随下载链接一起显示的简短描述。
   ...
   * **文件**
可在生成的网页上下载的文件。 从内容查找器中拖动资产，或选择区域，以便您可以上传要供下载的文件。
...
* **列表**
   ...
   * 根据您选择的项目，将显示一个新面板：
      ...
      * **子页面选项**
         ...
         * **子项** （父页面）手动或使用选择器指定路径。 将留空，以使用当前页面作为父页面。
      * **固定列表选项**
...
以下示例显示了 **列表** 组件，以显示子页面列表（设计由网站设计的自定义CSS定义控制）。
...
除了引用特定段落之外，还可以修改路径以指定整个段落系统。 为路径添加以下后缀：
...
例如，如果您在 `Geometrixxe`，系统会显示“您是说吗？ Geometrixx&quot;.
   ...
   * **相似页面文本**
      ...
      类似页面的结果旁边显示的文本。 要剪切出图像的一部分，请单击此链接。
...
>
>Adobe建议您使用 **表** 组件，但也可以使用 **文本** 组件。

的 **表** 组件已进行预配置，允许您构建、填充和设置表格式。 使用该对话框，您可以从头开始配置表格并创建内容，或者从外部编辑器（如Excel、OpenOffice或记事本）复制并粘贴电子表格或表格以创建内容。
...
当您打开 **样式** 选项卡 **编辑** 对话框中，您还可以设置以下内容：

* **分隔条**
* **文本样式**

格式化的文本随后会显示在页面上。 实际设计取决于网站CSS:
...
以下示例显示了正在使用的两列和三列组件。 您可以对新组件使用占位符：
...
>[!NOTE]
>
>有关开发和自定义表单的信息，请参阅 [开发Forms页面](/help/sites-developing/developing-forms.md). 此表单自定义包括添加操作、约束、预加载字段和使用脚本调用服务以执行操作等。
...
* **约束**
   ...
   在此，您可以指定字段是否为必填字段以及是否对该字段施加约束，如数字。
...
此字段为必填字段，且只应包含以下字符：
   ...
   * **描述**
      ...
      允许您根据需要为用户提供其他信息。 在表单上，此描述显示在字段下方，字体比标题字体小。
...
* **约束**
   ...
   * **必填**
      ...
      此约束取决于表单组件类型，但提供一个或多个单击框来指示此字段为必填字段，或此字段的某些部分为必填字段。

   * **必需的消息**

      通知用户此字段为必填字段的消息。 必填字段带有星号标记。
...
表单组件使用 **表单开始** 和 **表单结尾** 元素。 开始元素和结束元素始终是成对的，以确保正确定义表单。
...
此字段为可选字段，并指定存储库中节点的路径。 如果此节点具有与字段名称匹配的属性，则表单上的相应字段将预先加载这些属性的值。 如果不存在匹配项，则字段包含默认值。
使用 **加载路径** 您可以预载必填字段中包含值的表单。 请参阅 [预加载表单值](/help/sites-developing/developing-forms.md#preloading-form-values).

   * **客户端验证**

      指示此表单是否需要客户端验证（服务器验证） *always* 发生。) 通过 **Forms验证码** 组件。
...
指定所需的潜在客户列表。
      * **创建和更新帐户**
         ...
         * **查看数据...**
            ...
            选择按钮，以便您能够在批量编辑器中访问有关表单结果的信息。 从此处，您可以将信息导出到 `.tsv` （制表符分隔）文件（例如，在Excel电子表格中使用）。
      * **邮件**
...
此组件允许您创建隐藏字段。 这些隐藏字段可用于各种目的，例如，提交表单后必须执行某项操作，或在帖子处理中需要隐藏数据时。
