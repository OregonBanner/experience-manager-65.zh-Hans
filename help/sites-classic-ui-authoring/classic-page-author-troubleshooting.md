...
>[!NOTE]
>
>具有管理员权限的用户以及希望对AEM问题进行故障排除的用户，可以使用中所述的故障排除方法 [AEM故障诊断（适用于管理员）](/help/sites-administering/troubleshoot.md). 如果您没有足够的权限，请咨询系统管理员以解决AEM问题。

## 发布站点上仍显示旧的页面版本 {#old-page-version-still-on-published-site}

* **问题**：

   * 您对某个页面进行了更改，并将该页面复制到发布站点，但是 *旧* 发布网站上仍显示页面的版本。
...
* **原因**:
   ...
   * 在极少数情况下，您可能会将sidekick的标题放在当前窗口范围之外。 这意味着您无法再次调整其位置。

* **解决方案**:

   * 从当前会话注销，然后重新登录。 Sidekick将返回到默认位置。

## 查找和替换 — 并非所有实例都将被替换 {#find-replace-not-all-instances-are-replaced}

* **问题:**

   * 使用时 **查找和替换** 选项，并非所有实例 `find` 替换页面上的术语。

* **原因**:

   * 的功能 **查找和替换** 取决于内容的保存方式以及是否可搜索内容。 例如，博客文本存储在 `jcr:text` 未配置为要搜索的属性。 查找和替换servlet的默认范围包括以下属性：
...
* **解决方案**:
   ...
   * 这些定义可以使用以下配置进行更改 **Day CQ WCM查找替换Servlet** 使用 **Web控制台**；例如，在
