---
title: 了解元数据概念
description: 了解元数据的需求和类型，以便更轻松地分类和组织资产。
contentOwner: AG
role: User, Admin
feature: Metadata
...
都柏林核心元数据提供了一个用于描述资产的标准化惯例集，可使资产更易于查找。在 [!DNL Assets]，都柏林核心可描述数字资产，包括视频、声音、图像和文档。
...
| date | 与资产关联的日期或时间段。 |
| 描述 | 关于资产的详细信息。 |
| format | 资产的文件格式、物理介质或尺寸。[!DNL Experience Manager] 使用 `dc:format` 表示资产的MIME类型。 |
| 标识符 | 资产的唯一参考。 |
| 语言 | 资产的语言(例如， `en` （英语）。 |
...
XMP（可扩展元数据平台）是 [!DNL Adobe Experience Manager Assets] ，以用于所有元数据管理。 XMP 为在各种应用程序中创建、处理和交换元数据提供了一种标准格式。
...
例如，专业生产人士可以使用 Adobe 应用程序中内置的 XMP 支持，在多种文件格式之间传递信息。[!DNL Assets] 存储库会提取XMP元数据，并使用它来管理内容生命周期，并提供创建自动化工作流的功能。
...
例如， `Creator` 两个独立设计架构中的资产可能是指创建资产的人员，也可能是指创建资产的应用程序(例如，Adobe Photoshop)。
...
XMP允许您添加 `xml:lang` 属性来指定文本的语言。

## 使用IPTC元数据 {#support-for-iptc-metadata}

了解如何 [!DNL Adobe Experience Manager Assets] 支持通过向资产添加的IPTC元数据、创作评级和关键词 [!DNL Adobe Bridge] 其他 [!DNL Adobe Creative Cloud] 应用程序。

[!DNL Adobe Experience Manager Assets] 支持广泛用于描述资产的IPTC元数据标准。 这边， [!DNL Assets] 加强摄影师、创意机构、图书馆、博物馆等各方对其图像的接受。
...
1. 从 [!DNL Assets] 用户界面中，选择图像。
1. 单击 **[!UICONTROL 属性]** 中。
1. 单击 **[!UICONTROL IPTC]** 选项卡，以查看资产的IPTC元数据。
1. 根据需要编辑IPTC元数据属性。

   ![iptc_tab](assets/keywords-in-iptc-tab.png)

1. 单击 **[!UICONTROL IPTC扩展]** 选项卡，以查看资产的IPTC扩展元数据。
1. 根据需要编辑IPTC扩展元数据属性。
...
的 **[!UICONTROL IPTC]** 选项卡 [!UICONTROL 属性] 页面还显示通过Adobe Bridge和其他Adobe Creative Cloud应用程序添加到资产的关键词。 您还可以编辑这些关键词，并从 **[!UICONTROL IPTC]** 选项卡。
