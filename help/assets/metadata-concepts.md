...
都柏林核心元数据提供了一个用于描述资产的标准化惯例集，可使资产更易于查找。In [!DNL Assets], the Dublin Core describes digital assets including video, sound, images, and documents.
...
了解如 [!DNL Adobe Experience Manager Assets] 何支持通过和其他应用程序添加到资产的IPTC元数据、创意评 [!DNL Adobe Bridge] 级和关 [!DNL Adobe Creative Cloud] 键字。

[!DNL Adobe Experience Manager Assets] 支持广泛用于描述资产的IPTC元数据标准。 这样， [!DNL Assets] 包括摄影师、创意机构、图书馆、博物馆等各方对其图像的接受程度都得到提高。
...
“属 **[!UICONTROL 性]** ”页面的IPTC选 [!UICONTROL 项卡还显] 示通过Adobe Bridge和其他Adobe Creative Cloud应用程序添加到资产的关键字。 您还可以编辑这些关键字并从IPTC选项卡添加更多 **[!UICONTROL 关键字]** 。
