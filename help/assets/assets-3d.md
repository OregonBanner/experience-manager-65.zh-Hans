...
Dynamic Media让您能够上传、管理、视图和交付3D资产，成为沉浸式体验。
...
另请参阅支持的[3D格式。](/help/assets/assets-formats.md)

| 3D文件扩展名 | 文件格式 | MIME类型 | 注释 |
|---|---|---|---|
| GLB | 二进制GL传输 | model/gltf-binary | 将材料和纹理作为单个资源提供。 |
| OBJ | WaveFront 3D对象文件 | application/x-tgif |  |
| STL | 立体光刻 | application/vnd.ms-pki.stl |  |
| USDZ | 通用场景描述Zip存档 | model/vnd.usdz+zip | *仅支持摄取；没有可用的查看或交互。* USDZ是专有的3D格式，Safari和iOS设备可以本机查看。 |
...
1. **发布3D资产**
   ...
   * [使用Dimension Viewer发布Dynamic Media 3D资源的替代方法](#alternate-publish-methods)

## 关于查看3D资产并与之交互{#viewing-three-d-assets}

本节介绍如何通过两种不同的方式视图3D资产并与之交互：从资产详细信息页面和站点中的3D媒体组件。
...
>[!TIP]
>
>您可以在查看器预设编辑器中打开维查看器预设，以练习在3D资产上导航，而无需先上传任何3D文件。 “维”查看器预设包含一个内置的3D资源，可供您进行交互。
>
...
当网页处于&#x200B;**[!UICONTROL 编辑]**&#x200B;模式时，不能与3D资产进行交互。 要使资产具有交互性，您可以使用&#x200B;**[!UICONTROL 预览]**&#x200B;功能在页面编辑器中视图网页，以完全访问3D媒体组件的功能。

>[!IMPORTANT]
>
>只有在将3D媒体组件添加到网页并为该组件分配了3D资产后，才能完成此任务。 请参阅[将3D媒体组件添加到网页](#adding-the-three-d-media-component-to-a-web-page)和[将3D资产分配给3D媒体组件。](#assigning-a-three-d-asset-to-the-component)
...
一个完全交互式的3D资产，如    ![3D资产在3D媒体组件内显示在预](/help/assets/assets-dm/3d-asset-in-3d-media.png)
览模式中显示的完全交互式3D **** 资产。
...
Dynamic Media包含一个Dynamic Media 3D媒体组件，您可以在AEM Sites中使用它在网页上实现3D模型的交互式查看。
...
1. 点按&#x200B;**[!UICONTROL 布局容器]**&#x200B;区域中的空区域以选择它并打开其关联工具栏。
...
1. 点按&#x200B;**[!UICONTROL 完成]**&#x200B;以保存更改并关闭&#x200B;**[!UICONTROL 策略编辑器。]**
   ...
   您现在可以将Dynamic Media 3D媒体组件放置到使用此模板的所有页面上。
...
1. 打开AEM Sites，选择要向其添加Dynamic Media 3D媒体组件的网页。
...
1. 在AEM Sites页面编辑器中，选择您之前添加到页面的&#x200B;**[!UICONTROL 3D Media Viewer]**&#x200B;组件。
...
1. 将3D资产从“资产”侧面板中拖放到您之前添加到页面的&#x200B;**[!UICONTROL 3D Media]**&#x200B;组件上。

   ![将3d资产分配给3d媒体组件](/help/assets/assets-dm/3d-asset-add.png)

>[!NOTE]
>
>当网页处于AEM Sites **[!UICONTROL 编辑]**&#x200B;模式时，3D Media组件会显示3D资产，但无法与资产交互。 要使资产具有交互性，您可以使用&#x200B;**[!UICONTROL 预览]**&#x200B;功能在页面编辑器中视图网页，以完全访问3D媒体组件的功能。

## 发布静态Dynamic Media 3D资源{#publishing-three-d-assets}

Dynamic Media接受Dynamic Media中支持的各种3D文件格式为&#x200B;*static content*。 静态内容意味着您可以上传和发布3D资产，但不支持与3D资产关联的&#x200B;*动态*&#x200B;图像或图像重排。 原因是Dynamic Media Imaging Server无法识别3D格式。 因此，在Dynamic Media中发布3D资产后，您可以复制一个即时URL。 3D资产的URL采用通常的Dynamic Media URL结构。 但是，与Dynamic Media中的传统图像资产不同，您无法编辑资产URL中的任何参数。
...
1. 点按&#x200B;**[!UICONTROL URL]**&#x200B;可显示3D资产的直接生产URL，您可以在网页上复制和使用该URL。
...
* **[!UICONTROL 嵌入]**  — 当 **** 您希望使用维查看器视图嵌入到网页中的Dynamic Media 3D资源时，请使用“嵌入”。将嵌入代码复制到剪贴板，以便将其粘贴到网页中。 不允许在&#x200B;**[!UICONTROL 嵌入]**&#x200B;对话框中编辑代码。
