...
通过Dynamic Media，您可以上传、管理、查看和交付3D资源作为沉浸式体验。

* 一键式发布(使用 **[!UICONTROL 快速发布]** （在工具栏上）以生成URL。
* 优化支持使用由Adobe Dimension提供支持的高质量交互式Dimensional查看器预设查看3D资产。
* 通过3D媒体WCM组件，您可以轻松地将3D资源添加到Adobe Experience Manager Sites页面。
...
在Dynamic Media中使用3D资产之前，请确保您的Experience Manager管理员已在Dynamic Media - Scene7模式下启用和配置Dynamic MediaCloud Services。
...
1. **上传3D资产**
   ...
   * [Dynamic Media中支持的3D文件格式上传](#supported-three-d-file-formats-in-dm).
...
本节介绍如何以两种不同的方式查看3D资源并与之交互：在“资源详细信息”页面中查看，以及在Experience Manager Sites的3D媒体组件中查看。
...
此外，在交互操作相机时，客户端计算机的功能（如工作站、笔记本电脑或移动触摸设备）也非常重要。 一个功能相当强大、图形功能良好的系统，可以使交互式3D观看体验更顺畅、更顺畅。

>[!TIP]
>
>您可以在查看器预设编辑器中打开维查看器预设，以练习导航3D资产，而无需首先上传任何3D文件。 维查看器预设提供了一个内置的3D资产供您与之交互。
>
>参见 [管理查看器预设](/help/assets/managing-viewer-presets.md).

## 从资源详细信息页面查看并与3D资源交互 {#viewing-three-d-assets-from-asset-details-page}

另请参阅 [使用软件界面预览资源](/help/assets/previewing-assets.md).

**要从资产详细信息页面查看并与3D资产交互，请执行以下操作：**

1. 确保已将3D资源上传到Experience Manager。

   参见 [上传要在Dynamic Media中使用的3D资源](/help/assets/manage-assets.md#uploading-assets).

1. 从Experience Manager，在 **[!UICONTROL 导航]** 页面，转到 **[!UICONTROL 资产]** > **[!UICONTROL 文件]**.
1. 在页面的右上角附近，从 **[!UICONTROL 视图]** 下拉列表，选择 **[!UICONTROL 卡片视图]**.
1. 导航到要查看的3D资产。
1. 选择3D资产的卡。
1. 在3D资产的“详细信息视图”页面上，执行以下任一操作：
...
## 查看3D媒体组件中的3D资产并与之交互 {#interacting-with-asset-inside-three-d-media-component}

当网页位于 **[!UICONTROL 编辑]** 模式，无法与3D资产进行交互。 要使资源具有交互性，您可以使用 **[!UICONTROL 预览]** 可在页面编辑器中查看网页的功能，该功能可完全访问3D媒体组件的功能。

>[!IMPORTANT]
>
>只有在将3D媒体组件添加到网页并将3D资产分配给组件后，才能完成此任务。 参见 [将3D媒体组件添加到网页](#adding-the-three-d-media-component-to-a-web-page) 和 [将3D资产分配给3D媒体组件](#assigning-a-three-d-asset-to-the-component).

另请参阅 [使用软件界面预览资源](/help/assets/previewing-assets.md).

**要在3D媒体组件中查看并与3D资产交互，请执行以下操作：**

1. 当网页位于 **[!UICONTROL 编辑]** 模式，执行以下任一操作：
...
Dynamic Media包含一个Dynamic Media 3D媒体组件，您可以在Adobe Experience Manager Sites中使用它来启用在网页上以交互方式查看3D模型。
...
1. 选择空白区域 **[!UICONTROL 布局容器]** 区域，以便您选择它并打开其关联的工具栏。
...
如果您使用Experience Manager作为Web内容管理系统，则可以通过3D媒体组件将3D资源添加到网页。
...
1. 拖动 **[!UICONTROL 3D媒体]** 组件来自 **[!UICONTROL 组件]** 列出要在页面上显示3D查看器的位置。

现在，您可以将3D资产分配给组件。

参见 [将3D资产分配给3D媒体组件](#assigning-a-three-d-asset-to-the-component).

### 可选 — 配置3D媒体组件 {#configuring-the-three-d-component}

1. 在Experience Manager Sites页面编辑器中，选择 **[!UICONTROL 3D媒体查看器]** 之前添加到页面的组件。
1. 选择 **[!UICONTROL 配置]** 图标（扳手）打开“组件配置”对话框。
...
将3D媒体组件添加到网页后，可以为其分配3D资产。

参见 [将3D媒体组件添加到网页](#adding-the-three-d-media-component-to-a-web-page).
...
1. 将3D资产从“资产”侧面板拖放到 **[!UICONTROL 3D媒体]** 之前添加到页面的组件。

   ![将3D资产分配给3D媒体组件](/help/assets/assets-dm/3d-asset-add.png)

>[!NOTE]
>
>当网页在Experience Manager Sites中时 **[!UICONTROL 编辑]** 模式，3D媒体组件会显示3D资产，但无法与资产交互。 要使资源具有交互性，您可以使用 **[!UICONTROL 预览]** 可在页面编辑器中查看网页的功能，该功能可完全访问3D媒体组件的功能。

## 发布静态Dynamic Media 3D资源 {#publishing-three-d-assets}

Dynamic Media接受支持的各种3D文件格式 *静态内容* 在Dynamic Media中。 静态内容表示您可以上传和发布3D资产，但不支持 *可变成像* 或与3D资源关联的图像重新调整。 原因是Dynamic Media Imaging Server无法识别3D格式。 因此，在Dynamic Media中发布3D资源后，您就可以复制一个即时URL。 3D资源的URL遵循常规的Dynamic Media URL结构。 但是，与Dynamic Media中的传统图像资源不同，您无法编辑资源URL中的任何参数。

另请参阅 [获取静态资源的URL](/help/assets/linking-urls-to-yourwebapplication.md#obtaining-a-url-for-a-static-asset).

在 **[!UICONTROL 卡片视图]**，资产名称的正下方及其日期和时间的左侧会显示一个小地球图标，指示资产已发布。 在&#x200B;**[!UICONTROL 列表视图]**&#x200B;中，**[!UICONTROL 已发布]**&#x200B;列显示已发布的资产和未发布的资产。

如果您将Experience Manager用作WCM，请使用此发布方法直接在网页上添加Dynamic Media 3D资源。
...
1. 打开3D资产（GLB、OBJ或STL文件格式），以便您可以在资产详细信息页面中查看它。
...
1. 选择 **[!UICONTROL 原有]**. 发布（或“激活”）3D资产时， **[!UICONTROL URL]** 如果满足以下所有3D资产条件，则按钮将在页面的左下角附近显示：
...
1. 选择 **[!UICONTROL URL]** 这样您便可以显示3D资产的直接生产URL，以便复制该URL并在网页上使用。

### 使用维度查看器发布Dynamic Media 3D资产的替代方法 {#alternate-publish-methods}

如果这样做，请使用以下两种方法发布Dynamic Media 3D资产 *非* 使用Experience Manager作为WCM。
