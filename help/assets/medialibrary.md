...
# [!DNL Experience Manager Assets] 与 [!DNL Experience Manager] 媒体库  {#aem-assets-vs-aem-medialibrary}

[!DNL Adobe Experience Manager Assets] 是平台的一部分 [!DNL Experience Manager] 。这种顺畅的集成被视为[!DNL Experience Manager]的一个主要优势，它确保了内容管理的一致性和内容作者的高工作效率。

## 常见问题 {#frequently-asked-questions}

### [!DNL Assets]是什么？{#what-is-aem-assets}

[!DNL Assets] 是一项功能， [!DNL Experience Manager] 允许用户在基于Web的存储库中管理其数字资产(图像、视频、文档和音频剪辑)。[!DNL Assets] 包括元数据支持、再现、查找器和管理界面。
...
### 从不属于WCM的[!DNL Assets]中可以获得什么？{#what-do-i-get-from-aem-assets-that-is-not-part-of-aem-wcm}

只有[!DNL Assets]的客户才能使用的独特功能有：

* 能够提取和编辑除标题、标记和描述之外的元数据。
* [!DNL Assets]管理员，可从欢迎屏幕获取。
...
否. 为了简化安装和部署，所有[!DNL Experience Manager]应用程序和加载项都在一个包中提供，其中包含所有功能。 这并不表示您有权使用包中的所有功能。
...
否. 调整静态图像的大小和自动工作流程驱动的转换以及管理再现的能力是[!DNL Experience Manager]媒体库的一部分。 这些功能不需要[!DNL Assets]许可证。

### 我想使用自定义的图像组件调整图像大小。 我需要[!DNL Assets]吗？{#i-want-to-resize-images-using-a-customized-image-component-do-i-need-aem-assets}

图像组件是WCM的一部分。 图像组件（也由[!DNL Assets]使用）使用的图形库是[!DNL Experience Manager]平台的一部分，不需要[!DNL Assets]许可证。

### 如果我没有许可证[!DNL Assets]，如何防止我的用户使用[!DNL Assets]?{#how-can-i-prevent-my-users-from-using-aem-assets-if-i-did-not-license-aem-assets}
...
### [!DNL Assets]与媒体库{#listoffeatures}中可用功能的详细列表
