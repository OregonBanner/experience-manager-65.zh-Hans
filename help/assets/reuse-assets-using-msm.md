...
# 对[!DNL Assets] {#reuse-assets-using-msm-for-assets}使用MSM重用资源

[!DNL Adobe Experience Manager]中的多站点管理器(MSM)功能使用户能够重用一次创作的内容并在多个Web位置间重复使用。 数字资产与MSM在[!DNL Assets]功能方面的功能也相同。 将MSM用于[!DNL Assets]，您可以：
...
要将MSM用于[!DNL Assets]，请至少安装Service Pack 1。 有关详细信息，请参阅[发行说明](/help/release-notes/sp-release-notes.md)。

## 了解优势和概念{#concepts}

### 工作方式及优势{#how-it-works-and-the-benefits}

要了解在多个Web位置重复使用相同内容（文本和资产）的使用方案，请参阅[可能的MSM方案](/help/sites-administering/msm.md)。 [!DNL Experience Manager] 在原始资产及其链接的副本(称为Live Copy(LC))之间保留一个链接。维护的链接允许将集中的更改推送到多个Live Copy。 这样可以在消除管理重复副本的限制的同时加快更新。 更改的传播是无错的和集中的。 该功能允许仅允许选定Live Copy进行更新。 用户可以分离链接（即中断继承），并进行在下次更新主副本和转出更改时不会覆盖的本地编辑。 可以对若干选定的元数据字段或整个资产进行分离。 它允许灵活地本地更新最初从主副本继承的资产。
...
**转出：** 将修改推送到源下游的Live Copy的操作。可以使用转出操作在一次转出中更新一个或多个Live Copy。 请参阅转出。

**转出配置：** 确定同步哪些属性、方式和时间的规则。创建Live Copy时会应用这些配置；稍后可以编辑；并且子项可以从其父资产继承转出配置。 对于[!DNL Assets]的MSM，请仅使用标准转出配置。 其他转出配置对于[!DNL Assets]不可用。

**同步：** 除了转出之外，还有一个操作，通过从源向Live Copy发送更新，实现了源与其Live Copy之间的对等性。将为特定Live Copy启动同步，操作会从源中提取更改。 使用此操作，只能更新其中一个Live Copy。 请参阅同步操作。

**暂停：** 暂时删除Live Copy与其源资产／文件夹之间的Live关系。你可以恢复关系。 请参阅暂停操作。

**恢复：** 恢复Live关系，以便Live Copy再次开始从源接收更新。请参阅恢复操作。

**重置：重** 置操作会通过覆盖任何本地更改，使Live Copy再次成为源的副本。它还会删除继承取消并重置所有元数据字段的继承。 要在将来进行本地修改，您必须再次取消特定字段的继承。 查看对LC的本地修改。
...
### 文件夹{#statuslcfolder}的所有Live Copy的信息和状态
...
### Live Copy {#refraillc}的“引用”边栏中的快速操作
...
修改源后，可以使用同步操作或转出操作将更改传播到Live Copy。 要了解两个操作之间的区别，请参阅[词汇表](#glossary)。
...
1. 选择要更新的Live Copy。 单击&#x200B;**[!UICONTROL 转出]**。 要转出对子资产进行的更新，请选择&#x200B;**[!UICONTROL 转出源和所有子资产]**。
...
或者，在选择特定Live Copy后，也可以从引用边栏启动转出操作。 有关详细信息，请参阅[ References rail for Live Copy](#refraillc)中的快速操作。 在此转出方法中，只更新所选Live Copy及其子项（可选）。
...
|---|---|---|
| [!UICONTROL 引用边] 栏 | 在已选择源时快速同步。 | 请参阅[源](#refrailsource)引用边栏中的快速操作 |
| 工具栏。 | 在Live Copy属性已打开时启动同步。 | 请参阅[同步Live Copy](#synclc) |
| [!UICONTROL Live Copy概述控] 制台 | 选择源文件夹或[!UICONTROL Live Copy概述]控制台已打开时，快速同步多个资产（不一定全部）。 同步操作是一次为一个资产启动的，但是这是一次为多个资产执行同步的更快方式。 | 请参阅[对Live Copy文件夹中的许多资产执行的操作](#bulkactions) |
...
但是，您可以灵活地对Live Copy进行本地修改，以更改一些选定的属性。 要进行本地修改，请取消所需属性的继承。 取消一个或多个元数据字段的继承后，资产的实时关系和其他元数据字段的继承将保留。 任何同步或转出不会覆盖本地修改。 为此，请打开Live Copy资产的&#x200B;**[!UICONTROL 属性]**&#x200B;页，单击元数据字段旁的&#x200B;**[!UICONTROL 取消继承]**&#x200B;选项。
...
您可以使用分离操作完全删除源与Live Copy之间的关系。 Live Copy分离后将成为独立的资产或文件夹。 在分离后，它会立即在[!DNL Experience Manager]接口中显示为新资产。 要从Live Copy的源中分离Live Copy，请按照以下步骤操作。
...
1. 单击&#x200B;**[!UICONTROL Live Copy]**。 单击工具栏中的&#x200B;**[!UICONTROL 分离]**。 在显示的对话框中单击&#x200B;**[!UICONTROL 分离]**。
   ...
   *图：分离操作会完全删除源和Live Copy之间的关系。*
   ...
   >[!CAUTION]
   >
   >单击对话框中的&#x200B;**[!UICONTROL 分离]**&#x200B;时，该关系将立即删除。 无法通过单击“属性”页上的&#x200B;**[!UICONTROL 取消]**&#x200B;来撤消此操作。

或者，您也可以从&#x200B;**[!UICONTROL Live Copy概述]**&#x200B;控制台快速分离Live Copy文件夹中的多个资产。 请参阅[对 Live Copy 文件夹中的许多资产执行操作](#bulkactions)。

## 对Live Copy文件夹{#bulkactions}中的许多资产执行操作

如果您在Live Copy文件夹中包含多个资产，则启动每个资产的操作可能会非常繁琐。 您可以从[!UICONTROL Live Copy控制台]快速对许多资产启动基本操作。 以上方法继续适用于单个资产。
...
Live Copy和源是可以作为数字资产在一定程度上进行管理的资产或文件夹。 [!DNL Experience Manager]中的某些资产管理任务对Live Copy具有特定影响。
...
在更多情况下，[!DNL Assets]的MSM与“站点”功能的MSM行为相匹配。 需要注意的主要区别是：

* [!DNL Sites]的MSM中的Blueprint在[!DNL Assets]的MSM中称为Live Copy源。
* 在站点中，您可以比较Blueprint和其Live Copy，但是在[!DNL Assets]中，无法将源与其Live Copy进行比较。
* 不能在[!DNL Assets]中编辑Live Copy。
* 站点通常有子项，但[!DNL Assets]没有。 创建单个资产的Live Copy时，不提供包含或排除子项的选项。
* [!DNL Assets]的MSM不支持删除创建站点向导中的章节步骤。
* 对于[!DNL Assets],MSM不支持在页面属性（触屏优化UI）上配置MSM锁。
* 对于[!DNL Assets]的MSM，仅使用&#x200B;**[!UICONTROL 标准转出配置]**。 其他转出配置对于[!DNL Assets]不可用。
...
## [!DNL Assets] {#limitations}的MSM的限制和已知问题
