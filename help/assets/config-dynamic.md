...
需要启用并配置Dynamic Media-Hybrid以供使用。 根据您的用例，Dynamic Media具有多种受支 [持的配置](#supported-dynamic-media-configurations)。
...
了解有关在Dynamic Media中处 [理视](/help/assets/video.md) 频的更多信息。
...
[默认情况](https://www.adobe.com/solutions/web-experience-management/dynamic-media.html) 下，Dynamic Media处于禁用状态。 要利用Dynamic Media功能，您需要像运行模式一样 `dynamicmedia` 使用运行模式来启用 `publish` Dynamic Media。 在启用之前，请确保查看技 [术要求。](/help/sites-deploying/technical-requirements.md#requirements-for-aem-dynamic-media-add-on)

>[!NOTE]
>
>通过运行模式启用Dynamic Media将取代AEM 6.1和AEM 6.0中的功能，您可通过将标志设置为true来启 `dynamicMediaEnabled` 用Dynamic **[!UICONTROL Media。]** 此标志在AEM 6.2和更高版本中没有功能。 此外，您无需重新启动快速启动即可启用Dynamic Media。
...
在AEM QuickStart WAR部署中，无法派生端口号和上下文路径，因此必须配置 **自身** 域。 请参 [阅](/help/sites-developing/externalizer.md) externalizer文档，了解如何 **配置自** 定义域。
...
需要超过最大内存限制的图像将被拒绝。
要更改创建PTIFF的内存限制，请导航到“工 **[!UICONTROL 具”>“操作”>“Web控制台”>“Adobe CQScene7PTiffManager]** ”，然后 **[!UICONTROL 更改maxMemory]** 值。
...
1. 在“编 **[!UICONTROL 辑User Settings For dynamic-media]** -replication”页面上 **，展开“从KeyStore** 添加私钥”文件区域，并添加以下内容（请参阅下面的图像）:
   ...
   * 在“KeyStore **[!UICONTROL 文件口令]** ”字段中，输入KeyStore文件口令。 这不是 **您在** 第5步中创建的KeyStore密码，而是KeyStore文件密码Adobe在配置过程中发送给您的欢迎电子邮件中提供的密码。 如果您未收到KeyStore文件密码，请与Adobe客户服务部门联系。
...
* 添加的图像预设。
* 在 **[!UICONTROL Cloud Services下配置Dynamic Media配置(6.3之前]** )。 此测试需要图像服务URL
...
1. 在Dynamic Media配置浏览器页面的左窗格中，选择全局 **[!UICONTROL 性]**，然后点 **[!UICONTROL 按创建。]**
...
1. 在配置Dynamic Media配置(6.3 ***之前*** ，请先将 ***视频分析*** 预设包安装到任何新的作者节点。
1. 验证并调试包安装。
...
1. 构建包。
1. 下载或共享视频分析预设包，以便可以与后续的新作者节点共享。

### 在配置其他作者节点之前安装视频分析预设包 {#installing-the-video-analytics-preset-package-before-you-configure-additional-author-nodes}
...
例如，要在“作者”节点上查看分析预设，您可以发出以下请求：
   ...
   * **通过AEM中的视频报告工具检查视频分析预设。点按**&#x200B;工具 **[!UICONTROL >资产>视频报告]**
...
1. 在AEM中，在CRXDE Lite中导航到以下位置（需要管理员权限）:
...
如果您正在使用Dynamic Media进行(1)制 **作** 或(2)成像和视频，则您可以使用我们按原样提供的默认过滤器。 默认情况下，以下过滤器处于活动状态：
...
**入门项目**: *在配* 置Dynamic Media Image Server之前，请确保Windows虚拟机包括Microsoft Visual C++库的安装。 运行Dynamic Media Image Server时必须使用这些库。 您可 [以在此处下载Microsoft Visual C++ 2010 Redistributable Package(x64)](https://www.microsoft.com/en-us/download/details.aspx?id=14632)。
...
1. 在“Adobe Experience ManagerWeb控制台配置”页面，点 **[!UICONTROL 按OSGi > Configuration]** ，以列表AEM中当前运行的所有捆绑包。
...
1. 在捆绑包的列表中，点按Adobe CQScene7图像服务器右侧的编辑图标。
1. 在“Adobe CQScene7图像服务器”对话框中，设置以下配置值：
...
1. 在捆绑包的列表中，点按Adobe CQScene7平台服务器右侧的“编 **[!UICONTROL 辑]** ”图标。
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>这些值表示视频播放时间的快照，并传 <a href="https://encoding.com/">递给encoding.com</a>。 有关 <a href="/help/assets/video.md#about-video-thumbnails-in-dynamic-media-hybrid-mode">详细信息</a> ，请参阅“关于视频缩略图”。</td>
  </tr>
 </tbody>
</table>
...
1. 导航到“ [软件分发](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html) ”并下载其中一 `cq-6.3.0-featurepack-12445`项。
...
1. 在“ **[!UICONTROL 工具”>“常规”>]**“CRXDE Lite” `/conf/global/settings/dam/dm/imageserver/jcr:content` 中，导航到包含默认的Adobe Color用户档案。
