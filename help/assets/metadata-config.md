...
1. 单击某个组件，然后在 **[!UICONTROL 设置]** 选项卡。例如，单击 **[!UICONTROL 描述]** 组件并编辑其属性。
   ...
   * **[!UICONTROL 映射到属性]**:此属性的值提供资产节点在存储库中保存的相对路径或名称。 值应始终以开头 `./` 因为它表示路径位于资产的节点下。
   ...
   您为&#x200B;**[!UICONTROL 映射到属性]**&#x200B;指定的值会作为属性存储在资产的元数据节点下。例如，如果您指定 `./jcr:content/metadata/dc:desc` 作为的名称 **[!UICONTROL 映射到属性]**, [!DNL Assets] 存储值 `dc:desc` ，位于资产的元数据节点。 Adobe建议您只将一个字段映射到元数据架构中的给定属性。 否则，系统会选取映射到属性的最新添加字段。
