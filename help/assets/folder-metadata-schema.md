...
Adobe Experience Manager资产允许您为资产文件夹创建元数据模式，定义在文件夹属性页面中显示的布局和元数据。

## 添加文件夹元数据模式表单 {#add-a-folder-metadata-schema-form}

使用文件夹元数据模式表单编辑器创建和编辑文件夹的元数据模式。

1. 在Experience Manager界面中，转到工 **[!UICONTROL 具]** >资 **[!UICONTROL 产]** >文件夹 **[!UICONTROL 元数据模式]**。
1. 在“文件 [!UICONTROL 夹元数据模式表单] ”页面上，单 **[!UICONTROL 击创建]**。
1. Specify a name for the form, and click **[!UICONTROL Create]**. 新的模式表单列在模式表 [!UICONTROL 单页面中] 。
...
1. 在模式表单页面中，选择您创建的表单，然后单击工具栏中的 **[!UICONTROL 编辑]** 图标。
1. 在“文件夹元数据模式编辑器”页 `+` 面中，单击以向表单添加选项卡。 要重命名选项卡，请单击默认名称，然后在“设置”下指定新 **[!UICONTROL 名称]**。

   ![custom_tab](assets/custom_tab.png)

   要添加更多选项卡，请单击 `+`。 单击 `X` 选项卡可将其删除。
...
The **[!UICONTROL Build Form]** tab lists form items that you use in your folder metadata schema form. The **[!UICONTROL Settings]** tab displays the attributes for each item that you select in the **[!UICONTROL Build Form]** tab. 以下是“构建表单”选项卡中可用的表 **[!UICONTROL 单项的列表]** :
...
要编辑表单项的属性，请单击该组件，然后在设置选项卡中编辑以下全部或一个子 **[!UICONTROL 集]** 。
...
您可以从“文件夹元数据模式表单”页面删除文件夹元数据模式表单。 要删除表单，请选择该表单，然后单击工具栏中的删除图标。
...
如果为文件夹配置元数据模式，则模式表单的路径将存储在下的文件 `folderMetadataSchema` 夹节点的属性中。*/jcr:content*。

### 从“文件夹元数据模式”页面分配到模式 {#assign-to-a-schema-from-the-folder-metadata-schema-page}

1. 在Experience Manager界面中，转到工 **[!UICONTROL 具]** >资 **[!UICONTROL 产]**>文件 **[!UICONTROL 夹元数据模式]**。
1. 从“文件夹元数据模式表单”页中，选择要应用到文件夹的模式表单。
1. 在工具栏中，单 **[!UICONTROL 击“应用到文件夹”]**。

1. 选择要应用模式的文件夹，然后单击“应 **[!UICONTROL 用”]**。 如果已对文件夹应用元数据模式，则会显示一条警告消息，通知您将覆盖现有的元数据模式。 单击“ **[!UICONTROL 覆盖]**”。
1. 打开您应用元数据模式的文件夹的元数据属性。
...
### 在创建文件夹时分配模式 {#assign-a-schema-when-creating-a-folder}

在创建文件夹时，可以分配文件夹元数据模式。 如果系统中至少存在一个文件夹元数据模式，则“创建文件夹”对话框中会显示另 **[!UICONTROL 一个列表]** 。 您可以选择所需的模式。 默认情况下，不选择模式。

1. 在用户 [!DNL Experience Manager Assets] 界面中，单击工 **[!UICONTROL 具栏中]** 的创建。
1. 指定文件夹的标题和名称。
