...
* 您的组织已启用智能内容服务。
* 智能内容服务基础包只能添加到其中[!DNL Adobe Experience Manager Sites]基础包和[!DNL Assets]加载项已获得许可的部署。

智能标记仅应用于具有以下MIME类型的资产：
...
您还可以为标记指定更高的排名，以提高其与图像的相关性。 提升图像的标记可提高当基于特定标记执行搜索时在搜索结果中出现图像的可能性。

1. 在“搜索”框中，根据标记搜索资产。
1. Inspect搜索结果，以识别与搜索不相关的图像。
1. 选择图像，然后单击工具栏中的&#x200B;**[!UICONTROL 管理标记]**。
1. 在&#x200B;**[!UICONTROL 管理标记]**&#x200B;页中，检查标记。 如果您不希望根据特定标记搜索图像，请选择该标记，然后单击工具栏中的&#x200B;**[!UICONTROL 删除]**。 或者，单击标记旁边显示的`x`符号。
