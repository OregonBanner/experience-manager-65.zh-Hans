...
| 使用——打开资产 | 是 | 是 | [从Web界面或Finder](manage-assets.md#previewing-assets) 打开 |
...
* **对于在Photoshop、InDesign或Illustrator工作的创意用户：** Adobe资产链接提供最佳用户体验，包括对从中签出的资产的进行中工作的干净处理 [!DNL Experience Manager]。
...
[Experience Manager和Adobe Stock](/help/assets/aem-assets-adobe-stock.md)[!DNL Experience Manager] 集成使用户能够从中搜索、预览、许可和保存资 [!DNL Adobe Stock] 产 [!DNL Experience Manager]。 已授权和保 [!DNL Stock] 存的资产已选 [!DNL Stock] 择元数据，这些元数据可用于通过额外的过滤器搜索资产。

有关此集成的几个要点：

* 将Adobe库中的资源保存到时， [!DNL Experience Manager]它们将变为常规资源， [!DNL Assets]并将二进制文件保存到存储 [!DNL Experience Manager] 库中。 某些与相关的元数 [!DNL Adobe Stock] 据将保存在中， [!DNL Experience Manager]否则，摄取过程的外观与任何其他文件一样。 例如，如果智能标记处于活动状态，则在保存这些资产时会向其添加标记。
...
在具有扩展 [!DNL Creative Cloud] 功能的 [!DNL Adobe Asset Link] 应用程序中工作的创意用户除了能够从中访问已授权的资源 [!DNL Adobe Stock] 外，还 [!DNL Experience Manager]可以使用“库” [!DNL Creative Cloud] 面板搜索、预览和许可资 [!DNL Adobe Stock] 源。
[!DNL Assets] 从许 [!DNL Adobe Stock] 可和保存到中 [!DNL Experience Manager] 的资源可供更多团队访问 [!DNL Experience Manager Assets] 部署，而通过“库”面板授权 [!DNL Adobe Stock] 的创意 [!DNL Creative Cloud] 人员仅可在其帐户中默认自行 [!DNL Creative Cloud] 使用。
...
大多数组织选择只存储与下游营销/LOB流程相关的资产(通过Adobe Experience Cloud服务的渠道发布到Web渠道 [!DNL Experience Manager Sites] 等，或由Advertising CloudMarketing Cloud、Analytics Cloud评估、向用户／合作伙伴提供等渠道)。 此外，组织会存储在DAM中可能要经过审核／批准流程的资产。 这样，DAM存储的资产大多具有很高的杠杆利用机会，并避免存储闲置资产。
...
在这种情况下，Adobe资产链接或桌 [!DNL Experience Manager] 面应用程序有助于轻松访问最终／批准的资产，并允许您将可用于创意的资产保存到DAM。
...
* 当用户 [!DNL Photoshop]在 [!DNL InDesign]、或 [!DNL Illustrator] 打算编辑文件时，他们会对给定资产执行签出操作
* 资产在后台下载，通过Creative Cloud桌面应用程序将其同步到磁盘的用户帐户放入Creative Cloud桌面应用程序，并且资产上的注销标志也会被切换，以 [!DNL Experience Manager] 最大限度地减少编辑冲突
...
* 创意用户完成更改后，他们可以在其Creative Cloud应用程序中对该文件执行签入操作，并添加可选注释。 中的相应资产 [!DNL Experience Manager] 将进行版本控制，并使用新的二进制文件进行更新。 [!DNL Experience Manager] 像营销人员或LOB用户这样的用户可以通过资产时间线UI访问重大资产 [!DNL Experience Manager] 更改或里程碑。
