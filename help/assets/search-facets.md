...
[!DNL Adobe Experience Manager Assets]的企业范围部署具有存储许多资源的能力。 有时，如果您仅使用[!DNL Experience Manager]的通用搜索功能，则查找正确的资产可能会非常困难且耗时。
...
对于全文搜索，请将[!UICONTROL 全文]谓词添加到表单中。 使用“属性”谓词，可以搜索与您指定的单个属性匹配的资产。 使用“选项”谓词，可搜索与特定属性的一个或多个值相匹配的资产。 添加“日期范围”谓词，以搜索在指定日期范围内创建的资产。
...
1. 在[!UICONTROL 编辑搜索Forms]页面中，将谓词从&#x200B;**[!UICONTROL 选择谓词]**&#x200B;选项卡拖到主窗格。 例如，拖动&#x200B;**[!UICONTROL 属性谓词]**。
...
1. 单击&#x200B;**[!UICONTROL 预览]** ![预览](assets/do-not-localize/preview_icon.png)以生成过滤器面板的预览，该显示在添加谓词后。
1. 在“预览”模式下查看谓词的布局。
...
1. 在文本框中输入对要搜索的资产的描述。例如，输入`Adobe`。 执行搜索时，其描述与`Adobe`匹配的资产会列在搜索结果中。
...
通过“多值属性”谓词，您可以搜索资产中的多个值。 请考虑这样的情况：您在[!DNL Assets]中有多个产品的图像，并且每个图像的元数据都包含与产品关联的SKU编号。 您可以使用此谓词根据多个SKU编号搜索产品图像。
...
此标记谓词允许您对资产执行基于标记的搜索。 默认情况下，[!DNL Assets]会根据您指定的标记搜索资产中的一个或多个标记匹配项。 换句话说，搜索查询使用指定的标记执行OR操作。 但是，您可以使用“匹配所有标记”选项来搜索包含您指定的所有标记的资产。
...
| [!UICONTROL 文件大小] | 此搜索谓词用于根据资产的大小搜索资产. 它是基于滑块的谓词，您可以从可配置节点中选择滑块选项。 默认选项在CRXDE存储库的/libs/dam/options/predicates/filesize中定义。 文件大小以字节为单位。 | <ul><li>标签</li><li>属性名称</li><li>路径</li><li>描述</li></ul> |
...
默认情况下，在&#x200B;**[!UICONTROL 搜索Forms]**&#x200B;页面中，**[!UICONTROL 资产管理搜索边栏]**&#x200B;前会显示锁定图标![锁定关闭图标](assets/do-not-localize/lock_closed_icon.svg)。 “搜索Forms”页面上某个选项的锁图标表示默认设置保持不变且未进行自定义。 如果向表单中添加搜索彩块化，则图标![锁定已关闭的图标](assets/do-not-localize/lock_closed_icon.svg)会消失，指示默认表单已被修改。
...
1. 在确认对话框中，单击&#x200B;**[!UICONTROL 删除]**&#x200B;以删除自定义更改。
   ...
   在删除对搜索彩块化的自定义更改后，在&#x200B;**[!UICONTROL 搜索Forms]**&#x200B;页面的&#x200B;**[!UICONTROL 资产管理搜索边栏]**&#x200B;前，将重新显示锁定图标![锁定已关闭图标](assets/do-not-localize/lock_closed_icon.svg)。
...
| 操作 | 权限 |
| ------------------- | ---------------------------------------------------------------- |
| [!UICONTROL 编辑] | 在CRXDE中的`/apps`节点上读取和写入权限 |
| [!UICONTROL 删除] | 在CRXDE中的`/apps`节点上读取、写入和删除权限 |
| [!UICONTROL 预览] | 在CRXDE中的`/var/dam/content`节点上读取、写入和删除权限。 此外，`/apps`节点上的“读取”和“写入”权限。 |
