...
>[!NOTE]
>
>智能成像可以与现有图像预设配合使用，并在传送的最后一毫秒使用智能功能根据浏览器或网络连接速度进一步减小图像文件大小。 有关更 [多信息，请参阅](/help/assets/imaging-faq.md) “智能成像”。
...
>[!NOTE]
>
>当您在资产的详细信息视图中选择演绎版 **[!UICONTROL 时]** ，系统会显示各种演绎版。 您可以增加或减少显示的图像预设数。 请参 [阅增加显示的图像预设数](#increasingthenumberofimagepresetsthatdisplay)。
...
在打开资产时，您可以查看子资产或页面，点按内容菜单，然后选择子 **[!UICONTROL 资产]** 或 **[!UICONTROL 页面]**。 子资产是真实资产。 即，PDF页面由工作流组件 `Create Sub Asset` 提取。 然后，它们会作 `page1.pdf`为、 `page2.pdf`等存储在主资产下方。 在存储它们后，工作流 `DAM Update Asset` 会处理它们。
...
>[!NOTE]
>
>在DAM更新资产工作流程中， **[!UICONTROL EPS缩略图步骤]** ，可生成EPS文件的缩略图。
...
点按左上角的Adobe Experience Manager，导航到工具>工 **[!UICONTROL 作流>模型]**。 在“工作流模型”页面上，选择 **[!UICONTROL DAM更新资产]**，然后在工具栏中点按编 **[!UICONTROL 辑]**。 在DAM更新资产工作流页面上，双击流程组 `Rasterize PDF/AI Image Preview Rendition` 件以打开其步骤属性对话框。
...
“最大宽度”和“最大高度”限制栅格化时的分辨率。 例如，如果最大值保持不变，且“分辨率”设置为300 ppi，则“美国字母”文档将以186 ppi栅格化。 即，文档为1581 x 2046像素。

进 `Rasterize PDF/AI Image Preview Rendition` 程组件定义了最大值，以确保它不会在内存中创建过大的图像。 这样的大映像可能会溢出提供给JVM（Java虚拟机）的内存。 必须注意向JVM提供足够的内存来管理所配置的并行工作流数，每个工作流都有可能以最大配置大小创建映像。
...
工作流中的“媒体提取”进程组 `DAM Update Asset` 件运行多个预配置的扩展脚本以处理InDesign文件。
...
在Dynamic Media流 **[!UICONTROL 程图像资产步骤中]** ，缩略图由图像服务器生成，此配置与应用于流程缩略图步骤的配置无 **[!UICONTROL 关]** 。 通过“流程缩览 **[!UICONTROL 图]** ”步骤生成缩览图是创建缩览图的最慢、内存占用最多的方法。
...
>[!NOTE]
>
>* EPS文件的缩略图大小在 **[!UICONTROL EPS缩略图步骤中配置]** ，该步骤位于缩略图下的 **[!UICONTROL “参数]** ”选项卡中。
   >
   >
* 视频的缩略图大小在“FFmpeg缩略图 **[!UICONTROL ”步骤中的“进程”选项卡的“参数”下配置，“参数”选项卡中的“]** Process **[!UICONTROL ”（进程）选项]** 卡中 ****。
>
...
1. 点按Dynamic Media Process图 **[!UICONTROL 像资产步骤]** ，然后点按或单击缩览 **[!UICONTROL 图选项卡]** 。 根据需要更改缩略图大小，然后点按确 **[!UICONTROL 定]**。

   ![6_5_dynamicmediaprocessimageassets-thumbnailstab](assets/6_5_dynamicmediaprocessimageassets-thumbnailstab.png)

1. 点按“流 **[!UICONTROL 程缩略图]** ”步骤，然后点按“缩 **[!UICONTROL 略图]** ”选项卡。 根据需要更改缩略图大小，然后点按确 **[!UICONTROL 定]**。

   >[!NOTE]
   >
   >“流程缩略图”步骤中的“缩 **[!UICONTROL 略图]** ”参数中的值必须与“Dynamic Media Process图像资产”步骤中的“ **[!UICONTROL 缩略图”参数相匹配]** 。
...
1. 在AEM中，点按AEM徽标以访问全局导航控制台，然后点按工具> **[!UICONTROL 资产>图像预设]**。
1. 单击&#x200B;**[!UICONTROL 创建]**。此时将打开&#x200B;**[!UICONTROL 编辑图像预设]**&#x200B;窗口。
   ...
   >[!NOTE]
   >
   >要使此图像预设具有响应性，请擦除宽度和高 **[!UICONTROL 度字段]****[!UICONTROL 中的值]** ，并将其留空。

1. 根据需要在“基 **[!UICONTROL 本]** ”和“ **[!UICONTROL 高级]** ”选项卡中输入值，包括名称。图像预设选项中概 [述了这些选项](#image-preset-options)。 各项预设将出现在左侧窗格中，并且可以快速用于其他资产。
...
* **[!UICONTROL格式** (**[!UICONTROL 基本选项卡]** )-选择 **[!UICONTROL JPEG]** 或其他符合您要求的格式。 所有的 Web 浏览器均支持 JPEG 图像格式；这种格式能在小文件大小与图像质量之间实现良好平衡。但是，JPEG 格式图像使用有损压缩模式，如果压缩设置太低，会引入不必要的伪像。因此，Adobe建议将压缩质量设置为75。 此设置可在图像质量与小文件大小之间实现良好平衡。
...
1. 从图像预设列表中选择一个或多个图像预设，然后单击或点按发 **[!UICONTROL 布]**。
