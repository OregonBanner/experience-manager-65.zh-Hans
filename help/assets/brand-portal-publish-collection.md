...
如果您对AEM Assets中的原始收藏集进行后续修改，则在再次发布该收藏集之前，所做的更改不会反映在Brand Portal中。 此特性可确保进行中的工作更改在Brand Portal中不可用。 只有管理员发布的已批准更改才会出现在 Brand Portal 中。

>[!NOTE]
>
>无法将内容片段发布到 Brand Portal。因此，如果您在AEM创作中选择内容片段，则 **发布到Brand Portal** 操作不可用。
>
