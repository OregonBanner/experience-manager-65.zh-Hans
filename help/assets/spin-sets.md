...
旋转集由带有&#x200B;**[!UICONTROL SPINSET字样的横幅来指定。]**&#x200B;此外，如果旋转集已发布，则横幅上会显示发布日期（由&#x200B;**[!UICONTROL World]**&#x200B;图标指示）以及上次修改日期（由&#x200B;**[!UICONTROL Pencil]**&#x200B;图标指示）。
...
如果需要，可以[编辑旋转集](#editing-spin-sets)。 此外，您还可以查看和修改[旋转集属性](/help/assets/manage-assets.md#editing-properties)。
...
1. 在旋转集编辑器的&#x200B;**[!UICONTROL Title]**&#x200B;字段中，输入旋转集的名称。 该名称会显示在旋转集的横幅中。（可选）输入说明。

   ![6_5_spinset_spinseteditortitle](assets/6_5_spinset-spinseteditortitle.png)

   >[!NOTE]
   >
   >创建旋转集时，您可以更改旋转集缩略图，或允许AEM根据旋转集中的资产自动选择缩略图。 要选择缩略图，请单击&#x200B;**[!UICONTROL 更改缩略图]**&#x200B;并选择任意图像（您也可以导航到其他文件夹以查找图像）。 如果您选择了缩略图，然后决定让AEM从旋转集中生成缩略图，请选择&#x200B;**[!UICONTROL 切换到自动缩略图。]**
...
1. 执行以下操作之一：
   ...
   * 在“旋转集编辑器”页面的中间附近，点按&#x200B;**[!UICONTROL 点按以打开资产选择器。]**
   ...
   借助资产选择器，您可以通过键入关键字并点按&#x200B;**[!UICONTROL 返回]**&#x200B;来搜索资产。您还可以应用过滤器来优化搜索结果。您可以按路径、收藏集、文件类型和标记进行过滤。选择过滤器，然后点按工具栏上的&#x200B;**[!UICONTROL 过滤器]**&#x200B;图标。点按“视图”图标，然后选择&#x200B;**[!UICONTROL 列视图]**、**[!UICONTROL 卡片视图]**&#x200B;或&#x200B;**[!UICONTROL 列表视图，以更改视图。]**
...
1. 单击&#x200B;**[!UICONTROL 保存。]**&#x200B;新创建的旋转集将显示在创建时所用的文件夹中。
...
1. 从搜索中，可以选择&#x200B;**[!UICONTROL 筛选器]**，然后展开&#x200B;**[!UICONTROL Dynamic Media]**&#x200B;并选择&#x200B;**[!UICONTROL 集。]**

   搜索会返回在UI中手动创建的匹配集，或通过批集预设自动创建的匹配集。 对于自动集，搜索查询使用`Starts with`搜索条件执行，该条件与基于使用`Contains`搜索条件的AEM搜索条件不同。 将筛选器设置为&#x200B;**[!UICONTROL Sets]**&#x200B;是搜索自动集的唯一方法。
...
1. 要编辑旋转集，请执行以下任一操作：
   ...
   * 要添加资产或更新现有资产，请单击&#x200B;**[!UICONTROL 添加资产。]** 导航到资产，将其选中，然后点 **** 按右上角附近的选择。如果您通过将AEM用作缩略图的图像替换为其他图像来删除该图像，则仍会显示原始资产。
   * 要删除资产，请选择该资产，然后单击或点按&#x200B;**[!UICONTROL 删除资产。]**
   * 要应用预设，请点按或单击预设图标，然后选择一个预设。
   * 要删除整个旋转集，请导航到旋转集，将其选中，然后选择&#x200B;**[!UICONTROL Delete]**

   >[!NOTE]
   >
   >您可以导航到旋转集，点按左边栏中的&#x200B;**[!UICONTROL 设置成员]**，然后点按单个资产上的铅笔图标以打开编辑窗口，来编辑图像。
