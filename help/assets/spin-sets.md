...
旋转集由带有&#x200B;**[!UICONTROL SPINSET字样的横幅来指定。]**&#x200B;此外，如果旋转集已发布，则横幅上将显示由&#x200B;**[!UICONTROL 世界]**&#x200B;图标指示的发布日期以及由&#x200B;**[!UICONTROL 铅笔]**&#x200B;图标指示的上次修改日期。
...
1. [创建旋转集。](#creating-spin-sets)
   ...
   >[!NOTE]
   >
   >您还可以通过[批量集预设](/help/assets/config-dms7.md#creating-batch-set-presets-to-auto-generate-image-sets-and-spin-sets)自动创建旋转集。**重要：** 批处理集由IPS（图像生产系统）创建，作为资源摄取的一部分，并且仅在Dynamic Media - Scene7模式下可用。
...
如果需要，您可以[编辑旋转集](#editing-spin-sets)。 此外，您还可以视图和修改[旋转集属性](/help/assets/manage-assets.md#editing-properties)。
...
### 捕获旋转集{#guidelines-for-shooting-spin-set-images}的图像的指南
...
1. 在旋转集编辑器的&#x200B;**[!UICONTROL 标题]**&#x200B;字段中，输入旋转集的名称。 该名称会显示在旋转集的横幅中。（可选）输入说明。
   ...
   >[!NOTE]
   >
   >创建旋转集时，您可以更改旋转集缩略图，或允许AEM根据旋转集中的资产自动选择缩略图。 要选择缩略图，请单击&#x200B;**[!UICONTROL 更改缩略图]**&#x200B;并选择任何图像（您也可以导航到其他文件夹以查找图像）。 如果您选择了缩略图，然后决定要AEM从旋转集中生成缩略图，请选择&#x200B;**[!UICONTROL 切换到自动缩略图。]**
...
1. 单击&#x200B;**[!UICONTROL 保存。]**&#x200B;您新创建的旋转集将显示在创建时所用的文件夹中。

## 查看旋转集{#viewing-spin-sets}

您可以在用户界面中创建旋转集，也可以使用[批集预设](/help/assets/config-dms7.md#creating-batch-set-presets-to-auto-generate-image-sets-and-spin-sets)自动创建旋转集。 但是，使用批集预设创建的集，请执行&#x200B;*not*&#x200B;操作，以在用户界面中显示。 您可以通过三种不同方式访问通过批集预设创建的集。 （即使您在用户界面中创建了旋转集，这些方法也可用）。
...
1. 打开单个资产的属性时。 属性指示所选资产是其成员（位于“集成成员”]**下）。**[!UICONTROL &#x200B;单击集的名称可查看整个集。
...
1. 在搜索中，您可以选择&#x200B;**[!UICONTROL 过滤器]**，然后展开&#x200B;**[!UICONTROL Dynamic Media]**&#x200B;并选择&#x200B;**[!UICONTROL 集。]**
   ...
   搜索会返回在UI中手动创建或通过批集预设自动创建的匹配集。 对于自动集，搜索查询使用与基于使用`Contains`搜索条件的AEM搜索不同的`Starts with`搜索条件。 将筛选器设置为&#x200B;**[!UICONTROL Sets]**&#x200B;是搜索自动集的唯一方法。
...
1. 要编辑旋转集，请执行以下任意操作：
   ...
   * 要按升序或降序对项目排序，请单击列标题。
   * 要添加资产或更新现有资产，请单击&#x200B;**[!UICONTROL 添加资产。]** 导航到资产，选择它，然后点 **** 按右上角附近的选择。如果您通过将AEM用作缩略图的图像替换为其他图像来删除该图像，则仍会显示原始资产。
   ...
   * 要删除整个旋转集，请导航到该旋转集，将其选中，然后选择&#x200B;**[!UICONTROL 删除]**
   ...
   >[!NOTE]
   >
   >您可以通过导航到旋转集中来编辑旋转集中的图像，点按左边栏中的&#x200B;**[!UICONTROL 设置成员]**，然后点按单个资产上的铅笔图标以打开编辑窗口。
