...
360度视频同时记录每个方向的视图。 它们是用全方位相机或一组相机拍摄的。 在平面显示器上播放期间，用户可以控制观看角度；移动设备上的播放通常利用其内置的陀螺仪控件。
...
点按[空间站360](http://mobiletest.scene7.com/s7viewers/html5/Video360Viewer.html?asset=Viewers/space_station_360-AVS)以打开浏览器窗口并观看360度视频。 在视频播放期间，将鼠标指针拖动到新位置以更改观看角度。
...
上传到AEM中的360个视频资产在资产页面上标记为&#x200B;**Multimedia**，与普通视频资产类似。
...
1. [将自适应视频配置文件应用到该文件夹。](/help/assets/video-profiles.md#applying-a-video-profile-to-folders)
   ...
   您可以使用已随附的现成自适应视频配置文件Dynamic Media。 但是，请注意，对于使用非360视频查看器呈现的相同设置进行编码的非360视频，其视频质量将显着低于您获得的360视频质量。 因此，如果需要高品质的360视频，请执行以下操作：

   * 理想情况下，原始360视频内容应具有以下任一分辨率：

      * 1080p - 1920 x 1080，称为全高清或全高清分辨率，或
      * 2160p - 3840 x 2160，称为4K、UHD或UltraHD分辨率。 这种非常大的显示分辨率通常出现在高档电视机和电脑显示器上。 2160p分辨率通常称为“4K”，因为宽度接近4000像素。 换句话说，它的像素数是1080p的四倍。
   * [创建具有更高质量演绎版的自](/help/assets/video-profiles.md#creating-a-video-encoding-profile-for-adaptive-streaming) 定义自适应视频配置文件。例如，您可能要创建包含以下三个设置的自适应视频配置文件：
...
默认情况下，如果视频的宽高比（宽高）为2.0，则AEM会将视频检测为“360”。如果您是管理员，则可以通过在CRXDE Lite中设置可选的`s7video360AR`属性（如下所示）来覆盖默认宽高比设置2:
...
](https://helpx.adobe.com/experience-manager/6-5/help/assets/embed-code.html)请参阅[将 URL 关联到您的 Web 应用程序](https://helpx.adobe.com/experience-manager/6-5/help/assets/linking-urls-to-yourwebapplication.html)。请注意，如果您的交互式内容具有包含相对URL的链接，特别是指向AEM Sites页面的链接，则无法使用基于URL的链接方法。
请参阅[将Dynamic Media Assets添加到页面。](https://helpx.adobe.com/experience-manager/6-5/help/assets/adding-dynamic-media-assets-to-pages.html)

**预览360个视频**

1. 在&#x200B;**[!UICONTROL Assets]**&#x200B;中，导航到您创建的现有360视频。 点按360视频资产，以在预览模式下将其打开。
...
1. 在预览页面的左上角附近，点按下拉列表，然后选择&#x200B;**[!UICONTROL 查看器。]**
   ...
   * 在视频中拖动鼠标指针以更改静态场景的观看角度。
   * 点按视频的&#x200B;**[!UICONTROL Play]**&#x200B;按钮以开始播放；在视频播放时，将鼠标指针拖动到视频上以更改观看角度。
   ...
   * 从查看器列表中，点按&#x200B;**[!UICONTROL Video360VR。]**
      ...
      虚拟现实(VR)视频是一种沉浸式视频内容，通过使用虚拟现实耳机来访问该内容。 与普通视频一样，您可以在视频被录制或使用360度摄像机捕获时，在视频开始时创建VR视频。
...
您需要发布360视频才能使用它。 发布360视频可激活URL和嵌入代码。 它还会将360视频发布到Dynamic Media云，该云与CDN集成以进行可扩展且性能出众的交付。
...
另请参阅[将URL关联到Web应用程序](https://helpx.adobe.com/experience-manager/6-5/help/assets/linking-urls-to-yourwebapplication.html)。 请注意，如果您的交互式内容具有包含相对URL的链接，特别是指向AEM Sites页面的链接，则无法使用基于URL的链接方法。
另请参阅[将Dynamic Media Assets添加到页面。](https://helpx.adobe.com/experience-manager/6-5/help/assets/adding-dynamic-media-assets-to-pages.html)
