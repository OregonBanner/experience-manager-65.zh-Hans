...
通过 Dynamic Media，您可以在发布资产前对其进行管理。在中详细介绍了如何一般处理资产 [使用数字资产](manage-assets.md). 一般主题包括上传、下载、编辑和发布资产；查看和编辑属性，以及搜索资产。
...
选择图像资产时，资产的视图与Dynamic Media不同 [已启用](config-dynamic.md#enabling-dynamic-media). Dynamic Media使用按需HTML5查看器。
