...
要修改大小限制，请配置 `chunkUploadMinFileSize` 属性 `fileupload` CRX存储库中的节点(位于 `/apps/dam/gui/content/assets/jcr:content/actions/secondary/create/items/fileupload`.
...
您可以选择替换现有资产、创建其他版本，或通过重命名上传的新资产来同时保留这两个资产。 如果您替换现有资产，则资产的元数据以及您对现有资产所做的任何先前修改（例如添加批注或裁剪）都将被删除。 如果您选择保留这两个资产，则新资产将重命名为数字 `1` 附加到其名称。
...
|  | PDF选项 | 您可以栅格化文件、提取搜索词和链接、自动生成eCatalog、设置分辨率并选择色彩空间。<br>eCatalog在 [!DNL Experience Manager]. <br> 请参阅 [设置PDF上载选项](#setting-pdf-upload-options).<br>**注意**:要考虑提取的PDF的最大页面数是5000，用于新上载。 2022年12月31日，此限制将更改为100页(适用于所有PDF)。 另请参阅 [Dynamic Media限制](/help/assets/limitations.md). |
...
|  | 桌面 | 创建MP4文件，以将流式或渐进式视频体验交付到台式计算机。 选择一个或多个具有所需分辨率大小和目标数据速率的纵横比。 |
...
1. 在预览模式下，缩放选项可用于 [支持的图像类型](/help/assets/assets-formats.md#supported-raster-image-formats) （通过交互式编辑）。
   ...
   要放大资产，请单击 `+` （或单击资产上的放大镜）。 要缩小，请单击 `-`. 放大时，可以通过平移来仔细查看图像的任何区域。 使用重置缩放箭头可返回原始视图。 要将视图重置为原始大小，请单击 **[!UICONTROL 重置]** ![重置视图](assets/do-not-localize/revert.png).
...
1. 选择演绎版以查看或删除演绎版。
   ...
   **上传新演绎版**
   ...
   >[!NOTE]
   >
   >视频注释功能仅在具有与HTML5兼容的视频格式的浏览器上受支持。 此外，还支持不同的视频格式，具体视浏览器而定。 但是，视频批注尚不支持MXF视频格式。
...
只有使用与HTML5兼容的视频格式的浏览器才支持视频注释。 视频格式 [!DNL Assets] 支持依赖于浏览器。 但是，视频批注尚不支持MXF视频格式。
...
1. 要通知用户有关注释的信息，请指定用户的电子邮件地址并添加评论。 例如，要通知Aaron MacDonald有关注释的信息，请输入@aa。 所有匹配用户的提示都会显示在列表中。 从列表中选择Aaron的电子邮件地址，以便您可以标记具有评论的人员。 同样，您也可以在批注中的任意位置、批注前后标记更多用户。
...
1. 要查看通知，请登录 [!DNL Assets] 使用Aaron MacDonald的凭据，然后单击 **[!UICONTROL 通知]** 选项来查看通知。
   ...
   >[!NOTE]
   >
   >您也可以向视频资产中添加注释。 在对视频添加注释时，播放器会暂停，以允许您对帧添加注释。 有关详细信息，请参阅 [管理视频资产](/help/assets/managing-video-assets.md). 视频批注尚不支持MXF视频格式。
...
版本控制可创建数字资产在特定时间点的快照。 版本控制可帮助在以后将资产恢复到以前的状态。 例如，如果要撤消对资产所做的更改，请恢复该资产未经编辑的版本。 在 [!DNL Experience Manager]，您可以创建一个版本、查看当前修订版本、查看两个图像版本之间的并排差异，以及将资产恢复到之前的版本。
