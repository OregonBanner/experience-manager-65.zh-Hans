...
* **标记**：标记可帮助您更有效地对可浏览和搜索的资源进行分类。 标记有助于将适当的分类传播给其他用户和工作流程。[!DNL Experience Manager] 提供了一些方法，可使用Adobe Sensei的人工智能服务自动标记资源，从而更好地通过使用和培训来标记资源。 搜索资产时，如果帐户中启用了智能标记功能，则会将智能标记考虑在内。 它与内置搜索功能配合使用。 请参阅 [搜索行为](#searchbehavior). 要优化搜索结果的显示顺序，您可以 [提升搜索排名](#searchrank) 的一些精选资源的。
