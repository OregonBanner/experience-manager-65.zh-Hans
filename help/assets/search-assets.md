...
使用[!DNL Experience Manager] Web界面顶部的Omnisearch字段搜索资产。 转到&#x200B;**[!UICONTROL Assets]** > **[!UICONTROL [!DNL Experience Manager]中的文件]**，单击顶栏中的![search_icon](assets/do-not-localize/search_icon.png)，输入搜索关键字，然后选择`Return`。 或者，使用关键字快捷键`/`（正斜杠）打开Omnisearch字段。 `Location:Assets` 已预选以限制对DAM资产的搜索。[!DNL Experience Manager] 在您开始键入搜索关键词时提供建议。

使用&#x200B;**[!UICONTROL 过滤器]**&#x200B;面板可搜索资产、文件夹、标记和元数据。 您可以根据各种选项（谓词）筛选搜索结果，例如文件类型、文件大小、上次修改日期、资产状态、分析数据和Adobe Stock许可。 您可以自定义“过滤器”面板，并使用[搜索彩块化](/help/assets/search-facets.md)添加或删除搜索谓词。 [!UICONTROL 过滤器]面板中的[!UICONTROL 文件类型]过滤器具有混合状态复选框。 因此，除非选择所有嵌套谓词（或格式），否则会部分选中第一级复选框。
...
**A.将** 搜索另存为智能收藏集。**B.过** 滤或谓词以缩小搜索结果。**C.显** 示文件、文件夹或两者。**D.** 单击“过滤器”以打开或关闭左边栏。**E.** 搜索位置为 DAM。**F.Omnisearch** 字段中包含用户提供的搜索关键词。**G.选** 择加载的搜索结果。**H.显** 示的搜索结果在总搜索结果中的数量。**I.关** 闭搜索。**J.在** 卡片视图和列表视图之间切换。
...
您可以从OmniSearch字段运行关键词搜索。 关键字搜索不区分大小写，是全文搜索（在常用元数据字段中）。 如果使用多个关键词，则`AND`是关键词之间的默认运算符。

结果按相关性排序，从最接近的匹配开始。 对于多个关键字，更相关的结果是元数据中同时包含这两个术语的资产。 在元数据中，显示为智能标记的关键词的排名高于其他元数据字段中显示的关键词。 [!DNL Experience Manager] 允许为特定搜索词赋予更高的权重。此外，还可以[提升特定搜索词的少数目标资产的排名](#searchrank)。
...
当结果为多个资产时，[!DNL Experience Manager]在卡片视图中显示前100个，在列表视图中显示200个。 用户滚动时，会加载更多资产。 这是为了提高性能。 观看显示的[资产数量](https://www.youtube.com/watch?v=LcrGPDLDf4o)的视频演示。
...
默认情况下，[!DNL Experience Manager]搜索将搜索词与AND子句组合在一起。 例如，请考虑搜索关键词“女士正在运行”。 默认情况下，搜索结果中仅显示元数据中同时具有女性和运行关键词的资产。 在关键词中使用特殊字符（句点、下划线或短划线）时，将保留相同的行为。 以下搜索查询返回相同的结果：

* `woman running`
* `woman.running`
* `woman-running`

但是，查询`woman -running`会返回其元数据中没有`running`的资产。
使用智能标记可添加额外的`OR`子句，以查找任何搜索词作为应用的智能标记。 此类搜索查询中还会显示使用智能标记标记为`woman`或`running`的资产。 所以搜索结果是，
...
开始键入关键词时，[!DNL Experience Manager]建议可能的搜索关键词或短语。 建议基于现有资产的元数据。 [!DNL Experience Manager] 对所有元数据字段进行索引，以帮助进行搜索。为了提供搜索建议，系统使用以下几个元数据字段的值。 要提供搜索建议，请考虑使用适当的关键字填充以下字段：
...
若要接收多个搜索关键词的建议，请继续键入所有关键词，而不为单个关键词选择任何建议。

![键入多个关键词以查看适合所有关键词的建议](assets/search_suggestionsmanykeywords.gif)

*图：键入多个关键词，以查看适合所有关键词的建议。*
...
1. 从[!DNL Assets]用户界面中，打开资产的属性页面。 单击&#x200B;**[!UICONTROL 高级]**，然后单击&#x200B;**[!UICONTROL 提升搜索关键词]**&#x200B;下的&#x200B;**[!UICONTROL 添加]**。
1. 在&#x200B;**[!UICONTROL 搜索提升]**&#x200B;框中，指定要提升对图像搜索的关键字，然后单击&#x200B;**[!UICONTROL 添加]**。 您可以以相同方式指定多个关键词。
1. 单击&#x200B;**[!UICONTROL 保存并关闭]**。您为此关键词促销的资产会显示在热门搜索结果中。

您可以通过提升目标关键字的搜索结果中某些资产的排名，来使用此功能。 请观看下面的视频示例。 有关详细信息，请参阅 [!DNL Experience Manager]](https://experienceleague.adobe.com/docs/experience-manager-learn/assets/search-and-discovery/search-boost.html)中的[搜索。
...
[!DNL Experience Manager] 提供了各种方法，如应用于搜索的资产的过滤器，以帮助您更快地找到所需的资产。下面介绍了一些常用的方法。 下面共享了一些[图例](#samples)。

**搜索文件或文件夹**:在搜索结果中，查看文件、文件夹或两者。从&#x200B;**[!UICONTROL Filters]**&#x200B;面板中，可以选择相应的选项。 请参阅[搜索接口](#searchui)。

**在文件夹中搜索资产**:您可以将搜索限制为特定文件夹。在&#x200B;**[!UICONTROL 筛选器]**&#x200B;面板中，添加文件夹的路径。 一次只能选择一个文件夹。
...
不能将属性`path`、`limit`、`size`和`orderby`与任何其他属性一起使用`OR`运算符进行组合。
...
| 名称 | 值 | 示例 | 用途 |
|---|---|---|---|
| 资源后缀(B) | 作为URL中资源后缀的文件夹路径：[https://localhost:4502/aem/assetpicker.html/&lt;folder_path>](https://localhost:4502/aem/assetpicker.html) | 要启动资产选择器并选择特定文件夹（例如选择文件夹`/content/dam/we-retail/en/activities`），URL应采用以下格式：`https://localhost:4502/aem/assetpicker.html/content/dam/we-retail/en/activities?assettype=images` | 如果在启动资产选择器时需要选择特定文件夹，则会将其作为资源后缀传递。 |
...
* 如果选择`Return`而未在Omnisearch栏中键入内容，则[!DNL Experience Manager]将返回仅包含文件而不包含文件夹的列表。 如果您在不使用关键字的情况下专门搜索文件夹，则[!DNL Experience Manager]不会返回任何结果。
...
* 使用&#x200B;**[!UICONTROL 全选]**&#x200B;复选框选择搜索的资产。 [!DNL Experience Manager] 最初在卡片视图中显示100个资产，在列表视图中显示200个资产。滚动搜索结果时会加载更多资产。 您可以选择的资产比加载的资产多。 选定资产的计数显示在搜索结果页面的右上角。 您可以对所选内容进行操作，例如下载选定的资产、批量更新选定资产的元数据属性，或将选定的资产添加到收藏集。 选择的资产多于显示的资产时，会对所有选定的资产应用一项操作，或者出现一个对话框，显示所应用的资产数量。 要对未加载的资产应用操作，请确保明确选择了所有资产。
...
* 要从搜索结果中排除特定资产，请在Lucene索引中使用`excludedPath`属性。
...
* `run` 返回具有完全运行关键字的资产
* `run*` 通过、  `running`、  `run`等返 `runaway`回资产。
* `*run` 通过、 `outrun`等 `rerun`返回资产。
* `*run*` 返回所有可能的组合。
...
* `run????` 查询匹配后面 `running` 有四个字符的 `run`词。

* `??run` 查询匹配之前 `rerun` 包含两个字符的 `run`词。

![通过一个示例说明在资产搜索中使用问号通配符的方法](assets/search_with_questionmark_run.gif)

*图：通过示例说明了如何在资产搜索中使用问号通配符。*

**排除关键词**:使用短划线搜索不包含关键字的资产。例如， `running -shoe`查询会返回包含`running`但不包含`shoe`的资产。 同样， `camp -night`查询会返回包含`camp`但不包含`night`的资产。 查询`camp-night`返回同时包含`camp`和`night`的资产。
...
资产发现依赖于DAM内容的索引，包括元数据。 更快、更准确的资产发现依赖于优化的索引和适当的配置。 请参阅[搜索索引](/help/assets/performance-tuning-guidelines.md#search-indexes)、[oak查询和索引](/help/sites-deploying/queries-and-indexing.md)和[最佳实践](/help/sites-deploying/best-practices-for-queries-and-indexing.md)。
...
1. 在[!DNL Experience Manager] CRXDE的`/oak:index/lucene`节点中，添加以下属性和值并保存更改。
   ...
   * `costPerEntry` 属性， `Double` 其值为 `10`。
   * `costPerExecution` 属性， `Double` 其值为 `2`。
   * `refresh` 属性， `Boolean` 其值为 `true`。

   此配置允许从相应的索引进行搜索。

1. 要创建Lucene索引，请在CRXDE的`/oak:index/damAssetLucene/indexRules/dam:Asset/properties`处，创建名为`imageFeatures`的`nt-unstructured`类型的节点。 在`imageFeatures`节点中，

   * 添加`String`类型的`name`属性，其值为`jcr:content/metadata/imageFeatures/haystack0`。
   * 添加`Boolean`类型的`nodeScopeIndex`属性，其值为`true`。
   * 添加`Boolean`类型的`propertyIndex`属性，其值为`true`。
   * 添加`Boolean`类型的`useInSimilarity`属性，其值为`true`。
...
有关相关信息，请参阅[了解Experience Manager](https://experienceleague.adobe.com/docs/experience-manager-learn/assets/metadata/image-smart-tags.html)中的智能标记，以及[如何管理智能标记](/help/assets/enhanced-smart-tags.md)。
...
业务用户、管理员或DAM管理员可以将某些元数据定义为必须的元数据，这是业务流程正常工作所必须的元数据。 由于各种原因，某些资产可能缺少此元数据，例如批量迁移的旧版资产或资产。 系统会根据索引的元数据属性检测和报告元数据缺失或无效的资产。 要对其进行配置，请参阅[必需元数据](/help/assets/metadata-schemas.md#define-mandatory-metadata)。

### 修改搜索彩块化 {#searchfacets}

为了提高搜索速度，[!DNL Experience Manager Assets]提供了搜索Facet，您可以使用这些Facet筛选搜索结果。 默认情况下，“过滤器”面板包含一些标准Facet。 管理员可以自定义过滤器面板，以使用内置谓词修改默认Facet。 [!DNL Experience Manager] 提供了一组良好的内置谓词以及用于自定义facet的编辑器。请参阅[搜索彩块化](/help/assets/search-facets.md)。
...
谓词用于创建Facet。 管理员可以使用预配置的谓词，在“过滤器”面板中自定义搜索彩块化。 可以使用叠加图自定义这些谓词。 请参阅[创建自定义谓词](/help/assets/searchx.md)。
...
您可以对您在[!DNL Experience Manager]中搜索的资产执行以下操作：
...
对搜索结果排序，以更快地发现所需的资产。 只有在从&#x200B;**[!UICONTROL Filters]**&#x200B;面板中选择&#x200B;**[[!UICONTROL Files]](#searchui)**&#x200B;时，才能在列表视图中对搜索结果排序。 [!DNL Assets] 使用服务器端排序来快速对文件夹或搜索查询结果中的所有资产（无论数量多少）进行排序。与客户端排序相比，服务器端排序提供更快、更准确的结果。
...
可以批量更新多个资产的常用元数据字段。 从搜索结果中，选择一个或多个资产。 单击工具栏中的&#x200B;**[!UICONTROL 属性]** ，然后根据需要更新元数据。 完成后，单击&#x200B;**[!UICONTROL 保存并关闭]**。 更新字段中以前存在的元数据将被覆盖。
...
|---|---|---|
| 搜索缺少元数据的资产时，结果不正确。 | 在搜索缺少必需元数据的资产时，[!DNL Experience Manager]可能会显示一些具有有效元数据的资产。 结果基于索引的元数据属性。 | 更新元数据后，需要重新编制索引，以反映资产元数据的正确状态。 请参阅[必需元数据](metadata-schemas.md#define-mandatory-metadata)。 |
| 搜索结果过多。 | 广泛搜索参数。 | 请考虑限制[搜索范围](#scope)。 使用智能标记可能会提供比预期更多的搜索结果。 请参阅[使用智能标记的搜索行为](#withsmarttags)。 |
| 无关或部分相关的搜索结果。 | 搜索行为会随智能标记而发生更改。 | 了解[智能标记](#withsmarttags)后搜索的变化情况。 |
| 没有自动完成有关资产的建议。 | 尚未对新上传的资产建立索引。 当您开始在Omnisearch栏中键入搜索关键词时，元数据并非立即可用，因为提供了建议。 | [!DNL Experience Manager] 会等到超时时段（默认为一小时）到期，然后运行后台作业以索引所有新上传或更新的资产的元数据，然后将该元数据添加到建议列表。 |
...
| 搜索筛选器或谓词不可用。 | <ul><li>未配置搜索过滤器。</li><li>无法用于登录。</li><li>（不太可能）未在您使用的部署中自定义搜索选项。</li></ul> | <ul><li>请联系管理员以检查搜索自定义是否可用。</li><li>请联系管理员以检查您的帐户是否具有使用自定义设置的权限。</li><li>请联系管理员并检查您所使用的[!DNL Assets]部署的可用自定义设置。</li></ul> |
| 在搜索视觉上相似的图像时，缺少预期的图像。 | <ul><li>映像在[!DNL Experience Manager]中不可用。</li><li>图像未编入索引。 通常，在最近上传时。</li><li>图像未智能标记。</li></ul> | <ul><li>将图像添加到[!DNL Assets]。</li><li>请与管理员联系以重新编入存储库索引。 此外，请确保您使用了相应的索引。</li><li>请联系您的管理员以智能标记相关资产。</li></ul> |
...
| 选择并运行搜索结果时，所有搜索的资产都不会运行。 | [!UICONTROL 全选]选项仅选择卡片视图中的前100个搜索结果和列表视图中的前200个搜索结果。 |  |
