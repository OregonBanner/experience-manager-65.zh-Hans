...
要使用ImageMagick处理大型文件，请考虑高于通常的内存要求、IM策略所需的潜在更改以及总体性能影响。 内存要求取决于分辨率、位深度、颜色配置文件和文件格式等多种因素。 如果您要使用ImageMagick处理非常大的文件，请对 [!DNL Experience Manager] 服务器。 最后提供了一些有用的资源。
