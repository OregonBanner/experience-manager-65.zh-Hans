...
在进行正确配置后，您向“站点”页面添加的任何Dynamic Media查看器(使用Dynamic Media支持的WCM组件)将自动跟踪到Adobe Analytics或Adobe Analytics的视频数据，或同时跟踪到这两者。
...
请参阅[Dynamic Media查看器参考指南](https://experienceleague.adobe.com/docs/dynamic-media-developer-resources/library/viewers-aem-assets-dmc/c-html5-s7-aem-asset-viewers.html#viewers-aem-assets-dmc)，了解每个查看器类型所支持事件的列表;转到特定查看器部分，然后单击“支持Adobe Analytics跟踪”子部分。 目前，Dynamic Media查看器参考指南不文档事件参数。
...
* [!DNL Adobe I/O] 控制台——为Adobe启动创建集成。
...
有关设置Adobe启动云配置的详细信息，请参阅[将AEM与Adobe启动集成（通过 [!DNL Adobe I/O]](https://helpx.adobe.com/experience-manager/using/aem_launch_adobeio_integration.html)）。
