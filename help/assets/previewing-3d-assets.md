...
Experience manager支持3D资产的上传、交付和交互式预览，这是创作流程的一部分。
...
| GLTF | GL传输格式 | model/gltf+json | 请参 **阅以下** 注释。 |
...
**注意**:如果材料未在gLTF模型的预览中呈现，请确保它们的名称正确，并且位于与模型相同的根文件夹中的文件夹中，如下所示： `textures`
...
在资产详细信息视图页面中打开3D资产所花费的时间取决于几个因素，如带宽、图像复杂性和服务器延迟。

此外，在以交互方式操作相机时，客户机（如工作站、笔记本或移动触控设备）的功能也很重要。 具备良好图形功能的相当强大的系统可以使交互式 3D 查看体验更流畅、更舒适。

**预览3D资产**

1. 确保您已将 3D 资产上传到 AEM。请参 [阅3D预览和上传资产的](#supported-3d-previewing-assets)[支持格式](/help/assets/managing-assets-touch-ui.md#uploading-assets)。
1. 从AEM的导航页面 **[!UICONTROL 上]** ，点按资 **[!UICONTROL 产>文件]**。

   ![导航页面](/help/assets/assets-dm/navigation-assets.png)

1. 在页面的右上角附近，从“视图”下拉列表中，点按卡片视图 ****，然后导航到要预览的3D资产。
...
1. 在3D资产的资产详细信息视图页面上，执行下列任一操作：
   ...
   * **重新输入相机**-将相机重新输入到3D场景中对象上的某个点。
      * _鼠标_:双击。
      * _触摸屏_:双击。
   * **重置**-在页面的右下角附近，点按重置图标以将视图目标点恢复到3D资产的中心。 重置还会使相机更近或更远地移动，以便以合理的查看大小显示整个资产。
   * **全屏模式**-要进入全屏模式，请点按页面右下角的全屏图标。

1. 完成后，在页面的右上角附近，点按关 **[!UICONTROL 闭]**。
