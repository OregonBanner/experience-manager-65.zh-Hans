---
title: 管理数字资产集合
description: 了解用于管理资产集合的任务，如创建、视图、删除、编辑和下载集合。
contentOwner: AG
mini-toc-levels: 1
...
要打开收藏 **[!UICONTROL 集]**，请在Experience Manager界面中，转到“资产” **[!UICONTROL >“收藏]** 集” ****。
...
1. 将资产拖至“放 **[!UICONTROL 入收藏集”区域]** 。 或者，单击工 **[!UICONTROL 具栏中的]** “到集合”图标。
...
1. 在“Omnisearch”（搜索）框中键入search关键字，然后按 `Enter`键。 打开过滤器面板并应用搜索筛选器。
...
1. 在“资产”用户界面中，选择资产，然后单击工 **[!UICONTROL 具栏中的]** “收藏集”图标。
...
1. 单击 **[!UICONTROL 覆盖]** ，将原始智能收藏集替换为已编辑的收藏集。 或者，选择另 **[!UICONTROL 存为]** ，以单独保存编辑的集合。
...
1. 根据需要修改元数据，然后单击工 **[!UICONTROL 具栏中的保存并关闭]** ，以保存更改。
...
1. 单击 **[!UICONTROL 工具栏中的保存并关闭]** ，然后关闭确认对话框以完成该过程。
1. To append the new metadata with the existing metadata, select **[!UICONTROL Append mode]**. 如果不选中此选项，则新元数据将替换字段中的现有元数据。单击 **[!UICONTROL 提交]**。
   ...
   >[!NOTE]
   >
   >您为所选集合添加的元数据将覆盖这些集合的先前元数据。 使用“ [!UICONTROL 追加”模式] ，在可包含多个值的字段中向现有元数据添加新值。 始终覆盖单值字段。 您在“标记”字段中添 [!UICONTROL 加的任何标记] ，都会附加到元数据中标记的现有列表中。
...
1. 选择一个集合，然后单击工 **[!UICONTROL 具栏中]** 的设置图标。 或者，也可以使 **[!UICONTROL 用集合缩览图]** 中的设置快速操作。
...
1. 在工具栏中，单击下载图标。
1. 在“下 **[!UICONTROL 载]** ”对话框中，单 **[!UICONTROL 击“下载]**”。 如果要下载集合中资产的演绎版，请选择演 **[!UICONTROL 绎版]**。 选择“电 **[!UICONTROL 子邮件]** ”选项，向集合所有者发送电子邮件通知。
...
1. 从收藏集控制台中，选择所需的收藏集或收藏集组，然后单击工 **[!UICONTROL 具栏中的]** “到收藏集”。
...
1. 单击&#x200B;**[!UICONTROL 添加]**。系统会显示一条消息，确认该集合已添加到“选择目标”页面中的 **[!UICONTROL 目标集合]** 。 关闭消息以完成该过程。
...
1. 单击GlobalNav图标，然后从列表 **[!UICONTROL 中选择]** “时间轴”。
...
1. 在对话框中，单击“继 **[!UICONTROL 续”]**。 该工作流将在集合中的所有资产上运行。
