...
集合是Adobe Experience Manager(AEM)资产中的一组资产。 使用收藏集可在用户之间共享资产。

* 一个收藏集可以包含来自不同位置的资产。
* 您可以与分配了不同权限级别（包括查看、编辑等）的不同用户共享集合。
...
1. 点按或单击AEM徽标。
1. 在导航页面中，转到资产 **** >收藏 **[!UICONTROL 集]**。 将显 **[!UICONTROL 示“集合]** ”控制台。
...
1. 向收藏集添加成员并分配适当的权限。或者，选择 **[!UICONTROL 公共集合]** ，以允许所有用户访问该集合。
...
1. （可选）为集合添加缩略图。
1. 点按／单 **[!UICONTROL 击创建]**，然后点按／单 **[!UICONTROL 击确定]** ，关闭对话框。 具有指定标题和属性的集合将在“集合”控制台中打开。

   >[!NOTE]
   >
   >AEM资产允许您为收藏集创建审核任务，这与为资产文件夹创建审核任务的方式类似。
...
1. 在添加到 **[!UICONTROL 收藏集页面中]** ，点按／单击工具 **[!UICONTROL 栏中的创建收藏集图标]** 。

   如果要将资产添加到现有收藏集，请从页面中选择它，然后点按／单击添 **[!UICONTROL 加]**。 默认情况下，将选择最近更新的集合。

1. 在“创 **[!UICONTROL 建新集合]** ”对话框中，指定集合的名称。 如果希望所有用户都可以访问该集合，请选择“公 **[!UICONTROL 共集合”]**。
...
1. 在Omnisearch框中输入搜索关键字，然后按Enter。 点按／单击GlobalNav图标以显示“筛选器”面板，并从“搜索”面板应用搜索筛选器。
...
1. 选择资产，然后点按／单击工 **[!UICONTROL 具栏中的]** “到集合”图标。

   ![chlimage_1-8](assets/chlimage_1-113.png)

   或者，您也可以将资产拖动到放 **[!UICONTROL 入收藏集区]** 。 当放置区域变为活动状态且其标签变为“放置到添加”时， **[!UICONTROL 松开鼠标按钮]**。
...
1. 在资产UI中，点按／单击工具 **[!UICONTROL 栏中的]** “搜索”图标。
...
1. 在“集合 **[!UICONTROL 元数据]** ”页面中，从“基本”和“高级”选项卡中查 **[!UICONTROL 看集合]** 元 **数据** 。
...
1. 在“集合 **[!UICONTROL 元数据]** ”页面中，根据需要编辑“基本”和“高级”选 **[!UICONTROL 项卡下]** 的元数据 **** 。
1. 点按／单 **[!UICONTROL 击工具栏中的保存并关闭]** ，然后关闭确认对话框以完成该过程。
1. 要将新元数据与现有元数据一起附加，请选择“ **[!UICONTROL 结束”模式]**。 如果不选择此选项，则新元数据将替换字段中的现有元数据。 点按／单击 **[!UICONTROL 提交]**。

   >[!NOTE]
   >
   >“追加”模式仅适用于可包含多个值的字段。 对于只能包含单个值的字段，即使选择“追加”模式，新元数据也不会附加到字段中的现有 **[!UICONTROL 值中]**。
...
在资产UI中，您可以根据某些规则、搜索条件或自定义搜索彩块化来搜索或筛选资产。 If you save these as **[!UICONTROL Saved Searches]**, you can access them later from the **[!UICONTROL Saved Searches]** list in the Filter panel. 创建保存的搜索也会创建智能收藏集。
...
1. 导航到资产UI，然后点按／单击工具栏中的搜索图标。

   ![chlimage_1-13](assets/chlimage_1-114.png)

1. 在“Omni Search”（全景搜索）框中，按Return键。
1. 单击或点按GlobalNav图标以显示“滤镜”面板。

1. 从“保 **[!UICONTROL 存的搜索]** ”列表中，点按 **** 要删除的智能收藏集旁边的删除。

   ![select_smart_collection_1](assets/select_smart_collection-1.png)

1. 在对话框中，点按删 **[!UICONTROL 除]** ，以删除保存的搜索。
...
1. 在时间轴中，单击或点按底部的尖角图标，然后点按开始工 **[!UICONTROL 作流]**。

   ![chlimage_1-14](assets/chlimage_1-137.png)

1. 在“开始 **[!UICONTROL 工作流]** ”(Start Workflow)部分，从列表中选择工作流模型。 例如，选择 **[!UICONTROL DAM更新资产模型]** 。
1. 输入工作流的标题，然后点按／单击开 **[!UICONTROL 始]**。
1. 在对话框中，点按／单击继 **[!UICONTROL 续]**。 该工作流将在集合中的所有资产上运行。

>[!MORELIKETHIS]
>
>* [配置AEM资产电子邮件通知](/help/sites-administering/notification.md#assetsconfig)
>* [编辑多个集合的元数据属性](/help/assets/managing-multiple-assets.md)
