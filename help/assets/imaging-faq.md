...
* 使用其规则集中的自定义标头(例如，[向图像响应添加自定义标头值|Dynamic Media Classic](https://helpx.adobe.com/experience-manager/scene7/kb/base/scene7-rulesets/add-custom-header-val-image.html)中建议的“定时允许来源”、“X-Robot”)的客户将从最新的智能映像中受益，因为这些标头不会被阻止，这与以前版本的智能映像不同。

## 是否存在与智能成像相关的许可成本？{#are-there-any-licensing-costs-associated-with-smart-imaging}

否. 您现有的Dynamic Media Classic或Adobe Experience Manager- Dynamic Media(作为Cloud Service，在Prem、AMS和AEM上)许可证中包含智能成像。
...
1. 在您的支持案例中提供以下信息：
   ...
   1. 主要联系人姓名、电子邮件、电话。
   1. 要启用智能映像的所有域（即`images.company.com`或`mycompany.scene7.com`）。
   ...
   1. 验证您使用的是专用域（如`images.company.com`或`mycompany.scene7.com`），而不是通用域（如`s7d1.scene7.com`、`s7d2.scene7.com`和`s7d13.scene7.com`）。
      ...
      查找标有&#x200B;**[!UICONTROL “已发布服务器名称”的字段。]** 如果您当前使用的是通用的Dynamic Media Classic域，则可以请求移至您自己的自定义域作为此过渡的一部分。
...
1. 为最大限度地提高智能成像的性能，Adobe建议将生存时间(TTL)设置为24小时或更长。 TTL定义CDN缓存资源的时长。 要更改此设置：
   ...
   1. 如果您使用Dynamic Media Classic，请单击&#x200B;**[!UICONTROL 设置>应用程序设置>发布设置>图像服务器。]** 将默认客 **[!UICONTROL 户端缓存时间设置]** 为“实时值”24或更长。
