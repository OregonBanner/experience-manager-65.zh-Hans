...
仅当您 *not* 使用Experience Manager作为WCM。 链接 — 与嵌入 — 用于将视频播放器作为弹出窗口或模式窗口进行传递。 如果您使用Experience Manager作为WCM， [您可以直接在页面上添加资产](adding-dynamic-media-assets-to-pages.md).
...
1. 在 **[!UICONTROL Dynamic Media Cloud设置]** 页面，下 **[!UICONTROL 视频服务URL]**，向下复制整个URL路径。 您需要在后续步骤中访问复制的URL路径。
