---
title: AEM Assets HTTP API中的内容片段支持
seo-title: AEM Assets HTTP API中的内容片段支持
...
Adobe Experience Manager(AEM) [Assets REST API](/help/assets/mac-api-assets.md) 允许开发人员通过HTTP API直接访问内容（存储在AEM中），方法是通过CRUD操作（创建、读取、更新、删除）。
...
虽然AEM核心组件提供了非常全面、灵活且可自定义的API，可为此目的提供所需的读取操作，并且其JSON输出可以自定义，但它们确实需要AEM WCM（Web内容管理）专业知识来实现，因为这些知识必须在基于专用AEM模板的(API)页面中托管。 并非每个SPA开发组织都能够访问此类资源。

此时，可以使用资产REST API。 它允许开发人员直接访问资产（例如，图像和内容片段），无需先将资产嵌入到页面中，然后以序列化JSON格式交付其内容。 （请注意，无法从Assets REST API自定义JSON输出）。 Assets REST API还允许开发人员通过创建新资产、更新或删除现有资产、内容片段和文件夹来修改内容。
...
资产REST API允许 [REST](https://en.wikipedia.org/wiki/Representational_state_transfer)-style访问存储在AEM实例中的资产。 它使用端 `/api/assets` 点并需要资产的路径才能访问它(不带前导 `/content/dam`)。
...
API参考文档中定义了支持请求的 [确切格式](/help/assets/assets-api-content-fragments.md#api-reference) 。
...
<table>
 <tbody>
  ...
  <tr>
   <td>支持的用例</td>
   <td>一般用途。</td>
   <td><p>针对单页应用程序(SPA)或任何其他（内容消耗）上下文中的使用情况进行了优化。</p> <p>还可以包含布局信息。</p> </td>
  </tr>
  <tr>
   <td>支持的操作</td>
  ...
  <tr>
   <td>访问</td>
   <td><p>可以直接访问。</p> <p>使用映射 <code>/api/assets </code>到（在存储库中） <code>/content/dam</code> 的端点。</p> <p><code class="code">
       /content/dam/we-retail/en/experiences/arctic-surfing-in-lofoten</code><br /> 例如，要访问：请求：<br /> <code>/api/assets/we-retail/en/experiences/arctic-surfing-in-lofoten.model.json</code></p> </td>
   <td><p>需要通过AEM页面上的AEM组件引用。</p> <p>使用选 <code>.model</code> 择器创建JSON表示。</p> <p>示例URL如下：<br /> <code>https://localhost:4502/content/we-retail/language-masters/en/experience/arctic-surfing-in-lofoten.model.json</code></p> </td>
  </tr>
  <tr>
   <td>安全</td>
   <td><p>可能有多个选项。</p> <p>OAuth被提出；可以与标准设置分开配置。</p> </td>
   <td>使用AEM的标准设置。</td>
  </tr>
  <tr>
   <td>建筑评论</td>
...
如果在没有特定身份验证要求的环境中使用资产REST API，则需要正确配置AEM的CORS过滤器。
...
内容片段是资产的一种特定类型，请参阅 [使用内容片段](/help/assets/content-fragments.md)。

有关通过API提供的功能的更多信息，请参阅：

* [Assets](/help/assets/mac-api-assets.md#available-features) REST API的可用功能
...
资产REST API公开了对文件夹属性的访问权；例如，其名称、标题等。 资产会作为文件夹的子实体公开。

>[!NOTE]
>
>根据资产类型，子实体列表可能已包含定义相应子实体的全套属性。 或者，对于子实体列表中的实体，只能公开缩减的属性集。

### 资产 {#assets}

如果请求资产，响应将返回其元数据；如标题、名称和由各个资产架构定义的其他信息。

资产的二进制数据会作为类型(也称为 `content` SIREN)的SIREN链接公 `rel attribute`开。

资产可以有多个演绎版。 这些属性通常作为子实体公开，一个例外是缩略图再现，它作为类型()的链接公 `thumbnail` 开 `rel="thumbnail"`。

### 内容片段 {#content-fragments}

内 [容片段](/help/assets/content-fragments.md) ，是一种特殊类型的资产。 它们可用于访问结构化数据，例如文本、数字、日期等。
...
使用情况可能因您使用的是AEM创作环境还是发布环境以及特定用例而异。

* 创建严格绑定到作者实例([目前没有方法使用此API将片段复制到发布](/help/assets/assets-api-content-fragments.md#limitations))。
* 可以同时从两者进行交付，因为AEM仅以JSON格式提供请求的内容。

   * 从AEM作者实例进行存储和交付应足以用于防火墙后的媒体库应用程序。
...
>[!NOTE]
>
>有关更多详细信息，请参 [阅API参考](/help/assets/assets-api-content-fragments.md#api-reference)。 特别是， [Adobe Experience Manager Assets API —— 内容片段](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/assets-api-content-fragments/index.html)。
...
这可以只是内容片段的标题或描述，或单个元素，或所有元素值和／或元数据。 此外，必须为更新提供有 `cq:model` 效的属性。
...
* **无法编写和更新变体。** 如果将这些变量添加到有效负荷（例如，对于更新），则忽略它们。 但是，变体将通过交付( `GET`)提供。

* **当前不支持内容片段模型**:无法读取或创建它们。 要能够创建新内容片段或更新现有内容片段，开发人员必须知道内容片段模型的正确路径。 目前，只有通过管理UI才能获得这些内容的概述。
* **将忽略引用**。 当前不检查是否引用了现有内容片段。 因此，例如，删除内容片段可能会导致包含引用的页面上出现问题。
...
1. 500（内部服务器错误）
   ...
   下面列出了返回此错误状态时的常见情况以及生成的错误消息（等宽）:

   * 父文件夹不存在(通过创建内容片段时 `POST`)
   * 未提供内容片段模型（null值），资源为null（可能是权限问题），或者资源不是有效的片段模板：
