...
Adobe Experience Manager(AEM) [Assets REST](/help/assets/mac-api-assets.md) API允许开发人员通过CRUD操作（创建、读取、更新、删除）直接通过HTTP API访问内容(存储在AEM中)。
...
资产REST API优惠 [对](https://en.wikipedia.org/wiki/Representational_state_transfer)AEM实例中存储的资产进行REST样式访问。 它使用端 `/api/assets` 点，并需要资产的路径才能访问它(不带前导 `/content/dam`)。
