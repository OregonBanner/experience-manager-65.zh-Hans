...
虽然AEM核心组件提供非常全面、灵活且可自定义的API，可为此目的提供所需的读取操作，并且其JSON输出可进行自定义，但它们确实需要AEM WCM(Web内容管理)知识才能实施，因为它们必须在基于专用AEM模板的(API)页面中托管。 并非每个SPA开发组织都有权访问此类资源。
...
在最新AEM版本的每次现成安装中，均提供资产REST API。

## 重要概念 {#key-concepts}

资产REST API可优惠 [对](https://en.wikipedia.org/wiki/Representational_state_transfer)AEM实例中存储的资产进行REST样式访问。 它使用端 `/api/assets` 点，并需要资产的路径才能访问它(不带前导 `/content/dam`)。
...
<table>
 <tbody>
  ...
  <tr>
   <td>访问</td>
   <td><p>可以直接访问。</p> <p>使用映 <code>/api/assets </code>射到(在 <code>/content/dam</code> 存储库中)的端点。</p> <p>例如，要访问：<code class="code">
   ...
   <td><p>需要通过AEM页面上的AEM组件进行引用。</p> <p>使用选 <code>.model</code> 择器创建JSON表示。</p> <p>示例URL如下所示：<br /> <code>https://localhost:4502/content/we-retail/language-masters/en/experience/arctic-surfing-in-lofoten.model.json</code></p> </td>
  </tr>
  <tr>
   <td>安全</td>
