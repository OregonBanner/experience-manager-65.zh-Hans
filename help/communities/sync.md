...
当 [拓扑](/help/communities/topologies.md) 是 [发布场](/help/sites-deploying/recommended-deploys.md#tarmk-farm)，则对一个发布实例进行的注册和修改需要与其他发布实例同步。 成员需要能够登录并查看其任何发布节点上的数据。
...
在作者中，从 [社区站点控制台](/help/communities/sites-console.md)，效果为 [复制](/help/sites-deploying/configuring.md#replication-reverse-replication-and-replication-agents) 关联页面，Sling则分发动态创建的社区用户组，包括其成员资格。

### 用户在发布时创建或编辑配置文件 {#user-is-created-or-edits-profile-on-publish}

根据设计，在发布环境中创建的用户和配置文件（例如，通过自注册、社交登录、LDAP身份验证）不会显示在创作环境中。

当拓扑为 [发布场](/help/communities/topologies.md) 和用户同步已正确配置， *用户* 和 *用户配置文件* 会使用Sling分发在发布场中同步。
...
对于用户生成的内容(UGC)，将通过 [已配置SRP](/help/communities/srp-config.md).
...
创作实例ping发布者的导出程序端点。 每当在特定发布者(n)上创建或更新用户时，作者都会从其导出端点获取内容，并且 [推送内容](/help/communities/sync.md#main-pars-image-1413756164) 到其他发布者（n-1，即除获取内容的发布者之外）。
...
1. 定位 **Apache Sling Distribution Agent — 同步代理工厂**.
...
1. 定位 **AdobeGranite分发 — 加密密码传输密钥提供程序。**
...
1. 定位 **Apache Sling Distribution Agent — 队列代理工厂**.
1. 选择要打开以进行编辑的现有配置（铅笔图标）。
...
1. 定位 **AdobeGranite分布 — 差异观察器工厂**.
1. 选择要打开以进行编辑的现有配置（铅笔图标）。

   验证 **代理名称：socialpubsync -reverse**.

1. 选中&#x200B;**启用**&#x200B;复选框。
1. 指定 **rep:members** 作为 **已查找属性名称**，并保存。
...
1. 浏览到 [https://_主机：端口_/system/console/status-slsettings](https://localhost:4503/system/console/status-slingsettings).
1. 检查的值 **Sling ID**.
...
1. 在 `crx-quickstart/launchpad/felix` 目录，搜索并删除名为 *sling.id.file.*
...
1. 访问 [Web控制台](/help/sites-deploying/configuring-osgi.md)
   ...
   例如， [https://localhost:4503/system/console/configMgr](https://localhost:4503/system/console/configMgr).
1. 找到 **Apache Sling Distribution包 — 保管库包生成器工厂**
...
1. **检查配置**
   ...
   确保 [配置](/help/communities/sync.md#bestpractices) 已在AEM创作实例中正确设置，如 [最佳实践](/help/communities/sync.md#main-pars-header-863110628).

1. **检查授权用户权限**

   如果包安装不正确，请检查 [授权用户](/help/sites-administering/sync.md#createauthuser) 在第一个Publish实例中创建的ACL正确。
...
1. **检查差异观察器工厂配置**
   ...
   如果仅特定节点未在发布场之间同步（例如，组成员未同步），请确保 [AdobeGranite分布 — 差异观察器工厂](/help/sites-administering/sync.md#diffobserver) 配置已启用，并且 **代表：成员** 在 **已查找属性名称**.
...
1. 在AEM创作实例上，使用管理员权限登录。
   ...
   1. 访问 [Web控制台](/help/sites-deploying/configuring-osgi.md). 例如， [https://localhost:4502/system/console/configMgr](https://localhost:4502/system/console/configMgr).
   1. 找到配置 **Apache Sling Distribution Agent — 同步代理工厂**.
   1. 取消选择 **已启用** 复选框。
...
1. 检查 [日志顺序](/help/communities/sync.md#troubleshoot-sling-distribution-in-aem-communities)，创建时。
1. 检查 **vlt** 包创建于 **/var/sling/distribution/packages/socialpubsync-vlt/data**.
1. 现在，在AEM创作实例上启用用户同步。
1. 在发布者上，在 **Apache Sling Distribution Agent — 同步代理工厂**.
