...
在AEM Communities中，从发布环境（取决于配置的权限）中，站点访客可能变为 *成员* 、创建用户组 *，以及编*&#x200B;辑其成 **** 员配置文件。
...
当拓扑为发布场 [且用户同步已正确配置](/help/communities/topologies.md) ，则使用Sling分发在发布场 *内同步用户* 和用户配置文件 ** 。
...
对于订阅和后续订阅中存在差异的Sling分发问题，请检查 **AEM Communities用户同步监听器配置中是否设置了以下属性** :
...
1. 在目录 `crx-quickstart/launchpad/felix` 中，搜索并删除名为sling.id.file的 *文件。*
   ...
   使用Windows资源管理器和搜索 `sling.id.file`
...
在 **/home/users中**，将创建***/rep:cache **节点。 它是一个缓存，用于查找如果我们查询节点的主体名称，则可以直接使用该缓存。
...
1. **检查配置**。 确保在AEM作 [](/help/communities/sync.md#bestpractices)者实例中正确设置所有配置，如最佳实 [践所述](/help/communities/sync.md#main-pars-header-863110628)。
...
1. **检查Diff Observer Factory配置**。 如果仅特定节点未在发布场之间同步（例如，组成员未同步），则确保启用 [Adobe Granite Distribution - Diff Observer Factory](/help/sites-administering/sync.md#diffobserver) （差异观察器工厂）配置并 **表示：成员** 在查看的属 **性名称中设置**。
...
现在，如果用户是在发布实例上创建的， **vlt** package将在 */var/sling/distribution/packages/ socialpubsync - vlt /data节点中创建* 。 如果作者将这些包推送到其他服务。 您可以下载并提取此数据以检查将哪些属性推送到其他服务。
   ...
   1. 检查是否在 **/var/sling/distribution/packages/socialpubsync-vlt/data上创建**vlt **包**。
