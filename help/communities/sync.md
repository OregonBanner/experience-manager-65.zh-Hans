...
当[topology](/help/communities/topologies.md)是[发布场](/help/sites-deploying/recommended-deploys.md#tarmk-farm)时，在一个发布实例上进行的注册和修改需要与其它发布实例同步。 成员需要能够登录并查看其任何发布节点上的数据。
...
* **vlt包**
   ...
   它是发布者所做的所有更改的zip文件，需要在发布者之间分发。 发布者上的更改生成由更改事件侦听器选取的事件。 这将创建一个包含所有更改的vlt包。
...
当拓扑为[发布场](/help/communities/topologies.md)且用户同步已正确配置时，使用Sling分发在发布场中同步&#x200B;*用户*&#x200B;和&#x200B;*用户用户档案*。
...
创作实例ping发布器的导出器端点。 每当在特定发布者(n)上创建或更新用户时，作者将从其导出者端点获取内容，并[将内容](/help/communities/sync.md#main-pars-image-1413756164)推送至其他发布者（n-1，即与从中获取内容的发布者不同）。
...
1. 找到&#x200B;**AdobeGranite分发 — 加密密码传输密码提供程序。**
...
1. 选中&#x200B;**已启用**&#x200B;复选框。
1. 指定&#x200B;**rep:members**&#x200B;作为&#x200B;**中的propertyName的描述，查找属性名称**&#x200B;和“保存”。
...
1. 策略处理
   ...
   * 要防止策略被分发，请设置：`Acl Handling: IGNORE`
   ...
   ![存储包生成器工厂](assets/vault-package-builder-factory.png)
...
1. **检查配置**
   ...
   确保在您的AEM作者实例中正确设置了所有[配置](/help/communities/sync.md#bestpractices)，如[最佳实践](/help/communities/sync.md#main-pars-header-863110628)中所述。
...
1. **检查比较观察器工厂配置**
   ...
   如果仅特定节点未在发布场之间同步 — 例如，组成员未同步 — 则确保[AdobeGranite Distribution - Diff Observer Factory](/help/sites-administering/sync.md#diffobserver)配置已启用且&#x200B;**rep:成员**&#x200B;在&#x200B;**查找的属性名称**&#x200B;中设置。
...
1. **检查AEM Communities用户同步监听器配置。** 如果创建的用户已同步，但订阅和后续操作不起作用，请确保AEM Communities用户同步监听器配置具有：
   ...
   * “已分发文件夹”(Distributed Folders) — 设置为要分发的文件夹。

1. **检查在发布实例上创建用户时生成的日志**

   如果上述配置设置得当，但用户同步不起作用，则检查创建用户时生成的日志。
...
1. 在发布者上，在&#x200B;**Apache Sling Distribution Agent - Sync Agent Factory**中更改导出器或导入器终结点。
