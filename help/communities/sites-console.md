...
* **选择持久性选项**
   ...
   * **根据用户请求翻译稿件，**&#x200B;然后保留如果选择，则直到发出请求后，内容才会翻译。 翻译完成后，翻译会存储在存储库中。
...
当选 `ENABLEMENT`定的社区站点模板包含分配功能时，这些设 [置将适用](/help/communities/functions.md#assignments-function)，在授权功能获得许可和配置时，这 [些功能可用](/help/communities/enablement.md)。 包含分配功能的参考站点模板是 `Reference Structured Learning Site Template.`
