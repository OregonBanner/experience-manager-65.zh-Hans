...
如果为false，则为已分配，表示已授予奖励积分的徽章，并且该徽章应显示为图像。
...
**发布时JSRP**：同样，如果在发布环境中进行测试，则需要使用发布实例上的管理权限访问CRXDE Lite。 如果发布实例在中运行 [生产模式](/help/sites-administering/production-ready.md) (nosamplecontent runmode)，将需要 [启用CRXDE Lite](/help/sites-administering/enabling-crxde-lite.md).
...
1. 已使用cURL为用户分配了审查方徽章：
   ...
   由于用户已获得两个铜牌并获得了版主徽章，因此这是用户在其论坛条目中的显示方式。
...
对于GeometrixxEngage站点，用户及其分数位于一条由评分规则名称、社区站点的站点ID ( `engage-ba81p`)、唯一ID和用户的ID ：
...
分数存储在属性中 `scoreValue_tl` 可以直接包含值，也可以间接引用atomicCounter。
