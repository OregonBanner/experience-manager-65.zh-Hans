...
* [批量审核控制台](moderation.md)
   ...
   管理员和[社区管理者](/help/communities/users.md)可以在公共环境下以及作者环境下通过管理员访问协调控制台。 当社区内容存储在[公用商店](/help/communities/working-with-srp.md)中时，这是可能的。
...
UGC预先审核后，在审核操作批准之前，发布的站点上不会显示该帖子。 在创建[社区站点](/help/communities/sites-console.md)期间，选中“内容为预审核](sites-console.md#moderation)”框将为整个站点启用预审核。 [将组件放置到页面上后，可以使用编辑对话框中的设置将支持仲裁的组件配置为进行预仲裁：

* [“用](comments.md) 户协调 [](reviews.md)
”>“预协调 **[!UICONTROL ”中的]** 评论和审阅 ****。
...
但是，要自定义或扩展默认的垃圾邮件字词，请通过[overlay](/help/communities/overlay-comments.md)在/apps目录中创建一组默认的垃圾邮件字词结构。
...
1. 找到&#x200B;**AEM Communities自动协调**&#x200B;配置并编辑它。
...
从[批量审核控制台](/help/communities/moderation.md)中，可以根据情绪是否为负、中性还是正性来筛选和视图UGC。
...
**sentimentprocess.name**&#x200B;也可以被修改为引用自定义情绪规则集的位置。
...
* **表词节点的显式路径**
   ...
   包含指定默认观察字的默认`positive`和`negative`属性的节点的存储库位置。 默认值为`/libs/settings/community/watchwords/default`。

* **情绪规则**

   用于基于正反表词计算情绪的规则的存储库位置。 默认值为`/libs/cq/workflow/components/workflow/social/sentiments/rules`（但是，不再涉及任何工作流）。
