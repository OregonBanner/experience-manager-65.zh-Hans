...
社区内容，也称为用户生成的内容(UGC)，是在成员（登录网站访客）通过与以下社区组件之一交互而发布已发布社区网站中的内容时创建的：
...
审核UGC对于识别积极贡献以及限制消极贡献（例如垃圾邮件和辱骂语言）非常有用。 UGC可以从多个环境中审核：
...
可以将上下文审核配置为允许成员在标记帖子时选择原因。 可选择标志原因的列表是可配置的，包括是否可以输入自定义原因。 标记原因会与UGC一起保存，但不会触发任何特定操作。 只有标记的数量会触发通知。 已标记的内容将进行相应的批注，以便审查方可以对其进行处理。

系统将跟踪所有已标记的标记以及标记原因，并在达到阈值时发送事件。 如果社区审查方允许UGC，则会存档这些标记。 在允许并存档之后，如果存在后续标记，则会将其存档，就像之前没有标记一样。

### 允许 {#allow}

对于在预审核系统中已标记、已拒绝或未批准的UGC，允许操作是一个选项。 允许操作将清除任何存在的已标记或已拒绝/垃圾邮件状态，并存档任何已标记的数据。

## 常见的审核概念 {#common-moderation-concepts}

### 预审 {#premoderation}

预审UGC时，帖子只有在获得审核操作批准之后才会显示在已发布的网站上。 创建期间 [社区站点](/help/communities/sites-console.md)，勾选方框 [内容已通过预审](sites-console.md#moderation) 将为整个站点启用预审。 将组件放到页面上后，可以使用其“编辑”对话框中的设置将支持审核的组件配置为预审：
...
垃圾邮件检测是一种自动审核功能，它通过将用户生成的提交内容标记为垃圾邮件来过滤掉不需要的这些内容。 一旦启用，它就根据预先配置的垃圾邮件词语集合来识别用户生成的内容是否为垃圾邮件。 默认垃圾邮件词位于

`/libs/settings/community/sites/moderation/spamdetector-conf/profiles/spam_words.txt`。

但是，要自定义或扩展默认垃圾邮件单词，请按照默认垃圾邮件单词的结构在/apps目录中创建一组单词，方法是 [叠加](/help/communities/overlay-comments.md).

用户生成的包含垃圾邮件的帖子（涵盖所有内容类型，例如博客、论坛和评论）在帖子上方标有“此帖子被分类为垃圾邮件”文本。
...
AEM communities提供 *关注词分析器* 作为评估过程中的一个步骤 [情绪](#sentiment). 对标语提供的情绪值的贡献是由于对发布内容中使用的负面和正面标语以及禁止单词的比较。

#### 配置情绪和标语 {#configure-sentiment-and-watchwords}

正面和负面标语列表可以自定义，就像情绪规则一样。

可以将默认关注字列表作为存储库中节点的属性输入，这与默认列表类似，也可以通过配置OSGi服务来覆盖默认列表 `sentimentprocess.name` 和单词清单。
...
* **正面标语**
  ...
  逗号分隔的单词列表，这些单词有助于覆盖默认值的正面情绪。 默认列表为空。
