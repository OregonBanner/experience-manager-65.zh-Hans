...
AEM Communities功能通常要求在加入发布环境的社区之前先注册并登录站点访客。 他们的用户注册只需存在于发布环境中，通常称为 *成员* ，以区分他们 *与在作者环境* 中注册的用户。

### 发布时的成员（用户） {#members-users-on-publish}

使用“社区成员”和“组”控制台，可以从作者环境创建 *和管理* “发布”环境中注册的成员和 *成员组* 。 只有启用隧道服务 [时，才可](deploy-communities.md#tunnel-service-on-author) 以这样做。
...
当用户是启 `Views`用Adobe `Posts`Analytics的 `Follows` 一个或多个社区站点的成员时，将更新显示 `Likes` 、显示 [和列](sites-console.md#analytics)。
...
要访问“组”控制台，请执行以下操作：
* 从全局导航中，选 **[!UICONTROL 择导航]** > **[!UICONTROL 社区]** > ****&#x200B;组。
