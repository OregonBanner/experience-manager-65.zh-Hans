...
此体验假定 [Aaron mcDonald](/help/communities/tutorials.md#demo-users) 被分配为社区经 [理和主持人的角色](/help/communities/create-site.md#roles)。 如果没有，则返回创作环境以修 [改站点设置](/help/communities/sites-console.md#modifying-site-properties) ，并选择Aaron mcDonald作为社区经理和版主。
...
* **通过** “配置文件”选项，可以查看和编辑您的配置文件。
* [“消息](/help/communities/configure-messaging.md) ”选项将指导您进入“直接消息”部分，在该部分，您可以：

1. 查看您收到的（收件箱）、发送（已发送的项目）和删除（垃圾桶）的直接邮件。
1. 编写新的直接消息以发送给个人和组。

* [“通知](/help/communities/notifications.md) ”选项会引导您进入通知部分，您可以在该部分查看您感兴趣的活动并编辑通知设置。
...
当社区成员具有审核权限时，“审核”链接将可见，选择该链接后，将显示发布的社区内容并允许其以类似于创作环境中的审核控制台的方式 [进行审核](/help/communities/moderate-ugc.md)[](/help/communities/moderation.md) 。
