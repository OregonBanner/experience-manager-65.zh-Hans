...
* [向会员授予基本的徽章](#enable-scoring) ，以鼓励其参与（创建的内容数量）
...
在UGC中，徽章是按规则分配的还是赚取的。 目前，已分配的标记显示为文本，而已获得的标记显示为图像。
...
* if page2 contains scontains scroing rule2, its contaster page1 contains scroing rule1
...
>成 *员-profile-url*
>
>* 如果启用了隧道服务 [](/help/communities/users.md#tunnel-service) ，则可引用作者实例
>* 可能是一个模糊的随机名称——请参阅有关可授权 [ID的安全清单](/help/sites-administering/security-checklist.md#verify-that-you-are-not-disclosing-personally-identifiable-information-in-the-users-home-path) （仅英文版）
>
...
对于交付的社区组件，下表描述了为 `verbs`可用于评 `topic`分子规则的每 [个组件定义的](#scoring-sub-rules)。
...
**[日记帐组](/help/communities/blog-feature.md)**件SocialEvent`topic`= com/adobe/cq/social/journal
...
* 必须刷新页面才能显示标记
   ...
   * 注销并以其他社区成员的身份登录(例如：aaron.mcdonald@mailinator.com)
