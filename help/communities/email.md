...
`AEM Communities Email Reply Configuration` OSGi config与“名称”电子邮件的实例是立即启用功能。 此功能不包括电子邮件回复。
...
* **[!UICONTROL 最大主题长度]**
   ...
   如果将跟踪器ID添加到主题行，则这是主题的最大长度（不包括跟踪器ID），之后将裁切它。 请注意，这应尽可能小，以避免丢失跟踪的ID信息。 默认为200。
...
* **[!UICONTROL 回复分隔符]**
   ...
   如果将跟踪器ID添加到“回复”标题，则将使用此分隔符。 默认值为`+`（加号）。

* **[!UICONTROL 主题中的跟踪器ID前缀]**

   如果将跟踪器ID添加到主题行，将使用此前缀。 默认值为`post#`。
...
* **[!UICONTROL 以秒为单位的更新时间间隔]**
   ...
   *（可选）为* 默认邮件服务配置的邮件服务器可能要求更新时间间隔值。例如，Gmail可能需要`300`的间隔。
...
* 使用管理员权限登录到主发布者，并浏览到[https://&lt;server>:&lt;port>/crx/de/index.jsp#/etc/importers/polling](http://localhost:4503/crx/de/index.jsp#/etc/importers/polling)。
* 选择新创建的配置并修改以下属性：
