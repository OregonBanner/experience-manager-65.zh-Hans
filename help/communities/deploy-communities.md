...
**对于 [社区功能](/help/communities/overview.md)**
...
* 确定 [共享UGC存储](/help/communities/working-with-srp.md) (**SRP**)
...
AEM 6.5 Communities GA包含Communities包。 了解AEM 6.5的更新 [社区](/help/release-notes/release-notes.md#experiencemanagercommunities)，请参阅 [AEM 6.5发行说明](/help/release-notes/release-notes.md#communities-release-notes.html).
...
* 对于 [启用](/help/communities/enablement.md):记录SCORM活动和学习者
...
AEM Communities SCORM引擎是 [启用](/help/communities/overview.md#enablement-community) 功能。 AEM 6.5 Communities支持的Scorm包：
...
1. 安装 [cq-social-scorm-package，版本2.3.7](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=%2Fcontent%2Fsoftware-distribution%2Fen%2Fdetails.html%2Fcontent%2Fdam%2Faem%2Fpublic%2Fadobe%2Fpackages%2Fcq640%2Fsocial%2Fscorm%2Fcq-social-scorm-2017-pkg)  从包共享。
...
在AEM Communities中，通用商店用于存储用户生成的内容(UGC)，通常称为 [存储资源提供程序(SRP)](/help/communities/working-with-srp.md). 推荐的部署中心是为公共存储选择SRP选项。

该常用商店支持在发布环境中审核UGC并对其进行分析，同时不需要 [复制](/help/communities/sync.md) UGC的。
...
当选择的部署是 [发布场](/help/communities/topologies.md#tarmk-publish-farm)，则必须将一个AEM发布实例标识为 **`primary publisher`** 对于不应出现在所有实例的活动，例如依赖于 **通知** 或 **Adobe Analytics**.
...
* 对于两个代理，请遵循相同的过程：
   ...
   * **反向复制代理（发布反向）**
      ...
      1. 如果不是用户 `admin`，编辑 **用户** 和 **密码** 指定 `administrators` 用户组
...
使用创作环境时 [创建站点](/help/communities/sites-console.md), [修改网站属性](/help/communities/sites-console.md#modifying-site-properties) 或 [管理社区成员](/help/communities/members.md)，则必须访问在发布环境中注册的成员（用户），而不是在作者中注册的用户。
...
AEM Communities有两项功能，要求所有AEM服务器实例使用相同的加密密钥。 这些是 [Analytics](/help/communities/analytics.md) 和 [ASRP](/help/communities/asrp.md).
...
* 访问包含要复制的关键材料的AEM实例（通常为创作实例）
   ...
   * 找到 `com.adobe.granite.crypto.file` 包，例如，
