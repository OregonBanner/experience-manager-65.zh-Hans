...
包括所 [需的客户端库](/help/communities/ideation.md#essentials-for-client-side) ，组件的显示 `Ideation`方式如下：
...
* **关闭**
   ...
   如果选中此项，则会关闭构思论坛以查看新的构思和评论。 默认为未选中。
...
* **允许的文件类型**
   ...
   仅在选中时 `Allow File Uploads` 相关。 以逗号分隔的文件扩展名列表，带有“点”分隔符。 例如：.jpg、.jpeg、.png、.doc、.docx、.pdf。 如果指定了任何文件类型，则不允许上传那些未指定的文件类型。 默认值未指定，因此允许** **所有文件类型。
...
* **允许专题内容**
   ...
   如果选中此项，则该构思可被标识为特色 [内容](/help/communities/featured.md)。 默认为未选中。
...
在**用户协调**选项卡下，指定如何管理已发布的构思和评论（用户生成的内容）。 有关详细信息，请参阅 [审核用户生成的内容](/help/communities/moderate-ugc.md)。
...
* **允许的命名空间**
   ...
   如果选中 `Allow Tagging` 了“设置”选项卡下 **的选项** ，则相关。 可应用的标记仅限于选定的命名空间类别中的标记。 命名空间列表包括“标准标记”（默认命名空间）和“包括所有标记”。 默认值为“无”选中，这意味着允许所有命名空间。

* **建议限制**

   输入要作为建议显示给发布到论坛的成员的标记数。 值**-**1表示无限制。 默认值为0。
