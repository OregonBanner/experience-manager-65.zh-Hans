...
可以为AEM Communities配置Adobe Analytics，以便在成员与支持的Communities功能交互时，事件将发送到Adobe Analytics，并从中生成报告。

例如，当某个启用社区站点的成员查看分配给他们的视频资源时，资源播放器会将事件（包括视频心率数据）发送到Analytics。 在社区站点中，管理员能够查看有关视频播放的各种报告。
...
此 `Marketing Cloud Org Id` 可能输入以下时间 [社区站点创建](/help/communities/sites-console.md#enablement) 或更高版本，操作者 [修改](/help/communities/sites-console.md#modifying-site-properties) 社区站点属性。
...
以下是遵循以下规则后的默认映射示例 [入门教程](/help/communities/getting-started-enablement.md)：
