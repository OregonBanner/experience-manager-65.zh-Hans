...
>[!NOTE]
>
>使用MTOM传输协议的基于JAX WS的应用程序仅限于25MB的发送和接收数据。 此限制是由JAX-WS中的错误造成的。 如果已发送和已接收文件的总大小超过25MB，请使用SwaRef传输协议，而不是MTOM协议。 否则，可能会出现 `OutOfMemory`*异常。*
...
1. 使用使用服务WSDL的JAX-WS创建Java代 `MyApplication/EncryptDocument` 理类。 使用以下WSDL端点：
   ...
   >[!NOTE]
   >
   >替 `hiro-xp`换为承载AEM Forms的J2EE应用程序服务器的IP地址。
