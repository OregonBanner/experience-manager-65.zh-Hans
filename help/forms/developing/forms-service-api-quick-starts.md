...
description: 使用JAVA API呈现交互式PDF、HTMLForms和片段。
...
>[!NOTE]
>
>使用v编程中的快速启动基于您正在使用其他操作系统（如Unix）的表单服务器，使用适用的操作系统支持的路径替换Windows特定路径。 同样，如果您使用的是其他J2EE应用程序服务器，请确保指定有效的连接属性。 请参阅 [设置连接属性](/help/forms/developing/invoking-aem-forms-using-java.md#setting-connection-properties).
...
以下代码示例呈现基于片段的表单。 表单设计的名称为 *PurchaseOrderDynamic.xdp* 它位于AEM Forms存储库中（XDP文件存储在存储库中名为FormsFolder的文件夹中）。 此外，POFragment表单引用的片段还必须位于存储库中。 (请参阅 [基于片段渲染Forms](/help/forms/developing/rendering-forms.md#rendering-forms-based-on-fragments).)
...
以下代码示例使用Forms服务Java API渲染HTML表单。 一个工具栏以及两个文件附件将添加到HTML表单中。 此外，用户代理值也可从 `HttpServletRequest` 对象。 (请参阅 [将Forms渲染为HTML](/help/forms/developing/rendering-forms.md#rendering-forms-as-html).)
...
以下Java快速入门呈现基于名为的表单设计的交互式PDF表单 *Loan.xdp* 按值。 请注意，表单设计用于填充 `com.adobe.idp.Document` 对象已命名 *inputXDP*. (请参阅 [按值呈现Forms](/help/forms/developing/rendering-forms.md#rendering-forms-by-value).)
...
以下Java快速入门从内容服务中检索文件Loan.xdp（已弃用）。 此XDP文件位于空间 `/Company Home/Form Designs`. XDP文件将在 `com.adobe.idp.Document` 实例。 此 `com.adobe.idp.Document` 实例将传递到Forms服务。 交互式表单将写入客户端Web浏览器。 (请参阅 [将文档传递到Forms服务](/help/forms/developing/passing-documents-forms-service.md).)
