...
您还必须引用安全凭据才能对PDF文档进行数字签名。 要引用安全凭据，请指定别名。 别名是对实际凭据的引用，该凭据可能位于PKCS#12文件（扩展名为.pfx）或硬件安全模块(HSM)中。 有关安全凭据的信息，请参阅适用于应用程序服务器的&#x200B;*安装和部署AEM Forms*&#x200B;指南。
...
循环访问每个签名。 即，对于每个签名，验证数字签名，并检查签名者的身份和每个签名的状态。 （请参阅[验证数字签名](#verify-digital-signatures-using-the-java-api)。）
