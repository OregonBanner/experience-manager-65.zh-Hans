...
1. 设置EJB端点属性。
   ...
   * 指定通过调用对象的方法调用 `CreateEndpointInfo` 的操作，并 `setOperationName` 传递一个指定操作名称的字符串值。 对于SOAP和EJB端点，指定通配符( `*`)，表示所有操作。
...
此过程接受一个不安全的PDF文档作为输入值，然后将该不安全的PDF文档传递给加密服务的操 `EncryptPDFUsingPassword` 作。 此过程使用密码加密PDF文档并返回密码加密的PDF文档作为输出值。 输入值的名称（不安全的PDF文档） `InDoc` 和数据类型为 `com.adobe.idp.Document`。 输出值的名称（密码加密的PDF文档） `SecuredDoc` 和数据类型为 `com.adobe.idp.Document`。
