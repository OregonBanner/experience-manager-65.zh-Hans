...
* 集合对象， `java.util.List` 如 `java.util.Map`
   ...
   ` <result> <output-paramater1>output-parameter-value-as-string</output-paramater1> . . . <output-paramaterN>output-parameter-value-as-string</output-paramaterN> </result>`**注意&#x200B;**:该`output-paramater1`值表示输出参数名称。
...
如果该过程要求将PDF文档作为输入值，请确保您将表单提交为PDF，如上图所示。 此外，要成功调用进程，该进程必须返回PDF文档。 否则，Acrobat无法处理返回值，并且会发生错误。 您不必指定输入进程变量的名称。 例如， *MyApplication/EncryptDocument进程有一个名为的输入变量*`inDoc`。 只要表单提交为PDF，您就不必指定inDoc。
