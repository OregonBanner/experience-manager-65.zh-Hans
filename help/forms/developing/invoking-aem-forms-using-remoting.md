...
>[!NOTE]
>
创建新角色并希望该角色的成员上载安全文档时，请确保指定“文档上载”权限。
...
您可以使用远程调用位于自定义组件中的服务。 例如，考虑包含客户服务的银行组件。 您可以使用在Flex中编写的客户端应用程序调用属于客户服务的操作。 您必须先创建Bank自定义组件，然后才能执行与此部分关联的快速入门。
...
ActionScript类的字段与属于AEM Forms复杂类型的字段匹配。 CustomerActionScript类中的六个字段与所属的字段匹配 `com.adobe.livecycle.sample.customer.Customer`.
