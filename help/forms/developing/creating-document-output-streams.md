...
1. 设置打印到文件所需的打印运行时选项。
   ...
   * 通过调用PrintedOutputOptionsSpec对象的`setFileURI`方法并传递一个字符串值来指定文件，该字符串值表示文件的名称和位置。 例如，如果希望输出服务打印到位于C:\Adobe的名为MortgageForm.ps的PostScript文件，请指定C:\\Adobe\MortgageForm.ps。
