...
您可以使用用户管理API创建客户端应用程序，这些应用程序可以管理角色、权限和承担者（可以是用户或组），以及对用户进行身份验证。 用户管理API包含以下AEM Forms API：
...
In [添加用户](users.md#adding-users) 您将了解如何以编程方式添加用户。 本节使用目录管理器服务API。

In [删除用户](users.md#deleting-users) 您将了解如何以编程方式删除用户。 本节使用目录管理器服务API。

In [管理用户和组](users.md#managing-users-and-groups) 您将了解本地用户和目录用户之间的区别，并查看有关如何使用Java和Web服务API以编程方式管理用户和组的示例。 本节使用目录管理器服务API。

In [管理角色和权限](users.md#managing-roles-and-permissions) 您将了解系统角色和权限，以及可以通过编程方式增强这些角色和权限的功能，并查看有关如何使用Java和Web服务API以编程方式管理角色和权限的示例。 本节同时使用目录管理器服务API和授权管理器服务API。

In [验证用户](users.md#authenticating-users) 您将看到有关如何使用Java和Web服务API以编程方式对用户进行身份验证的示例。 本节使用授权管理器服务API。

**了解身份验证过程**

“用户管理”提供了内置的身份验证功能，还让您能够将其与自己的身份验证提供程序连接。 当用户管理收到身份验证请求（例如，用户尝试登录）时，会将用户信息传递给身份验证提供程序以进行身份验证。 User Management在对用户进行身份验证后，会从身份验证提供程序接收结果。
...
<table>
 ...
 <tbody>
  ...
  <tr>
   <td><p>2</p></td>
   <td><p>“用户管理”将用户名和密码以及配置信息发送给身份验证提供程序。</p></td>
  </tr>
  <tr>
   <td><p>3</p></td>
   <td><p>身份验证提供程序连接到用户存储区并对用户进行身份验证。</p></td>
  </tr>
  <tr>
   <td><p>4</p></td>
   <td><p>身份验证提供程序将结果返回给“用户管理”。</p></td>
  </tr>
  <tr>
   <td><p>5</p></td>
...
User Management与支持连接到LDAP目录的目录服务提供程序(DirectoryManagerService)一起打包。 如果您的组织使用非LDAP存储库来存储用户记录，则可以创建自己的目录服务提供程序，以便与存储库配合使用。

目录服务提供者应用户管理的请求，从用户存储中检索记录。 用户管理定期缓存数据库中的用户和组记录，以提高性能。

目录服务提供程序可用于将User Management数据库与用户存储同步。 此步骤可确保所有用户目录信息以及所有用户和组记录都是最新的。

此外， DirectoryManagerService还让您能够创建和管理域。 域定义了不同的用户群。 域的边界通常根据组织的构建方式或用户存储的设置方式定义。 用户管理域提供身份验证提供程序和目录服务提供程序使用的配置设置。

在User Management导出的配置XML中，具有属性值的根节点 `Domains` 包含为用户管理定义的每个域的XML元素。 这些元素中的每一个都包含定义与特定服务提供者相关联的域的各方面的其他元素。

**了解objectSID值**

在使用Active Directory时，请务必了解 `objectSID` 值不是跨多个域的唯一属性。 此值存储对象的安全标识符。 在多个域环境中（例如域的树）， `objectSID` 值可以不同。
...
使用目录管理器服务API添加新用户时，请为该用户定义信息。 通常，在添加新用户时，您会定义以下值：
...
1. 包括项目文件。
   ...
   在Java项目的类路径中包含客户端JAR文件，例如adobe-usermanager-client.jar。

1. 创建DirectoryManagerServices客户端。

   创建 `DirectoryManagerServiceClient` 对象通过使用该对象的构造函数传递 `ServiceClientFactory` 包含连接属性的对象。
...
1. 定义用户信息。
   ...
   * 通过调用 `UserImpl` 对象的 `setCanonicalName` 方法。 传递一个字符串值，该值指定用户的规范名称。 例如，您可以指定 `wblue`.
   ...
   * 通过调用 `UserImpl` 对象的 `setFamilyName` 方法。 传递一个指定用户姓氏的字符串值。 例如，您可以指定 `Blue`.
   ...
   >[!NOTE]
   >
   >调用属于以下项的方法： `UserImpl` 对象以设置其他值。 例如，您可以通过调用 `UserImpl` 对象的 `setLocale` 方法。

1. 将用户添加到AEM Forms。

   调用 `DirectoryManagerServiceClient` 对象的 `createLocalUser` 方法，并传递以下值：
...
1. 创建DirectoryManagerService客户端。
   ...
   * 创建 `DirectoryManagerServiceClient.Endpoint.Address` 对象 `System.ServiceModel.EndpointAddress` 构造函数。 将指定WSDL的字符串值传递给AEM Forms服务(例如， `http://localhost:8080/soap/services/DirectoryManagerService?blob=mtom`)。 您无需使用 `lc_version` 属性。 创建服务引用时使用此属性。 确保您指定 `?blob=mtom`.
...
1. 定义用户信息。
   ...
   * 通过为指定字符串值来设置规范名称值 `UserImpl` 对象的 `canonicalName` 字段。
...
1. 验证是否已添加用户。
   ...
   * 创建 `PrincipalSearchFilter` 对象。
   * 通过将表示用户标识符值的字符串值分配给 `PrincipalSearchFilter` 对象的 `userId` 字段。
   * 调用 `DirectoryManagerServiceClient` 对象的 `findPrincipals` 方法并传递 `PrincipalSearchFilter` 对象。 此方法会返回 `MyArrayOfUser` 收藏集对象，其中每个元素为 `User` 对象。 循环访问 `MyArrayOfUser` 收藏集以查找用户。
...
1. 指定要删除的用户。
   ...
   * 调用 `DirectoryManagerServiceClient` 对象的 `findPrincipals` 方法并传递 `PrincipalSearchFilter` 对象。 此方法会返回 `MyArrayOfUser` 收藏集对象，其中每个元素为 `User` 对象。 循环访问 `MyArrayOfUser` 收藏集以查找用户。 此 `User` 对象检索自 `MyArrayOfUser` 收藏集对象用于删除用户。
...
创建组时，请确保该组不存在于同一域中。 也就是说，两个组在同一域中不能具有相同的名称。 要执行此任务，请执行搜索并根据两个值筛选搜索结果。 将主体类型设置为 `com.adobe.idp.um.api.infomodel.Principal.PRINCIPALTYPE_GROUP` 以确保仅返回组。 另外，请确保指定域名。
...
创建组后，可以使用组执行操作。 例如，您可以向组中添加用户。 要将用户添加到组，请检索用户和组的唯一标识符值。 将这些值传递给 `addPrincipalToLocalGroup` 方法。
...
1. 创建组。
   ...
   * 如果组不存在，则调用 `Group` 对象的 `setCommonName` 方法，并传递一个指定组名称的字符串值。
...
>[!NOTE]
>
>配置域时，必须为组和用户设置唯一标识符。 选择的属性不仅必须在LDAP环境中是唯一的，还必须不可变，并且不会在目录中更改。 此属性还必须属于简单字符串数据类型(Active Directory 2000/2003当前允许的唯一例外是 `"objectsid"`，这是一个二进制值)。 Novell eDirectory属性 `"GUID"`例如，不是简单的字符串数据类型，因此将无法使用。
...
>[!NOTE]
>
>不支持在进行LDAP目录同步时创建多个本地用户和组。 尝试此进程可能会导致错误。
...
必须先创建目录管理器服务客户端，然后才能以编程方式执行目录管理器服务操作。 使用Java API可通过创建 `DirectoryManagerServiceClient` 对象。 使用Web服务API，可通过创建 `DirectoryManagerServiceService` 对象。
...
[用户管理器API快速启动](/help/forms/developing/user-manager-java-api-quick.md#user-manager-java-api-quick-start-soap)
...
1. 包括项目文件。
   ...
   在Java项目的类路径中包含客户端JAR文件，例如adobe-usermanager-client.jar。 有关这些文件位置的信息，请参见 [包括AEM Forms Java库文件](/help/forms/developing/invoking-aem-forms-using-java.md#including-aem-forms-java-library-files).

1. 创建DirectoryManagerService客户端。

   创建 `DirectoryManagerServiceClient` 对象通过使用该对象的构造函数传递 `ServiceClientFactory` 包含连接属性的对象。 有关信息，请参阅 [设置连接属性&#x200B;](/help/forms/developing/invoking-aem-forms-using-java.md#setting-connection-properties)*.*
...
1. 调用相应的用户或组操作。
   ...
   因为在此例中，返回值是 `java.util.List` 包含 `Principal` 对象，遍历结果并强制转换 `Principal` 对象可以为 `User` 或 `Group` 对象。

   使用结果 `User` 或 `Group` 对象(继承自 `Principal` 界面)，检索工作流中所需的信息。 例如，域名和规范名值组合在一起唯一标识承担者。 这些权限是通过调用 `Principal` 对象的 `getDomainName` 和 `getCanonicalName` 方法。
...
要使用Directory Manager服务API （Web服务）以编程方式管理用户、组和域，请执行以下任务：
...
1. 调用相应的用户或组操作。
   ...
   要查找用户或组，请调用 `DirectoryManagerServiceService` 对象查找承担者的方法（因为承担者可以是用户或组）。 在以下示例中， `findPrincipalsWithFilter` 使用搜索过滤器(a)调用方法 `PrincipalSearchFilter` 对象)。 使用时 `PrincipalSearchFilter` 对象，仅当 `isLocal` 属性设置为 `true`. 此行为与Java API的行为不同。

   >[!NOTE]
   >
   >如果未在搜索筛选条件中指定最大结果数(通过 `PrincipalSearchFilter.resultsMax` 字段)，最多可返回1000个结果。 这与使用Java API的行为不同，在Java API中，默认最大值为10个结果。 此外，搜索方法如 `findGroupMembers` 将不会产生任何结果，除非在搜索筛选条件中指定了最大结果数(例如，通过 `GroupMembershipSearchFilter.resultsMax` 字段)。 这适用于继承自以下位置的所有搜索过滤器： `GenericSearchFilter` 类。 有关更多信息，请参阅 [AEM Forms API参考](https://www.adobe.com/go/learn_aemforms_javadocs_63_en).

   因为在此例中，返回值是 `object[]` 包含 `Principal` 对象，遍历结果并强制转换 `Principal` 对象可以为 `User` 或 `Group` 对象。

   使用结果 `User` 或 `Group` 对象(继承自 `Principal` 界面)，检索工作流中所需的信息。 例如，域名和规范名值组合在一起唯一标识承担者。 这些权限是通过调用 `Principal` 对象的 `domainName` 和 `canonicalName` 字段。
...
在AEM Forms中， *角色* 是一组用于访问一个或多个系统级资源的权限。 这些权限通过“用户管理”创建，并由服务组件强制执行。 例如，管理员可以将“策略集作者”的角色分配给一组用户。 然后，Rights Management将允许具有该角色的用户通过管理控制台创建策略集。

有两种类型的角色： *默认角色* 和 *自定义角色*. 默认角色(*系统角色)* 已驻留在AEM Forms中。 假定管理员不能删除或修改默认角色，因此这些角色是不可变的。 因此，管理员创建的自定义角色是可变的，管理员随后可以修改或删除这些角色。
...
创建服务客户端后，您可以调用角色或权限操作。 服务客户端允许您分配、删除和确定角色和权限。
...
1. 创建AuthorizationManagerService客户端。
   ...
   创建 `AuthorizationManagerServiceClient` 对象通过使用该对象的构造函数传递 `ServiceClientFactory` 包含连接属性的对象。

1. 调用相应的角色或权限操作。

   要为主体分配角色，请调用 `AuthorizationManagerServiceClient` 对象的 `assignRole` 方法，并传递以下值：

   * A `java.lang.String` 包含角色标识符的对象
   * 一个数组 `java.lang.String` 包含主体标识符的对象。

   要从主体中删除角色，请调用 `AuthorizationManagerServiceClient` 对象的 `unassignRole` 方法，并传递以下值：
...
1. 创建AuthorizationManagerService客户端。
   ...
   * 创建 `AuthorizationManagerServiceClient.Endpoint.Address` 对象 `System.ServiceModel.EndpointAddress` 构造函数。 将指定WSDL的字符串值传递给AEM Forms服务(例如， `http://localhost:8080/soap/services/AuthorizationManagerService?blob=mtom`.) 您无需使用 `lc_version` 属性。 创建服务引用时使用此属性。
...
1. 调用相应的角色或权限操作。
   ...
   * A `MyArrayOf_xsd_string` 包含主体标识符的对象。
   ...
   要从主体中删除角色，请调用 `AuthorizationManagerServiceService` 对象的 `unassignRole` 方法，并传递以下值：

   * A `string` 包含角色标识符的对象。
   * 一个数组 `string` 包含主体标识符的对象。
...
例如，假设用户在Web页中输入用户名和密码，并将值提交到托管Forms的J2EE应用程序服务器。 Forms自定义应用程序可以使用Authentication Manager服务对用户进行身份验证。
...
<table>
 ...
 <tbody>
  ...
  <tr>
   <td><p>4</p></td>
   <td><p>用户信息与表单设计合并，并将表单呈现给用户。 </p></td>
  </tr>
 </tbody>
</table>
...
在以编程方式验证用户之前，必须创建AuthenticationManagerService客户端。 使用Java API时，创建 `AuthenticationManagerServiceClient` 对象。
...
在用户通过身份验证后，即可基于通过身份验证的用户创建上下文。 然后，您可以使用内容调用其他AEM Forms服务。 例如，您可以使用上下文创建 `EncryptionServiceClient` 并用密码加密PDF文档。 确保已验证的用户具有名为的角色 `Services User` 调用AEM Forms服务时所需的服务。
...
### 使用Java API对用户进行身份验证 {#authenticate-a-user-using-the-java-api}
...
1. 创建AuthenticationManagerServices客户端。
   ...
   创建 `AuthenticationManagerServiceClient` 对象通过使用该对象的构造函数传递 `ServiceClientFactory` 包含连接属性的对象。

1. 调用身份验证操作。

   调用 `AuthenticationManagerServiceClient` 对象的 `authenticate` 方法，并传递以下值：

   * A `java.lang.String` 包含用户名的对象。
   * 字节数组(a) `byte[]` 对象)，其中包含用户的密码。 您可以获取 `byte[]` 对象 `java.lang.String` 对象的 `getBytes` 方法。
...
### 使用Web服务API对用户进行身份验证 {#authenticate-a-user-using-the-web-service-api}
...
1. 调用身份验证操作。
   ...
   * A `string` 包含用户名的对象
   * 字节数组(a) `byte[]` 对象)，其中包含用户的密码。 您可以获取 `byte[]` 对象转换 `string` 包含密码的对象 `byte[]` 数组，其逻辑如下例所示。
...
您可以使用用户管理API以编程方式同步用户。 同步用户时，您会使用位于用户存储库中的用户数据更新AEM Forms。 例如，假设您将新用户添加到用户存储库。 执行同步操作后，新用户将成为AEM forms用户。 此外，您用户存储库中不再存在的用户也将从AEM Forms中删除。
...
在以编程方式同步用户之前，必须创建 `UserManagerUtilServiceClient` 对象。

**指定企业域**

在使用用户管理API执行同步操作之前，请指定用户所属的企业域。 您可以指定一个或多个企业域。 必须先使用Administration Console设置企业域，然后才能以编程方式执行同步操作。 (请参阅 [管理帮助](https://www.adobe.com/go/learn_aemforms_admin_63).)
...
1. 包括项目文件。
   ...
   在Java项目的类路径中包含客户端JAR文件，例如adobe-usermanager-client.jar和adobe-usermanager-util-client.jar。

1. 创建UserManagerUtilServiceClient客户端。

   创建 `UserManagerUtilServiceClient` 对象通过使用该对象的构造函数传递 `ServiceClientFactory` 包含连接属性的对象。

1. 指定企业域。

   * 调用 `UserManagerUtilServiceClient` 对象的 `scheduleSynchronization` 用于启动用户同步操作的方法。
   * 创建 `java.util.Set` 使用实例 `HashSet` 构造函数。 确保您指定 `String` 作为数据类型。 此 `Java.util.Set` 实例存储了应用同步操作的域名。
