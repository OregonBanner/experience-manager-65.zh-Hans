...
1. 打开`http://<hostname>:<port>/ system/console/bundles`并点按&#x200B;**[!UICONTROL 安装／更新]**&#x200B;以上传OSGi捆绑包。 验证已上载包的状态是否显示为&#x200B;**活动**。 如果软件包的状态未显示为&#x200B;**活动**，请重新启动服务器。
...
### 将交互通信另存为草稿{#save-as-draft-agent-ui}

执行以下步骤将交互通信另存为草稿：

1. 在“Forms管理器”中选择交互式通信，然后点按&#x200B;**[!UICONTROL 打开代理UI]**。

1. 在代理UI中进行适当的更改，然后点按&#x200B;**[!UICONTROL 另存为草稿]**。

1. 在&#x200B;**[!UICONTROL 名称]**&#x200B;字段中指定草稿的名称，然后点按&#x200B;**[!UICONTROL 完成]**。

将交互通信另存为草稿后，点按&#x200B;**[!UICONTROL 保存更改]**&#x200B;以保存对草稿所做的任何进一步更改。
...
[拖] 动指在将交互式通信另存为草稿后生成的草稿版本的唯一标识符。
...
`save`、`update`、`get`和`getAll`操作调用数据库服务，将交互式通信另存为草稿，更新交互式通信，从数据库检索数据，并检索数据库中所有可用的交互式通信的数据。 此示例使用`mySQLDataBaseServiceCRUD`作为数据库服务的名称。
...
>[!NOTE]
>
>确保将`pom.xml`文件中的`aemfd-client-sdk`依赖项更新为6.0.160。
