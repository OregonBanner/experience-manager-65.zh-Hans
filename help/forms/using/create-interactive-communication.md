...
1. 输入以下内容：
   ...
   * **[!UICONTROL Web]**:选择此选项可生成Web通道或交互式通信的响应式输出。
   * **[!UICONTROL 交互通信Web模板]**:浏览并选择Web模板。
   * **[!UICONTROL 主题]** , **[!UICONTROL 选择主题]**:浏览并选择主题，以设置交互式通信的Web频道的样式。 有关详细信息，请参 [阅AEM Forms中的主题](/help/forms/using/themes.md)。

   * **[!UICONTROL 将打印作为Web渠道的主渠道]**:选择此选项可创建与打印渠道同步的Web渠道。 使用打印渠道作为Web渠道的主渠道可确保Web渠道的内容和数据绑定从打印渠道派生，并且当您点按同步时，打印渠道中所做的更改会反映在Web渠道中。 但是，允许作者根据需要中断Web渠道中特定组件的继承。 有关详细信息，请参 [阅将Web渠道与打印渠道同步](../../forms/using/create-interactive-communication.md#synchronize)。
如果选择“ **[!UICONTROL 使用打印为Web渠道的主视图]** ”选项，则可以选择以下任一模式以生成Web渠道：

      * **[!UICONTROL 自动布局]**:选择此模式可自动从打印渠道为Web渠道生成占位符、内容和数据绑定。
      * **[!UICONCONTROL手动组织**:选择此模式，以使用“数据源”选项卡中提供的主内容手动选择并将“打印”渠道元素添加 **[!UICONTROL 到Web渠道]** 。 有关详细信息，请参 [阅选择打印渠道元素以创建Web渠道内容](#selectprintchannelelements)。
...
文档片段中使用的未绑定变量有时不能显示在创作界面上。 这可能是由于文档片段中的内联文本规则或条件片段的情况。 在这种情况下，以蓝色突出显示的工具提示将作为文档片段的一部分显示。 工具提示显示文档片段中使用的未绑定变量的数量。

![未绑定变量](assets/df_unbound_variable_new.png)

点按文档片段，点 ![按configure_icon](assets/configure_icon.png) (Configure)，然后从交互式通信的Sidekick中点 **[!UICONTROL 按属性]** 。 “变 **[!UICONTROL 量和数据模型对象]** ”部分列出变量，包括隐藏变量以及文档片段中使用的数据模型对象。 使用每 ![个数据模型对象](https://helpx.adobe.com/content/dam/help/images/en/edit.png) 或变量旁边的编辑（编辑）图标编辑属性。

1. 要设置变量的绑定，请点按变量，选择 ![configure_icon](assets/configure_icon.png) (Configure)，然后在提要栏的“属性”面板中设置绑定属性。

   * **无**:代理将填写变量的值。
   * **文本片段**:如果选中此项，则可以浏览并选择其内容呈现在字段中的文本文档片段。 只有这些文本文档片段才能绑定到其中没有变量的变量。
   * **数据模型对象**:选择在字段中填充其值的表单数据模型属性。
   * **** 默认值：您可以使用此字段为变量定义默认值。 预览交互式通信时或在代理UI中显示该值。
   * **** 显示图案：您还可以为变量定义显示格式。 从“类型”( **Type** )下拉列表中选择任意预定义选项，以将显示格式应用于变量。 选择 **“自定义** ”(Custom)可定义列表中不可用的显示模式。 有关详细信息，请参阅 [数据显示模式](../../forms/using/create-interactive-communication.md#datadisplaypatterns)。
...
1. 切换到 **[!UICONTROL Web渠道]**。 Web频道显示在交互式通信编辑器中。 首次从“打印”通道切换到Web通道时，会进行自动同步。 有关详细信息，请参 [阅从打印渠道同步Web渠道](../../forms/using/create-interactive-communication.md#synchronize)。

   由于在此示例中我们将“打印”用作Web的主页，因此打印渠道占位符、内容和数据绑定将同步到Web渠道。 但是，您可以更改和自定义Web渠道中的特定内容。 [取消继承](../../forms/using/create-interactive-communication.md#main-pars-header-103384010) ，以取消使用打印渠道生成的目标区域和变量，以便能够自定义内容。

   ![webchannelassets](assets/webchannelassets.png)

   点按文档片段，点 ![按configure_icon](assets/configure_icon.png) (Configure)，然后从交互式通信的Sidekick中点 **[!UICONTROL 按属性]** 。 “变 **[!UICONTROL 量和数据模型对象]** ”部分列出变量，包括隐藏变量以及文档片段中使用的数据模型对象。 使用每 ![个数据模型对象](https://helpx.adobe.com/content/dam/help/images/en/edit.png) 或变量旁边的编辑（编辑）图标编辑属性。 此外，对于在Web通道中使用打印通道自动生成的文档片段 [，请使用每个数据模型对象和变量旁边的](../../forms/using/create-interactive-communication.md#main-pars-header-1213963149) （取消继承）图标来取消继承 ![](assets/cancelinheritance.png)[](../../forms/using/create-interactive-communication.md#main-pars-header-103384010) ，并能够编辑继承。
...
可以使用“预 **览”选项** ，评估交互式通信的外观。 交互式通信的Web频道还提供了一个选项，用于模拟各种设备的交互式通信体验。 例如，iPhone、iPad和Desktop。 您可以将“预 **览** ”和“模拟器 ****![](assets/ruler.png) ”标尺选项结合使用，以预览不同屏幕尺寸的设备的Web输出。 预览中的范例数据是从指定的表单数据模型填充的。
...
1. 指定以下内容：
   ...
   * **[!UICONTROL 绑定类型]**:为字段选择以下绑定类型之一。
      ...
      * 无：代理将填写该属性的值。
      * 文本片段：如果选中此项，则可以浏览并选择其内容呈现在字段中的文本文档片段。 或者，将文本文档片段拖放到字段名称以设置它们之间的绑定。 文本文档片段不得包含任何变量。
   ...
   * **[!UICONTROL 显示图案]**:您还可以为字段定义显示格式。 从“类型”( **Type** )下拉列表中选择任意预定义选项，以将显示格式应用于字段。 选择 **“自定义** ”(Custom)可定义列表中不可用的显示模式。 有关详细信息，请参阅 [数据显示模式](../../forms/using/create-interactive-communication.md#datadisplaypatterns)
...
创作界面使您能够为在创建用于打印和Web渠道的交互式通信时可用的字段、变量和表单数据模型元素定义数据显示模式。

要配置数据显示模式，请点按元素，选择 ![configure_icon](assets/configure_icon.png) (Configure)，然后在提要栏的“属性”面板中设置 **[!UICONTROL 显示模式]** 。 从“类型”( **[!UICONTROL Type]** )下拉列表中选择任意预定义选项，以查看与选定类型关联的图案。 从“ **[!UICONTROL 类型]****[!UICONTROL ”(Type]** )下拉列表中选择“自定义”(Custom)，以定义列表中不可用的模式。 在“图案”( **[!UICONTROL Pattern]** )字段中编辑值会自动将类型修改为“自 **[!UICONTROL 定义”(Custom)]**。
...
在从打印渠道生成Web内容后，您可以为字段、变量或表单数据模型元素重新定义显示模式。 因此，元素可以具有为打印和网络通道定义的不同显示模式。 如果未为打印通道中的元素定义显示图案并使用打印通道自动生成Web内容，则为打印通道中的元素定义的数据绑定定义了“类型 **[!UICONTROL ”(Type]** )下拉列表中可用的显示图案选项。 如果没有为元素定义绑定，则元素的数据类型将定义可用的显示模式选项。 例如，如果为打印通道中的元素创建Number类型的数据绑定，则“ **[!UICONTROL Type]** ”（类型）下拉列表中可用的显示模式选项的类型为各种格式的Number。

切换至“预 **览** ”模式或打开代理UI以查看应用于这些元素的显示模式。
...
如果不将显示模式应用于数字数据类型的变量，“打印预览”会根据全局文档片段配置显示该模式。 如果对默认的全局文档片段配置应用更改，代理UI仍会根据为区域设置定义的默认分隔符显示模式。
...
1. 在表单元格中选择默认文本框，然后点按( ![](https://helpx.adobe.com/content/dam/help/images/en/edit.png) 编辑)。
...
1. 切换到“预 **览** ”模式以查看输出。 表会根据表的第一列自动排序。
1. 单击列标题可根据列对值进行排序。
...
1. 在“属 **性** ”页面上，选择“渠 **道** ”选项卡。
1. 选中“ **Web** ”复选框，然后为Web渠道选择模板。
1. 选 **择“使用打印作为Web渠道的主渠道** ”，以启用Web渠道与打印渠道之间的同步。
1. 点按 **保存并关闭** ，以保存更改。

   同样，您也可以点按 **渠道选项卡** 上的 **Web** 复选框，从交互通信中删除Web渠道。

## 将“按钮”组件添加到Web渠道 {#add-button-component-to-the-web-channel}

您可以将按钮作为组件添加到交互式通信的Web频道。 使用规则编 [辑器定义规则](../../forms/using/rule-editor.md) ，以便能够在点击按钮时导航到其他交互通信、自适应表单、图像或文档片段等其他资产或外部URL。
...
1. 在“时 **间** ”部分中：
   ...
   1. 指定交互通信、自适应表单、资产或网页的URL。 例如，以下格式指定URL以导航到另一个交互通信：https://&lt;server-name>:&lt;port>/editor.html/content/forms/af/&lt;Interactive Communication name>/channels/&lt;channel name - print or web>.html
...
当您在创建交互式通信时选择“打印为Web渠道的主渠道”时，将创建与打印渠道同步的Web渠道，并且Web渠道的内容和数据绑定从打印渠道派生，并且点按同步时，打印渠道中所做的更改可能会反映在Web渠道中。
...
* **[!UICONTROL 自动布局]**:选择此模式可自动从打印渠道为Web渠道生成占位符、内容和数据绑定。
* **[!UICONTROL 手动组织]**:选择此模式，可使用“数据源”选项卡中提供的主内容手动选择并将“打印”渠道元素添加到Web渠道。 有关详细信息，请参 [阅选择打印渠道元素以创建Web渠道内容](#selectprintchannelelements)。

![创建IC选项](assets/create_ic_options_updated_new.png)

>[!NOTE]
>
>同步渠道仅同步文档片段、图像、条件、列表和布局片段，它们从打印渠道到Web渠道。 包含此类元素的子表单或父节点不会同步。

### 选择打印渠道元素以创建Web渠道内容 {#selectprintchannelelements}

如果在创建交互式通信时选择“打印为主设备”，但不选择自动同步选项，则还可以将打印渠道元素拖放到Web渠道创作界面。

导航到“ **数据源** ”>“ **主内容** ”以查看“打印”渠道元素。 将目标区域、字段或表拖放到Web渠道创作界面。 元素名称旁边的蓝色圆圈图标表示“打印”渠道元素已包含在Web渠道中。
...
将指针悬停在Web渠道中的相关目标区域或变量上，选择取消继 ![承](assets/cancelinheritance.png) （取消继承），然后在取消继承对话框中，点按 **[!UICONTROL 是]**。

目标区域中组件的继承将被取消，现在您可以根据需要编辑它们。
...
如果您使用“打印为Web渠道的主渠道”并对“打印”渠道进行更改，则可以同步内容以将新做的更改引入Web渠道。
...
* **** 父项：选择组件的父项。
* **** 查看SOM表达式：查看组 [件的SOM表达式](../../forms/using/using-som-expressions-adaptive-forms.md) 。
...
* **** 取消继承：取 [消目标区域](../../forms/using/create-interactive-communication.md#main-pars-header-103384010) 内组件的继承以对其进行编辑。

### Group objects in Panel {#groupobjectspanel}

Web渠道创作界面便于将面板中的组件分组以能够同时对这些组件执行操作。 “内 **容** ”选项卡将分组的组件列为内容树中面板的子元素。
