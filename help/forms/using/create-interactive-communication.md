...
1. 输入以下内容：
   ...
   * **[!UICONTROL 将“打印为主控”用于Web渠道]**:选择此选项可创建与打印渠道同步的Web渠道。 将打印渠道用作Web渠道的主控，可确保Web渠道的内容和数据绑定是从打印渠道派生的，并且在点按“同步”时，在打印渠道中所做的更改会反映在Web渠道中。 但是，作者可以根据需要中断Web渠道中特定组件的继承。 有关更多信息，请参阅 [将Web渠道与打印渠道同步](../../forms/using/create-interactive-communication.md#synchronize).
如果您选择 **[!UICONTROL 将“打印为主控”用于Web渠道]** 选项，则可以选择以下任何模式来生成Web渠道：

      * **[!UICONTROL 自动布局]**:选择此模式可自动从打印渠道为Web渠道生成占位符、内容和数据绑定。
      * **[!UICONTROL 手动组织]**:选择此模式，可使用 **[!UICONTROL 数据源]** 选项卡。 有关更多信息，请参阅 [选择打印渠道元素以创建Web渠道内容](#selectprintchannelelements).
...
您还可以通过使用 **[!UICONTROL 资产]** 选项卡。 拖动片段时目标区域的蓝色底纹表示文档片段可以拖放到目标区域。
...
点按文档片段，点按 ![configure_icon](assets/configure_icon.png) （配置），然后点按 **[!UICONTROL 属性]** 的Sidekick中。 的 **[!UICONTROL 变量和数据模型对象]** 部分列出变量，包括隐藏变量以及文档片段中使用的数据模型对象。 使用 ![编辑](assets/edit.svg) 用于编辑属性的每个数据模型对象或变量旁边的（编辑）图标。
...
1. 要设置变量绑定，请点按变量并选择 ![configure_icon](assets/configure_icon.png) （配置），然后在侧栏的“属性”面板中设置绑定属性。
   ...
   * **显示模式：** 您还可以为变量定义显示格式。 从 **类型** 下拉列表来将显示格式应用到变量。 选择 **自定义** 定义列表中不可用的显示模式。 有关更多信息，请参阅 [数据显示模式](../../forms/using/create-interactive-communication.md#datadisplaypatterns).

   导航到 [变量和数据模型对象](../../forms/using/create-interactive-communication.md#hiddenvariables) 设置文档片段中隐藏变量的绑定。

   您还可以拖放数据源元素或文本文档片段以设置变量绑定。  要使用任何数据源元素创建绑定，请选择 **数据源** ，然后将元素拖放到变量名称中。 数据源元素和变量必须是相同类型才能成功设置绑定。 如果将数据源元素拖放到已绑定的变量，则新元素会替换前一个元素，以使用该变量创建新绑定。 同样，选择 **资产** 选项卡，并将文本文档片段拖放到变量名称中，以设置它们之间的绑定。 文本文档片段不得包含任何变量。
...
1. 切换到 **[!UICONTROL Web渠道]**. Web渠道显示在交互式通信编辑器中。 首次从“打印”渠道切换到Web渠道时，会进行自动同步。 有关更多信息，请参阅 [从打印渠道同步Web渠道](../../forms/using/create-interactive-communication.md#synchronize).
   ...
   由于在本例中，我们将“打印”用作Web的主控，因此“打印”渠道占位符、内容和数据绑定会同步到Web渠道。 但是，您可以更改和自定义Web渠道中的特定内容。 [取消继承](#cancelinheritance) 对于使用打印渠道生成的目标区域和变量，以便能够自定义内容。

   ![webchannelassets](assets/webchannelassets.png)

   点按文档片段，点按 ![configure_icon](assets/configure_icon.png) （配置），然后点按 **[!UICONTROL 属性]** 的Sidekick中。 的 **[!UICONTROL 变量和数据模型对象]** 部分列出变量，包括隐藏变量以及文档片段中使用的数据模型对象。 使用 ![编辑](assets/edit.svg) 用于编辑属性的每个数据模型对象或变量旁边的（编辑）图标。 此外，对于已 [自动生成](#synchronize) 在使用“打印”渠道的Web渠道中，使用 ![取消继承](assets/cancelinheritance.png) （取消继承）每个数据模型对象和要 [取消继承](#cancelinheritance) 并能够编辑它们。
...
1. 要在选择Web渠道的Web渠道中添加其他组件，请点按 **[!UICONTROL 组件]**. 根据需要将组件拖放到交互式通信的Web渠道中，然后继续配置这些组件。
   ...
   | 图像 | 允许插入图像。 |
   | 面板 | 用于添加 [面板](../../forms/using/create-interactive-communication.md#add-panel-component-to-the-web-channel) 到交互式通信。 |
   | 表 | 添加表格以便按行和列整理数据。 |
   | 目标区域 | 在Web渠道中插入目标区域以组织特定于Web渠道的组件。 目标区域是一个纯容器，用于对特定于Web渠道的组件进行分组。 |
   | 文本 | 向交互式通信的Web渠道中添加富文本。 文本还可以利用表单数据模型对象来使内容动态。 |
   | 按钮 | 用于添加 [按钮](../../forms/using/create-interactive-communication.md#add-button-component-to-the-web-channel) 到交互式通信。 您可以使用按钮组件导航到其他交互式通信、自适应表单、其他资产（如图像或文档片段）或外部URL。 |
...
您可以使用 **预览选项** 以评估交互式通信的外观。 交互式通信的Web渠道还提供了用于模拟各种设备的交互式通信体验的选项。 例如，iPhone、iPad和桌面。 您可以同时使用 **预览** 和 **模拟器** ![标尺](assets/ruler.png) 选项，以预览不同屏幕大小的设备的web输出。 预览中的示例数据是从指定的表单数据模型填充的。
...
1. 指定以下内容：
   ...
   * **[!UICONTROL 绑定类型]**:为字段选择以下绑定类型之一。
      ...
      * 数据模型对象：选择在字段中填充其值的表单数据模型属性。 或者，选择 **数据源** ，然后将属性拖放到字段中。
   * **[!UICONTROL 默认值]**:默认值可确保当指定的数据模型对象或文本片段没有提供任何值时，该字段不为空。 如果数据绑定类型为“无”，则会在字段中预填充默认值。
   * **[!UICONTROL 显示图案]**:您还可以为字段定义显示格式。 从 **类型** 下拉列表来应用显示格式。 选择 **自定义** 定义列表中不可用的显示模式。 有关更多信息，请参阅 [数据显示模式](../../forms/using/create-interactive-communication.md#datadisplaypatterns)
...
要配置数据显示模式，请点按元素，选择 ![configure_icon](assets/configure_icon.png) （配置）并在 **[!UICONTROL 属性]** 的子菜单。 从 **[!UICONTROL 类型]** 下拉列表，查看与选定类型关联的模式。 选择 **[!UICONTROL 自定义]** 从 **[!UICONTROL 类型]** 下拉列表，用于定义列表中不可用的模式。 编辑 **[!UICONTROL 图案]** 字段会自动将类型修改为 **[!UICONTROL 自定义]**.

要应用显示模式，“模式”字段中定义的字符或位数必须匹配或超过字段、变量和表单数据模型元素值中定义的字符或位数。 有关更多信息，请参阅 [示例](../../forms/using/create-interactive-communication.md#greaternumberofdigits).

![data_display_pattern_example](assets/data_display_patterns_ssn_new.png)

在从打印渠道生成Web内容后，可以为字段、变量或表单数据模型元素重新定义显示模式。 因此，元素可以为打印和Web渠道定义不同的显示模式。 如果您没有为打印渠道中的元素定义显示模式，并且使用打印渠道自动生成Web内容，则为打印渠道中的元素定义的数据绑定定义了 **[!UICONTROL 类型]** 下拉列表。 如果没有为元素定义绑定，则元素的数据类型将定义可用的显示模式选项。 例如，如果为打印渠道中的元素创建Number类型的数据绑定，则 **[!UICONTROL 类型]** 下拉列表具有各种格式的“数字”类型。

切换到 **预览** 模式或打开代理UI，以查看应用于这些元素的显示模式。
...
如果未为变量或表单数据模型元素指定显示模式，则 [全局文档片段配置](https://helpx.adobe.com//experience-manager/6-5/forms/using/interactive-communication-configuration-properties.html) 默认情况下使用。
...
1. 为表的每个单元格指定数据绑定。 要创建可重复行，请在属于公共集合属性的行中插入表单数据模型属性。
   ...
   1. 配置属性：
      ...
      * **[!UICONTROL 绑定类型]**:为字段选择以下绑定类型之一。
         ...
         * **[!UICONTROL 无]**
         * **[!UICONTROL 数据模型对象]**:字段中会填充表单数据模型属性的值。 或者，选择 **数据源** ，然后将属性拖放到字段中。
...
点按Web模板中的根面板，然后点按 **+** 添加 **表** 组件。 在交互式通信中插入包含两行的表。 表的第一行表示表标题。
...
1. 选择 **添加行** 或 **删除行** 分别添加或删除表行。 使用 **上移** 和 **向下移动** 用于重新排列表格中行的选项。

![组件工具栏](assets/component_toolbar_table_row_new.png)

**A.** 添加行 **B.** 删除行 **C.** 上移 **D.** 向下移动

#### 在表格单元格中添加或编辑文本 {#addedittexttable}

1. 在表格单元格中选择默认文本框，然后点按 ![编辑](assets/edit.png) （编辑）。
1. 在表格单元格中键入文本，然后点按 ![done_icon](assets/done_icon.png) 来保存它。
...
1. 在表格单元格中选择默认文本框，然后点按 ![编辑](assets/edit.svg) （编辑）。
...
1. 在 **属性** 页面，选择 **渠道** 选项卡。
1. 选择 **Web** 复选框，然后选择Web渠道的模板。
1. 选择 **将“打印为主控”用于Web渠道** 启用Web渠道与打印渠道之间的同步。
1. 点按 **保存并关闭** 以保存更改。

   同样，您也可以点按 **Web** 复选框 **渠道** 选项卡，以从交互式通信中删除Web渠道。

## 将按钮组件添加到Web渠道 {#add-button-component-to-the-web-channel}

您可以将按钮作为组件添加到交互式通信的Web渠道中。 使用定义规则 [规则编辑器](../../forms/using/rule-editor.md) 以便能够通过点按按钮导航到其他交互式通信、自适应表单、其他资产（如图像或文档片段）或外部URL。

要添加按钮并在其上定义规则，请执行以下操作：

1. 点按Web模板中的根面板，然后点按 **+** 添加 **按钮** 组件。
1. 点按按钮组件，然后点按 ![edit-rules](assets/edit-rules.png) 用于在点按按钮时定义规则。
1. 在 **When** 选择 **点击** 从“按钮”(Button)下拉列表的状态中。
1. 在 **然后** 部分：

   1. 从下拉列表中选择一个操作。 例如，选择 **导航到** 作为操作类型。

   1. 指定交互式通信、自适应表单、资产或网页的URL。 例如，以下格式指定URL以导航到另一个交互式通信：https://&lt;server-name>:&lt;port>/editor.html&lt;interactive communication=&quot;&quot; name=&quot;&quot;>/channels/&lt;channel name=&quot;&quot; print=&quot;&quot; or=&quot;&quot; web=&quot;&quot;>.html
   1. 指定在同一选项卡、新选项卡或新窗口中打开资产的选项。
   1. 点按 **完成** 然后点按 **关闭** 来保存规则。
...
1. 插入 **面板** 组件（使用以下任意选项）：
   ...
   * 点按 **面板** 在 **内容** 浏览器面板，点按 **添加子面板**. 选择 **添加子面板** 选项 **添加子面板** 对话框。 输入面板组件的标题以及可选描述和名称。

1. 点按 **内容** 浏览器以在面板上执行其他操作，例如配置、编辑规则、复制、删除和插入组件。

   您还可以在 **内容** 浏览器，以在右侧窗格中反映交互式通信结构的变化。
...
导航到 **数据源** > **主控内容** 查看打印渠道元素。 将目标区域、字段或表拖放到Web渠道创作界面中。 元素名称旁边的蓝色圆图标表示“打印”渠道元素已包含在Web渠道中。
...
将鼠标悬停在Web渠道中的相关目标区域或变量上，然后选择 ![取消继承](assets/cancelinheritance.png) （取消继承），然后在取消继承对话框中，点按 **[!UICONTROL 是]**.
...
* **群组：** 如果要剪切、复制或粘贴多个组件，请选择多个组件。
* **父项：** 选择组件的父项。
* **查看SOM表达式：** 查看 [SOM表达式](../../forms/using/using-som-expressions-adaptive-forms.md) 的子菜单。

* **在面板中对象组：** 在面板中对组件进行分组，以便能够同时对这些组件执行操作。 有关详细信息，请参阅 [在面板中对象组](#groupobjectspanel).

* **取消继承：** [取消继承](#cancelinheritance) 来编辑组件。

### 在面板中对象组 {#groupobjectspanel}

Web渠道创作界面便于将面板中的组件分组以便能够同时对这些组件执行操作。 的 **内容** 选项卡将分组的组件列为内容树中面板的子元素。

1. 点按组件并选择群组( ![群组](assets/group.jpg))操作。
1. 选择多个组件并点按 **在面板中对象组**.

   ![组对象](assets/component_toolbar_group_objects_new.png)

1. 在 **在面板中对象组** 对话框，输入面板的名称。
1. 为面板输入可选标题和描述。
...
使用PrintChannel API为交互式通信的打印渠道定义输出格式。 如果不定义输出格式，则AEM Forms将以PDF格式生成输出。
...
要以任何其他格式生成输出，请指定输出格式类型。 请参阅 [PrintChannel API](https://helpx.adobe.com/experience-manager/6-5/forms/javadocs/com/adobe/fd/output/api/PrintConfig.html) ，以查看支持的输出格式类型列表。
