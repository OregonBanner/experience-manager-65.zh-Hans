...
1. 输入以下内容：
   ...
   * **[!UICONTROL 将打印作为主控用于Web渠道]**:选择此选项可创建与打印渠道同步的Web渠道。将打印渠道用作Web渠道的主控，可确保Web渠道的内容和数据绑定是从打印渠道派生的，并且点按同步时，打印渠道中所做的更改会反映在Web渠道中。 但是，作者可以根据需要中断Web渠道中特定组件的继承。 有关详细信息，请参阅[将Web渠道与打印渠道同步](../../forms/using/create-interactive-communication.md#synchronize)。
如果选择**[!UICONTROL 将“打印为主控”作为Web渠道]**&#x200B;选项，则可以选择以下任何模式以生成Web渠道:

      * **[!UICONTROL 自动布局]**:选择此模式可自动为Web渠道从打印渠道生成占位符、内容和数据绑定。
      * **[!UICONTROL 手动组织]**:选择此模式，以使用“数据源”选项卡中提供的主控内容手动选择“打印渠道”元素并将其 **[!UICONTROL 添加]** 到Web渠道。有关详细信息，请参阅[选择“打印渠道元素”以创建Web渠道内容](#selectprintchannelelements)。
...
您还可以使用&#x200B;**[!UICONTROL 资产]**&#x200B;选项卡将新片段拖放到目标区域，以替换文档区域与目标片段之间的绑定。 拖动片段时目标区域的蓝色底纹表示文档片段可以拖放到目标区域。
...
点按文档片段，点按![configure_icon](assets/configure_icon.png)(Configure)，然后点按交互通信的Sidekick中的&#x200B;**[!UICONTROL 属性]**。 **[!UICONTROL 变量和数据模型对象]**&#x200B;部分列表变量，包括隐藏变量和文档片段中使用的数据模型对象。 使用每个数据模型对象或变量旁的![edit](assets/edit.svg)（编辑）图标编辑属性。
...
1. 要设置变量的绑定，请点按变量，选择![configure_icon](assets/configure_icon.png)(Configure)，然后在提要栏的“属性”面板中设置绑定属性。
   ...
   * **显示图案：** 您还可以为变量定义显示格式。从&#x200B;**类型**&#x200B;下拉列表中选择任何预定义的选项，将显示格式应用于变量。 选择&#x200B;**自定义**&#x200B;以定义列表中不可用的显示模式。 有关详细信息，请参阅[数据显示模式](../../forms/using/create-interactive-communication.md#datadisplaypatterns)。

   导航到[变量和数据模型对象](../../forms/using/create-interactive-communication.md#hiddenvariables)以设置隐藏变量在文档片段中的绑定。

   您还可以拖放数据源元素或文本文档片段以设置变量绑定。  要创建与任何数据源元素的绑定，请选择&#x200B;**数据源**&#x200B;选项卡，并将元素拖放到变量名。 数据源元素和变量的类型必须相同，才能成功设置绑定。 如果将数据源元素拖放到已绑定的变量，则新元素将替换前一个元素，以使用该变量创建新绑定。 同样，选择&#x200B;**资产**&#x200B;选项卡，将文本文档片段拖放到变量名以设置它们之间的绑定。 文本文档片段不能包含任何变量。
...
1. 切换到&#x200B;**[!UICONTROL Web渠道]**。 Web渠道显示在交互式通信编辑器中。 首次从“打印”渠道切换到“Web”渠道时，将进行自动同步。 有关详细信息，请参阅[从打印渠道同步Web渠道](../../forms/using/create-interactive-communication.md#synchronize)。
   ...
   由于本例中我们将“打印”作为Web的主控，因此“打印”渠道占位符、内容和数据绑定将同步到Web渠道。 但是，您可以在Web渠道中更改和自定义特定内容。 [取](#cancelinheritance) 消继承已使用打印目标生成的渠道区域和变量，以便能够自定义内容。

   ![Web频道资源](assets/webchannelassets.png)

   点按文档片段，点按![configure_icon](assets/configure_icon.png)(Configure)，然后点按交互通信的Sidekick中的&#x200B;**[!UICONTROL 属性]**。 **[!UICONTROL 变量和数据模型对象]**&#x200B;部分列表变量，包括隐藏变量和文档片段中使用的数据模型对象。 使用每个数据模型对象或变量旁的![edit](assets/edit.svg)（编辑）图标编辑属性。 此外，对于在使用打印渠道的Web渠道中已经[自动生成的](#synchronize)的文档片段，请使用每个数据模型对象和变量旁的![取消继承](assets/cancelinheritance.png)（取消继承）图标[取消继承](#cancelinheritance)并能够编辑它们。

1. 要在Web渠道中添加其他组件，在选择Web渠道时，点按&#x200B;**[!UICONTROL 组件]**。 根据需要在交互通信的Web渠道中拖放组件，然后继续配置它们。

   | 组件 | 功能 |
   |---|---|
   | 图表 | 添加一个图表，您可以在交互通信中使用该图表来直观地表示从表单数据模型集合检索的二维数据。 有关详细信息，请参阅[使用图表组件](../../forms/using/chart-component-interactive-communications.md)。 |
   | 文档片段 | 允许您向交互式通信中添加可重用的组件、文本、列表或条件。 您添加到交互式通信的可重用组件可以是基于表单数据模型的组件，也可以是没有表单数据模型的组件。 |
   | 图像 | 允许插入图像。 |
   | 面板 | 允许您将[面板](../../forms/using/create-interactive-communication.md#add-panel-component-to-the-web-channel)添加到交互式通信中。 |
   | 表 | 添加表格以便按行和列整理数据。 |
   | 目标区域 | 在Web渠道中插入目标区域以组织Web渠道特定的组件。 目标区域是一个简单容器，它允许您对Web渠道特定组件进行分组。 |
   | 文本 | 将富文本添加到交互式通信的Web渠道。 文本还可以利用表单数据模型对象来使内容动态化。 |
   | 按钮 | 允许您向交互通信添加[Button](../../forms/using/create-interactive-communication.md#add-button-component-to-the-web-channel)。 您可以使用“按钮”组件导航到其他交互通信、自适应表单、其他资产(如图像或文档片段)或外部URL。 |
...
可以使用&#x200B;**预览选项**&#x200B;评估交互通信的外观。 交互通信的Web渠道还提供了一个选项，用于模拟各种设备的交互通信体验。 例如，iPhone、iPad和Desktop。 您可以将&#x200B;**预览**&#x200B;和&#x200B;**模拟器** ![标尺](assets/ruler.png)选项相互结合，以针对不同屏幕大小的设备预览网络输出。 预览中的示例数据从指定的表单数据模型填充。
...
1. 指定以下内容：
   ...
   * **[!UICONTROL 显示图案]**:您还可以为字段定义显示格式。从&#x200B;**类型**&#x200B;下拉列表中选择任何预定义的选项，将显示格式应用于字段。 选择&#x200B;**自定义**&#x200B;以定义列表中不可用的显示模式。 有关详细信息，请参阅[数据显示模式](../../forms/using/create-interactive-communication.md#datadisplaypatterns)
...
要配置数据显示模式，请点按元素，选择![configure_icon](assets/configure_icon.png)(Configure)，然后在提要栏的&#x200B;**[!UICONTROL 属性]**&#x200B;面板中设置显示模式。 从&#x200B;**[!UICONTROL 类型]**&#x200B;下拉列表中选择任何预定义选项，以视图与选定类型关联的模式。 从&#x200B;**[!UICONTROL 类型]**&#x200B;下拉列表中选择&#x200B;**[!UICONTROL 自定义]**&#x200B;以定义列表中不可用的模式。 在&#x200B;**[!UICONTROL Pattern]**&#x200B;字段中编辑值会自动将类型修改为&#x200B;**[!UICONTROL Custom]**。
...
在从打印渠道生成Web内容后，您可以重新定义字段、变量或表单数据模型元素的显示模式。 因此，元素可以具有为打印和Web渠道定义的不同显示模式。 如果未为打印渠道中的元素定义显示模式，并使用打印渠道自动生成Web内容，则为打印渠道中的元素定义的数据绑定定义了&#x200B;**[!UICONTROL Type]**&#x200B;下拉列表中可用的显示模式选项。 如果没有为元素定义绑定，则元素的数据类型将定义可用的显示模式选项。 例如，如果为打印渠道中的元素创建Number类型的数据绑定，则&#x200B;**[!UICONTROL Type]**&#x200B;下拉列表中可用的显示模式选项为各种格式的Number类型。
...
点按Web模板中的根面板，然后点按&#x200B;**+**，将&#x200B;**Table**&#x200B;组件添加到交互通信中。 包含两行的表将插入交互通信中。 表的第一行表示表标题。
...
1. 选择&#x200B;**添加行**&#x200B;或&#x200B;**删除行**&#x200B;以分别添加或删除表行。 使用工具栏中的&#x200B;**向上移动**&#x200B;和&#x200B;**向下移动**&#x200B;选项重新排列表中的行。

![组件工具栏](assets/component_toolbar_table_row_new.png)

**A.添** 加行 **B.** 删除 **行C.** 上 **移D.** 下移

#### 添加或编辑表单元格{#addedittexttable}中的文本

1. 在表单元格中选择默认文本框，然后点按![edit](assets/edit.png)（编辑）。
...
1. 在表单元格中选择默认文本框，然后点按![edit](assets/edit.svg)（编辑）。
...
1. 选择&#x200B;**将“打印为主控”作为Web渠道**，以启用Web渠道与打印渠道之间的同步。
1. 点按&#x200B;**保存并关闭**&#x200B;以保存更改。
...
您可以将按钮作为组件添加到交互式通信的Web渠道。 使用[规则编辑器](../../forms/using/rule-editor.md)定义规则，以便能够导航到其他交互通信、自适应表单、其他资产(如图像或文档片段)或点击按钮时的外部URL。
...
1. 使用以下任意选项将&#x200B;**Panel**&#x200B;组件插入Web渠道:
   ...
   * 从&#x200B;**组件**&#x200B;浏览器面板，将&#x200B;**面板**&#x200B;组件拖放到交互通信上。

   * 点按&#x200B;**内容**&#x200B;浏览器面板中的&#x200B;**面板**，然后点按&#x200B;**添加子面板**。 选择&#x200B;**添加子面板**&#x200B;选项将显示&#x200B;**添加子面板**&#x200B;对话框。 输入面板组件的标题和可选描述和名称。

1. 从&#x200B;**Content**&#x200B;浏览器点按面板，对面板执行其他操作，如配置、编辑规则、复制、删除和插入组件。

   您还可以在&#x200B;**内容**&#x200B;浏览器中拖放面板，以反映右侧窗格中交互通信结构的更改。
...
* **[!UICONTROL 自动布局]**:选择此模式可自动为Web渠道从打印渠道生成占位符、内容和数据绑定。
* **[!UICONTROL 手动组织]**:选择此模式，以使用“渠道源”选项卡中提供的主控内容手动选择“打印渠道”元素并将其添加到Web。有关详细信息，请参阅[选择“打印渠道元素”以创建Web渠道内容](#selectprintchannelelements)。
...
将指针悬停在Web目标中相关的渠道区或变量上，选择![cancelinheritance](assets/cancelinheritance.png)（取消继承），然后在“取消继承”对话框中，点按&#x200B;**[!UICONTROL 是]**。
...
如果需要，请选择&#x200B;**[!UICONTROL 还原继承后同步页面]**。 选择此选项可同步整个交互通信。 如果不选择此选项，则恢复继承时只同步相关目标区域。
...
* **父项：** 选择组件的父项。
* **视图SOM表达式:** 视图 [组件](../../forms/using/using-som-expressions-adaptive-forms.md) 的SOM表达式。

* **在面板中组合对** 象：在面板中组合组件，以便能够同时对这些组件执行操作。有关详细信息，请参阅面板](#groupobjectspanel)中的[对象分组。

* **取消继承：** [取消](#cancelinheritance) 组件在目标区域内的继承以编辑它们。

### 在面板{#groupobjectspanel}中对象分组

Web渠道创作界面便于将面板中的组件分组，以便能够同时对这些组件执行操作。 **内容**&#x200B;选项卡将分组的组件列表为内容树中面板的子元素。
