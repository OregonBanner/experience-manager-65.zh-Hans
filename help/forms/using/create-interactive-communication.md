...
点按文档片段，点按 ![configure_icon](assets/configure_icon.png) （配置），然后点按 **[!UICONTROL 属性]** 互动式交流的副手。 此 **[!UICONTROL 变量和数据模型对象]** 部分列出了变量，包括隐藏变量以及文档片段中使用的数据模型对象。 使用 ![编辑](assets/edit.svg) 每个数据模型对象或变量旁边的（编辑）图标以编辑属性。
