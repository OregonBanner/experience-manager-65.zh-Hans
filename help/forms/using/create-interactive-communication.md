...
1. 输入以下内容：
   ...
   * **[!UICONTROL 主]** 题和 **[!UICONTROL 选择主题]**:浏览并选择主题，以设置交互式通信的Web渠道的样式。有关详细信息，请参阅AEM Forms](/help/forms/using/themes.md)中的[主题。

   * **[!UICONTROL 将“打印为主控”用于Web渠道]**:选择此选项可创建与打印渠道同步的Web渠道。将打印渠道用作Web渠道的主控，可确保Web渠道的内容和数据绑定是从打印渠道派生的，并且点按“同步”时，打印渠道中所做的更改会反映在Web渠道中。 但是，作者可以根据需要中断Web渠道中特定组件的继承。 有关详细信息，请参阅[将Web渠道与打印渠道同步](../../forms/using/create-interactive-communication.md#synchronize)。
如果选择**[!UICONTROL “将打印为Web渠道的主控”选项]**，则可以选择以下任何模式以生成Web渠道:

      * **[!UICONTROL 自动布局]**:选择此模式可自动为Web渠道从打印渠道生成占位符、内容和数据绑定。
      * **[!UICONTROL 手动组织]**:选择此模式，可使用“渠道源”选项卡中提供的主控内容手动选择“打印”元素并将其添 **[!UICONTROL 加到]** Web渠道。有关详细信息，请参阅[选择“打印渠道元素”以创建Web渠道内容](#selectprintchannelelements)。
...
您还可以通过使用&#x200B;**[!UICONTROL 资产]**&#x200B;选项卡将新片段拖放到目标区域上，来替换文档区域与目标片段之间的绑定。 拖动片段时目标区域的蓝色阴影表示文档片段可以拖放到目标区域。
...
点按文档片段，点按![configure_icon](assets/configure_icon.png)(Configure)，然后点按交互通信的Sidekick中的&#x200B;**[!UICONTROL 属性]**。 **[!UICONTROL 变量和数据模型对象]**&#x200B;部分列表变量，包括隐藏变量和文档片段中使用的数据模型对象。 使用每个数据模型对象或变量旁边的![edit](assets/edit.svg)（编辑）图标编辑属性。
...
1. 要设置变量的绑定，请点按一个变量并选择![configure_icon](assets/configure_icon.png)(Configure)，然后在提要栏的“属性”面板中设置绑定属性。
   ...
   * **显示图案：** 您还可以为变量定义显示格式。从&#x200B;**Type**&#x200B;下拉列表中选择任何预定义选项，以将显示格式应用于变量。 选择&#x200B;**自定义**&#x200B;以定义列表中不可用的显示模式。 有关详细信息，请参阅[数据显示模式](../../forms/using/create-interactive-communication.md#datadisplaypatterns)。
...
1. 切换到&#x200B;**[!UICONTROL Web渠道]**。 Web渠道显示在交互式通信编辑器中。 首次从“打印”渠道切换到“Web”渠道时，将进行自动同步。 有关详细信息，请参阅[从打印渠道同步Web渠道](../../forms/using/create-interactive-communication.md#synchronize)。

   由于我们在此示例中将“打印”用作Web的主控，因此“打印”渠道占位符、内容和数据绑定将同步到Web渠道。 但是，您可以在Web渠道中更改和自定义特定内容。 [取](#cancelinheritance) 消继承已使用打印渠道生成的目标区域和变量，以便能够自定义内容。

   ![webchannelassets](assets/webchannelassets.png)

   点按文档片段，点按![configure_icon](assets/configure_icon.png)(Configure)，然后点按交互通信的Sidekick中的&#x200B;**[!UICONTROL 属性]**。 **[!UICONTROL 变量和数据模型对象]**&#x200B;部分列表变量，包括隐藏变量和文档片段中使用的数据模型对象。 使用每个数据模型对象或变量旁边的![edit](assets/edit.svg)（编辑）图标编辑属性。 此外，对于在Web渠道中使用打印渠道自动生成[](#synchronize)的文档片段，请使用每个数据模型对象和变量旁的![canceliheration](assets/cancelinheritance.png)（取消继承）图标[取消继承](#cancelinheritance)并能够编辑它们。

1. 要在Web渠道中添加其他组件，请点按&#x200B;**[!UICONTROL 组件]**，选择Web渠道。 在您的交互式通信的Web渠道中根据需要拖放组件，然后继续进行配置。

   | 组件 | 功能 |
   |---|---|
   | 图表 | 添加一个图表，您可以在Interactive Communication中使用该图表，以可视方式表示从表单数据模型集合检索的二维数据。 有关详细信息，请参阅[使用图表组件](../../forms/using/chart-component-interactive-communications.md)。 |
   | 文档片段 | 允许您向交互式通信中添加可重用的组件、文本、列表或条件。 您添加到交互式通信的可重用组件可以是基于表单数据模型的组件，也可以是没有表单数据模型的组件。 |
   | 图像 | 允许插入图像。 |
   | 面板 | 允许您将[Panel](../../forms/using/create-interactive-communication.md#add-panel-component-to-the-web-channel)添加到交互式通信中。 |
   | 表 | 添加表格以便按行和列整理数据。 |
   | 目标区域 | 在Web渠道中插入一个目标区域，以组织Web渠道特定的组件。 目标区域是一个简单容器，它允许您将Web渠道特定的组件进行分组。 |
   | 文本 | 将富文本添加到交互式通信的Web渠道。 文本还可以利用表单数据模型对象使内容动态化。 |
   | 按钮 | 允许您向交互通信添加[Button](../../forms/using/create-interactive-communication.md#add-button-component-to-the-web-channel)。 您可以使用按钮组件导航到其他Interactive Communications、自适应表单、图像或文档片段等其他资产或外部URL。 |
...
可以使用&#x200B;**预览选项**&#x200B;来评估交互式通信的外观。 交互式通信的Web渠道还提供了一个选项，用于模拟不同设备的交互式通信体验。 例如，iPhone、iPad和Desktop。 您可以将&#x200B;**预览**&#x200B;和&#x200B;**模拟器** ![标尺](assets/ruler.png)选项相互结合使用，以针对不同屏幕大小的设备预览网络输出。 预览中的样本数据是从指定的表单数据模型填充的。
...
1. 指定以下内容：
   ...
   * **[!UICONTROL 显示图案]**:您还可以定义字段的显示格式。从&#x200B;**类型**&#x200B;下拉列表中选择任何预定义选项，以将显示格式应用于字段。 选择&#x200B;**自定义**&#x200B;以定义列表中不可用的显示模式。 有关详细信息，请参阅[数据显示模式](../../forms/using/create-interactive-communication.md#datadisplaypatterns)
...
要配置数据显示模式，请点按元素，选择![configure_icon](assets/configure_icon.png)(Configure)，然后在提要栏的&#x200B;**[!UICONTROL 属性]**&#x200B;面板中设置显示模式。 从&#x200B;**[!UICONTROL 类型]**&#x200B;下拉列表中选择任何预定义选项，以视图与选定类型关联的模式。 从&#x200B;**[!UICONTROL 类型]**&#x200B;下拉列表中选择&#x200B;**[!UICONTROL 自定义]**&#x200B;以定义列表中不可用的模式。 编辑&#x200B;**[!UICONTROL Pattern]**&#x200B;字段中的值会自动将类型修改为&#x200B;**[!UICONTROL Custom]**。

要应用显示模式，在“模式”字段中定义的字符或数字数必须与字段、变量和表单数据模型元素的值中定义的字符或数字匹配或超过。 有关详细信息，请参阅[example](../../forms/using/create-interactive-communication.md#greaternumberofdigits)。

![data_display_pattern_example](assets/data_display_patterns_ssn_new.png)

在从打印渠道生成Web内容后，可以重新定义字段、变量或表单数据模型元素的显示模式。 因此，元素可以具有为打印和Web渠道定义的不同显示模式。 如果您没有为打印渠道中的元素定义显示模式，并使用打印渠道自动生成Web内容，则为打印渠道中的元素定义的数据绑定定义了&#x200B;**[!UICONTROL 类型]**&#x200B;下拉列表中可用的显示模式选项。 如果没有为元素定义绑定，则元素的数据类型将定义可用的显示图案选项。 例如，如果为打印渠道中的元素创建Number类型的列表绑定，则&#x200B;**[!UICONTROL Type]**&#x200B;下拉中可用的显示模式选项为各种格式的Number类型。
...
| 类型 | 默认值 | 显示图案 | 显示值 | 描述 |
|---|---|---|---|---|
| 社会安全号码 | 123456789 | text{999-99-9999} | 123-45-6789 | 默认值字段中的位数与“模式”字段中的位数匹配。 成功显示基于图案的值。 |
| 社会安全号码 | 1234567 | text{999-99-9999} | 1-23-4567 | 默认值字段中的位数小于“模式”字段中的位数。 该模式适用于7个可用数字。 |
| 社会安全号码 | 1234567890 | text{999-99-9999} | 1234567890 | 默认值字段中的位数大于“模式”字段中的位数。 因此，显示值没有变化。 |

如果未为变量或表单文档模型元素指定显示模式，则默认情况下使用[全局数据片段配置](https://helpx.adobe.com//experience-manager/6-5/forms/using/interactive-communication-configuration-properties.html)。
...
1. 为表的每个单元格指定数据绑定。 要创建可重复行，请在属于公用集合属性的行中插入表单数据模型属性。
   ...
   1. 配置属性：
      ...
      * **[!UICONTROL 绑定类型]**:为字段选择以下绑定类型之一。
         ...
         * **[!UICONTROL 无]**
         * **[!UICONTROL 数据模型对象]**:表单数据模型属性的值将填充在字段中。或者，选择&#x200B;**数据源**&#x200B;选项卡，然后将属性拖放到字段。
      * **[!UICONTROL 数据模型对象]**:在字段中填充其值的表单数据模型属性。
      * **[!UICONTROL 默认值]**:默认值确保在指定数据模型对象没有提供值时字段不为空。默认值将预填充在字段中。
...
1. 选择&#x200B;**添加行**&#x200B;或&#x200B;**删除行**&#x200B;可分别添加或删除表行。 使用工具栏中的&#x200B;**向上移动**&#x200B;和&#x200B;**向下移动**&#x200B;选项可重新排列表中的行。

![组件工具栏](assets/component_toolbar_table_row_new.png)

**A.添** 加行 **B.** 删 **除行C.** 上 **移D.** 下移

#### 在表单元格{#addedittexttable}中添加或编辑文本

1. 选择表单元格中的默认文本框，然后点按![edit](assets/edit.png)（编辑）。
1. 在表单元格中键入文本，然后点按![done_icon](assets/done_icon.png)以保存它。

#### 在表单元格和数据模型对象元素{#createbindingtablecells}之间创建绑定

1. 选择表行中的默认文本框，然后点按![edit](assets/edit.png)（编辑）。
1. 点按数据模型对象下拉列表并选择属性。
1. 点按可保存并创建表单元格与数据模型对象属性之间的绑定。

![创建数据绑定](assets/create_data_binding_table_new.png)

#### 为表单元格{#createhyperlinktable}中的文本创建超链接

1. 选择表单元格中的默认文本框，然后点按![edit](assets/edit.svg)（编辑）。
...
## 编辑交互通信属性{#edit-interactive-communication-properties}
...
1. 选中&#x200B;**Web**&#x200B;复选框，并为Web渠道选择模板。
1. 选择&#x200B;**“使用打印作为Web渠道的主控”**&#x200B;以启用Web渠道与打印渠道之间的同步。
1. 点按&#x200B;**保存并关闭**&#x200B;以保存更改。

   同样，您也可以点按&#x200B;**渠道**&#x200B;选项卡上的&#x200B;**Web**&#x200B;复选框，从交互通信中删除Web渠道。

## 将Button组件添加到Web渠道{#add-button-component-to-the-web-channel}

您可以将按钮作为组件添加到交互式通信的Web渠道。 使用[规则编辑器](../../forms/using/rule-editor.md)定义规则，以便能够导航到其他交互通信、自适应表单、图像或文档片段等其他资产，或点按按钮时的外部URL。

要添加按钮并定义其规则，请执行以下操作：

1. 点按Web模板中的根面板，然后点按&#x200B;**+**&#x200B;以将&#x200B;**Button**&#x200B;组件添加到交互通信中。
1. 点按按钮组件，然后点按![edit-rules](assets/edit-rules.png)以在点按按钮时定义规则。
1. 在&#x200B;**When**&#x200B;部分，从按钮下拉列表的状态中选择&#x200B;**clicked**。
1. 在&#x200B;**Then**&#x200B;部分中：

   1. 从下拉列表中选择操作。 例如，选择&#x200B;**导航到**&#x200B;作为操作类型。

   1. 指定交互式通信、自适应表单、资产或网页的URL。 例如，以下格式指定URL以导航到另一个交互式通信：https://&lt;server-name>:&lt;port>/editor.html/content/forms/af/&lt;Interactive Communication name>/渠道/&lt;渠道名称 — 打印或web>.html
...
1. 预览交互式通信，然后点按按钮以视图第4(b)步中指定的交互式通信、自适应表单、资产或网页。
...
1. 从&#x200B;**Content**&#x200B;浏览器点按面板，对面板执行其他操作，如配置、编辑规则、复制、删除和插入组件。
...
* **[!UICONTROL 自动布局]**:选择此模式可自动为Web渠道从打印渠道生成占位符、内容和数据绑定。
* **[!UICONTROL 手动组织]**:选择此模式，可使用“渠道源”选项卡中提供的主控内容手动选择打印渠道元素并将其添加到Web。有关详细信息，请参阅[选择“打印渠道元素”以创建Web渠道内容](#selectprintchannelelements)。
...
将鼠标悬停在Web渠道中的相关目标区域或变量上，选择![cancerientance](assets/cancelinheritance.png)（取消继承），然后在“取消继承”对话框中，点按&#x200B;**[!UICONTROL 是]**。
...
### 使用组件工具栏对继承的组件{#componenttoolbar}执行操作
...
* **父项：** 选择组件的父项。
* **视图 SOM表达式:** 视图 [组](../../forms/using/using-som-expressions-adaptive-forms.md) 件的SOM表达式。

* **在面板中对象组** 合：在面板中组合组件，以便能够同时对这些组件执行操作。有关详细信息，请参阅Panel](#groupobjectspanel)中的[对象分组。

* **取消继承：** [取消](#cancelinheritance) 组件在目标区域内的继承以编辑它们。

### 在面板{#groupobjectspanel}中对对象进行编组

Web渠道创作界面便于将面板中的组件分组，以便能够同时对这些组件执行操作。 **内容**&#x200B;选项卡将编组的组件列表为内容树中面板的子元素。

1. 点按组件并选择“组（![组](assets/group.jpg)）”操作。
1. 选择多个组件，然后点按面板&#x200B;**中的**&#x200B;对象组。

   ![对象组](assets/component_toolbar_group_objects_new.png)

1. 在Panel **对话框的**&#x200B;对象组中，输入Panel的名称。
1. 输入面板的可选标题和说明。
