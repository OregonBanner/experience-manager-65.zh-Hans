...
数据集成支持现成的OAuth2.0、基本身份验证和API密钥身份验证类型，并允许实施用于访问Web服务的自定义身份验证。 虽然在AEM云服务中配置了RESTful、基于SOAP和OData服务，但在AEM Web控制台中配置了关系数据库的JDBC和AEM用户配置文件的连接器。
...
1. 为RESTful服务指定以下详细信息：
   ...
   * 根据Swagger源输入，以下字段预填充了值：
      ...
      * 基本路径：所有API路径的URL前缀。 它是一个可选字段。\
         如有必要，请编辑这些字段的预填充值。
   * 选择身份验证类型 — 无、OAuth2.0、基本身份验证、API密钥、自定义身份验证或相互身份验证 — 以访问RESTful服务，并相应地提供身份验证的详细信息。
