...
1. 在文本中键入，并使用以下选项来设置格式、设置条件以及在文本中插入表单数据模型属性和变量：
   ...
   * [键盘快捷键](/help/forms/using/keyboard-shortcuts.md)
   ...
   >[!NOTE]
   >
   >您可以在文本编辑器中使用@符号添加表单数据模型元素、数据字典元素和变量。 在文本编辑器中输入以@开头的字符串时，将搜索所有数据模型元素、数据字典元素和变量，并显示包含搜索字符串的元素或变量。 您可以在搜索结果中导航并选择一个元素或变量。 如果没有匹配的结果， *未找到匹配结果* 消息。
...
* 要将表单数据模型属性插入文本，请将光标放在要插入该属性的位置，然后选择 **[A]** 属性，然后点按 **[!UICONTROL [B] 添加选定项]**. 您也只需双击资产，即可在 **[C]** 光标位置。 表单数据模型属性以褐色背景色突出显示。
...
* 允许代理在代理UI中编辑表单数据模型属性的值，同时 [准备和发送交互式通信](/help/forms/using/prepare-send-interactive-communication.md) 使用代理UI，点按 **[D]** 该属性的锁定图标，并确保其处于已解锁状态。 属性的默认状态已锁定，且代理无法在代理UI中编辑该属性。
