...
* **重复**:您的交互式通信中可能包含动态信息，如信用卡对帐单中的事务处理，其发生次数可能随每个生成的交互式通信而不断变化。 使用重复，您可以格式化和构造此类动态信息。 有关详细信息，请参 [阅内联条件和重复](https://helpx.adobe.com/experience-manager/6-3/forms/using/cm-inline-condition.html)。
...
1. 指定以下信息：
   ...
   * **[!UICONTROL 表单数据模型]**:（可选）选择“表单数据模型”单选按钮，以基于表单数据模型创建文本。 选择“表单数据模型”单选按钮时，将 **[!UICONTROL 显示“表单数据模]** 型”字段。 浏览并选择表单数据模型。 在为交互式通信创建文本和条件时，请确保您使用的数据模型与要在交互式通信中使用的数据模型相同。 有关表单数据模型的详细信息，请参阅 [数据集成](/help/forms/using/data-integration.md)。
...
* 要将表单数据模型属性插入文本，请将光标放在要插入属性的位置，然后点按左窗格 **[中]** A属性，然后点按 **[!UICONTROL [B]添加选定项]**。 您也只需多次点按属性，将其插入到C **[光标]** 位置。 表单数据模型属性以褐色背景色突出显示。
...
* 要允许代理在使用代理UI准备和发送交互 [式通信时编辑代理UI中表单数据模型属性的值](/help/forms/using/prepare-send-interactive-communication.md) ，请点按该属性的 **[D]** lock图标，并确保它处于解锁状态。 属性的默认状态已锁定，代理无法在代理UI中编辑该属性。
