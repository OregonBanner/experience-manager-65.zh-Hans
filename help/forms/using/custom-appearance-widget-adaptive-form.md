...
1. 根据需要更新文件夹 `integration/javascript` 中的JavaScript文件。
   ...
   * 当事 `getCommitValue` 件发生时，自适应表单框架将调用 `commit`该方法。 通常，它是退出事件，但下拉列表、单选按钮和复选框元素除外，它们在更改时发生)。 有关详细信息，请参阅自 [适应表单表达式](../../forms/using/adaptive-form-expressions.md#p-value-commit-script-p)。
