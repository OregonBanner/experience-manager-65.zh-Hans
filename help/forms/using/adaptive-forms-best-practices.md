...
* 设置 **每个请求的调用数** 在 **Apache Sling主Servlet** 数量相当大。 它允许Forms服务器允许其他调用。 配置显示默认值1500。 值为1500调用，适用于其他Experience Manager组件，如站点和资产。 自适应表单的默认值集为20000。 如果您遇到 `too many calls` 日志中错误或表单无法呈现，请尝试将值增加到大数以解决此问题。 如果调用数超过20000，则表示表单复杂，可能需要一些时间在浏览器中渲染表单。 只有在首次加载表单时，才会发生这种情况，在缓存表单后，以及缓存表单后，性能不会受到显着影响。
