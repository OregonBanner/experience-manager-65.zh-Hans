...
1. 在apps文件夹中，创建一个名为items的文件夹，其路径/结构与selection文件夹中的items文件夹类似，具体步骤如下：
   ...
   1. 右键单击 **个项目** 文件夹并选中 **覆盖节点**：
      ...
      >[!NOTE]
      >
      >此路径专用于创建可与选择一个或多个资产/字母一起使用的操作。 如果要创建无需选择即可正常运行的操作，则需要为以下路径创建一个覆盖节点，并相应地完成其余步骤：
      >
      >
...
1. 在新创建的项目文件夹下，使用以下步骤为特定资产（例如：downloadFlatPDF）中的自定义按钮/操作添加节点：
   ...
   1. 将以下属性添加到节点（此处为downloadFlatPDF），然后单击 **全部保存**：
      ...
      <table>
        ...
        <td>图标</td>
        <td>字符串</td>
        <td>图标 — 下载<br /> <br /> “通信管理”显示在命令/菜单左侧的图标。 有关可用的不同图标和设置，请参阅 <a href="https://docs.adobe.com/docs/en/aem/6-3/develop/ref/coral-ui/coralui3/Coral.Icon.html" target="_blank">CoralUI图标文档</a>.<br /> </td>
        </tr>
        <tr>
...
1. 使用命令操作处理代码创建名为POST.jsp的文件，该代码位于以下位置：
   ...
   1. 将以下代码添加到POST.jsp文件，然后单击 **全部保存**：
      ...
      此代码特定于信件渲染服务。 对于任何其他资源，请将该资源的Java库添加到此代码中。 有关AEM Forms API的更多信息，请参阅 [AEM FORMS API](https://adobe.com/go/learn_aemforms_javadocs_63_en).
