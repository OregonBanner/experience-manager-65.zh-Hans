...
默认情况下，AEM Forms会为每个资源类型提供一组定义的元数据。 除了默认元数据之外，您还可以将自定义元数据添加到每个资源类型。 AEM Forms还为您提供了正确的方法，以便高效地为表单创建、管理和交换所有这些元数据。

如果您是开发人员或站点所有者，则可以自定义AEM Forms的最终用户界面Forms Portal，以反映您在组织中使用的元数据。 有关Forms门户的更多信息，请参阅 [在门户上发布表单的简介](../../forms/using/introduction-publishing-forms.md).
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td><p>指定在创作自适应表单时使用的表单模型。 此属性可以具有以下值：</p> 
    <ul> 
     <li>表单模板：从存储库中存在的表单模板中选择一个表单模板。 可以更新此值。</li> 
     <li>XML架构：已上传XSD文件。 可以更新此值。</li> 
     <li>无</li> 
    </ul> 
    <div>
      表单模型一旦选定便可以更新，但不能删除。 
...
1. 使用以下方式之一打开属性页面：
   ...
   1. 单击视图属性 ![e_reviewmode_properties_n](assets/e_reviewmode_properties_n.png) 图标。

      >[!NOTE]
      >
      >快速操作是在鼠标悬停时显示在缩略图上的操作项。

   1. 选择表单并单击“查看属性” ![e_reviewmode_properties_n](assets/e_reviewmode_properties_n.png) 工具栏中显示的图标。
...
1. 按照上一节中所述的步骤打开属性页面，可以在其中查看所选表单的现有元数据。
...
对于自适应表单，还提供了附加功能，允许用户生成缩略图作为当前自适应表单预览的快照。 由于AEM Forms还支持自适应表单的创作，因此每次更改自适应表单时，自适应表单的预览可能会发生更改。 此功能可生成缩略图，以帮助您根据当前预览状态获取自适应表单的新缩略图。 单击 **[!UICONTROL 生成预览]** 执行这项操作。
...
1. 单击刚刚拖动的组件。 在右侧面板中打开的“设置”选项卡中，填写以下字段的信息：
   ...
   1. 指定字段标签，该标签将用作架构中放置的字段上方的显示名称（例如：Department）
...
AEM Forms中的所有自适应表单现在都包含此额外的元数据属性。 您可以从属性页面中编辑它。
