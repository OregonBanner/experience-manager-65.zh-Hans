...
如果您正在执行全新安装或计划为JEE环境中的AEM 6.5 Forms使用最新软件，则Adobe建议使用 [AEM 6.5.12.0 Forms on JEE完整安装程序](https://experienceleague.adobe.com/docs/experience-manager-release-information/aem-release-updates/forms-updates/aem-forms-releases.html) 2022年3月3日发布，而不是2019年4月8日发布的AEM 6.5 Forms安装程序。
...
>
>对于新安装，建议在可行的情况下，在现代OSGi栈栈上部署AEM Forms，以利用有关响应式自适应Forms的最新创新，实现使用表单数据模型的移动、多渠道交互式通信以及后端数据集成。
>
>我们认识到现有用户需要继续在JEE栈栈上部署AEM Forms。 在此类情况下，Adobe需要在受支持的基础设施上部署AEM Forms JEE，如本文档所述。 如果您要升级到AEM 6.5 Forms，并且在以前的AEM Forms版本中使用不受支持的平台，则可以联系Adobe支持部门以获取有关升级到受支持平台的帮助。

### Java虚拟机(JVM) {#java-virtual-machines-jvm}

Adobe Experience Manager Forms需要由Java开发工具包(JDK)分发提供的Java虚拟机才能运行。 Adobe Experience Manager与以下版本的Java虚拟机一起运行：
...
>[!NOTE]
>
>- 建议跟踪Java供应商的安全公告，以确保生产环境的安全并安装最新的Java更新。
...
- 为了充分利用AEM部署，Adobe建议许可MongoDB Enterprise版本，以便从专业支持中获益。
- Adobe客户关怀团队将协助解决与AEM使用MongoDB相关的资格确认问题。 欲了解更多信息，请参见 [适用于Adobe Experience Manager的MongoDB页面](https://www.mongodb.com/lp/contact/mongodb-adobe-experience-manager).
- “文件系统”包括符合POSIX的块存储。 这包括网络存储技术。 请注意，文件系统性能可能会有所不同，并影响整体性能。 建议将AEM与网络/远程文件系统结合使用来进行负载测试。
...
1. 除IBM® JDK外，JEE上的AEM Forms不支持任何包含IBM® WebSphere®的JDK。
...
1. JEE上的AEM Forms仅支持用于应用程序服务器JBoss EAP 7.4的OracleJava™ SE 11 （64位） SDK。

此外，在选择用于AdobeJEE部署上的AEM Forms的软件时，请考虑以下几点：

- JEE上的AEM Forms支持在受支持软件的指定主要版本和次要版本之上进行更新、修补程序和修复包。 但是，除非另外指定，否则不支持更新到下一个主要或次要版本。
- 基于群集的安装不支持TarMK持久性。 有关支持的持久性的信息，请参见 [为AEM Forms安装选择持久性类型](/help/forms/using/choosing-persistence-type-for-aem-forms.md).
- AEM Forms根据 [第三方软件支持政策](../../forms/using/aem-forms-jee-supported-platforms.md#p-third-party-patch-support-policy-p).
- 根据第三方供应商提供的支持，JEE上的AEM Forms支持平台。 第三方供应商可能不允许某些组合。 例如，许多供应商尚未对其应用程序服务器进行Oracle认证。 因此，JEE上的AEM Forms也不支持这些组合。 要确保您选择支持的软件版本，请同时查看第三方供应商的支持列表。
...
- 仅在Windows和Linux上支持OpenOffice的PDF Generator转换。
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>英特尔®至强® E5-2680,2.4 GHz处理器或同等处理器<br /> VMWare ESX 5.1或更高版本<br /> RAM：6 GB（带64位JVM的64位操作系统）<br /> 可用磁盘空间：15GB临时空间加上22GB<br /> 适用于AEM Forms on JEE的</td>
  </tr>
  <tr>
   <td>SUSE Linux Enterprise Server</td>
   <td>英特尔至强E5-2670v2,1个vCPU，2.5 GHz处理器<br /> AWS m3.medium（3个ECU）<br /> RAM：6 GB（带64位JVM的64位操作系统）<br /> 可用磁盘空间：6 GB临时空间加上22 GB<br /> 适用于AEM Forms on JEE的</td>
  </tr>
  <tr>
   <td>Red Hat Enterprise Linux</td>
   <td>英特尔至强E5-2670v2,1个vCPU，2.5 GHz处理器<br /> AWS m3.medium（3个ECU）<br /> RAM：6 GB（带64位JVM的64位操作系统）<br /> 可用磁盘空间：6 GB临时空间加上22 GB<br /> 适用于AEM Forms on JEE的<br /> </td>
  </tr>
  <tr>
   <td>小型生产环境的硬件要求</td>
   <td>
    <ul>
     <li><strong>英特尔支持的环境</strong>：英特尔®至强® E5-2680,2.4 GHz或更高。 使用双核处理器将进一步提高性能</li>
...
>[!NOTE]
>
Acrobat DC产品系列为Acrobat和Reader引入了两个途径，它们本质上是不同的产品：“Classic”和“Continuous”。 有关详细信息和两个跟踪的比较，请参阅 [https://www.adobe.com/go/acrobatdctracks.](https://www.adobe.com/go/acrobatdctracks)
...
- Forms portal支持Internet Explorer 11上的JAWS 14.0屏幕阅读器软件以实现可访问性。
...
| Google Android | Android 5.1及更高版本。 AEM Forms应用在7英寸和10英寸Samsung Galaxy平板电脑和流行智能手机上获得了认证。 |
...
JEE上AEM Forms的第三方软件要求记录在其各自产品文档的“系统要求”部分。 所有文档都可从以下位置访问： [https://adobe.com/go/learn_aemforms_documentation_65](https://adobe.com/go/learn_aemforms_documentation_65) .
...
在Adobe无法控制的情况下，声称向后兼容的第三方修补程序可能会对Adobe产品或客户环境产生负面影响。 在这种情况下，Adobe建议客户在将第三方提供的任何紧急修补程序应用于关键系统之前，评估这些修补程序的影响。 Adobe将与第三方合作，通过正常的Adobe支持计划或通过第三方纠正补丁中的问题，利用合理的商业努力来解决此类问题。 这并不保证新发布的受Adobe支持的第三方修补程序将按供应商或AEM Forms on JEE的文档记录工作。
...
以下平台在2022年6月2日发行的AEM Forms 6.5.13.0中标记为已弃用：
...
>[!NOTE]
>
平台标记为 [AEM Forms 6.5.12.0和6.5.10.0已弃用，在AEM Forms 6.5 Service Pack 18 (6.5.18.0)版本发布之前，仍将继续支持这两个版本](https://helpx.adobe.com/cn/support/programs/eol-matrix.html).
