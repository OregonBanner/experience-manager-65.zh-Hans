...
description: 在JEE上安装AEM Forms所需和支持的基础架构组件列表
seo-description: 在JEE上安装AEM Forms所需和支持的基础架构组件列表
...
>[!NOTE]
>
>为帮助AEM Forms客户降低拥有成本、简化部署体系结构并实现开发堆栈的现代化，Adobe Experience Manager企业平台正从基于应用程序服务器的部署转向基于OSGi的独立部署。 Adobe通过减少的基础架构组件矩阵继续支持AEM Forms JEE堆栈。
>
...
>
>对于新安装，建议在可行时在现代OSGi堆栈上部署AEM Forms，以利用与响应式自适应表单相关的最新创新，使用表单渠道模型进行移动、多交互式通信和后端数据集成。
>
>我们认识到现有用户需要继续在JEE堆栈上部署AEM Forms。 在这种情况下，Adobe需要按照本文档所述在受支持的基础架构上部署AEM Forms JEE。 如果您要升级到AEM 6.5 Forms并使用上一个AEM Forms版本上不支持的平台，则可以联系Adobe支持以获得有关升级到受支持平台的帮助。

### Java虚拟机(JVM) {#java-virtual-machines-jvm}

Adobe Experience Manager Forms需要运行Java虚拟机，该虚拟机由Java开发工具包(JDK)分发提供。 Adobe Experience Manager可运行以下版本的Java虚拟机：
...
* Adobe客户关怀将协助解决与在AEM中使用MongoDB相关的资格问题。 有关详细信息，请参 [阅MongoDB for Adobe Experience Manager页](https://www.mongodb.com/lp/contact/mongodb-adobe-experience-manager)。
...
您可以在物理机或虚拟环境上在JEE上运行AEM Forms。 但是，如果您在虚拟环境上遇到AEM Forms的任何问题，请尝试在物理机上复制该问题。 如果物理机上仍存在该问题，请与Adobe支持联系以获得解决。 对于无法在物理机上复制的问题，请与虚拟环境供应商联系。
...
在选择平台以在JEE服务器上设置AEM表单时，请考虑以下例外情况。
...
1. JEE上的AEM Forms不支持IBM® WebSphere®（IBM® JDK除外）中的任何JDK。
1. CRX-repository支持TarMK、MongoDB和关系数据库(RDBMK)类型的持久性。 应用程序服务器和CRX-repository之间不能有两个不同的数据库系统。 但是，在JEE环境的AEM Forms上，您可以将MongoMK与CRX-repository结合使用，将受支持的关系数据库与应用程序服务器结合使用。
...
* AEM Forms on JEE根据我们的第三方软件支持政策支 [持各种第三方软件](../../forms/using/aem-forms-jee-supported-platforms.md#p-third-party-patch-support-policy-p)。
* JEE上的AEM Forms根据第三方供应商提供的支持支持支持平台。 某些组合可能不允许由第三方供应商进行。 例如，许多供应商尚未通过Oracle对其应用程序服务器进行认证。 因此，JEE上的AEM Forms也不支持这些组合。 要确保选择支持的软件版本，请查看第三方供应商的支持列表。
...
AEM Forms App现在支持Apache Cordova。 以下是支持的特定于平台的Cordova版本：
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>Intel® Xeon® E5-2680、2.4 GHz处理器或等效的VMWare<br /> ESX 5.1或更高版本<br /> RAM: 6 GB（64位操作系统，带64位JVM）可用磁盘空间<br /> : 15GB临时空间，外加22GB<br /> （适用于JEE上的AEM Forms）</td>
  </tr>
  <tr>
   <td>SUSE Linux Enterprise Server</td>
   <td>Intel Xeon E5-2670v2,1个vCPU,2.5 GHz处理器<br /> AWS m3.medium（3个ECU）<br /> RAM: 6 GB（64位操作系统，带64位JVM）可用磁盘空间<br /> : 6 GB临时空间，外加22GB<br /> （适用于JEE上的AEM Forms）</td>
  </tr>
  <tr>
   <td>Red Hat Enterprise Linux</td>
   <td>Intel Xeon E5-2670v2,1个vCPU,2.5 GHz处理器<br /> AWS m3.medium（3个ECU）<br /> RAM: 6 GB（64位操作系统，带64位JVM）可用磁盘空间<br /> : 6 GB临时空间，外加22GB<br /> （适用于JEE上的AEM Forms）<br /> </td>
  </tr>
  <tr>
   <td>小型生产环境的硬件要求</td>
   <td>
...
| Google Android | Android 5.1及更高版本。 AEM Forms应用程序已通过7英寸和10英寸Samsung Galaxy平板电脑和流行智能手机的认证。 |
...
JEE上的AEM Forms的第三方软件要求在其各自产品文档的“系统要求”部分中进行说明。 所有文档均可从https://adobe.com/go/learn_aemforms_documentation_65 [访问](https://adobe.com/go/learn_aemforms_documentation_65) 。
...
在Adobe无法控制的情况下，声称向后兼容的第三方修补程序可能会对Adobe产品或客户环境产生负面影响。 在这种情况下，Adobe建议客户先评估来自第三方的任何紧急补丁程序的影响，然后再将其应用到关键系统。 Adobe将与第三方合作，通过正常的Adobe支持项目或第三方在他们的补丁中解决该问题，通过合理的业务努力解决此类问题。 这不保证将受Adobe支持的新发布的第三方修补程序能按供应商的说明或与JEE上的AEM Forms一起使用。

Adobe保留在任何给定点更改由AEM Forms on JEE版本支持的第三方参考平台及其支持的修补程序定义的权利。
