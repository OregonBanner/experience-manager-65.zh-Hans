---
title: 配置外出设置
description: 为AEM Forms实例配置外出设置。
...
1. 选择 **[!UICONTROL 在此期间转发我的项目]** 复选框。 如果未选择选项并且未指定被分配人，则不会将您的项目转发给任何用户。 虽然您离开了，并且已启用设置，但项目仍会保留在您的收件箱中。
1. 点按 **[!UICONTROL 添加被分派人]**. 在中指定用户 **[!UICONTROL 被分派人]** 要将项目委派到的字段。 指定 **[!UICONTROL 工作流模型]** 以委派给指定的用户。 您可以选择多个工作流模型。
   ...
   要为除少数工作流模型之外的所有工作流模型将项目分配给特定用户，请选择 **[!UICONTROL 所有工作流]** 从工作流模型下拉列表中，点按 **[!UICONTROL +添加例外]**，并指定要忽略的工作流模型。
   <br>
   ...
   >[!NOTE]
   >
   >受让人的顺序很重要。 当将项目分配给启用了外出设置的用户时，将按照添加受分配人的顺序根据指定的受分配人列表评估项目。 当项目符合条件时，项目会分配给被分配人，且不会选中下一个被分配人。

1. 点按 **[!UICONTROL 保存]**. 该设置将在指定的开始日期和时间生效。 如果您在办公室外时登录，则在更改设置之前，不会将您视为办公室成员。
...
>[!NOTE]
>
>(仅适用于以Forms为中心的工作流项目)启用 **允许被分派人使用“外出”设置进行委派** 的选项 **分配任务** 步骤。 只有启用了上述选项的项目才会委托给其他用户。
