...
description: 创建自定义查询以在流程报告中搜索AEM Forms on JEE流程和任务详细信息
...
Process Reporting中的临时查询允许您创建自定义查询，您可以使用这些查询搜索在AEM Forms环境中定义的AEM Forms流程实例的流程和任务详细信息。
...
为流程详细信息创建筛选器和运行查询所遵循的步骤与任务相同。

这意味着“进程搜索”和“任务搜索”的用户界面仅在可搜索的字段和搜索结果中返回的字段上有所不同。 这只是因为，虽然许多字段相同，但某些字段特定于流程，而某些字段特定于任务。

本文详细介绍了“流程/任务搜索”和“流程/任务详细信息”章节。 在适当的地点，任何具体的差异都将被特别指出。

## 进程/任务搜索 {#process-task-search}

您可以使用“进程/任务搜索”来定义查询进程/任务实例的筛选器。
...
1. 要创建查询，请单击 **添加** (+)。
   ...
   **要定义过滤器，请执行以下操作**
   ...
   1. 要将其他筛选器添加到查询，请单击 **添加(+)** 筛选器行的右侧。

      要从查询中删除筛选器，请单击 **删除(-)** 筛选器行的右侧。
...
* **保存**：保存过滤器。 然后，可以从查看和执行过滤器 **我的筛选器** 面板。
...
1. 要执行查询，请单击 **我的筛选器** 面板或单击 **运行** 按钮。
1. 查询结果显示在 **报告** 面板 **进程报告** 窗口。

   ![process_search_result](assets/process_search_result.png)

   借助报表底部显示的分页面板，可以对搜索结果进行分页。

   ![process_result_pgn](assets/process_result_pgn.png)

   在 **显示** 下拉列表中，选择每页显示的结果数。

   在 **页面** 文本框中，输入页码以直接转到该页面。

1. “进程搜索”结果中显示以下字段：

   * **进程ID**：进程的ID。 字段是超链接的。 如果单击此字段中的进程ID，您将被重定向到 **[!UICONTROL 进程详细信息]** 用于进程的面板。
   ...
   * **任务编号**：任务的ID。 字段是超链接的。 如果单击此字段中的任务ID，您将被重定向到 **[!UICONTROL 任务详细信息]** 任务面板。
   * **发起者**：启动流程实例的AEM Forms用户
   * **创建时间**：流程实例开始的日期和时间
   * **完成时间**：流程实例完成的日期和时间
   * **持续时间**：流程实例从开始到完成的持续时间
...
1. 单击边栏按钮 ![lc_pr_rail_button](assets/lc_pr_rail_button.png) 左上角折叠 **我的筛选器** 窗格并展开可用空间 **报告** 面板。
...
您使用 **进程详细信息** 面板以查看特定进程的详细信息。

同样，您使用 **任务详细信息** 面板以查看特定任务的详细信息。
...
1. 执行进程/任务搜索。 有关详细信息，请参阅 [执行进程搜索查询](#to-execute-a-search-query).
   ...
   请注意，结果中返回的显示的进程ID是超链接的。

   ![process_id_list](assets/process_id_list.png)

1. 单击列表中的进程ID可在 **进程详细信息** 面板。

   此 **进程/任务详细信息** 查询结果显示流程/任务中包含的任务/表单的详细信息。

   默认情况下，结果按任务/表单ID排序。 但是，要按任意字段对结果进行排序，请单击字段标题。 排序结果所依据的列由列标题旁边的深色箭头指示。
...
1. 单击 **返回流程/任务搜索** 以返回到从中向下钻取进程/任务详细信息的搜索结果。

   ![back_to_search](assets/back_to_search.png)

   但是，如果通过输入特定的流程/任务ID找到流程/任务详细信息，则单击“返回至流程/任务搜索”将返回至 **进程/任务搜索**，不显示任何搜索结果。
...
1. 在“进程/任务ID”文本框中，输入进程/任务ID。
   ...
   中的字段 **进程/任务详细信息** 查询结果是AEM Forms进程/任务特定的字段。

   对于进程，查询结果会显示该进程中所包含任务的详细信息。
