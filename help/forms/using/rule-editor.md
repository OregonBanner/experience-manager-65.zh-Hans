...
的 **设置属性** 利用规则类型，可根据条件操作设置指定对象的属性值。

它允许您定义规则以动态地向自适应表单添加复选框。 您可以使用自定义函数、表单对象或对象属性来定义规则。

![设置属性](assets/set_property_rule_new.png)

要根据自定义函数定义规则，请选择 **函数输出** 从下拉列表中，并从 **函数** 选项卡。 如果满足条件操作，则将自定义函数中定义的复选框数量添加到自适应表单。

要根据表单对象定义规则，请选择 **表单对象** 从下拉列表中，并从 **表单对象** 选项卡。 如果满足条件操作，则表单对象中定义的复选框数量将添加到自适应表单。
...
的 **设置选项** 规则类型允许您定义规则以动态地将复选框添加到自适应表单。 您可以使用表单数据模型或自定义函数来定义规则。

要根据自定义函数定义规则，请选择 **函数输出** 从下拉列表中，并从 **函数** 选项卡。 自定义函数中定义的复选框数将添加到自适应表单中。
...
1. 选择 **服务输出** 从下拉列表中。
1. 选择数据模型对象。
1. 从 **显示值** 下拉列表。 自适应表单中的复选框数量源自数据库中为该属性定义的实例数。
1. 从 **保存值** 下拉列表。
...
1. 点按要为其写入规则的表单对象，然后点按组件工具栏中的 ![edit-rules](assets/edit-rules.png). 将显示规则编辑器用户界面。
   ...
   此视图中列出了所选表单对象上的任何现有规则。 有关管理现有规则的信息，请参阅 [管理规则](#manage-rules).
