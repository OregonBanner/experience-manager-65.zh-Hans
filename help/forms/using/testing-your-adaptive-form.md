...
1. 以管理员身份登录到AEM Forms作者实例。 打开CRXDE Lite。 您可以点按AEM徽标>工 **具** >常规 **>** CRXDE Lite **，或在浏览器**[](https://localhost:4502/crx/de/index.jsp) 中打开https://localhost:4502/crx/de/index.jsp URL，以打开CRXDE Lite。

1. 导航到CRXDE Lite中的/etc/clientlibs。 右键单击/etc/clientlibs子文件夹，然后单击“创 **建** ” > “ **创建节点”。** 在“名称”字段中，键 **入WeRetailFormTestCases**。 选择类型 **cq:ClientLibraryFolder** ，然后单 **击确定**。 它创建一个节点。 您可以使用任何名称代替WeRetailFormTestCase。
1. 将以下属性添加到WeRetailFormTestCases节点，然后点按保 **存全部**。
...
![依赖性](assets/dependencies.png)

1. 右键单击“ **[!UICONTROL WeRetailFormTestCases”节点]** ，单击“创建 **”** >“ **创建文件”**。 在“名称”字段中，键入并 `js.txt` 单击“确 **定”**。
...
1. 在节点中创建一个文件init.js `WeRetailFormTestCases`。 将以下代码添加到文件，然后点按全 **[!UICONTROL 部保存]**。
...
1. 打开AEM测试UI（AEM >工具>操作>测试）。 测试套件- **We retail - Tests** —— 列在UI中。
...
1. 在CRXDE lite中，导览至该文 `/content/forms/af/create-first-adaptive-form` 件夹。 右键单击 **[!UICONTROL create-first-adaptive-form文件夹节点]** ，然后单击创建 **[!UICONTROL >创]**&#x200B;建文件 ****。 在“名称”字段中，键入并 `prefill.xml` 单击“确 **[!UICONTROL 定”]**。 将以下代码添加到文件：
...
1. 导航至 `/etc/clientlibs`. 右键单击子文 `/etc/clientlibs` 件夹，然后单 **[!UICONTROL 击创建]**> **[!UICONTROL 创建节点]**。

   在“名 **[!UICONTROL 称]** ”字段类型 `WeRetailFormTests`。 选择类型，然后 `cq:ClientLibraryFolder` 单击“确 **[!UICONTROL 定”]**。

1. 将以下属性添加到 **[!UICONTROL WeRetailFormTests节点]** 。
...
1. 在WeRetailFormTests节点中创建js.txt **[!UICONTROL 文件]** 。 将以下内容添加到文件：
