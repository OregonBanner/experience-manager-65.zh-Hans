...
1. 在页面中选择组件，然后点按编辑按钮 ![编辑按钮](assets/edit-button.png). 样式属性在侧栏中打开。

   您也可以从侧栏中的表单层次结构树中选择组件。 表单层次结构树在侧栏中可用作表单对象。

   您还可以从侧栏中选择组件。 在“样式”模式下，您可以看到列在“表单对象”下的组件。 但是，侧栏中的表单对象列表列出了字段和面板等组件。 字段和面板是可以包含文本框和单选按钮等组件的通用组件。
...
1. 同样，您也可以为组件的其他部分（如小组件、题注和帮助）应用样式。
