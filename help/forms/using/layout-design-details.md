...
分析创建交互式通信所需的布局后，您便可以使用Forms Designer在XDP模板中创建子表单。 XDP模板中使用的空白子表单组件会导致在交互式通信的打印通道中显示目标区域。

>[!NOTE]
>
>将内容添加到交互通信的打印渠道，而不是将内容添加到XDP模板中的子表单组件。 使用文档片段、图表、图像和布局片段将内 [容添加到打印渠道中的目标区](../../forms/using/create-interactive-communication.md#main-pars-header-741471925)域。
...
1. 选择“ **子表单** ”选项卡，然后从“内容 **”下拉列** 表中选择“ **Frued** ”。 在“绑 **定** ”选项卡中：

   1. 在“名称”字段中指定子表单 **的名** 称。
   1. 从“ **数据绑定** ”下拉列 **表中选择“无数** 据绑定”。
   重复第2步到第5步，向XDP模板添加更多子表单。 仅 [在创作交互式通信时，将文本、文档片段、图像和图表添加到目标区域](../../forms/using/create-interactive-communication.md#main-pars-header-741471925) 。
...
>[!NOTE]
>
>将图像添加到交互通信的打印通道，而不是将图像添加到XDP模板中的图像字段或子表单组件。 有关详细信息，请参 [阅将内容添加到交互式通信](../../forms/using/create-interactive-communication.md#step2)。
...
>[!NOTE]
>
>在表单元格中输入文本，或 [仅在创作交互式通信时创建与表单数据模型对象的绑定](../../forms/using/create-interactive-communication.md#main-pars-header-570999227) 。
...
1. 在“设计”视图中单击表头文本字 **段** ，并重命名它们。
1. 将“文本字段”组 **件从“对象库** ”拖放到“设计”视 **图中的每个表单元** 格中 ****。 执行此步骤，可在创作交互通信时将表单元格与表单数据模型对象绑定。
...
* 在“设计器”中，单 **击“文件** ”>“表单属性 **”>“默认值”******，并确保“PDF渲染／格式”设置设置为“动态XML表单”。
