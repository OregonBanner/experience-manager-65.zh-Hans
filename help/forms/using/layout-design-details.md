...
>[!NOTE]
>
>将内容添加到交互通信的打印渠道，而不是将内容添加到XDP模板中的子表单组件。 使用目标片段、图表、图像和布局片段将内 [容添加到打印渠道的文档](create-interactive-communication.md#step2)区域。

执行以下步骤以在XDP模板中使用子表单：

1. 打开Forms设计器，选 **择“文件** ” > “新 **建** ” > **“使用空白表单**”，点 ******** 按下一个，然后点按完成表单以打开表单，以创建模板。
...
1. 选择 **子表单** 选项卡， **然后从** “内容 **”下拉** 列表中选择“流”。 在“绑 **定** ”选项卡中：

   1. 在“名称”字段中指定子表 **单的** 名称。
   1. 从“ **数据绑定** ”下 **拉列表** 中选择“无数据绑定”。

   重复第2步到第5步，向XDP模板添加更多子表单。 仅 [在创作交互式通信时，将文本](create-interactive-communication.md#step2) 、文档片段、图像和图表添加到目标区域。
...
>[!NOTE]
>
>在表单元格中输入文本，或 [仅在创作交互式通信时创建与表单数据模型](create-interactive-communication.md#step2) 对象的绑定。
...
1. 点 **按** 表名称旁左侧窗格中的+，右键单击标题和其他行中包含的单元格名称，然后选择重命名 **对象** ，重命名表单元格。
1. 在“设计”视图中单击表标题文 **本字段** ，并重命名它们。
1. 将文本字段组件 **从对象** 库拖 **放到设计** 视图 **中的每个表**&#x200B;单元格。 执行此步骤，可在创作交互通信时将表单元格与表单数据模型对象绑定。

   ![表中的文本字段](assets/text_fields_table_new.png)

1. 从左窗格中选择行的名称，然后选择“对 **象** ”>“ **绑定** ” **>“对每个**&#x200B;数据项重复行”。 执行此步骤可确保，如果在此行的表单元格之间创建了具有集合类型的表单数据模型对象的绑定，则会自动为数据库中可用的每个数据项重复表行。
...
1. 选择 **Forms** > **Forms和文档**。
1. 点按 **创建** > **文件上传**。
1. 导航到本地文件系统上XDP模板的位置，然后点 **按打开** ，将XDP模板导入到AEM Forms服务器。
