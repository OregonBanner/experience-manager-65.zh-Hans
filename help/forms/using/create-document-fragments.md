...
* 文本 — 文本资源是由一个或多个文本段落组成的一段内容。 段落可以是静态的或动态的。
* 列表 — 列表是一组文档片段，包括文本、列表、条件和图像。
* 条件 — 通过条件，可根据从表单数据模型收到的数据定义要包含在交互式通信中的内容。

本教程将指导您完成根据中提供的解剖创建多个文本文档片段的步骤。 [规划交互式通信](/help/forms/using/planning-interactive-communications.md) 部分。 在本教程结束时，您将能够：
...
* [账单汇总](../../forms/using/create-document-fragments.md#step-create-bill-summary-text-document-fragment)
* [费用汇总](../../forms/using/create-document-fragments.md#step-create-summary-of-charges-text-document-fragment)

每个文档片段包括具有静态文本的字段、从表单数据模型接收的数据以及使用代理UI输入的数据。 所有这些字段在 [规划交互式通信](/help/forms/using/planning-interactive-communications.md) 部分。

在本教程中创建文档片段时，将使用代理UI为接收数据的字段创建变量。

使用 **FDM_Create_First_IC**，如 [创建表单数据模型](../../forms/using/create-form-data-model0.md) 部分，作为表单数据模型，用于在本教程中创建文档片段。

## 步骤1：创建账单详细信息文本文档片段 {#step-create-bill-details-text-document-fragment}
...
执行以下步骤，为使用代理UI作为数据源的字段创建变量、创建静态文本并在文档片段中使用表单数据模型元素：
...
1. 将光标放在旁边 **发票编号** 字段并双击 **发票编号** 变量来自 **变量** 选项卡。
1. 将光标放在旁边 **记帐期间** 字段并双击 **Billperiod** 变量。
1. 将光标放在旁边 **记帐日期** 字段并双击 **记帐日期** 变量。
1. 选择 **数据模型对象** 选项卡。
1. 将光标放在旁边 **您的计划** 字段并双击 **客户** > **customerplan** 属性。
...
1. 选择 **数据模型对象** 选项卡，将光标放在右窗格中，然后双击 **客户** > **name** 属性。
...
1. 将光标放在旁边 **手机号码** 字段并双击 **客户** > **mobilenum** 属性。
1. 将光标放在旁边 **备用联系电话** 字段并双击** customer** > **alternatemobilenumber** 属性。
1. 将光标放在旁边 **关系编号** 字段并双击 **客户** > **关系编号** 属性。
1. 选择 **变量** 选项卡，将光标放置在 **供应地点** 字段并双击 **Placesupply** 变量。
1. 将光标放在旁边 **州代码** 字段并双击 **Statecode** 变量。
1. 将光标放在旁边 **连接数** 字段并双击 **连接数** 变量。
...
1. 将光标放在旁边 **上一个余额** 字段并双击 **以前的平衡** 变量。
1. 将光标放在旁边 **支付** 字段并双击 **支付** 变量。
1. 将光标放在旁边 **调整** 字段并双击 **调整** 变量。
1. 将光标放在旁边 **到期金额** 字段并双击 **金额** 变量。
1. 将光标放在旁边 **到期日期** 字段并双击 **Duedate** 变量。
1. 选择 **数据模型对象** 选项卡，将光标放置在 **费用当前帐单期间** 字段，然后双击 **帐单** > **使用费用** 属性。

   ![账单汇总](assets/bill_summary_static_variables_new.png)
...
1. 选择 **数据模型对象** 选项卡。
1. 将光标放在旁边 **呼叫费用** 字段并双击 **帐单** > **callcharges** 属性。
1. 将光标放在旁边 **电话会议费用** 字段并双击 **帐单** > **confcallcharges** 属性。
1. 将光标放在旁边 **短信费用** 字段并双击 **帐单** > **smscharges** 属性。
1. 将光标放在旁边 **移动互联网费用** 字段并双击 **帐单** > **internetcharges** 属性。
1. 将光标放在旁边 **国家漫游费** 字段并双击 **帐单** > **roamingnational** 属性。
1. 将光标放在旁边 **国际漫游费** 字段并双击 **帐单** > **roamingintel** 属性。
1. 将光标放在旁边 **增值服务费用** 字段并双击 **帐单** > **vas** 属性。
1. 将光标放在旁边 **总费用** 字段并双击 **帐单** > **使用费用** 属性。
1. 将光标放在旁边 **应付总额** 字段并双击 **帐单** > **使用费用** 属性。

   ![费用汇总](assets/summary_charges_static_fdm_new.png)

1. 选择文本 **增值服务费用** 行并点按 **创建规则** 要创建行在交互式通信中显示时所依据的条件，请执行以下操作：
1. 在 **创建规则** 弹出窗口：
