...
* 在编写表达式时，要访问字段和面板，可以使用字段或面板的名称。 要访问字段的值，请使用value属性。 For example, `field1.value`
...
* **[单击表达式](../../forms/using/adaptive-form-expressions.md#p-click-expression-p)**:以处理按钮的单击事件上的操作。
* **[](../../forms/using/adaptive-form-expressions.md#p-initialization-script-p)初始化脚本&#x200B;**:对字段的初始化执行操作。
* **[选项表达式](../../forms/using/adaptive-form-expressions.md#p-options-expression-p)**:以动态填写下拉列表。
* **[摘要表达式](#summary)**:动态计算折叠面板的标题。
* **[验证表达式](../../forms/using/adaptive-form-expressions.md#p-validate-expression-p)**:来验证字段。
* **[](../../forms/using/adaptive-form-expressions.md#p-value-commit-script-p)值提交脚本&#x200B;**:更改字段值后表单的组件。
...
**示例**:要根据另一个字段的值填充字段的值，请提供一个简单的选项表达式。 例如，要根据在另一个字段 **中表示的“婚姻状态****** ”填充字段“子女数”，该表达式为：

**`marital_status.value == "married" ? ["1=One", "2=two"] : ["0=Zero"]`.**

只要finery_status字 **段的值发生更改** ，表达式就会重新触发。 您还可以从REST服务填充下拉列表。 有关详细信息，请参 [阅动态填充下拉列表](../../forms/using/dynamically-populate-dropdowns.md)。
...
>[!NOTE]
>
>如果为非强制或强制字段编写验证表达式，则无论字段的可见性状态如何，都将计算表达式。 要停止对隐藏字段的验证，请将“初始化”或“值提交脚本”中的validationsDisabled属性设置为true。 For example, `this.validationsDisabled=true`
...
>[!NOTE]
>
>在以编程方式更改字段的值时，可以禁用Value Commit script的执行。 为此，请转到https://[server]:[port]/system/console/configMgr并将Adaptive Forms Version for Compatibility **（兼容性的自适应表单版本）更改为****** AEM Forms 6.1。 此后，仅当用户从UI更改字段的值时，才执行“值提交脚本”。
...
也可以使用不同字段上的表达式计算字段的验证。 这些表达式编写在组 **件的“编辑** ”对话框的“脚本 **”选项卡的“****** 验证脚本”字段内。 字段的验证状态取决于表达式返回的值。 有关如何编写此类表达式的信息，请参阅验 [证表达式](../../forms/using/adaptive-form-expressions.md#p-validate-expression-p)。
