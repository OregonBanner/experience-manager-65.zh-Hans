...
* 确保将AEM Forms云服务配置为使用Adobe Sign。 有关详细信息，请参阅[将Adobe Sign与AEM Forms整合](../../forms/using/adobe-sign-integration-adaptive-forms.md)。
...
1. 在&#x200B;**[!UICONTROL 基本]**&#x200B;选项卡中：
   ...
   1. 选择在配置Adobe Sign时创建的[配置容器](../../forms/using/adobe-sign-integration-adaptive-forms.md#configure-adobe-sign-with-aem-forms)。
...
1. 在&#x200B;**[!UICONTROL 基本]**&#x200B;选项卡中，选择在将Adobe Sign配置为AEM Forms时创建的[配置容器](../../forms/using/adobe-sign-integration-adaptive-forms.md#configure-adobe-sign-with-aem-forms)。
...
1. 点按&#x200B;**Adobe Sign字段** ![ aem_6_3_adobesign](assets/aem_6_3_adobesign.png)图标。 它显示用于选择和添加Adobe Sign字段的选项。
   ...
   如果从下拉列表中选择&#x200B;**数字签名**，则可以将数字签名应用到自适应表单：
...
1. 从已配置的Adobe SignCloud Services列表中选择云服务。
   ...
   如果&#x200B;**Adobe SignCloud Service**&#x200B;列表为空，请按照[使用AEM Forms](../../forms/using/adobe-sign-integration-adaptive-forms.md)文章配置Adobe Sign以配置服务。
...
1. 在属性浏览器中，展开&#x200B;**电子签名**&#x200B;折叠面板，然后选择&#x200B;**启用Adobe Sign**&#x200B;选项。 它使Adobe Sign能够获得一种适应性表单。
1. 点按&#x200B;**签署方配置**&#x200B;下的&#x200B;**添加签署方**。 它会向自适应表单中添加一个签名者。 您可以向自适应表单中添加多个Adobe Sign签名者。
...
在您将Adobe Sign字段添加到自适应表单、启用Adobe Sign表单容器、选择Adobe SignCloud Service并添加Adobe Sign签名者后，为自适应表单选择适当的提交操作。 有关自适应表单提交操作的详细信息，请参阅[配置提交操作](../../forms/using/configuring-submit-actions.md)。

此外，只有在所有签名者签署表单后，才会提交启用Adobe Sign的自适应表单。 您可以在表单门户的“待签名”部分找到部分签名的表单。 Adobe Sign配置服务以[定期间隔](../../forms/using/adobe-sign-integration-adaptive-forms.md)对Adobe Sign服务器进行轮询以验证签名状态。 如果所有签署方都完成了表单的签名，则会启动提交操作服务并提交表单。 如果您使用自定义提交操作，并且表单使用Adobe Sign，请更新自定义提交操作以使用提交操作服务。
...
1. 点按&#x200B;**Adobe Sign字段** ![ aem_6_3_adobesign](assets/aem_6_3_adobesign.png)图标。 它显示用于选择和添加Adobe Sign字段的选项。
   ...
   展开&#x200B;**Type**&#x200B;下拉字段以选择&#x200B;**数字签名**，然后点按完成图标以将所选字段添加到Adobe Sign块。
...
1. 选择新添加的签名步骤组件，然后点按&#x200B;**配置** ![配置](assets/configure.png)图标。 它打开属性浏览器并显示签名步骤属性。 配置以下属性：
   ...
   * **使用旧版电子签名组件**:如果您在AEM Forms工作区、AEM Forms应 [用程序中使用相应的自适应表单](../../forms/using/introduction-html-workspace.md)，或者基础自适应表单具有旧版电子签名组件，请选择 **使用旧版电子签名** 组件。
...
**问：可** 以在另一个自适应表单中嵌入自适应表单。嵌入的自适应表单是否可以启用Adobe Sign?
**答：不** 支持，AEM Forms不支持使用嵌入支持Adobe Sign的自适应表单进行签名的自适应表单
...
**问：我** 是否可以在自适应表单的静态文本组件中使用Adobe Sign文本标记？**答：是** 的，您可以在文本组件中使用文本标记将Adobe Sign字段添加到 [记录文档](../../forms/using/generate-document-of-record-for-non-xfa-based-adaptive-forms.md) (仅自动生成记录文档选项)启用的自适应表单。要了解创建文本标记的过程和规则，请参阅[Adobe Sign文档](https://helpx.adobe.com/sign/using/text-tag.html)。 另外，自适应表单对文本标记的支持有限。 您可以使用文本标记仅创建[Adobe Sign块](../../forms/using/working-with-adobe-sign.md#configure-cloud-signatures-for-an-adaptive-form)支持的字段。

**问：** AEM Forms提供Adobe Sign块和签名步骤组件。能否同时在自适应形式中使用它们？
...
**问**
题：当Adobe Sign服务配置为自适应表单时，该服务无法为基础自适应表单创建Adobe Sign协议。
...
* 如果您使用多个Adobe Sign云服务，请将所有服务的&#x200B;**[!UICONTROL oAuth URL]**&#x200B;指向相同的&#x200B;**[!UICONTROL Adobe Sign共享]**。

* 使用单独的电子邮件地址为Adobe Sign帐户以及第一个签署方和单个签署方配置电子邮件地址。 第一个签署方或唯一签署方（对于单个签署方）的电子邮件地址不能与用于配置AEM云服务的Adobe Sign帐户相同。

### 为启用Adobe Sign的自适应表单配置的AEM Forms工作流不开始{#adobe-sign-aem-form-workflow-failures}

**问**
题当Adobe Sign为自适应表单配置时，使用调用Forms Workflow选项配置的工作流不开始。

**分辨率**

* 如果您使用Adobe Sign时没有签名步骤或表单需要多人签名，AEM Forms服务器会等待调度程序确认所有人已签署表单。 调度程序只有在所有人完成签名后才提交自适应表单，而工作流开始仅在自适应表单成功提交后才提交。 您可以缩短[调度程序](adobe-sign-integration-adaptive-forms.md)的时间间隔，以快速时间间隔检查表单签名的状态并加快表单提交。
