...
从下拉列表中选择&#x200B;**布局** ，该下拉列表显示在自适应表单创作界面顶部的&#x200B;**预览**&#x200B;选项旁边。 表单将在布局模式下显示。

1. 登录到AEM创作实例，然后导航到&#x200B;**Adobe Experience Manager** > **Forms** > **Forms和文档**。
1. 创建新表单或打开现有[自适应表单](../../forms/using/creating-adaptive-form.md)。
...
**目标：** 您想要插入一个表组件和一个图像组件，并在自适应表单中将它们平行放置。
...
1. 从显示在&#x200B;**预览**&#x200B;选项旁边顶部的下拉列表中选择&#x200B;**布局**。 此时会显示一条确认消息，用于启用“布局”模式。
1. 点按&#x200B;**Yes**&#x200B;以启用表单的&#x200B;**布局**&#x200B;模式。
...
### 将具有新响应式布局的片段嵌入到旧自适应表单{#embed-a-fragment-with-new-responsive-layout-in-an-old-adaptive-form}中

如果您在具有旧响应式布局的自适应表单中嵌入具有新响应式布局的片段，则系统会提示您为表单启用布局模式并重新嵌入该片段。

要启用布局模式，请从&#x200B;**预览**&#x200B;选项旁边顶部显示的下拉列表中选择&#x200B;**布局**，然后点按&#x200B;**是**&#x200B;进行确认。 选择&#x200B;**编辑**&#x200B;模式以重新嵌入片段。
