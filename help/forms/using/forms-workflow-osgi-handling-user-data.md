...
description: Forms上以OSGi为中心的工作流 |处理用户数据
...
1. 对步骤2中查询返回的所有工作流实例重复步骤3 - 5。
   ...
   >[!NOTE]
   >
   >AEM [!DNL Forms]应用程序还将数据存储在脱机模式下。 当应用程序与服务器同步时，工作流实例的数据可能会本地存储在单个设备上，并被提交到[!DNL Forms]服务器。
...
1. 识别并删除来自工作流参与者的AEM [!DNL Forms]应用程序外框的脱机草稿和提交数据，以避免任何提交到服务器。
