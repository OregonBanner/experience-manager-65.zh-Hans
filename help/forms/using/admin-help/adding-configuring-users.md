...
使用MySQL时，用户ID可以包含扩展字符。 但是，当在两个字符串（如abcde和âbcdè）之间进行比较时，它们会被视为相同。 例如，在同步时，如果向数据库中添加了新用户，则会进行比较以检查数据库中是否存在具有相同用户ID的用户。 如果用户 *abcde* 新用户 *阿布迪* 添加，则比较无法区分这两个名称。 假定用户存在于数据库中，并且新用户将被忽略且不会添加。
