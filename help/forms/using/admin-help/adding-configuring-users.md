...
使用MySQL时，用户ID可以包含扩展字符。 但是，在比较两个字符串（例如abcde和abcde）时，会将其视为相同。 例如，在同步时，如果将新用户添加到数据库中，则会进行比较以检查数据库中是否存在具有相同用户ID的用户。 If用户 *abcde* 当新用户存在时，数据库中已存在 *abcde* 添加，则比较无法区分这两个名称。 假定数据库中已存在该用户，并且新用户被忽略而不添加。
...
有关如何自定义错误处理页面的详细信息，请参阅 [自定义错误处理程序显示的页面](https://docs.adobe.com/docs/en/cq/current/developing/customizing_error_handler_pages.html) 在Adobe Experience Manager文档中。
