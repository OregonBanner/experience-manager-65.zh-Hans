...
通过创建用户组，您可以将角色分配给组，而不是分配给个人用户。

提供了两种不同类型的组。 您可以手动创建组并将用户和其他组添加到其中。 您还可以创建动态组，以自动包含满足一组指定规则的所有用户。

如果用户属于多个组（例如，500个或更多）或者这些组嵌套得很深（例如，30个级别），则用户响应速度可能会较慢。 如果您遇到此问题，可以将AEM表单配置为从特定域预取信息。 (请参阅 [配置AEM表单以预取域信息](/help/forms/using/admin-help/configure-aem-forms-prefetch-domain.md#configure-aem-forms-to-prefetch-domain-information).)

## 手动创建组 {#create-a-group-manually}

手动创建组时，可以向组添加用户和其他组，并为组分配角色。 您还可以将组与父组关联。

如果您使用的是Content Services（已弃用），则可以选择“域管理”页面上的选择此选项以将用户和组推送到已注册的外部承担者存储提供程序中，以推送您在Content Services中创建的任何新用户或组的信息（已弃用）。

1. 在管理控制台中，单击设置>用户管理>用户和组，然后单击新建组。
...
1. 单击下一步。
1. 要将此新组添加到其他现有组，请单击“查找组”，然后执行以下任务：
...
在动态组中，不会单独选择属于该组的用户。 相反，您需要指定一组规则，并且符合这些规则的所有用户都会自动添加到动态组中。

使用以下两种方法之一创建动态组：

* 启用基于电子邮件域(如@adobe.com)的动态组的自动创建。 启用此功能后，用户管理会为AEM Forms数据库中的每个唯一电子邮件域创建一个动态组。 使用cron表达式可指定用户管理搜索AEM表单数据库中新电子邮件域的频率。 这些动态组将添加到DefaultDom本地域中，并命名为“具有 *`[email domain]`* 邮件ID。”
* 根据指定的条件（包括用户的电子邮件域、描述、规范名称和域名）创建动态组。 要属于动态组，用户必须满足所有指定的标准。 要设置“或”条件，请创建两个不同的动态组，并将它们添加到本地组中。 例如，使用该方法可创建一组属于@adobe.com电子邮件域或其规范名称包含ou=adobe.com的用户。 但是，用户不一定必须同时满足这两个条件。
...
1. 在“自动创建动态组”下，选中复选框。
1. 指定用户管理器何时检查新电子邮件域。 该时间应晚于域同步时间，因为只有在域同步完成后，动态组的创建才符合逻辑。

   * 要每天启用自动同步，请在“Occurs Daily At”（每日发生）框中键入24小时格式的时间。 保存设置时，此值将转换为cron表达式，该表达式显示在下面的框中。
   * 要安排在一周或一个月中的某一天或某一特定月份进行同步，请在框中选择键入相应的cron表达式。 默认值为 `0 00 4 ? * *`（这意味着每天凌晨4点查看）。
...
1. 在“动态组标准”下，指定一个或多个用于填充动态组的属性。
   ...
   **域名：** 用户所属的域的名称，例如 `DefaultDom`. 使用Contains运算符时，“域名”属性区分大小写。 使用Starts With、Ends With或Equals运算符时，它不区分大小写。

1. 单击“测试”。 “测试”页面会显示前200名符合所定义标准的用户。 单击关闭。
...
1. 单击组的名称可显示有关的详细信息。 此时将显示“组详细资料”页。
...
1. 要编辑组中的用户和组，请单击“子承担者”选项卡，然后执行以下任务：
   ...
   * 要查找要添加的用户和组，请单击“查找用户/组”并填写搜索信息。
