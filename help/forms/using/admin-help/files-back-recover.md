...
内容存储根目录的默认位置是 `[aem-forms root]/lccs_data`.
...
内容存储根目录的默认位置是 `[GDS root]/lccs_data`，其中 `[GDS root]` 是 [GDS位置](files-back-recover.md#gds-location). 备份位于内容存储根目录中的以下目录：
...
索引根目录的默认位置为 `[aem-forms root]/lucene-indexes` 在每个节点上。
...
>[!NOTE]
>
>默认情况下，随AEM Forms一起安装的Adobe字体位于 `[aem-forms root]/fonts` 目录访问Advertising Cloud的帮助。
