...
* 创建一个公钥/私钥对，将公钥封装在X.509 v1自签名证书中（该证书存储为单元素证书链），然后将证书链和私钥存储到一个新的密钥库中。 此密钥库是应用程序服务器的自定义标识密钥库。
* 提取证书并将其插入新密钥库中。 此密钥库是应用程序服务器的自定义信任密钥库。

然后，配置WebLogic，使其使用您创建的自定义身份密钥库和自定义信任密钥库。 此外，禁用WebLogic主机名验证功能，因为用于创建密钥库文件的可分辨名称不包含承载WebLogic的计算机的名称。

## 创建要在WebLogic Server上使用的SSL凭据 {#creating-an-ssl-credential-for-use-on-weblogic-server}

keytool命令通常位于Java jre/bin目录中，并且必须包含下表中所列的多个选项和选项值。
...
<table>
 ...
 <tbody>
  ...
  <tr>
   <td><p>-keystore</p></td>
   <td><p>密钥库文件的位置和名称。</p><p>该位置可以包括文件的绝对路径。 或者，它可以相对于输入keytool命令的命令提示符的当前目录。</p></td>
   <td>
    <ul>
  ...
  <tr>
   ...
   <td><p>3650</p><p>您可以使用不同的值，具体取决于贵公司的策略。</p></td>
  </tr>
  <tr>
   <td><p>-storepass</p></td>
   ...
   <td>
    <ul>
     <li><p>自定义身份密钥库：密钥库密码必须与为Administration Console的Trust Store组件指定的SSL凭据密码对应。</p></li>
...
1. 通过输入以下命令从ads-credentials密钥库中提取证书：
   ...
   >[!NOTE]
   >
   >Replace `[JAVA_HOME]` 替换为安装JDK的目录，并替换 `store`*_* `password`*包含自定义身份密钥库的密码。*
   ...
   名为“ads-ca.cer”的证书文件创建于 [appserverdomain]/adobe/[*服务器名称*] 目录。

1. 将ads-ca.cer文件复制到任何需要与应用程序服务器进行安全通信的主机上。
1. 通过输入以下命令，将证书插入到新的密钥库文件（自定义信任密钥库）中：
   ...
   >[!NOTE]
   >
   >Replace `[JAVA_HOME]` 替换为安装JDK的目录，并替换 `store`*_* `password` 和 `key`*_* `password` *使用您自己的密码。*
...
配置WebLogic，使其使用您创建的自定义身份密钥库和自定义信任密钥库。 此外，禁用WebLogic主机名验证功能，因为用于创建keystore文件的可分辨名称不包含承载WebLogic Server的计算机名。
...
1. 如果此服务器是受控服务器，请将侦听端口更改为未使用的端口值（例如8001），并将SSL侦听端口更改为未使用的端口值（例如8002）。 在独立服务器上，默认SSL端口为7002。
1. 单击 **发行配置**.
...
1. 默认情况下，为“标识位置”和“信任位置”选择“密钥库”。 如果不能，请将其更改为keystore。
1. 在“标识”下，指定以下值：
