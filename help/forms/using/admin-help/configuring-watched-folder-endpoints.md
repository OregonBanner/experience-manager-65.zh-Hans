...
**注意**:指 *定产生唯一输出文件名的名称可提高性能。 例如，考虑服务返回一个输出文档，而“输出参数映射”将其映射到（输入文件的文件名和扩展名）的情`%F.%E`况。 在这种情况下，如果用户每分钟都会删除同名的文件，并将结果文件夹配置为“覆盖重复文件名”设置为关闭，则“监视文件夹”将尝试解析重复文件名。`result/%Y/%M/%D`解析重复文件名的过程可能会影响性能。 在这种情况下，将“输出参数映射”更改为`%F_%h_%m_%s_%l`向名称中添加小时、分钟、秒和毫秒，或者确保丢弃的文件具有唯一的名称可能会提高性能。*
