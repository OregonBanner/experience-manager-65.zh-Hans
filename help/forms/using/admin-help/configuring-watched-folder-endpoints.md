...
**变量：** 输入的值是watched文件夹用来选取输入的文件模式。 例如，在encrypt password service的情况下，输入文件必须是PDF文件，用户可以使用&amp;ast；.pdf作为文件模式。 观察文件夹将拾取观察文件夹中与此模式匹配的所有文件，并为每个文件调用服务。 使用变量时，所有输入文件都会转换为文档。 仅支持使用文档作为输入类型的API。
...
为监视文件夹端点启用限制时，它会限制可在任意给定时间处理的监视文件夹作业的数量。 最大作业数由批处理大小值决定，该值也可以在Watched文件夹端点中配置。 当达到限制限制时，将不会轮询观察文件夹输入目录中的传入文档。 文档还将保留在输入目录中，直到其他观察的文件夹作业完成并再次尝试轮询为止。 在同步处理的情况下，在单个轮询中处理的所有作业都将计入限制限制，即使这些作业在单个线程中连续处理。
...
* 如果表单服务器正在运行，但在作业管理器服务回调时观察文件夹未运行（当服务未按顺序启动时会发生这种情况），则管理员可以将文件移出暂存文件夹。 有关信息，请参阅 [故障点和恢复](configuring-watched-folder-endpoints.md#failure-points-and-recovery).


## 性能和可扩展性 {#performance-and-scalability}

“观察文件夹”可以在单个节点上提供总计100个文件夹。 观察文件夹的性能取决于表单服务器的性能。 对于异步调用，性能更取决于系统负载和作业管理器队列中的作业。

通过向群集添加节点，可以改进观察文件夹的性能。 监视文件夹作业通过Quartz调度程序跨群集节点分发，在异步请求的情况下，通过Job Manager服务分发。 所有作业都保留在数据库中。
