...
您可以从系统级别和内部级别监控AEM表单部署。 您可以使用诸如HP OpenView、IBM® Tivoli和CA UniCenter等专业管理工具，以及名为 *JConsole* 专门监控Java™活动。 实施监控策略可提高AEM表单部署的可用性、可靠性和性能。
...
* **运算统计：** 此MBean提供每个AEM Forms服务器服务的统计信息。 在此MBean中，管理员可以获取有关特定服务的信息，如调用时间和错误数。
...
**调用时间：** 执行方法所花费的时间。 此调用不包括序列化请求、从客户端到服务器传输请求和反序列化请求的时间。
...
1. 编辑位于 `[WebLogic home]/user_projects/domains/Adobe_Live_Cycle/bin`.
1. 找到JAVA_OPTS行并添加以下内容：
