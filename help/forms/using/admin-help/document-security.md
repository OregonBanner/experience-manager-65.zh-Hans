...
有关策略如何保护受支持文件类型的详细信息，请参阅 [更多文档安全信息](https://experienceleague.adobe.com/docs/experience-manager-65/forms/use-document-security/document-security-offerings.html?lang=en).
...
* 文档安全管理员创建策略和策略集，并根据需要为用户管理受策略保护的文档。 他们还创建受邀的用户帐户，并监视系统、文档、用户、策略、策略集和自定义事件。 他们还可能负责与系统管理员配置全局服务器、网页和策略设置。
...
* 对您的组织外部（例如，业务合作伙伴）的用户，如果文档位于文档安全目录中，或者管理员为其创建帐户，或者他们通过自动电子邮件邀请流程通过文档安全注册，则可以使用受策略保护的文档。 根据管理员如何启用访问设置，受邀用户还可以有权将策略应用于文档、创建、修改和删除其策略，以及邀请其他外部用户使用其受策略保护的文档。
...
* 在Microsoft® Outlook中以电子邮件附件形式发送文档。 在这种情况下，您可以从策略列表中选择策略。 或者，您也可以选择Acrobat使用一组默认的保密设置创建的自动生成的策略，以仅保护电子邮件收件人的文档。
...
在将策略应用于文档时，其中包含的信息(包括PDF文档中的任何包含文件（文本、音频或视频）)将受策略中指定的保密性设置的保护。 文档安全性生成许可证和加密信息，然后将其嵌入到文档中。 当您分发文档时，文档安全可以验证尝试打开文档的收件人，并根据策略中指定的权限授权访问。
...
打开受策略保护的文档时，Acrobat、Adobe Reader或Acrobat Reader DC扩展会连接到文档安全以验证您的身份。 然后，您可以继续登录。 如果正在审核文档使用情况，则会显示通知消息。 在文档安全确定要授予哪些文档权限后，它会管理文档的解密。 然后，您可以根据策略保密性设置使用文档。
...
如果策略允许脱机访问，则还可以脱机使用受策略保护的文档（无Internet或网络连接）。 首先登录文档安全以同步文档。 然后，您可以在策略中指定的脱机租用期内使用文档。

当脱机租用期结束时，通过联机并打开受策略保护的文档或使用客户端应用程序中的命令，再次将文档与文档安全同步。 请参阅 *Acrobat帮助* 或适当 *Acrobat Reader DC扩展帮助* 以了解详细信息。
...
每个策略集可以具有一个或多个关联的策略集协调器。 策略集协调器是管理员或具有更多权限的用户。 的 *策略集协调器* 通常是组织中最擅长创作特定策略集中策略的专家。
...
通过指定域中哪些用户或组可以使用策略集中的策略来保护文档，使有限数量的用户可以使用策略集。

安装文档安全后，将创建一个默认策略集，名为 *全局策略集*. 安装软件的管理员将管理此策略设置。

## 最佳实践 {#best-practices}

策略是可以应用于各种文档的可重用权限集和用户组。 保护文档。 这些策略可确保只有授权用户才能使用允许的功能。 随着部门内不同用户角色和文档的增加，策略和策略集的数量预计会增加。 要创建和管理策略，请考虑以下事项和最佳实践：

* **创建可重复使用的策略：** Adobe建议在各种文档中重复使用策略。 它有助于将策略数量保持在最低，提供最佳性能，并且更便于管理策略。 要创建可重复使用的策略，请执行以下操作：
...
将用户组添加到策略中，而不是单个用户。 它更便于管理策略并将其应用于许多用户。
...
* **使用外部授权程序动态应用权限：** 您可以使用 [外部授权器](https://help.adobe.com/en_US/livecycle/11.0/ProgramLC/WS624e3cba99b79e12e69a9941333732bac8-6f26.2.html) 以根据外部条件评估和动态应用权限。 在根据外部条件动态评估权限时，您可以：

   * 为组织中的文档提供集中的访问控制。

   * 通过动态确定用户是否可以访问受策略保护的文档来控制对受策略保护文档的访问。 例如，动态确定用户是否可以打印受策略保护的文档。

   * 使用内容管理系统所使用的访问控制机制以及标准策略评估过程。 例如，当服务确定用户是否可以打印受策略保护的文档时，它可以使用标准策略评估过程。 此外，它还可以使用内容管理系统所使用的访问控制机制。
   虽然可以用外部授权处理程序完全替换文档安全策略评估过程，但建议将外部授权处理程序与策略评估过程结合使用。 因此，文档访问可以由内容管理系统所使用的相同控制机制进行控制。 例如，当文档安全服务确定用户是否可以打印受策略保护的文档时，它使用标准策略评估过程。 它还使用内容管理系统使用的访问控制机制。 有关更多信息，请参阅 [创建外部授权处理程序](https://help.adobe.com/en_US/livecycle/11.0/ProgramLC/WS624e3cba99b79e12e69a9941333732bac8-6f26.2.html).

* **将策略集保留为有限数量：** 若干因素导致政策及政策组合持续增长。 一些常见因素包括：
