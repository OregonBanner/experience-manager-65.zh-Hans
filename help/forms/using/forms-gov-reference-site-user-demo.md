...
1. **移动设备视图**：We.Gov用户使用重新调整的浏览器复制移动视图。
...
**本节**： Camila CX销售线索有一个基于PDF的现有表单，该表单用作基于纸张的流程的一部分。 作为现代化工作的一部分，她希望使用此PDF表单自动创建新的现代化自适应Forms。
...
1. 然后，Camilla选择PDF表单并单击 **启动自动转换** 以开始转换过程。 您可能需要单击 **覆盖转换** 如果您已转换表单。
   ...
   >[!NOTE]
   >
   >请注意，AFC中的设置是为最终用户预先配置的，这意味着这些设置不应更改。
...
1. 然后，Camilla开始编辑表单。 她选择“根面板”>“编辑（扳手）”>“从面板布局下拉菜单中选择顶部选项卡”>“选中复选框”。
...
**本节**：在文档已转换并生成自适应表单后，Camila需要将自适应表单连接到数据源。
...
1. Camila单击 **输出** 文件夹，选择自适应表单并单击 **编辑** 以打开已完成的We.Gov表单。
1. Camila选择自适应表单字段并单击 ![“配置”图标](assets/configure-icon.svg). 她使用创建与表单数据模型实体的绑定 **绑定引用** 字段。 她对自适应表单中的所有字段重复此步骤。

### 表单无障碍测试(Camila) {#form-accessibility-testing}

Camila还会验证已创建的内容是否已根据公司标准正确构建并可完全访问。

1. Camila单击 **输出** 文件夹，选择自适应表单并单击 **预览** 以打开已完成的We.Gov表单。
...
1. 重新调整浏览器窗口的大小或使用浏览器的模拟器复制移动设备大小。

### We.Gov网站(Aya) {#aya-user-story-we-gov-website}

![虚拟用户](/help/forms/using/assets/aya_tan_new-1.png)

**本节**：Aya是公民。 她从一位朋友那里听说，她可能有资格获得政府机构提供的服务。 Aya通过手机导航到We.Gov网站，进一步了解她有资格使用的服务。
...
Aya需要先在家中查看一些文档，然后才能完成服务请求申请。 她保存并从移动设备中退出应用程序。
...
**本节：** 回到家后，Aya找到了所需的信息，并从桌面上恢复应用程序。 Aya导航到在线表单门户以继续执行其应用程序。 通过一些简单的自定义设置，代理机构还可以自动生成链接并通过电子邮件发送以恢复应用程序。
...
1. 单击右侧“圆形CTA”可移动到下一截面。
   ...
   >[!NOTE]
   >
   >当Aya填写“电话号码”字段时，她必须将其填写为不带破折号、空格或连字符的连续11位数。

   提交Aya后，会收到“感谢”页面。 （可选）她还将会收到一封电子邮件，她可以打开该邮件以使用Adobe Sign电子方式签署记录文档。
...
**本节：** Camila CX Lead与Aya建立了欢迎电话，解释如何使用她已获批准的政府服务。
...
1. 然后Camila选择AEM Forms **We.gov FDM** 然后编辑 **We.gov注册FDM**.

1. 然后Camila选择 **联系人** > **读取服务** 将进行测试。

   ![联系读取服务](assets/aftia-contact-read-service.jpg)

1. 然后，Camila提供具有联系人ID的测试服务，然后单击“测试”按钮。 例如，1或2（如果您已提交表单）。 如果您尚未提交表单，则不会返回任何数据。
...
**本节：** Camila导航到一个仪表板，她可以看到整个机构的KPI，例如开始填写服务请求表单并放弃的公民百分比，从请求提交到批准/拒绝响应的平均时间长度，以及她发送给公民的福利手册的参与统计数据。
