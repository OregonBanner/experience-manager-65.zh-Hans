...
按照 [设置和配置We.Gov和We.Finance参考网站](../../forms/using/forms-install-configure-gov-reference-site.md).
...
**此部分**:Camila CX Lead现有的基于PDF的表单，在基于纸张的流程中使用。 作为现代化努力的一部分，她希望使用此PDF表单自动创建新的现代自适应Forms。
...
1. 然后，Camilla选择PDF表单并单击 **开始自动化转化** 来启动转换过程。 您可能需要单击 **覆盖转化** 表单已转换。
   ...
   * **可选**:如果您希望使用无障碍超海洋主题，只需单击指定自适应表单主题并选择选项列表中显示的无障碍超海洋主题。
   ...
   完成百分比状态会在转化期间显示。 显示状态后 **已转换**，请单击 **输出** 文件夹中，选择自适应表单并单击 **编辑** 打开转换后的表单。
...
1. 卡米拉会在表单上打开属性，该表单已在 [automated forms conversion- We.Gov](#automated-forms-conversion-wegov).
...
1. 卡米拉单击 **输出** 文件夹，选择自适应表单并单击 **编辑** 以打开已填写的We.Gov表单。
1. Camila选择自适应表单字段并单击 ![“配置”图标](assets/configure-icon.svg). 她使用 **绑定引用** 字段。 她对自适应表单中的所有字段重复此步骤。

### 表单无障碍测试(Camila) {#form-accessibility-testing}

Camila还将验证创建的内容是否按照公司标准正确构建且完全可访问。

1. 卡米拉单击 **输出** 文件夹，选择自适应表单并单击 **预览** 以打开已填写的We.Gov表单。
...
**此部分：** 回到家后，Aya找到了她需要的信息，并从她的桌面上恢复了该应用程序。 Aya导航到在线表单门户以恢复其应用程序。 借助一些简单的自定义功能，代理机构还可以自动生成链接并通过电子邮件发送以恢复应用程序。
...
1. 单击右侧的Circle CTA以移到下一部分。
   ...
   表单填充到Aya最后一个条目的点。 Aya已输入了她的所有信息并准备提交。
...
**此部分：** 乔治是政府机构的商业分析师Aya是请求提供服务的。 George有一个仪表板，他可以在仪表板中看到分配给他审阅的所有服务请求应用程序。
...
1. 导航到 *https://&lt;aemserver>:&lt;port>/aem/start.html*
1. 单击用户图标（右上角），然后使用“**注销**“ ”或“**模拟为**“ ”菜单选项。
...
**此部分：** Camila the CX Lead与Aya打了一个欢迎电话，解释如何利用她获得批准的政府服务。
...
**此部分：** Aya收到一封电子邮件，其中包含一个指向交互式通信的链接，该链接总结了Aya的优势，还包括要填写的表单字段。 附加了PDF优惠声明，并链接到邮件中的交互式通信信件（与交互式通信具有相同的主题/品牌）。
...
1. 滚动到PDF页面底部的附件。
1. 单击以打开PDF附件。
1. 在电子邮件客户端中向上滚动，然后单击“**联机查看欢迎工具包**&quot;
...
1. 要直接快速引用IC，请执行以下操作：
   ...
   *https://&lt;aemserver>:&lt;port>/content/dam/formsanddocuments/adobe-gov-forms/welcome-handbook/we-gov-welcome-handbok/jcr:content?channel=web&amp;mode=preview&amp;wcmmode=disabled*

   ![欢迎福利手册](/help/forms/using/assets/welcome_benefits_handbook.png) ![交互式通信链接](/help/forms/using/assets/interactive_communication.png)

## 续约提醒公民(Aya) {#renewal-reminder-citizen-aya}

**此部分：** 卡米拉还安排了一次沟通提醒，这样一年以后。 （可自动执行和电子邮件的工作流步骤）。
...
1. 用户体验完成并完成用户提交后，Camila将导航到AEM Forms中的数据源选项卡(**Forms** > **数据集成**)

1. 然后，卡米拉选择AEM Forms **We.gov FDM** 然后编辑 **We.gov注册FDM**.

1. 然后，卡米拉选择 **联系人** > **读取服务** 要测试。
...
**此部分：** 卡米拉导航到仪表板，在仪表板中，她可以看到机构KPI的各个部分，如开始填写并放弃服务请求表的公民百分比、从请求提交到批准/拒绝响应的平均时间，以及她发送给公民的福利手册的参与度统计。
...
1. 选择一个网站页面（例如主页），然后选择“**Analytics和Recommendations**&quot;
...
1. 返回页面查看页面（在步骤3中访问）后，您还可以通过更改显示设置以查看“**列表视图**&quot;
1. 找到“**查看**&quot;下拉菜单，选择&quot;**列表视图**&quot;

   ![“视图”下拉菜单中的列表视图](/help/forms/using/assets/list_view_view_dropdown.jpg)

1. 从同一菜单中，选择“**视图设置**，然后从“**Analytics**“ ”部分。
...
### Adobe AnalyticsForms报道（卡米拉） {#camila-reviews-forms-reporting-we-gov-adobe-analytics}

1. 导航至

   *https://&lt;aemserver>:&lt;port>/aem/forms.html/content/dam/formsanddocuments/adobe-gov-forms*

1. 选择“**健康福利的注册申请**“自适应表单”并选择“**Analytics报表**“ ”选项。
