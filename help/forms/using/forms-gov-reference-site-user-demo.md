...
### 虚拟用户和用户组{#fictitious-users-and-groups}
...
1. **Forms门**: *https://&lt;aemserver>:&lt;port>/content/we-gov/formsportal.html*
...
1. **Adobe收件箱**:位于AEM后端上 [的](assets/bell.svg) 顶部菜单栏Bell。
...
## automated forms conversion语（卡米拉）{#automated-forms-conversion}

**本节**:CX潜在客户已有一个基于PDF的表单，该表单用作基于纸张的流程的一部分。作为现代化努力的一部分，她希望使用此PDF表单自动创建新的现代自适应Forms。
...
1. 登录方式：
   * **用户**:卡米拉·桑托斯
   * **密码**:口令
1. 从主页选择Forms>Forms和文档>AEM Forms州政府Forms> AFC。
1. 卡米拉将PDF上载到AEM Forms。

   ![上传表单](assets/aftia-upload-form.jpg)

1. 然后，Camilla选择PDF表单并单击&#x200B;**开始自动转换**&#x200B;以开始转换过程。 如果已转换表单，则可能需要单击&#x200B;**覆盖转换**。
   ...
   * **可选**:如果您希望使用“可访问的Ultramarine”主题，只需单击“指定自适应表单主题”，然后选择选项列表中显示的“可访问的Ultramarine”主题。
   ...
   转换过程中将显示完成百分比状态。 状态显示&#x200B;**Converted**&#x200B;后，单击&#x200B;**output**&#x200B;文件夹，选择自适应表单，然后单击&#x200B;**Edit**&#x200B;打开转换的表单。

1. 然后，Camilla会查看表单并确保所有字段都存在

   ![查看转化](assets/aftia-review-conversion.jpg)

1. 然后，卡米拉开始编辑表单。 她选择“根面板”>“编辑”（扳手）>从“面板布局”下拉菜单中选择“顶部的选项卡”>选中“复选框”。
...
1. Camila以在[Automated forms conversion- We.Gov](#automated-forms-conversion-wegov)中转换的形式打开“属性”。
...
1. 卡米拉单击&#x200B;**output**&#x200B;文件夹，选择自适应表单，然后单击&#x200B;**编辑**&#x200B;以打开完成的We.Gov表单。
1. Camila选择一个自适应表单字段并单击![配置图标](assets/configure-icon.svg)。 她使用&#x200B;**Bind Reference**&#x200B;字段创建与表单数据模型实体的绑定。 她在自适应表单中对所有字段重复此步骤。

### 表单辅助功能测试(Camila){#form-accessibility-testing}

Camila还验证创建的内容是否根据公司标准构建正确且完全可访问。

1. 卡米拉单击&#x200B;**output**&#x200B;文件夹，选择自适应表单，然后单击&#x200B;**预览**&#x200B;以打开完成的We.Gov表单。
...
1. 使用以下&#x200B;**动态逻辑**&#x200B;来演示使用&#x200B;**系列状态**&#x200B;下拉菜单的动态功能：
...
**本节：回** 到家后，Aya已找到所需信息并从桌面恢复应用程序。Aya导航到在线表单门户以恢复其应用程序。 借助一些简单的自定义功能，机构还可以自动生成一个链接并通过电子邮件发送该链接以恢复应用程序。
...
1. 单击右侧的“圈子CTA”移至下一节。
   ...
   提交Aya后，将收到“感谢”页面。 （可选）她还会收到一封电子邮件，可以打开它，与Adobe Sign以电子方式签署记录文档。
...
1. 导航到&#x200B;*https://&lt;aemserver>:&lt;port>/aem/start.html*
1. 单击用户图标（右上角），并使用“**注销**”或“**模拟为**”菜单选项（如果您当前使用管理用户登录）。
...
1. 打开并检查只读自适应表单。
1. 单击“**打开MS Dynamics**”按钮，在新窗口中打开MS Dynamics记录。
...
**本节：** CX Lead与Aya建立欢迎电话联系，以说明如何利用她获得批准的政府服务。
...
**本节：Aya收** 到一封电子邮件，其中包含一个指向交互式通信的链接，该链接概括了Aya的优势，还包括要填写的表单字段。附加PDF优势声明并链接到邮件中的交互式通信信件（与交互式通信具有相同的主题／品牌）。
...
1. 要直接快速参考IC:
   ...
   *https://&lt;aemserver>:&lt;port>/content/dam/formsanddocuments/adobe-gov-forms/welcome-handbook/we-gov-welcome-handbook/jcr:content?渠道=web&amp;mode=预览&amp;wcmmode=disabled*

   ![欢迎优势手](/help/forms/using/assets/welcome_benefits_handbook.png) ![册交互式通信链接](/help/forms/using/assets/interactive_communication.png)

## 续约提醒市民(Aya){#renewal-reminder-citizen-aya}

**本节：卡** 米拉也计划了一年后的通信提醒。（自动／执行和通过电子邮件发送的工作流步骤）。
...
**本节**:卡米拉导航到AEM Forms数据集成，在那里她可以运行快速测试，看是否确实存在通过表单数据模型集成发送到外部数据源的信息。

### 表单数据模型(Camila){#form-data-model-camila}

**本节**:Camila导航到“数据源”页以验证服务器在Derby数据库中复制的数据。

1. 用户体验完成且用户提交完成后，Camila将导航到AEM Forms的“数据源”选项卡(**Forms** > **数据集成**)
...
1. 然后，Camila为测试服务提供联系人id，然后单击“测试”按钮。 例如，1或2，如果您提交了表单。 如果您尚未提交表单，则不返回任何数据。
...
## （可选）分析(Camila){#analytics-cx-lead-camila}

**本节：** Camila导航到一个仪表板，她可以在该中查看机构KPI，如开始填写服务请求表并放弃的公民百分比、从请求提交到批准／拒绝响应的平均时间以及她发送给公民的福利手册的参与统计。

### Adobe Analytics站报告（卡米拉）{#camila-reviews-sites-reporting-we-gov-adobe-analytics}

1. 导航到&#x200B;*https://&lt;aemserver>:&lt;port>/sites.html/content*
1. 选择“**AEM FormsWe.Gov Site**”以视图网页。
1. 选择其中一个网站页面（例如“主页”），然后选择“**分析和Recommendations**”。
...
1. 从同一菜单中，选择“**视图设置**”，并从“**分析**”部分选择要显示的列。
...
### Adobe AnalyticsForms报告（卡米拉）{#camila-reviews-forms-reporting-we-gov-adobe-analytics}

1. 导航至

   *https://&lt;aemserver>:&lt;port>/aem/forms.html/content/dam/formsanddocuments/adobe-gov-forms*

1. 选择“**健康福利登记申请**”自适应表单并选择“**分析报告**”选项。
