...
* 已监视文件夹具有各种属性，请参阅 [监视的文件夹属性](watched-folder-in-aem-forms.md#watchedfolderproperties).
...
1. 点按 **高级**. 高级选项卡包含更多字段。 这些字段中的大多数都包含默认值。
   ...
   * **包含模式为的文件：** 指定分号(;)分隔的模式列表，已监视文件夹使用这些模式确定要扫描和选取的文件夹和文件。 例如，如果输入“Include File Pattern（包含文件模式）”，则会选取所有与输入匹配的文件和文件夹。 有关更多信息，请参阅 [管理帮助](/help/forms/using/admin-help/configuring-watched-folder-endpoints.md)
