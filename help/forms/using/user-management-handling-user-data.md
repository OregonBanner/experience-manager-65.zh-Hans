...
用户管理将用户数据存储在数据库中，如My Sql、Oracle、MS SQL Server和IBM DB2。 此外，任何在AEM作者的Forms应用程序中至少登录过一次的用户， `https://'[server]:[port]'lc`，则会在AEM存储库中创建用户。 因此，用户管理存储在以下数据存储中：
...
对于在以下位置至少访问过Forms应用程序一次的用户，其用户管理数据 `https://'[server]:[port]'lc` 也存储在AEM存储库中。
