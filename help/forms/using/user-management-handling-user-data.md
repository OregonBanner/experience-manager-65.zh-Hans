...
用户管理将用户数据存储在数据库中，如My Sql 、 Oracle 、 MS SQL Server和IBM DB2。 此外，任何已在AEM作者的Forms应用程序中至少登录一次的用户(`https://'[server]:[port]'lc`)，都将在AEM存储库中创建该用户。 因此，用户管理存储在以下数据存储中：
...
对于在`https://'[server]:[port]'lc`下至少访问过Forms应用程序的用户，用户管理数据也存储在AEM存储库中。
