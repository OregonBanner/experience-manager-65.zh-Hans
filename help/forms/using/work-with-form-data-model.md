...
1. 点 ![按aem_6_3_edit](assets/aem_6_3_edit.png) ，获取读取服务参数，将该参数绑定到“用户配置文件属性”、“请求属性”或“文本”值 [](../../forms/using/work-with-form-data-model.md#main-pars-header-2140694395) ，然后指定绑定值。
1. 点 **[!UICONTROL 按完成]** ，保存参数， **[!UICONTROL 单击完成]** ，保存属性，然后 **[!UICONTROL 单击保存]** ，保存表单数据模型。

### 绑定读取服务参数 {#bindargument}

根据绑定值将读取服务参数绑定到用户配置文件属性、请求属性或文本值。 该值作为参数传递给服务以从数据源获取与指定值关联的详细信息。
...
从“ **[!UICONTROL 绑定到”下拉菜]** 单中选择 **[!UICONTROL “用户配置文件属性”，并在“绑定值”字段中输入]** 属性名称 **** 。 登录到AEM实例的用户的详细信息将基于属性名称从数据源检索。
...
该参 `id` 数使用用户配置文件 `empid` 的属性的值，并将它作为参数传递给读取服务。 它从员工数据模型对象中读取并返回与登录用户关 `empid` 联的相关属性值。
...
>[!NOTE]
>
>您为输入数据中的某个属性提供的值必须与为元数据文档中的属性定义的数据类型相匹配。\
