...
1. 在新创建的节点（此处为afTestRegistration）中指定以下属性值，然后单击“全 **部保存”**。 这些属性帮助霍布斯将文件夹识别为测试文件夹。 要将此客户端库作为其他客户端库中的依赖关系重复使用，请将其命名为granite.testing.calvin.tests。
...
1. 右键单击测试节点(此处为 **afTestRegistration)** ，然后单击“ **Create** > **Create File**”。 将文件命名为js.txt并单击“ **确定”**。
1. 在js.txt文件中，添加以下文本：
...
1. 右键单击测试节点(此处为 **afTestRegistration)** ，然后单击“ **创建** ” **>“**&#x200B;创建文件”。 命名文件init.js，然后单击“确 **定”**。
...
1. 单击 **创建** >创 **建节点** ，以在要测试的每个表单的clientlib文件夹下创建一个节点。 此示例使用名为 **testForm的节点** ，测试名为testForm的自适应 **表单**。 指定以下属性，然后单击“ **确定”**:
...
1. 右键单击您为测试表单创建的文件夹（此处为testForm），然后选择“创 **建** ”>“ **创建文件”**。 将文件命名为scriptingTest.js，并将以下代码添加到文件中，然后单击“全 **部保存”。**
...
测试AEM自适应表单的步骤与测试AEM UI的步骤类似。 有关测试自适应表单的详细信息，请参阅测试UI中 [的以下主题](https://helpx.adobe.com//experience-manager/6-3/help/sites-developing/hobbes.html):
