...
1. **本地化支持**:要本地化任何静态文本，请使 `${localize-YOUR_TEXT}` 用属性并使本地化的值可用（如果尚不存在）。
   *在讨论的示例中，属性和`${localize-Apply}`用于`${localize-Download}`本地化“应用”和“下载”文本。*
...
1. **使用配置属性**:Search &amp; Lister组件具有多个可在用户界面上使用的配置。 例如，要显示通过编辑对话框保存的HTML工具提示文本，请使用属 `${config-htmlLinkText}` 性。 **同样，对于PDF工具提示文本，请使用属性**`${config-pdfLinkText}` 。
