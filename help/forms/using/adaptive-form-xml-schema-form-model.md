---
title: 如何使用XML模式创建自适应Forms?
description: 了解如何在自适应表单中使用XML模式作为表单模型。 您可以应用现有的XSD模板来创建自适应表单，并将模式元素从XSD拖放到自适应表单上。 使用XML模式的示例深入挖掘，使用XML模式向字段添加特殊属性，并限制自适应表单组件的可接受值。
feature: 自适应表单
...
[!DNL Experience Manager Forms] 支持使用现有XML模式作为表单模型创建自适应表单。此XML模式表示组织中的后端系统生成或使用数据的结构。
...
在内容查找器中按住多次单击某个元素时，将显示一个弹出窗口，其中显示一个字段名和一个名为`bindRef`的属性。 此属性将树元素映射到模式中的元素或属性。
...
在可重复的子表单中，您必须使用“完成”子表单。 如果只希望有选择的字段，请使用整个结构并删除不需要的字段。
