...
1. 在apps文件夹中，创建一个名为`defaultApp`的文件夹，其路径/结构与defaultApp文件夹（位于配置文件夹中）类似。 请按照以下步骤创建文件夹：
   ...
   1. 右键单击以下路径中的&#x200B;**defaultApp**&#x200B;文件夹，然后选择&#x200B;**覆盖节点**:
...
1. 复制/apps分支下的acmExtensionsConfig.xml文件（位于/libs分支下）。
   ...
   1. 右键单击acmExtensionsConfig.xml文件，然后选择&#x200B;**Copy**。
...
1. 右键单击以下路径中的&#x200B;**locale**&#x200B;文件夹，然后选择&#x200B;**覆盖节点**:
...
1. 找到AdobeLiveCycle客户端SDK配置，然后单击&#x200B;**编辑**（编辑图标）。 此时会打开“配置”面板。

1. 输入以下详细信息，然后单击&#x200B;**Save**:
...
1. 在“服务名称”列表中，单击+图标，然后添加serviceName **SendLetterForReview/SendLetterForReviewProcess**。
