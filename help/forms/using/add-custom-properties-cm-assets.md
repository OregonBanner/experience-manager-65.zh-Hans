...
1. 在新创建的叠加节点(/apps/fd/cm/gui/content/cmmetadataproperties/commonproperties/col1/items)下，为需要为其创建下拉列表的每个属性（字段）创建一个节点(此处为 `geographicallocation`)类型。
   ...
   1. 将以下属性添加到新创建的节点（此处为项目1），然后单击 **全部保存**:
      ...
      <table>
         ...
         <tr>
          <td>text</td>
          <td>字符串</td>
          <td>这是用户可见的下拉选项的值。 将值留空（默认）或输入值，例如 <strong>国际</strong> 或 <strong>美国</strong>.<br /> </td>
         </tr>
         <tr>
          <td>选定</td>
          <td>字符串</td>
