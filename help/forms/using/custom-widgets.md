...
您可以将自定义构件插入到Mobile Forms。 您可以扩展现有的jQuery构件或使用外观框架开发自己的自定义构件。 XFA引擎使用各种构件，请参见 [自适应表单和HTML5表单的外观框架](/help/forms/using/introduction-widgets.md) 以了解详细信息。

![默认和自定义小部件的示例](assets/custom-widgets.jpg)
...
您可以创建配置文件或选择现有配置文件以添加自定义构件。 有关创建用户档案的更多信息，请参阅 [创建自定义配置文件](/help/forms/using/custom-profile.md).

### 创建构件 {#create-a-widget}

HTML5表单提供了一种构件框架的实现，该框架可以扩展以创建新构件。 实施是一个jQuery构件 *抽象构件* 可以扩展以编写新的构件。 只有通过扩展/覆盖以下提及的函数，才能使新构件正常工作。
...
<table>
 <tbody>
  ...
  <tr>
   <td>渲染</td>
   <td>渲染函数为小部件的默认HTML元素返回jQuery对象。 默认HTML元素应为可聚焦类型。 例如， &lt;a&gt;， &lt;input&gt;、和 &lt;li&gt;. 返回的元素用作$userControl。 如果$userControl指定上述约束，则AbstractWidget类的函数将按预期工作，否则一些常见的API（集中、单击）需要更改。 </td>
  </tr>
  <tr>
   <td>getEventMap</td>
  ...
  <tr>
   <td>getOptionsMap</td>
   <td>返回一个映射，该映射提供在选项更改时执行什么操作的详细信息。 键是提供给小部件的选项，值是每当检测到该选项中的更改时调用的函数。 该构件为所有常用选项（value和displayValue除外）提供处理程序</td>
  </tr>
  <tr>
   <td>getcommitvalue</td>
   <td>每当小部件的值保存在XFAModel中（例如，在textField的退出事件中）时，小部件框架就会加载函数。 该实施应返回保存在构件中的值。 为处理程序提供了选项的新值。</td>
  </tr>
  <tr>
   <td>show值</td>
...
要创建自己的构件，请在上面创建的配置文件中，包含对JavaScript文件的引用，该文件包含被覆盖的函数和新添加的函数。 例如， *sliderNumericFieldWidget* 是数字字段的构件。 要在用户档案的标题部分中使用构件，请包括以下行：
...
| 标识符类型 | 标识符 | 描述 |
|---|---|---|
| 名为fieldname的特定字段 | 标识符：&quot;div.fieldname&quot; | 所有名为“fieldname”的字段都使用构件渲染。 |
