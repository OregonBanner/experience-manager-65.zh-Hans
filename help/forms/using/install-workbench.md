...
本文档面向负责安装、配置、管理或部署Workbench的管理员或开发人员。 还包含有关配置系统以支持已升级的AEM Forms进程的信息。 提供的信息基于以下假设：阅读本文档的任何人都熟悉Microsoft® Windows®操作系统。
...
>[!NOTE]
>
>Flex Workspace已在AEM Forms中弃用。 它适用于AEM Forms版本。
...
* 硬件要求：Intel® Pentium® 4或AMD®等效处理器，1 GHz处理器。
...
请参阅Workbench支持的平台的完整列表() [AEM Forms支持的平台](https://www.adobe.com/go/learn_aemforms_supportedplatforms_65).

## Designer安装注意事项 {#designer-installation-considerations}

默认情况下，Workbench安装包含相应的仅英文版Designer。 如果Workbench安装应用程序在您的计算机上检测到Designer的现有版本，则安装可能会终止，您需要先删除Designer的当前版本，然后才能继续。
下表提供了安装Workbench时，您可能遇到的Designer安装方案以及必须执行的任何操作的完整列表。
...
Designer的不同版本可以共存于同一系统中，例如Workbench 6.4的Designer 6.4.x和Workbench 6.5的Designer 6.5.0.x。无需卸载随Acrobat 10 Pro或Acrobat 10 Pro Extended或更高版本一起安装的Designer版本。
<br /></p> </td>
  </tr>
  <tr>
   <td><p>设计人员（独立）</p> </td>
...
1. 执行以下任务之一：
   ...
   * 将Workbench下载并解压缩到您的文件系统。 下载后，导航到\workbench目录并双击run_windows_installer.bat文件。
   ...
   >[!NOTE]
   >
   >如果遇到“无法创建Java™虚拟机”错误，请创建名为_JAVA_OPTIONS（值为 — Xmx512M）的环境变量并运行安装程序。
...
要使用Workbench，您必须运行一个AEM Forms实例，通常在单独的计算机上运行。 您必须具有用户名和密码才能登录AEM Forms，以及有关服务器位置的详细信息。

>[!NOTE]
如果您将AEM Forms配置为使用EMC Documentum®或IBM® FileNet存储库提供程序，并且您希望登录到除在AEM Forms管理控制台中配置为默认存储库的存储库之外的其他存储库，请提供用户名为username@Repository。
...
要通过HTTPS将Workbench连接到AEM Forms服务器，您必须确保颁发公共密钥的证书颁发机构(CA)被Workbench识别为受信任。 如果证书未被识别为来自可信来源，则必须在 [Workbench_HOME]/workbench/jre/lib/security目录。
...
1. 当系统提示您信任证书时，请键入Yes并按Enter键。 键工具会继续将缓存文件导入 [Workbench_HOME]/workbench/jre/lib/security目录。
...
* **通过引用传递的模板**:使用内容根和表单名称。 如果使用此方法在每个请求中传递具有不同文件名的唯一模板，则磁盘缓存会无休止地增长，并且永远不会重复使用。 为防止出现这种情况，应使用相同的文件名传递唯一模板，以确保为所有请求更新相同的缓存。
* **传递值的模板**:使用使用theinDataDoc参数随数据一起传递的模板字节。 如果使用此方法传递具有不同UUID的唯一模板，则磁盘缓存会无休止地增长，并且永远不会重复使用。 为防止出现这种情况，应从所有模板中去除UUID属性，以确保不会为模板创建缓存。 或者，传递相同的非空UUID可以创建缓存对象，但可以确保使用每个请求更新相同的缓存。

为防止缓存无休止地增长，请考虑以下因素来使用新的AEM Forms API（renderHTMLForm2和renderPDFForm2的API）渲染动态生成的模板。
...
同一缓存仅用于具有相同内容根目录和模板文件名的请求。
以下最佳实践可确保在将动态生成的模板传递到Forms服务时，缓存不会无限增长：
