...
本文档面向负责安装、配置、管理或部署Workbench的管理员或开发人员。 此外，还包括配置系统以支持升级后的AEM Forms流程所需的信息。 所提供的信息基于以下假设：阅读本文档的任何人都熟悉Microsoft® Windows®操作系统。
...
>[!NOTE]
>
>AEM Forms已弃用Flex工作区。 它适用于AEM Forms版本。
...
>[!NOTE]
>
>AEM Forms服务器需要单独的安装程序。 有关更多信息，请参阅AEM Forms on JEE安装文档。
...
请参阅Workbench支持的平台的完整列表，网址为 [AEM Forms支持的平台](https://adobe.com/go/learn_aemforms_supportedplatforms_65).
...
下表包含您在安装Workbench时可能遇到的设计器安装场景的完整列表，以及必须执行的任何操作。
...
1. 执行以下任务之一：
   ...
   * 将Workbench下载并解压缩到您的文件系统。 下载后，导航到\workbench目录并双击run_windows_installer.bat文件。
   ...
   >[!NOTE]
   >
   >如果遇到错误“无法创建Java虚拟机”，请创建一个名为_JAVA_investigations的环境OPTIONS（值为 — Xmx512M），然后运行安装程序。
...
1. 接受列出的默认目录，或单击“选择”并导航到安装Workbench的目录，然后单击“下一步”。
...
>[!NOTE]
>
>如果您将AEM Forms配置为使用EMC Documentum或IBM FileNet存储库提供程序，并且希望登录到其他存储库(而不是在AEM Forms管理控制台中配置为默认的存储库)，请以username@Repository的形式提供用户名。
...
要通过HTTPS将Workbench连接到AEM Forms服务器，您必须确保将颁发公钥的证书颁发机构(CA)识别为Workbench受信任。 如果无法识别证书来自受信任的来源，则必须更新位于中的cacert文件 [Workbench_HOME]/workbench/jre/lib/security目录。
...
1. 当提示信任证书时，键入Yes并按Enter键。 keytool继续将cacerts文件导入到 [Workbench_HOME]/workbench/jre/lib/security目录。
...
* **通过引用传递的模板**：使用内容根和表单名称。 如果使用此方法在每个请求中传递具有不同文件名的唯一模板，则磁盘缓存将无限增加，并且永远不会被重复使用。 为防止出现这种情况，应使用相同的文件名传递唯一的模板，以确保为所有请求更新相同的缓存。
* **按值传递的模板**：使用通过inDataDoc参数与数据一起传递的模板字节。 如果使用此方法传递具有不同UUID的唯一模板，则磁盘缓存将无限增长，并且永远不会被重复使用。 要防止出现这种情况，应从所有模板中剥除UUID属性，以确保不为该模板创建缓存。 或者，传递相同的非null UUID可以创建缓存对象，但可以确保使用每个请求更新相同缓存。
...
**获取示例XDC文件**：
1. 在AEM Forms服务器上，找到中的XDC文件夹 [AEM Forms根目录]\sdk\samples\Output\IVS.
