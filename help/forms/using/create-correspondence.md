...
在通信 [管理中创建信函模板后](../../forms/using/create-letter.md)，最终用户／代理／声明调整器可以在“创建通信”用户界面中打开信函，并通过输入数据、设置内容和管理附件创建通信。 最后，理赔员或代理人可以在预览模式下管理内容并提交信件。
...
1. 对于基于预览的字母，选择“ **** ”> **预览**。 或者，对于非基于数据字典的字母，选择 **预览**。 您还可以将鼠标悬停在字母上（无需选择它），然后点按字母预览图标以预览它。
   ...
   >[!NOTE]
   >
   >如果数据字典与字母不关联，则字母预览打开。 否则，如果字母是基于预览字典的，则“对应管理”会在“预览”菜单中显示“”和“自定义”选项，您可以选择两个选项之一。 还可以将测试数据与数据字典相关联。 当数据 [字典具有关联的测试数据](../../forms/using/data-dictionary.md#p-working-with-test-data-p)，然后选择预览选项时，将打开普通预览并填充测试数据。
...
1. 如果已选择“ **预览** ” **>“自**&#x200B;定义”，则会打开对话框。 在对话框中，选择与数据字典对应的预览文件，将字母，然后选择 **预览**。 基于特定字母的数据字典创建数据文件。 有关数据文件的详细信息，请参 [阅数据字典](../../forms/using/data-dictionary.md#p-working-with-test-data-p)。
...
1. 根据需要，在字段中输入数据和内容变量。 填写标有星号(*)的所有必填字段，以启用“提 **交** ”按钮。
...
1. 根据需要，在“内容”选项卡中编辑内容模块。 要将焦点放在内容层次结构中的相关内容模块，您可以点按字母预览中的相关行或段落，或直接点按内容层次结构中的内容模块。
   ...
   有关“创建对应”用户界面中各个模块可用的操作的详细信息，请参 [阅“创建对应”用户界面中的操作和信息](#actions-and-info-available-in-the-create-correspondence-content-tab)。
...
1. 如果需要，在通信中插入特殊字符。 例如，您可以使用“特殊字符”调板插入：
   ...
   * 标点符号，‟如和
   ...
   Commendergence Management内置了210个特殊字符。 管理员可 [通过自定义添加对更多／自定义特殊字符的支持](../../forms/using/custom-special-characters.md)。
...
1. 要将信件另存为草稿（以后可以继续处理），请点按另存为草稿。 要使用此选项，您的信件需要 [发布](../../forms/using/publishing-unpublishing-forms.md#publishanasset)。 有关详细信息，请参阅保存草稿 [和提交信函实例下的草稿实例](#savingdrafts)。
...
1. 如果您认为拼合的PDF预览令人满意，请点 **按** “提交”以提交信函。 或者，要更改信件，请点 **击“退出** ”预览，返回信件的“创建对应UI”预览，以更改信件。 点按提交时，如果在发布实例上启用了管理信函实例配置，则会生成提交信函实例。
   ...
   有关HTML5表单与PDF forms之间差异的信息，请参 [阅HTML5表单与PDF forms之间的功能区别](../../forms/using/feature-differentiation-html5-forms-pdf-forms.md)。
...
* **草稿实例**:草稿实例捕获您正在预览的字母的当前状态。 要保存草稿实例，请首先确保字母引用的所有资产处于“已发布”状态。 有关发布信件的信息，请参阅 [发布资产](../../forms/using/publishing-unpublishing-forms.md#publishanasset)。 您需要先发布字母，然后才能将其保存为草稿，因为当您发布字母时，您会创建该字母的版本、其从属资产和数据。 信件的已发布版本不能由您或其他用户编辑，稍后可以恢复，而不会与已发布版本出现任何意外差异。 您以后可以返回到此实例，并从离开的位置继续。
