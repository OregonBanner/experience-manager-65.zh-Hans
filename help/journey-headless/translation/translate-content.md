...
1. 单击文件夹以打开该文件夹。
1. 在新项目文件夹中，单击 **创建** > **项目**.
1. 项目是基于模板的。单击 **翻译项目** 模板以选择它，然后单击 **下一个**.
...
1. 您会看到，该作业未包含任何内容。单击 **添加** 按钮，然后 **资产/页面** 从下拉菜单中查找。
...
您可以使用前面的步骤向项目的所有语言（作业）添加必要的内容。选择所有内容后，即可开始翻译。
...
[如前所述，](#using-translation-project) 机器翻译的内容流回AEM，状态为 **已批准** 因为假定使用的是机器翻译，所以不需要人工干预。 但仍可以查看已翻译的内容。
