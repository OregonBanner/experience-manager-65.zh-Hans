...
* 支持&#x200B;**优先实施和上下文实施：**
WCAG 2.1 准则被赋予优先权，其中涉及不遵循准则对特定残障用户群体可能产生的影响。这使得作者能够针对特定情况根据最重要的准则做出明智的决策。此外，还引入了“支持无障碍”**&#x200B;的概念。这可以让作者决定如何更好地利用那些可能没有全面的无障碍支持，或者可能要求用户具有特定的辅助型技术和/或浏览器的 Web 技术，以便从无障碍功能中受益。
*这些目标对 WCAG 2.1 的结构有显著的影响。*

[!NOTE]

>[!NOTE]创建出满足各种残障人士或各种类型人员的网站几乎是不可能的。WCAG 2.1 旨在帮助 Web 作者尽可能创建能够在特定条件和合理范围内能进行访问的站点。
>
>结构 {#structure}

## 在构建 WCAG 2.1 时引入了以逐步细化的方式引创建无障碍 Web 内容的概念。这可能给人们留下这样的印象，即 WCAG 2.1 是一组非常复杂的相互关联的文档，但其目标是，在作者需要时（逐步）提供更详细的信息，而不是在一个非常大的文档中提供所有信息。{#structure}
...
* 成功标准可能包括其中一项/或多种选项，也可能包括例外，即不符合成功标准的情况。`True``False`
* 成功标准按照父准则和原则编号，从 1.1.1 - 4.1.1。它们还有一个简短的名称，用于总结标准的目的，以便于参考。例如，成功标准 [1.1.1 是非文本内容[#$tu42]。
* 
* 支持资源 {#supporting-resources}**

## 除了原则、准则和成功标准的核心 WCAG 2.1 组成部分外，还有一系列支持文档。其中一些文档就如何符合准则的某些方面提供了具体建议，而另一些文档则是一般性参考资料，可帮助拥有各种能力的 Web 作者、设计人员和开发人员尽可能有效地理解和使用 WCAG 2.1。{#supporting-resources}

尽管 WCAG 2.1 本身是一份稳定的文档，不会发生更改，但大多数支持资源是动态文档；它们会随着新兴技术的出现而进行更改和增加，您可以从中发现一些关于如何实现 Web 无障碍的新示例。

WCAG 2.1 资源 {#wcag-resources}

### 本列表并非详尽内容，它介绍了一些可用资源：{#wcag-resources}

[所有 WCAG 相关文档的概要[#$tu50]
* 
* 
* 
* 
...
### 这些准则提供了有关 WCAG 2.1 新增内容的信息：{#what-is-new}

[WCAG 2.1 的新增内容[#$tu64]提供了有关 WCAG 2.0 与 WCAG 2.1 之间新增内容的重要信息。

* 

* 

### 用于 WCAG 2.1 的技术可在[用于 WCAG 2.1 的技术[#$tu69]页面上找到。
...
* 每种技术都有一个与其收藏集相关的唯一编号。例如，ARIA 技术之一是[技术 ARIA2：使用“aria-required”属性标识必填字段[#$tu76]。
...
### WCAG 2.1 是指一系列文档，它提供了一些建议可帮助读者理解具体准则和成功标准的目的。您可以[下载简介，以及指向更详细信息的链接[#$tu85]。
...
* 示例可在[了解成功标准 1.1.1（“非文本内容”）[#$tu97]中找到。



### “如何满足”部分显示在[如何满足 WCAG 2.1[#$tu100] 页面上。本节提供了 WCAG 的替代形式，允许读者根据与自身兴趣和/或情况最相关的准则优化内容。读者可以通过指定特定 Web 内容技术（如层叠样式表或脚本）或指定特定优先级来筛选他们希望查看的成功标准技术。
