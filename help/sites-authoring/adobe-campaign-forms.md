...
description: 通过Adobe Experience Manager，您可以创建和使用与您网站上的Adobe Campaign交互的表单
...
首先，检查创作实例和发布实例之间的连接，确保Adobe Campaign正常运行。 参见 [与Adobe Campaign Standard集成](/help/sites-administering/campaignstandard.md) 或 [与Adobe Campaign Classic集成](/help/sites-administering/campaignonpremise.md).
...
1. 在 **名称** 字段中，添加页面名称。 它必须是有效的JCR名称。
1. 在 **标题** 字段，输入标题并单击 **创建**.
1. 打开页面并选择 **打开属性** 和(在Cloud Services中)添加Adobe Campaign配置，然后选择复选标记以保存更改。

   ![chlimage_1-44](assets/chlimage_1-44a.png)

1. 在页面上，在 **表单开始** 组件中，选择表单的类型 —  **订阅，取消订阅，**&#x200B;或&#x200B;**保存配置文件**. 每个表单只能有一个类型。 您现在可以 [编辑表单内容](#editing-form-content).
...
此部分仅详细介绍指向Adobe Campaign的特定链接。 有关如何在Adobe Experience Manager中使用表单的更一般概述的更多信息，请参阅 [Editmode组件](/help/sites-authoring/default-components-foundation.md).

1. 选择 **打开属性** 和(在Cloud Services中)添加Adobe Campaign配置，然后选择复选标记以保存更改。
...
1. 单击 **高级** 选项卡并选择表单类型 —  **订阅，取消订阅，** 或 **保存配置文件** 并单击 **好的。** 每个表单只能有一个类型。
...
1. 您必须拥有 **加密的主密钥** 每个表单上的组件。 此组件定义哪个URL参数将用于接受Adobe Campaign配置文件的加密主密钥。 在组件中，选择Adobe Campaign ，以便仅显示这些组件。
1. 拖动组件 **加密的主密钥** 到表单（任何位置），然后单击或点按 **配置** 图标。 在 **Adobe Campaign** 选项卡，为URL参数指定任意名称。 单击或点按复选标记可保存更改。

   生成的指向此表单的链接需要使用此URL参数并将其分配给Adobe Campaign配置文件的加密主密钥。 加密的主密钥必须正确URL（百分比）编码。

   ![chlimage_1-47](assets/chlimage_1-47a.png)

1. 根据需要将组件添加到表单，例如文本字段、日期字段、复选框字段、选项字段等。 参见 [Adobe Campaign表单组件](/help/sites-authoring/adobe-campaign-components.md) 以了解有关每个组件的更多信息。
1. 单击配置图标以打开组件。 例如，在 **文本字段（营销活动）** 组件，更改标题和文本。
   ...
   >[!NOTE]
   >
   >您可以按照下面的说明添加/删除收件人表中显示的字段： [https://blogs.adobe.com/experiencedelivers/experience-management/aem-campaign-integration/](https://blogs.adobe.com/experiencedelivers/experience-management/aem-campaign-integration/)

1. 单击 **发布页面**. 页面已在您的网站上激活。 您可以通过转到AEM发布实例来查看它。 您还可以 [测试表单](#testing-a-form).

   >[!CAUTION]
   >
   >您需要向云服务上的匿名用户提供读取权限，才能在发布中使用表单。 但是，请注意向匿名用户提供读取权限的潜在安全问题，并确保通过配置Dispatcher等方法缓解此问题。
...
>
>要创建该链接，您需要使用变量 **主要资源标识符**(Adobe Campaign Standard)或 **加密标识符** (Adobe Campaign Classic)(例如，在 **文本和个性化（营销活动）** 组件)，用于链接到Adobe Campaign中的epk。
...
1. 要获取Adobe Campaign配置文件的加密主密钥(EPK)，请执行以下操作：
   ...
   * 在Adobe Campaign Classic中，转到 **配置文件和目标** >  **收件人**，其中列出了现有用户档案。 确保表显示 **加密标识符** 列中的字段(这可以通过右键单击条目并选择 **配置列表……**)。 复制所需配置文件的加密标识符。
