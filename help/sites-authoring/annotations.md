...
向网站页面添加内容时，通常会先进行讨论，然后再实际发布。 为帮助实现此目的，许多与内容直接相关的组件（例如，与布局相反）允许您添加注释。
...
>[!NOTE]
>
>经典UI中创建的注释将显示在触屏UI中。 但是，草图是UI专属的，并且仅在创建它们的UI中显示。
...
1. 编辑页面时，您可以使用工具栏（右上方）中的图标进入注释模式：
   ...
   >[!NOTE]
   >
   >要退出“注释”模式，请点按/单击顶部工具栏右侧的“注释”图标（x符号）。
...
1. 单击/点按所需组件（可添加注释的组件将以蓝色边框突出显示）以添加注释，并打开对话框：
...
![注释指示器](assets/chlimage_1-242.png)
