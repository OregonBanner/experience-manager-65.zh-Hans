...
* 创建页面的版本。
* 例如，将页面还原到以前的版本，以便撤消您对页面所做的更改。
* 将页面的当前版本与突出显示的文本和图像有差异的先前版本进行比较。
...
>1. 恢复第一个版本；即 1.0。
>1. 再次创建新版本。
>1. 生成的标签和节点名称现在将为1.0.0、1.0.1、1.0.2等。
...
1. 选择 **与当前比较**. 此 [页面差异](/help/sites-authoring/page-diff.md) 将打开并显示差异。
...
* 这意味着显示的版本已创建/激活 *早于* 在时间扭曲中选择的时间点。
...
时间扭曲是 [模式](/help/sites-authoring/author-environment-tools.md#page-modes) 页面编辑器的。 要启动它，只需像切换任何其他模式一样切换它即可。
...
1. 根据日期集显示页面。 时间扭曲模式通过窗口顶部的蓝色状态栏指示。 使用状态栏中的链接选择新的目标日期或退出时间扭曲模式。
