...
登录后，您将立即转到[“导航”面板](#navigation-panel)。选择其中一个选项将打开相应的控制台。
...
每当用户首次访问控制台时，就会启动产品导航教程。 请花上一分钟的时间，通过单击或点按通读该教程，以大致了解 AEM 的基本处理。
...
标题始终显示在屏幕顶部。 尽管无论您在系统中的什么位置，标题中的大多数选项都保持不变，但有些选项特定于上下文。
...
* 痕迹导航
   ...
   只需单击痕迹导航文本，即可显示一个下拉列表，其中列出了当前选定项目的层次结构级别。 单击某个条目以跳转到该位置。
...
* [视图](/help/sites-authoring/basic-handling.md#viewingandselectingyourresourcescardlistcolumn)
   ...
   视图图标位于AEM工具栏的最右侧。 由于它还指示了当前视图，因此它会发生更改。 例如，在默认视图&#x200B;**列视图**&#x200B;中显示：
...
>[!NOTE]
>
>与相应的控制台/功能/方案相关的各个可用图标都记录在案。

## 快速操作 {#quick-actions}

在[信息卡视图](#cardviewquickactions)中，某些操作以快速操作图标的形式呈现，同时也在工具栏中提供。快速操作图标一次只可用于单个项目，无需预先选择。
...
>[!NOTE]
>
>默认情况下，AEM Assets不会将UI中资源的原始演绎版显示为任何视图中的缩略图。 如果您是管理员，则可以使用叠加将AEM Assets配置为将原始演绎版显示为缩略图。
...
列视图允许通过一系列的层叠列，对内容树进行可视化导航。利用此视图，可可视化并遍历网站的树结构。

在最左侧的列中选择资源，会在右侧的列中显示子资源。 在右侧列中选择资源后，子资源将显示在右侧另一列中，依此类推。
...
* 信息卡视图显示各个项目在当前级别的信息信息卡。这些信息包括：
   ...
   * （在适当时）您何时需要在工作流中采取相应的操作.
      ...
      * 指示所需操作的标记可能与中的条目相关 [收件箱](/help/sites-authoring/inbox.md).

* [快速操作](#quick-actions) 也可用于此视图，例如选择和常见操作，例如编辑。
...
* 可使用位于“视图”按钮下方的&#x200B;**查看设置**&#x200B;选项选择要显示的列。以下列可供显示：
   ...
   * **页面逗留时间**
   ...
   默认将显示&#x200B;**名称**&#x200B;列，它构成了页面 URL 的一部分。在某些情况下，作者可能需要访问采用不同语言的页面，如果作者不知道页面的语言，查看页面的名称（通常不会更改）会很有帮助。
...
内容树可用于在侧面板中快速导航站点层次结构，并查看有关当前文件夹中页面的许多信息。
...
>[!NOTE]
>
>在层次结构视图中选择了一个条目后，可以使用箭头键快速浏览层次结构。
>
>请参阅 [键盘快捷键](/help/sites-authoring/keyboard-shortcuts.md) 了解更多信息。
...
**引用** 显示与选定资源之间的任何连接。 例如，在 **Sites** 控制台中，页面的[引用](/help/sites-authoring/author-environment-tools.md#showingpagereferences)会显示以下项目：
