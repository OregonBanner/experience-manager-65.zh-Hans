...
* [创建内容片段时，您还可以选择模板](#templates-content-fragments). 此模板定义结构、初始元素和变体。
...
>[!NOTE]
>
>将模板用于 [创建页面](/help/sites-authoring/managing-pages.md#creating-a-new-page)，则（对于页面作者）和使用的模板类型没有可见差异。
...
* 创建新页面后，页面和模板之间会保持动态连接。 此连接意味着对模板结构的更改会反映在使用该模板创建的任何页面上；对初始内容的更改不会反映出来。
...
>[!NOTE]
>
>参见 [使用可编辑的页面模板开发Experience Manager站点](https://experienceleague.adobe.com/docs/experience-manager-learn/sites/page-authoring/template-editor-feature-video-use.html?lang=en).
...
>
>一个额外的好处是 `cq:allowedTemplates` 作者可以在以下位置更新属性： **高级** 的选项卡 **页面属性**. 无法使用（标准）UI更新其他模板属性，因此需要开发人员维护规则并为每次更改部署代码。

在站点管理界面中创建页面时，可用模板的列表取决于新页面的位置和每个模板中指定的版面限制。

以下属性确定模板是否 `T` 用于将新页面作为页面的子页面放置 `P`. 以下每个属性都是一个多值字符串，其中包含零个或多个用于与路径匹配的正则表达式：
...
参见 [内容片段模板](/help/sites-developing/content-fragment-templates.md).
