...
* [SPA Blueprint](/help/sites-developing/spa-blueprint.md) 的技术要求
...
>[!CAUTION]
>
>将 `cq.authoring.pagemodel.messaging` 类别应限于SPA编辑器的上下文。

### 通信数据类型 {#communication-data-type}

当 `cq.authoring.pagemodel.messaging` 类别时，它会向页面编辑器发送消息，以建立JSON通信数据类型。 当通信GET类型设置为JSON时，数据请求将与组件的Sling模型端点通信。 在页面编辑器中发生更新后，更新组件的JSON表示形式将发送到页面模型库。 然后，页面模型库会通知SPA进行更新。
...
要使作者能够使用页面编辑器编辑SPA的内容，必须实施SPA应用程序才能与AEM SPA Editor SDK进行交互。 请参阅 [AEM SPA快速入门](/help/sites-developing/spa-getting-started-react.md) 文档，以便您运行。
...
可以实施其他SPA框架以与AEM SPA Editor SDK一起使用。 请参阅 [SPA Blueprint](/help/sites-developing/spa-blueprint.md) 记录框架必须满足的要求，以便创建由模块、组件和服务组成的特定于框架的层，以便与AEM SPA Editor一起使用。

### 使用多个选择器 {#multiple-selectors}

其他自定义选择器可以定义，并用作为为AEM SPA SDK开发的SPA的一部分。 但是，此支持要求 `model` 选择器是第一个选择器，扩展是 `.json` as [JSON导出程序所需的ID。](json-exporter-components.md#multiple-selectors)
...
有关 `editElementQuery` 属性和富文本编辑器的配置，请参阅 [配置富文本编辑器。](/help/sites-administering/rich-text-editor.md)
