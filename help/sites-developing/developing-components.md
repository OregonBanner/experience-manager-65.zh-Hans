...
>[!NOTE]
>
>组件也可以通过基于搜索路径逻辑的叠加进行重新定义。 然而，在此情况下， [Sling资源合并器](/help/sites-developing/sling-resource-merger.md) 未触发，并且 `/apps` 必须定义整个叠加。
...
[HTL Java Use-API让HTL文件可以访问自定义Java类中的helper方法](https://experienceleague.adobe.com/docs/experience-manager-htl/content/java-use-api.html?lang=en). 这使您可以使用Java代码实施用于选择和配置组件内容的逻辑。

### 使用JavaScript {#using-javascript}

[HTL JavaScript Use-API让HTL文件可以访问使用JavaScript编写的帮助程序代码](https://experienceleague.adobe.com/docs/experience-manager-htl/content/java-use-api.html?lang=en). 这允许您使用JavaScript代码实施用于选择和配置组件内容的逻辑。
...
此 [WCM模式](https://developer.adobe.com/experience-manager/reference-materials/6-5/javadoc/com/day/cq/wcm/api/WCMMode.html) 切换到 **预览** 模式（即使页面未刷新）。

对于渲染时对WCM模式敏感的组件，需要定义它们以专门刷新自身，然后依赖Cookie的值。

>[!NOTE]
>
>在启用了触屏操作的UI中，仅 `EDIT` 和 `PREVIEW` 用于 [WCM模式](https://developer.adobe.com/experience-manager/reference-materials/6-5/javadoc/com/day/cq/wcm/api/WCMMode.html) Cookie。
...
[Granite UI提供大量基本组件（构件）](https://developer.adobe.com/experience-manager/reference-materials/6-5/granite-ui/api/jcr_root/libs/granite/ui/index.html) 需要在创作环境中创建对话框。 如有必要，您可以扩展此选择并 [创建您自己的构件](#creatinganewwidget).
...
>请参阅：
>
>* 的AEM Gems讲座 [自定义对话框字段](https://experienceleague.adobe.com/docs/experience-manager-gems-events/gems/gems2015/aem-customizing-dialog-fields-in-touch-ui.html?lang=en).
...
更具体地说，Granite UI提供了一系列字段组件，这些组件适合用于对话框(或者，更一般地说，用于 [表单](https://developer.adobe.com/experience-manager/reference-materials/6-5/granite-ui/api/jcr_root/libs/granite/ui/components/foundation/form/index.html))。
...
要实现此目的，您需要了解要与之交互的底层构件库。 请参阅 [Coral UI文档](https://developer.adobe.com/experience-manager/reference-materials/6-5/coral-ui/coralui3/index.html) 以识别您要对哪个事件做出反应。 这非常类似于过去使用ExtJS执行的过程：查找给定小部件的文档页面，然后检查其事件API的详细信息。
...
通过使用，在Granite UI和Granite UI组件（等效于构件）中进行字段验证 `foundation-validation` API。 [请参阅 `foundation-valdiation` Granite文档以了解详细信息。](https://developer.adobe.com/experience-manager/reference-materials/6-5/granite-ui/api/jcr_root/libs/granite/ui/components/coral/foundation/clientlibs/foundation/js/validation/index.html)
...
* 对话框
   ...
   * 创建要在触屏UI中使用的对话框。 但是，出于兼容性目的，如果没有为触屏UI定义对话框，则触屏UI可以使用经典UI对话框的定义。
   ...
   * 自定义字段，有关更多信息，请参阅上的AEM Gems会话 [自定义对话框字段](https://experienceleague.adobe.com/docs/experience-manager-gems-events/gems/gems2015/aem-customizing-dialog-fields-in-touch-ui.html?lang=en).
...
但是，如果您的项目必须在迁移期间（通常情况下）同时满足经典UI和触屏UI的要求，则必须实施开关以区分引用相应对象的单独代码。
