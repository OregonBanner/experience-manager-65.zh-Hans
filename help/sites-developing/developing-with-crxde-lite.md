...
1. 安装AEM。
1. 在浏览器中，输入`https://<host>:<port>/crx/de`。 默认情况下为`https://localhost:4502/crx/de`。
...
仅当安装了CQ5时（即，如果存储库中有节点类型`cq:Component`），此处描述的功能才可用。
...
通过CRXDE Lite，您可以在[CND（紧凑命名空间和节点类型定义）符号](https://jackrabbit.apache.org/jcr/node-type-notation.html)中导入和/或导出节点类型定义。
