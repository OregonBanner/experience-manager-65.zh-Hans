...
请注意，您管理的翻译工具及词典用于以不同语言呈现组件UI。 如果要翻译页面或用户生成的内容，请参阅 [翻译多语言站点的内容](/help/sites-administering/translation.md) 和 [用户生成内容的翻译](/help/communities/translate-ugc.md).

>[!CAUTION]
>
>仅编辑为项目创建并位于下的词典 `/apps`.
>
>此工具中还提供了AEM系统词典。 请勿更改AEM系统词典，因为这可能会导致AEM UI出现问题。 此外，升级时更改可能会丢失。 AEM系统词典位于 `/libs`.

>[!NOTE]
>
>尽管翻译工具具有经典的UI界面，但它用于翻译短语，无论这些短语位于哪个界面。

翻译人员列出了AEM中使用的文本，以及各种语言翻译：

![chlimage_1-205](assets/chlimage_1-205.png)

您可以搜索、筛选和编辑英语和已翻译文本。 您还可以将词典导出为XLIFF格式进行翻译，然后将翻译导入回词典。
...
1. 使用CRXDE Lite添加根节点( `sling:Folder`)作为保存语言定义的结构：
...
>仅编辑为项目创建并位于下的词典 `/apps`.
>
>请勿更改AEM系统词典，因为这可能会导致AEM UI出现问题。 此外，升级时更改可能会丢失。 AEM系统词典位于 `/libs`.

### 添加、更改和删除字符串 {#adding-changing-and-removing-strings}

将英语字符串添加到组件已国际化的词典中。 仅添加已国际化的字符串，以便您不会通过翻译未使用的字符串而浪费资源。

您添加到词典的字符串必须与代码中指定的字符串完全匹配。 如果代码中使用的默认英文字符串与词典中的英文字符串不匹配，则在需要时已翻译的字符串不会出现在UI中。 字符串区分大小写。

**提供翻译提示**

使用词典字符串的Commenet属性向翻译人员提供信息以阐明字符串的含义。 通常，UI会帮助用户确定模糊字词的含义。 但是，翻译人员不会在UI的上下文中看到字符串。 翻译提示可消除歧义。 例如，注释可帮助翻译人员了解英语单词Request用作名词而不是动词。
...
要在本地化字符串中包含变量，请将带括号的索引放置在get方法的第一个参数中的变量位置。 使用本地化提示描述这些值。 翻译人员必须理解变量的含义，因为不同的语言使用不同的句子结构。
...
在代码中更改或删除英文字符串时，更改或删除英文字符串。 当您更改字符串时，原始字符串将保留，并且会生成一个反映更改的新字符串。 在删除字符串之前，请确保没有代码使用它。
...
1. 在“添加字符串”对话框的“字符串”框中，键入英文字符串。 在“注释”框中，根据需要键入翻译员的翻译提示。
1. 单击确定。
1. 单击“保存”。
...
* **按文本筛选：** 匹配英语字符串、评论或翻译的模式。 只有与全部或部分阵列匹配的项目才会显示在表中。
...
将英语字符串添加到字典后，可以添加该字符串的翻译。 您还可以 [导出词典](/help/sites-developing/i18n-translator.md#exporting-a-dictionary) 由第三方翻译。
...
1. 要编辑其中一个字符串的翻译文本，您可以：
   ...
   * 双击 **字符串** 或 **注释** 必填字符串的字段，以打开 **编辑字符串** 对话框，根据需要编辑翻译，然后单击 **确定** 要关闭对话框，请执行以下操作：
...
导出XLIFF文件并包含语言时，存储库中词典的节点结构必须包含该语言。 如果未包含语言，则会发生错误。 例如，要导出法语XLIFF文件，词典文件夹必须包含 `mix:language` 子节点已命名 `fr`. (请参阅 [创建词典](/help/sites-developing/i18n-translator.md#creating-a-dictionary).)
...
1. 单击 **全部保存** CRXDE Lite并重新加载转换器。 网格将更新以显示定义的语言。
   ...
   >[!NOTE]
   >
   >翻译人员将只保存实际翻译的语言 [在词典中存在](#creating-a-dictionary) (即，在词典路径下，例如 `/apps/myProject/i18n`)。
   >
   >确保这些语言与网格中显示的语言对应。

### 使语言对作者可用 {#making-languages-available-to-authors}

在为AEM实例的新语言定义词典后，您需要使其可供作者选择(例如，用于 **首选项**)：

1. 要更改中可用的语言列表，请 **首选项** 的 **安全性** 控制台：
...
1. 使语言可用于 **首选项** 从 **网站** 控制台。您需要在应用程序中进行以下更改：
...
>[!NOTE]
>
>对于由上述翻译员管理的本地化，只有确切的语言才有效。 如果语言首选项下拉列表使用 `en_uk`，必须存在 `en_uk` 词典中的词典。
...
1. 语言列表存储在以下位置：
   ...
   然后在那里更改或扩展列表。 属性 `defaultCountry` 在语言节点上(例如， `ja`)必须包含完整代码，例如 `ja_jp`，这将定义 `jp` 作为语言的默认国家/地区 `ja`.
...
将您的词典纳入AEM应用程序的发行管理流程。 例如，将字典包含在应用程序的内容包中，以部署到发布实例。 此策略具有以下优势：

* 字典可用于其发布环境中的组件。
* 对组件UI字符串所做的更改将随更新的翻译一起部署。

同样，字典字符串的测试应作为正常软件开发生命周期的一部分来执行。

>[!NOTE]
>
>常规发布功能或复制不应用于词典。 相反，应按与代码和配置相同的方式处理词典。 这包括使用源代码管理来跟踪更改，以及使用内容包将更改应用于创作和发布。
