...
* 实施 [ `com.day.cq.wcm.msm.api.LiveAction`](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/msm/api/LiveAction.html) 执行操作的界面。
* 实现的OSGI组件 [ `com.day.cq.wcm.msm.api.LiveActionFactory`](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/msm/api/LiveActionFactory.html) 界面并创建实例 `LiveAction` 类。
...
的参数 ` [LiveActionFactory](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/msm/api/LiveActionFactory.html).createAction` 方法是 `Resource` 对象。 此 `Resource` 对象表示 `cq:LiveSyncAction` 转出配置中此实时操作的节点；请参阅 [创建转出配置](/help/sites-administering/msm-sync.md#creating-a-rollout-configuration). 与往常一样，在使用配置节点时，您应该将其调整为 `ValueMap` 对象：
...
* A [ `Resource`](https://helpx.adobe.com/cn/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/org/apache/sling/api/resource/Resource.html) 表示Live Copy源的对象。
* A `Resource` 表示Live Copy目标的对象。
* 此 [ `LiveRelationship`](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/msm/api/LiveRelationship.html) Live Copy的对象。
...
>[!NOTE]
>
>此 `Resource` 参数可以是 `null` 或 `Resources` 不适应的对象 `Node` 对象，例如 [ `NonExistingResource`](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/org/apache/sling/api/resource/NonExistingResource.html) 对象。
...
1. 重复上一步骤以创建 `/apps/wcm/core` 文件夹树。 创建节点类型 `sling:Folder` 在 `core` 已调用 `resources`. <!-- ![chlimage_1-77](assets/chlimage_1-77.png) -->
