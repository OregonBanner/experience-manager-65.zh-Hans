...
此 `Page` 组件扩展 `Container` 组件。 容器是一个组件，旨在包含和渲染子组件，包括子页面。 为此，容器在 `:itemsOrder`， `:items`、和 `:children` 模型的属性。 此 `Page` 组件从的存储中动态获取子组件 [组件映射](/help/sites-developing/spa-blueprint.md#componentmapping) 库。 此 `Page` 负责实例化子组件。
...
在上述实施中，项目组件在中实际注册之前，通过空值功能进行了扩展 [组件映射](/help/sites-developing/spa-blueprint.md#componentmapping) 商店。 这是通过封装和扩展 [`ComponentMapping`](/help/sites-developing/spa-blueprint.md#componentmapping) 库以介绍 `EditConfig` 配置对象：
...
* 与可编辑资源对应的元素带有 `data-cq-data-path` 属性。 请参阅 [与页面编辑器的合同](#contract-wtih-the-page-editor) 章节。
...
请参阅文章 [SPA模型路由](/help/sites-developing/spa-routing.md) 了解更多信息。

## SPA的实际操作 {#spa-in-action}

继续阅读本文档，了解简单的SPA的工作原理，并自行试验SPA [AEM中的SPA快速入门](/help/sites-developing/spa-getting-started-react.md).
...
* [AEM中的SPA快速入门](/help/sites-developing/spa-getting-started-react.md) 有关简单SPA及其工作方式的指南
