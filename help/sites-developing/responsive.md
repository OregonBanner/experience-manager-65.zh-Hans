...
>[!NOTE]
>
>AdobeSPA建议对需要基于单页应用程序框架的客户端渲染(例如 _React_)。 [了解更多](/help/sites-developing/spa-overview.md)。

>[!NOTE]
>
>各种示例都基于Geometrixx示例内容，AEM不再附带该内容，已被We.Retail取代。 查看文档 [We.Retail参考实施](/help/sites-developing/we-retail.md#we-retail-geometrixx) 了解如何下载和安装Geometrixx。
...
We.Retail媒体示例使用此策略来定义网站设计中的样式。 We.Retail使用的CSS文件位于 `*/apps/weretail/clientlibs/clientlib-site/less/grid.less`.
...
在页面组件的JSP脚本中包含client library文件夹，以生成包含媒体查询的CSS文件并引用该文件。
...
查看不同视区大小的页面的预览，以测试响应式设计的行为。 In **[!UICONTROL 预览]** 模式， **[!UICONTROL Sidekick]** 包括 **[!UICONTROL 设备]** 用于选择设备的下拉菜单。 选择设备后，页面会根据视区大小进行相应更改。
...
实施 [MediaQueryList接口](https://dev.w3.org/csswg/cssom-view/#the-mediaquerylist-interface) 通过W3C定义的，您可以使用javascript评估媒体查询。 您可以将逻辑应用于媒体查询结果，并执行针对当前窗口的脚本：

* 实施MediaQueryList接口的浏览器支持 `window.matchMedia()` 函数。 此函数针对给定的字符串测试媒体查询。 此函数返回 `MediaQueryList` 提供对查询结果的访问权限的对象。

* 对于未实施接口的浏览器，您可以使用 `matchMedia()` 聚合填充，如 [matchmedia.js](https://github.com/paulirish/matchMedia.js)，这是一个免费提供的javascript库。

#### 选择特定于媒体的资源 {#selecting-media-specific-resources}

W3C建议的 [图片元素](https://picture.responsiveimages.org/) 使用媒体查询来确定用于图像元素的源。 图片元素使用元素属性将媒体查询与图像路径相关联。
...
* GeometrixxCommons软件包安装可更改图像分辨率的图像引用修改Servlet示例servlet。
...
此 `AbstractImageServlet` 类提供 `doGet` 处理HTTP请求的方法。 此方法确定与请求关联的资源，从存储库中检索资源属性，并将它们返回到 [Imagecontext](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/commons/AbstractImageServlet.ImageContext.html) 对象。

>[!NOTE]
>
>此 [com.day.cq.commons.DownloadResource](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/commons/DownloadResource.html) 类提供 `getFileReference method`，以检索资源的 `fileReference` 属性。
...
info.geometrixx.commons.impl.servlets.ImageReferenceModificationServlet类扩展 [AbstractImageServlet](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/commons/AbstractImageServlet.html) 类。 如果已安装cq-geometrixx-commons-pkg软件包，则ImageReferenceModificationServlet源代码位于 `/apps/geometrixx-commons/src/core/src/main/java/info/geometrixx/commons/impl/servlets` 文件夹。
...
此 `AbstractImageServlet` 类提供 `doGet` 处理HTTP请求的方法。 此方法确定与调用关联的资源，从存储库中检索资源属性，并将它们保存在 [Imagecontext](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/commons/AbstractImageServlet.ImageContext.html) 对象。

此 `ImageReferenceModificationServlet` 类覆盖 `createLayer` 方法和实现了确定要渲染的图像资源的逻辑。 方法检索页面的子节点 `jcr:content` 节点已命名 `image`. An [图像](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/foundation/Image.html) 对象是从此处创建的 `image` 节点，以及 `getFileReference` 方法返回图像文件的路径，从 `fileReference` 图像节点的属性。

>[!NOTE]
>此 [com.day.cq.commons.DownloadResource](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/commons/DownloadResource.html) 类提供getFileReferencemethod。

## 开发流体网格 {#developing-a-fluid-grid}

AEM使您能够高效实施流式网格。 本页说明如何集成流动网格或现有网格实施(例如 [Bootstrap](https://twitter.github.com/bootstrap/))添加到您的AEM应用程序。
...
* 此 `.row-fluid .span*` 样式将元素宽度定义为父级的百分比。 百分比由绝对宽度计算。
* 较大视区的媒体查询分配较大的绝对宽度。

>[!NOTE]
>
>Geometrixx Media示例集成了 [Bootstrap](https://twitter.github.com/bootstrap/javascript.html) 将javascript框架融入到其流式网格中实现。 Bootstrap框架提供bootstrap.css文件。
...
将组件模块化，以便有效地使用代码。 您的网站可能会使用多种不同类型的页面，例如欢迎页面、文章页面或产品页面。 每种类型的页面包含不同类型的内容，并且可能使用不同的布局。 但是，当每个布局的某些元素在多个页面中很共有时，您可以重复使用实施该部分布局的代码。
...
作者应该意识到，内容块可以按各种大小和相对位置渲染。 内容文本不应使用相对方向来引用其他内容块。

如有必要，组件应提供其生成的HTML代码所需的任何CSS或JavaScript库。 使用组件内的客户端库文件夹生成CSS和JS文件。 要公开这些文件， [创建依赖项或嵌入库](/help/sites-developing/clientlibs.md#creating-client-library-folders) /etc文件夹下的另一个客户端库文件夹中。

**子网格**

如果组件包含多个内容块，请在行中添加内容块以在页面上建立子网格：

* 使用与包含页面组件相同的类名将div元素表示为行和内容块。
...
* 当这些div元素中的一个或多个共享公共父级div元素时，它们构成一行。
...
使用网格来缩放内容块，以适应不同的视区大小。 内容块跨越特定数量的列。 随着列宽度的增加或减少以适合不同的视区大小，内容块的宽度也会相应地增加或减少。 缩放功能可以支持宽到足以容纳并排放置内容块的大中型视区。
...
确定在页面上放置内容块所需的列和行。 页面布局决定了网格中跨越的列数和行数。

**列数**

包含足够的列，以便在所有版面中水平放置内容块（对于所有视区大小）。 您使用的列数应多于当前需要的列数，以适应未来的页面设计。
...
创建CSS类和样式以控制页面上内容块的布局。 页面设计通常基于视区中内容块的相对大小和位置。 视区确定内容块的实际大小。 CSS需要考虑相对大小和绝对大小。 您可以使用三种类型的CSS类实施流式网格：
...
1. 定义内容块元素的类名。 所有可能的宽度（以列跨度为单位）都需要一个类。 例如，使用 `span3` 类 `div` 跨越3列的元素，使用 `span4` 4列范围的类。 定义网格中列数的类数。
...
使用浮动样式 `.row-fluid` 类来控制行中的内容块是水平排列还是垂直排列。
