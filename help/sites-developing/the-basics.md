...
description: 概述了AEM的结构方式以及如何在其上进行开发的核心概念，包括了解JCR、Sling、OSGi、Dispatcher、工作流和MSM
...
Java™内容存储库(JCR)标准， [JSR 283](https://developer.adobe.com/experience-manager/reference-materials/spec/jcr/2.0/index.html)，指定在内容存储库的粒度级别上双向访问内容的与供应商无关和与实施无关的方法。

Adobe研究公司（瑞士）AG持有规范领先。

的 [JCR API 2.0](https://developer.adobe.com/experience-manager/reference-materials/spec/javax.jcr/javadocs/jcr-2.0/index.html) 包， javax.jcr。&amp;ast;用于直接访问和处理存储库内容。

## Experience Server(CRX)和Jackrabbit {#experience-server-crx-and-jackrabbit}

Experience Server提供AEM构建的Experience Services，并可用于构建自定义应用程序，它还基于Jackrabbit嵌入内容存储库。

[Apache Jackrabbit](https://jackrabbit.apache.org/jcr/index.html) 是JCR API 2.0的开放源代码，完全符合。

## Sling请求处理 {#sling-request-processing}

### Sling简介 {#introduction-to-sling}

AEM使用 [Sling](https://sling.apache.org/index.html)，基于REST原则的Web应用程序框架，可轻松开发面向内容的应用程序。 Sling使用JCR存储库（如Apache Jackrabbit）或CRX内容存储库(如AEM)作为其数据存储。 Sling已对Apache Software Foundation做出了贡献 — 有关更多信息，请参阅Apache 。
...
* 第一个目标是保存内容的资源（JCR节点）
* 其次，表示或脚本来自与请求的某些部分（例如，选择器和/或扩展）组合的资源属性
...
* 当需要方法(GET、POST)时，它将根据HTTP规范(例如jobs.POST.esp)以大写形式指定（请参阅下文）
* 支持各种脚本引擎：
...
此外，Apache Sling支持与其他常用脚本引擎（例如Groovy、JRuby、Freemarker）的集成，并提供了一种集成新脚本引擎的方法。
...
此架构允许您使用应用程序特定模块扩展Sling。 Sling，因此CQ5使用 [Apache Felix](https://felix.apache.org/documentation/index.html) 实施OSGi（开放服务网关计划），并基于OSGi服务平台版本4.2版规范。 它们都是在OSGi框架中运行的OSGi包集合。
...
有关处理项目对象的详细信息，请参阅 [Javaocs](https://developer.adobe.com/experience-manager/reference-materials/spec/javax.jcr/javadocs/jcr-2.0/javax/jcr/Item.html) 接口javax.jcr.Item的
...
有关操作节点对象的详细信息，请参阅 [Javaocs](https://developer.adobe.com/experience-manager/reference-materials/spec/javax.jcr/javadocs/jcr-2.0/javax/jcr/Node.html).
...
将currentPage作为当前页面对象。 有关处理页面对象的详细信息，请参阅 [Javaocs](https://developer.adobe.com/experience-manager/reference-materials/6-5/javadoc/com/day/cq/wcm/api/Page.html).
...
将pageManager作为页面管理器对象，将myResource作为资源对象。 有关页面管理器提供的方法的更多信息，请参阅 [Javaocs](https://developer.adobe.com/experience-manager/reference-materials/6-5/javadoc/com/day/cq/wcm/api/PageManager.html).
...
>[!CAUTION]
>
>请勿在 `/libs` 路径。 对于配置和其他更改，请从 `/libs` to `/apps` 在 `/apps`.
...
* `/libs`
   ...
   属于AEM核心的库和定义。 中的子文件夹 `/libs` 表示开箱即用的AEM功能，如搜索或复制。 中的内容 `/libs` 不应进行修改，因为它会影响AEM的工作方式。 您网站的特定功能应在 `/apps` (请参阅 [自定义组件和其他元素](/help/sites-developing/dev-guidelines-bestpractices.md#customizing-components-and-other-elements))。
...
例如，网站通常以多种语言为国际受众提供。 如果使用同一语言的网站数量较少（三到五个），则可以手动处理网站间的内容同步过程。 但是，当网站数量增加或涉及多种语言时，自动化该过程会变得更加高效。
