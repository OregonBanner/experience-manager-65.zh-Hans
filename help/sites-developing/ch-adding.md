...
* 首先，使用[地理位置](ch-samplestores.md#contexthub-geolocation-sample-store-candidate)存储确定用户的纬度。
* 然后，[surferinfo store](ch-samplestores.md#contexthub-surferinfo-sample-store-candidate)的月份数据项决定该纬度所在的季节。
...
ContextHub提供[诊断页面](ch-diagnostics.md)，您可在其中看到ContextHub框架的概述。
