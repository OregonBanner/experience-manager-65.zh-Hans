...
* 提供保留与从中创建的任何页面的动态连接的模板。 这可确保对模板所做的任何更改都会反映在页面本身中。
...
本文档假设您已经熟悉创建和编辑模板。 请参阅创作文档 [创建页面模板](/help/sites-authoring/templates.md)，其中详细说明了向模板作者公开的可编辑模板的功能。

>[!NOTE]
>
>以下教程可能也适合在新项目中设置可编辑页面模板：
>[AEM Sites快速入门第2部分 — 创建基础页面和模板](https://helpx.adobe.com/experience-manager/kt/sites/using/getting-started-wknd-tutorial-develop/part2.html)

## 创建新模板 {#creating-a-new-template}

创建可编辑模板主要是通过 [模板控制台和模板编辑器](/help/sites-authoring/templates.md) 模板作者。 本节概述了此过程，并在后面介绍了技术级别发生的情况。

有关如何在AEM项目中使用可编辑模板的信息，请参阅 [使用Lazybones创建AEM项目](https://helpx.adobe.com/experience-manager/using/aem_lazybones.html).
...
1. 选择 [模板类型](#template-type). 复制此选项是为了创建 [模板定义](#template-definitions).
   ...
   >[!NOTE]
   >
   >现成提供了一系列模板类型。 您还可以 [创建您自己的特定于站点的模板类型](/help/sites-developing/page-templates-editable.md#creating-template-types) 如果需要。
...
1. 配置新模板的结构、内容策略、初始内容和布局。
   ...
   * 不能在生成页面上移动在模板结构中定义的组件，也不能从任何生成页面中删除这些组件。
      ...
      * 如果您在We.Retail示例内容之外的自定义文件夹中创建模板，则可以选择Foundation Components或使用 [核心组件](https://helpx.adobe.com/experience-manager/core-components/using/developing.html).
   ...
   * 可以解锁组件，然后再将其锁定，以便定义初始内容。
   ...
   有关结构的技术详细信息，请参见 [结构](/help/sites-developing/page-templates-editable.md#structure) 在本文档中。
   ...
   * 模板的响应式布局与页面创作时的响应式布局功能相同。
   ...
   有关模板布局的技术详细信息，请参阅 [版面](/help/sites-developing/page-templates-editable.md#layout) 在本文档中。
...
>[!TIP]
>
>切勿在模板中输入任何需要国际化的信息。出于内部化目的， [核心组件的本地化功能](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/get-started/localization.html) 建议使用。
...
* **全局**
* 特定于站点：您为组织模板而创建的特定于站点的文件夹是使用具有管理员权限的帐户创建的。
...
>[!NOTE]
>
>最佳做法是创建一个新文件夹来存放自定义模板，而不是使用全局文件夹。
...
1. In *加法* 标准创作权限和特权(例如， `content-authors`)现在，您需要分配组并定义作者所需的访问权限(ACL)，以便能够在新文件夹中创建模板。

   此 `template-authors` group是需要分配的默认组。 请参阅以下部分 [ACL和组](/help/sites-developing/page-templates-editable.md#acls-and-groups) 了解详细信息。

   参见 [访问权限管理](/help/sites-administering/user-group-ac-admin.md#access-right-management) 有关管理和分配访问权限的完整详细信息。
...
>[!NOTE]
>
>在配置浏览器中，您可以编辑全局文件夹并激活 **可编辑的模板** 选项，但并非建议的最佳做法。
>
...
的模板文件夹 [We.Retail引用实施](/help/sites-developing/we-retail.md) 可用作示例。

#### 模板 — 作者组 {#the-template-authors-group}

此 `template-authors` group是用于管理对模板的访问权限的组，并且在AEM中是标准组，但为空。 必须将用户添加到项目/站点的组中。

>[!CAUTION]
>
>此 `template-authors` 组是 *仅限* 用户必须能够创建新模板。
>
...
模板不应再存储于 `/conf/global`但是，对于某些旧版安装，此位置可能仍存在模板。 仅在此类遗留情况下，应 `/conf/global` 明确配置路径。
...
* 模板类型允许您定义：
   ...
   * 建议为响应式网格定义断点，并在模板类型上设置移动模拟器。 这是可选的，因为还可以在单个模板上定义配置(请参阅 [模板类型和移动设备组](/help/sites-developing/page-templates-editable.md#p-template-type-and-mobile-device-groups-br-p))。

* AEM提供少量现成的模板类型，如“HTML5页”和“自适应表单页”。

   * 以下部分提供了其他示例 [We.Retail](/help/sites-developing/we-retail.md) 示例内容。
...
>[!CAUTION]
>
>您不得更改 `/libs` 路径。 这是因为 `/libs` 下次升级实例时将被覆盖（并且在应用修补程序或功能包时可能被覆盖）。
...
>[!NOTE]
>
>替换为 [静态模板](/help/sites-developing/page-templates-static.md)，的值 `cq:deviceGroups` 可以在站点的根目录中设置。
>
...
1. 像创建任何可编辑模板一样创建模板 [如此处记录的那样](/help/sites-authoring/templates.md#creating-a-new-template-template-author)，这将用作模板类型的基础。
...
可编辑模板的定义已存储 [用户定义的文件夹](/help/sites-developing/page-templates-editable.md#template-folders) （推荐）或另一种方法 `global`. 例如：
...
* 与结构合并( `/structure`)。
* 如果在创建后更改了初始内容，则任何现有页面都不会更新。
* 此 `root` node包含组件列表，用于定义生成的页面中可用的组件。
* 如果在结构模式下将内容添加到组件且随后解锁该组件（反之亦然），则此内容将用作初始内容。

### 布局 {#layout}

时间 [编辑模板您可以定义布局](/help/sites-authoring/templates.md)，此函数使用 [标准响应式布局](/help/sites-authoring/responsive-layout.md) 也可以 [已配置](/help/sites-administering/configuring-responsive-layout.md).

### 内容策略 {#content-policies}

内容（或设计）策略定义组件的设计属性，如组件的可用性或最小/最大维度。 这些属性适用于模板（和使用模板创建的页面）。可以在模板编辑器中创建和选择内容策略。
...
1. **允许的模板**
   ...
   * [在上定义允许的模板路径 **页面属性**](/help/sites-authoring/templates.md#allowing-a-template-author) 子分支的相应页面或根页面的属性。
   * 设置属性：
      `cq:allowedTemplates`
在 
`jcr:content` 所需分支的节点。
...
* 对模板和模板类型中包含的信息具有引用。 这可通过以下方式实现 `jcr:content` 节点具有以下属性：
...
* 模型 — OSGi捆绑包 [OSGi包](/help/sites-deploying/osgi-configuration-settings.md) 实施功能。

* 视图 —  `/apps/<my-site>/components`
在创作环境和发布环境中，内容渲染方式 [组件](/help/sites-developing/components.md).
