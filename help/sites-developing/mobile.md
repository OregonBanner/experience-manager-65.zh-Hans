...
创建移动站点与创建标准站点类似，因为它还包括创建模板和组件。 有关创建模板和组件的更多详细信息，请参阅以下页面： [模板](/help/sites-developing/templates.md)， [组件](/help/sites-developing/components.md) 和 [AEM Sites开发入门](/help/sites-developing/getting-started.md). 主要区别在于在站点内启用AEM内置移动功能。 这是通过创建依赖于移动页面组件的模板来实现的。

您还应考虑使用 [响应式设计](/help/sites-developing/responsive.md)，创建适合多种屏幕大小的单个站点。
...
使用多站点管理器(MSM)从标准站点创建Mobile Live Copy。 标准站点自动转换为移动站点：移动站点具有移动站点的所有功能（例如，在模拟器中编辑），可以与标准站点同步管理。 请参阅一节 [为不同渠道创建Live Copy](/help/sites-administering/msm.md) 在“多站点管理器”页面中。
...
* [com.day.cq.wcm.mobile.api.wurfl](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/workflow/api/package-summary.html)  — 定义WurlQueryEngine。
* [com.day.cq.wcm.mobile.core](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/mobile/core/package-summary.html)  — 定义MobileUtil，它提供围绕WCM Mobile的各种实用工具方法。
...
移动设备模拟器基于通用模拟器框架。 欲知更多详情，请参阅 [模拟器](/help/sites-developing/emulators.md) 页面。

设备模拟器会在页面上显示移动设备，而通常的编辑（parsys、组件）会在设备的屏幕内进行。 设备模拟器取决于为站点配置的设备组。 可以为一个设备组分配多个模拟器。 然后，所有模拟器都可在内容页面上使用。 默认情况下，将显示分配给分配给站点的第一个设备组的第一个模拟器。 可通过页面顶部的模拟器轮播或Sidekick的编辑按钮切换模拟器。

**创建模拟器**

要创建模拟器，请参阅 [创建自定义移动模拟器](/help/sites-developing/emulators.md) 部分。
...
* **智能** 手机，适用于黑莓等设备，支持基本HTML和图像，但不支持JavaScript。
...
1. 在浏览器中，转到 **工具** 控制台。
1. 在下方创建新页面 **工具** > **移动设备** > **设备组**. 在 **创建页面** 对话框：

   * 作为 **标题** 进入 `Special Phones`.
...
如前所述，可以将自定义CSS与设备组页面关联，这很像设计页面的CSS。此CSS用于影响设备组在创作和发布时特定于该页面内容的渲染。然后自动包含此CSS：
...
与AEM一起安装的截断的WURFL™数据库是早于2011年8月30日的版本。 如果您的WURFL版本在2011年8月30日后发布，请确保您的使用符合您的许可证。
...
1. In **CRXDE Lite**，在/apps/wcm/mobile/devicespecs/regexp下创建一个节点，例如apple_ipad_ver1。
1. 将以下属性添加到节点：
...
`BrowserMap` 为您提供了三种策略，您可以使用它们为客户提供替代网站，这些策略按以下顺序使用：
...
此 `PageVariantsProvider` OSGi服务能够为属于同一系列的网站生成备用链接。 为了配置要由服务考虑的站点， `cq:siteVariant` 节点必须添加到 `jcr:content` 从站点的根目录创建节点。

此 `cq:siteVariant` 节点需要具有以下属性：
...
如果您不想使用备用链接，则可以为每个链接配置全局URL `DeviceGroup`. 我们建议创建您自己的嵌入了 `browsermap.standard` 客户端库，但重新定义了设备组。

BrowserMap的设计方式允许通过创建具有相同名称的新设备组并将其添加到来覆盖设备组定义 `BrowserMap` 自定义客户端库中的对象。

>[!NOTE]
>
>欲知更多详情，请阅读 [自定义的浏览器图](#creatingacustomisedbrowsermap) 部分。

### 定义基于选择器的URL {#defining-selector-based-urls}

如果之前未采用任何机制来指示替代站点 `BrowserMap`，然后将使用 `DeviceGroups` 将被添加到 `URL`s，在这种情况下，您应该提供自己的将处理请求的servlet。
...
创建自定义后 `BrowserMap` 客户端库，我们建议采用以下方法：
...
您可以通过添加，强制使用任何请求的设备，以测试网站的特定版本 `device` 参数到您的URL。 以下URL将呈现Geometrixx Outdoors网站的移动版本。
...
被覆盖的设备值存储在Cookie中，因此您可以浏览网站而不添加 `device` 参数到每个 `URL`.

因此，您需要调用相同的 `URL` 使用 `device` 设置为 `browser` 以返回到网站的桌面版。

>[!NOTE]
>
>BrowserMap将覆盖的设备值存储在名为的Cookie中 `BMAP_device`. 删除此Cookie将确保CQ将根据您当前的设备（例如，桌面或移动设备）提供相应版本的网站。
...
1. AEM确定请求页面的网站是否为移动网站(通过检查第一级页面是否为 `/content/geometrixx_mobile` 扩展了移动页面组件)。 如果是，则：
...
移动设备页面通常可在Dispatcher上缓存，因为为设备组呈现的页面在页面URL中可由设备组选择器来区分，例如 `/content/mobilepage.touch.html`. 绝不会缓存没有选择器的移动页面请求，因为在这种情况下，设备检测会运行，并且最终重定向到匹配的设备组（或该内容的“不匹配”）。 使用设备组选择器呈现的移动页面由链接重写器处理，该链接重写器重写页面内的所有链接以也包含设备组选择器，从而防止对已经限定的页面上的每次点击重新执行设备检测。
...
要解决此问题，您可以在页面上包含一个简单的选择UI，最终用户可以在其中覆盖AEM选择的设备组。 在上例中，页面上的链接（或图标）允许最终用户切换到 `coolpage.touch.html` 如果他觉得他的设备足够好。
