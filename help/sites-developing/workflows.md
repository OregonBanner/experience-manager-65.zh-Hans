...
通过工作流，您可以自动执行在AEM环境中管理资源和发布内容的流程。 工作流由一系列步骤组成，每个步骤都完成一个离散的任务。 您可以使用逻辑和运行时数据来确定流程何时可以继续，并从多个可能的步骤中选择下一个步骤。
...
>* 有关端到端社区文章，请参阅 [使用Adobe Experience Manager工作流修改数字资源。](https://experienceleague.adobe.com/docs/experience-manager-65/assets/using/assets-workflow.html?lang=zh-Hans)
>* 请参阅 [提问AEM专家关于工作流的网络研讨会](https://communities.adobeconnect.com/p5s33iburd54/).
...
工作流模型已进行版本控制。 运行工作流实例时，它会使用和保留工作流的运行时模型（在工作流启动时可用）。
...
>
>如果在此时间点之后更改了工作流模型(无 **同步** )，则运行时实例不会反映这些更改。 只有更新后生成的运行时模型才会反映这些更改。 但基础ECMA脚本除外，这些脚本仅保留一次，以便进行这些更改。
...
有效负载实施引用存储库中的资源（按路径、UUID或URL）或序列化Java™对象。 引用存储库中的资源是灵活的，并且具有Sling的高效性。 例如，引用的节点可以呈现为表单。
...
* **旧版**
   ...
   在早期版本的AEM中创建的工作流。 这些工作流可以在升级期间保留，也可以从以前的版本导出为工作流包，然后导入到新版本中。

### 瞬态工作流 {#transient-workflows}

标准工作流在运行时保存运行时（历史记录）信息。 您还可以将工作流模型定义为 **瞬态** 以避免这样的历史继续存在。 此工作流用于性能优化，因为它节省了用于保留信息的时间和资源。
...
引入了瞬态工作流来加载许多资产，在这些资产中，资产信息很重要，但工作流运行时历史记录并不重要。
...
>[!CAUTION]
>
>当工作流模型标记为瞬态时，在少数情况下，运行时信息仍必须保留：
>
>* 有效负载类型（例如，视频）需要外部步骤进行处理；在这种情况下，需要运行时历史记录来确认状态。
>* 工作流输入 **AND拆分**. 在这种情况下，需要运行时历史记录来确认状态。
>* 瞬态工作流进入参与者步骤后，会在运行时将模式更改为非瞬态模式。 由于任务正在传递给人员，因此必须保留历史记录。
>
...
>在临时工作流中，不应使用 **跳转步骤**.
>
>原因在于 **跳转步骤** 创建一个sling作业以在 `goto` 点。 它否定了使工作流成为瞬态过程的目的，并在日志文件中生成错误。
>
>使用 **OR拆分** 在瞬态工作流中进行选择。
...
如果 **多资源支持** 不会为您的工作流模型激活，并且选择了多个资源，然后为每个资源启动单个工作流实例。
...
工作流暂存有助于在处理任务时可视化工作流的进度。 它们可用于概述工作流完成处理的程度。 当工作流运行时，用户可以查看以下描述的进度 **暂存** （相对于单个步骤）。
...
1. 运行工作流时，用户可以根据阶段名称（而不是步骤名称）查看进度。 工作流进度显示在中 [工作流项目的任务详细信息窗口的“工作流信息”选项卡](/help/sites-authoring/workflows-participating.md#opening-a-workflow-item-to-view-details-and-take-actions) 列于 [收件箱](/help/sites-authoring/inbox.md).
...
创建表单时，可以轻松地将表单提交与工作流模型相关联。 例如，将内容存储在存储库的特定位置，或者通知用户表单提交及其内容。
