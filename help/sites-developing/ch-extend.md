...
要创建商店候选，请使用该函 [ 数扩展一个基 `ContextHub.Utils.inheritance.inherit`](/help/sites-developing/contexthub-api.md#inherit-child-parent) 本商店：
...
注册一个存储候选项以将其与ContextHub框架集成，并允许从该框架创建存储。 要注册存储候选项，请使 [ 用类 `registerStoreCandidate`](/help/sites-developing/contexthub-api.md#registerstorecandidate-store-storetype-priority-applies) 的函 `ContextHub.Utils.storeCandidates` 数。
