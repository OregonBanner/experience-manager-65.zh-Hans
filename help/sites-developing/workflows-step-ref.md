...
* **超时**
   ...
   此步骤将在其后“超时”。
您可以选择：**Off**、**Immediate**、**1h**、**6h**、**12h**、**24h**。
...
一个容器步骤将开始另一个作为子工作流执行的工作流模型。

此容器可让您重复使用工作流模型以实现常见步骤序列。 例如，翻译工作流模型可用于多个编辑工作流。
...
* 如果您指定的条件不保持true，则&#x200B;**跳转步骤**&#x200B;将完成，并且常规路由逻辑将确定要执行的下一步。

使用&#x200B;**跳转步骤**&#x200B;可以在工作流模型中实现高级路由结构。 例如，要实现循环，可以定义&#x200B;**跳转步骤**&#x200B;以执行工作流中的前一步骤，路由表达式将评估循环条件。
...
* **进程**
   ...
   * **路由表达式**:选择“规则定义”、“外部脚本”或确定是否执行目标步骤的 **ECMA脚本**。
...
例如，要实现对多个JCR节点执行操作的工作流，可以使用循环计数器作为节点的索引。 要保留计数，请在工作流实例的数据映射中存储`integer`值。 使用&#x200B;**跳转步骤**&#x200B;的脚本可增加计数，并将计数与退出条件进行比较。
...
您还可以使用规则定义作为路由表达式来模拟for循环。 [创建长 **** ](/help/forms/using/variable-in-aem-workflows.md#create-a-variable) 数据类型的可变项。在&#x200B;**[设置变量](/help/sites-developing/using-variables-in-aem-workflows.md#set-a-variable)**&#x200B;步骤中，使用&#x200B;**表达式**&#x200B;作为映射模式，在每次执行&#x200B;**设置变量**&#x200B;步骤时，将&#x200B;**count**&#x200B;变量的值设置为&#x200B;**count + 1**。

![模拟for循环](assets/variable_use_case_count_new.png)

在&#x200B;**跳转步骤**&#x200B;中，使用&#x200B;**设置变量**&#x200B;作为&#x200B;**目标步骤**，将&#x200B;**计数&lt;5**&#x200B;用作路由表达式。

![模拟for循环的条件](assets/variable_use_case_count1_new.png)

每次执行&#x200B;**设置变量**&#x200B;步骤时，将每次执行的&#x200B;**count**&#x200B;变量的值重复递增1，直到值达到5。
...
* 编辑&#x200B;**或拆分属性**:
   ...
   * **分支(*x)***
      ...
      * **脚本路径**:如果选择“外部脚本”作为路由表达式，则包含用于评估活动分支的脚 **本** 的文件的路径。
      * **脚本**:在框中添加脚本以评估活动分支(如果选择ECMA Script **作** 为路由表达式)。
      * **默认路由**:如果存在多个分支，则会遵循默认分支。默认只能指定一个分支。
   ...
   >[!NOTE]
   >
   >请参阅[定义OR Split的规则](/help/sites-developing/workflows-models.md#defineruleecmascript)。
...
1. **对话框路径属性**
   ...
   **对话参与者步骤**&#x200B;具有&#x200B;**对话路径**&#x200B;属性（以及[参与者步骤](#participant-step)的属性）。 **对话路径**&#x200B;属性的值是对话框`dialog`节点的路径。
...
1. **示例对话框定义**
   ...
   以下XML代码片断表示一个对话框，该对话框将`String`值存储在有效负荷内容的`watchEmail`节点中。 标题节点表示[TextField](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/granite-ui/api/jcr_root/libs/granite/ui/components/coral/foundation/form/textfield/index.html)组件：
...
**动态参与者步骤**&#x200B;组件与&#x200B;**[参与者步骤](#participant-step)**&#x200B;类似，其差异是在运行时自动选择参与者。
...
* **参与者选择器**
   ...
   * **参与者选择器**:您创建的参 [加者选择器的名称](#developingtheparticipantchooser)。
   * **参数**:任何必需参数。
   * **电子邮件**:是否应向用户发送电子邮件通知。
...
* **OSGi服务**
   ...
   * `getParticipant` 方法：将动态解析的主体ID返回为 `String` 值。
   ...
   >[!NOTE]
   >
   >**随机参** 加者选择是选择随机用户()的示 `com.day.cq.workflow.impl.process.RandomParticipantChooser`例服务。**随机参与者选择** r步骤组件示例扩展了&#x200B;**动态参与者步骤**，并将此服务用作步骤实现。
...
“设置变量”(Set Variable)步骤允许您设置变量值并定义值的设置顺序。 变量按变量映射在设置变量步骤中列出的顺序设置。
...
* **映射**
   ...
   * **选择映射模** 式：选择映射模式以设置变量的值。根据变量的数据类型，您可以使用以下选项设置变量的值：

      * **文本：** 当您知道要指定的确切值时，请使用此选项。
      * **表达式:** 当根据表达式计算要使用的值时，请使用此选项。表达式是在提供的表达式编辑器中创建的。
      * **JSON点表示法：** 使用此选项可从JSON或FDM类型变量中检索值。
