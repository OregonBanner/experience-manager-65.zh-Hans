...
* **工作流暂存**
   ...
   要应用的下拉选择器 [阶段](/help/sites-developing/workflows.md#workflow-stages) 中。
...
* **通过电子邮件通知用户**
   ...
   * 如果启用，则会向属性定义的用户发送电子邮件 **用户/组** 或，如果定义了组，则发送给组的每个成员。
...
的 **和拆分** 在工作流中创建一个拆分，在该拆分之后，两个分支将处于活动状态。 您可以根据需要向每个分支添加工作流步骤。 此步骤允许您在工作流中引入多个处理路径。 例如，您可以允许并行执行某些审核步骤，从而节省时间。
...
例如，要实施对多个JCR节点执行操作的工作流，可以使用循环计数器作为节点的索引。 要保留计数，请存储 `integer` 值。 使用 **跳转步骤** 递增计数并将计数与退出条件进行比较。
...
的 **设置变量** 步骤会重复执行，以增加 **计数** 变量值，直到值达到5。

## OR 拆分 {#or-split}

的 **或拆分** 在工作流中创建一个拆分，在该拆分之后，只有一个分支将处于活动状态。 通过此步骤，您可以在工作流中引入条件处理路径。 您可以根据需要向每个分支添加工作流步骤。
...
A **参与者步骤** 允许您为特定操作分配所有权。 仅当用户手动确认该步骤时，工作流才会继续。 当您希望某人对工作流执行操作时，会使用此插件；例如，审核步骤。
...
>[!NOTE]
>
>需要配置某些属性才能启用电子邮件通知。 您还可以自定义电子邮件模板或为新语言添加电子邮件模板。 请参阅 [配置电子邮件通知](/help/sites-administering/notification.md#configuringemailnotification) 用于在AEM中配置电子邮件通知。
...
1. **示例对话框定义**
   ...
   以下XML代码段表示用于存储 `String` 值 `watchEmail` 有效负载内容的节点。 标题节点表示 [文本字段](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/granite-ui/api/jcr_root/libs/granite/ui/components/coral/foundation/form/textfield/index.html) 组件：
...
* **OSGi服务**
   ...
   服务必须实施 [com.day.cq.workflow.exec.ParticipantStepChooser](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/workflow/exec/ParticipantStepChooser.html) 界面。 界面定义以下成员：
   ...
   * `getParticipant` 方法：将动态解析的主体ID返回为 `String` 值。
   ...
   >
   >
   >但是，组ID只能用于 **参与者步骤**，返回参与者列表时。 对于 **动态参与者步骤** 返回空列表，该列表不能用于委派。

   使实施可用于 **动态参与者步骤** 组件，将您的Java类添加到导出服务的OSGi包中，然后将该包部署到AEM服务器。
...
以下Java类实现 `ParticipantStepChooser` 界面。 类返回启动工作流的参与者的名称。 该代码使用与示例脚本(`initiator-participant-chooser.ecma`)。
...
* **进程**
   ...
   * **进程**:要执行的进程实施。 使用下拉菜单选择ECMAScript或OSGi服务。 有关信息:
      ...
      * 为流程步骤创建OSGi服务，请参阅 [使用Java类实施流程步骤](/help/sites-developing/workflows-customizing-extending.md#implementing-a-process-step-with-a-java-class).
