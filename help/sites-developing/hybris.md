...
>[!NOTE]
>
>电子商务框架可与任何电子商务解决方案一起使用。 此处讨论的某些具体细节和示例将引用 [SAP Commerce Cloud解决方案](https://www.hybris.com/) 。
...
* 该属 `cq:commerceProvider` 性还用于引用相应的商务工厂定义。
   ...
   * 此处可以配置其他属性， [**如目录版本&#x200B;**(如果适用](#configuringthecatalogversion)，则可用)。
...
* 身份验证AEM假定为唯一 *的* Web前端，因此执行 *所有身份验证* 。

* Slave AccountsAEM在hybris中为每个购物者创建从帐户。 从帐户的用户名与AEM用户名相同。 密码随机密码是自动生成的，并在AEM中存储（加密）。
