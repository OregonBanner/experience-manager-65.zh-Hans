...
- selectionId(id、uid、sku、slug、combinedSlug、combinedSku) — 允许您选择将由选取器返回的产品属性（默认为id）。 使用sku可返回所选产品的sku，而使用combinedSku时可返回诸如base#variant之类的字符串以及基础产品和所选变体的sku，如果选择基础产品，则返回单个sku。
...
此外，标准对话框字段属性如 `name`， `fieldLabel`，或 `fieldDescription` 支持。
