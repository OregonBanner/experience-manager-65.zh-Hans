...
[!DNL Adobe Experience Manager] 6.5.12.0包括自2019年4月发布6.5版以来发布的新功能、客户请求的关键增强以及性能、稳定性和安全性改进。 服务包安装在 [!DNL Adobe Experience Manager] 6.5.
...
* 现在，默认情况下可以将Live Copy源推送转出到多个活动副本，无需Blueprint配置(CQ-4259951)。
* 正在进行中的异步操作的状态现在显示在用户界面中，以帮助防止用户意外触发同一路径上的多个异步操作(NPR-37611)。
* 为Analytics 2.0 API(CQ-4285474、NPR-37803、NPR-37701、NPR-37702、NPR-37703)提供对基于IMS的身份验证的支持。
* API支持JSON选件类型体验片段(NPR-37796)。
* 现在为IMS中的删除选件（体验片段API）提供选件请求(NPR-37668)。
...
* 关闭内容片段上横幅（这些片段在各种网站页面上引用）的选项不起作用。 此横幅会通知用户内容片段在一个或多个页面中被引用(SITES-4173)。
...
* 尝试使用编辑后的模板创建体验片段时不会显示对初始页面属性所做的编辑(NPR-37962)。
...
* 站点管理员触控用户界面中的引用无法正确显示传入链接(NPR-37934)。
* 在翻译作业中选择页面时，用于添加新页面或资产的启动路径不可用(NPR-37912)。
* 提升启动项时，在体验片段中添加的列表组件中的引用页面未更新到目标页面(NPR-37886)。
* 创作环境存在用户界面问题 — 例如，编辑模式页面标题未居中且策略编辑器上允许使用组件选择器：“组”复选框占据容器的整个宽度，因此标签将在下一行中呈现(NPR-37878)。
* [Platform] commons-httpclient的metatype.xml文件中的xmlns：metatype的版本号为“https://www.osgi.org/xmlns/metatype/v1.0.0”，而不是“https://www.osgi.org/xmlns/metatype/v1.2.0”(NPR-37865)。
* 尝试访问页面时，发现错误且页面无法移动(NPR-37864)。
* [富文本编辑器] 在富文本编辑器中将图像作为列表项添加时，图像不会在经典用户界面中渲染(NPR-37835)。
* 在对话框中使用标记字段时，作者能够应用配置的根路径之外的标记(NPR-37834)。
* 多字段在布局容器中无法正确呈现，并出现错误(NPR-37811)。
* 在页面编辑器中调整组件布局大小的尝试在移动布局中不起作用(NPR-37805)。
* 体验片段翻译不会更新对语言复制路径的循环引用(NPR-37745)。
* 在页面属性中使用cq-msm-lockable富文本字段不会在转出页面时禁用该字段，并且作者可以对其进行修改(NPR-37714)。
* 激活体验片段时，发布者向Dispatcher发送许多激活请求(NPR-37707)。
* 在拓扑更改时，用于资产处理的Sling作业会重置，从而导致拓扑更改时正在进行的作业被忽略(NPR-37706)。
* 当MacOS用户导出站点和资源URL时，引号、交叉号和短划线不会导出到CSV(NPR-37698)。
* 运行react SPA页面时，SPA页面模板中的布局容器无法注册模板策略中定义的自定义CSS类(NPR-37697)。
* 当用户选择定位容器中具有背景的体验片段时，背景图像不可见(NPR-37662)。
* 体验片段上的翻译作业未翻译该体验片段上的所有组件(NPR-37660)。
* 体验片段的翻译以及包含体验片段的页面不会更新体验片段链接中的启动路径(NPR-37659)。
* 上传文件并保存对话框后，文件上传小组件不显示文件名(NPR-37634)。
* 如果移动了包含资产的文件夹，则不会在计划时间触发资产的计划激活（发布）(NPR-37621)。
* [Platform] 外部链接检查器仪表板无法在中呈现结果 [!DNL Adobe Experience Manager] WCM (NPR-37614)。
* 在编辑器中编辑标记时，如果标记名称中使用大写字母，内容片段编辑器将无法正常工作(NPR-37601)。
* 传统用户界面编辑器在触控用户界面的比较视图中不会显示标记为(NPR-37588)。
* 向翻译作业添加体验片段时记录了间歇性500错误(NPR-37587)。
* 即使已禁用日期选择器，作者也能够选择和使用日期选择器日期(NPR-37583)。
* [基础] 作者无法在触控用户界面的组件对话框结构的数字字段资源类型中输入一些小数值(NPR-37059)。
* 在安装以前的Service Pack时，会删除libs文件夹中的路径(NPR-36815)。
* [商务] 停用根文件夹不会更改中子产品的停用状态 [!DNL Experience Manager Commerce] 控制台；此外，在停用时根文件夹的子文件夹计数在用户界面中错误地显示(CQ-4338261)。
* [本地化工作流程] 列自定义和品牌自定义的内容未在“管理员控制”对话框（在选择中配置文件图标下的图标时）中本地化 [!DNL Adobe Experience Manager] 收件箱(CQ-4334864)。
* [Communities] 不可单击表内组成员的内容(CQ-4334404)。
* [Oak] 冷备用同步进程不工作，正在记录错误(CQ-4333868)。
* [平台基础UI] [!DNL Experience Manager] 用户选择 [!DNL Adobe Experience Manager] 图标的位置(CQ-4317409)。
* 对于没有复制权限的用户，要删除或移动页面（即使页面未激活），请 `Page Subtree Activation Check` 在配置下 `Page Manager Factory` 需要启用(NPR-37936)。
...
* 添加资源或文件夹时(包含 `single quote` 在“连接的资产”中，引用路径会失败并产生异常(NPR-37712)。
* 向资产添加水印时，无论用户定义的颜色如何，水印始终以黑色显示(NPR-37720)。
* 使用连接的资产时，即使非管理员用户被限制访问DAM存储库，非管理员用户也能够搜索资产(NPR-37644)。
* 使用批量编辑更新资源元数据时，未保存应用于下拉字段的更改，这些更改会重置为默认值(NPR-37345)。
* 删除文件夹耗时过长，影响整体性能(NPR-37107)。
* 在元数据架构中应用规则时，用户无法查看下拉列表的完整值 `Field Value` 和 `Field Choices` 如果值大于文本框(CQ-4338074)。
* 升级到版本6.5.10.0后，资源属性页面反映不必要的HTML渲染消息(CQ-4336994)。
...
* 以下修复与资源属性中的级联元数据相关。
   * 强制下拉列表会为多值字段中的每个选择反映多个错误消息(NPR-37859)。
...
* 在中创建图像预设时 `tiff` 格式，则会创建预设，但格式会更改为 `jpeg` (CQ-4335985)。
* 修改 `Progressive JPEG Scan` 图像预设编辑器中的值，则下拉列表值始终重置为 `auto`(CQ-4335971)。
* 的视频元数据不可见 `mxf` 资产属性页面上的视频(CQ-4335499)。
* 重新处理视频资产时，会从发布服务器(CQ-4335461)取消发布AVS（自适应视频集）和视频演绎版。
...
* 当自适应表单中的文本组件包含表时，将内容粘贴到该组件导致在编辑器中擦除该表(NPR-38078)。

* 表单仅在打开保存的表单时显示工具栏(NPR-38060)。

* 撤消操作不适用于规则编辑器(NPR-37973)。

* `getAemFormContainer` 安装AEM Forms 6.5.10.0后返回空指针(NPR-37881)。

* 辅助功能 — 当选项卡焦点转移到字段时，屏幕阅读器会立即朗读文本框的详细说明，而不是仅在单击字段时朗读(NPR-37855)。

* 为文本框启用允许富文本属性时，允许的最大字符长度存在问题(NPR-37825)。

* 在自适应表单中复制任何组件时出现CSS问题(NPR-37812)。

* 在生成自适应表单翻译时，生成的XLIFF文件包含的文本序列与自适应表单中的文本序列不同。 在某些情况下，需要查看文本的上下文。 如果XLIFF中的序列是字母顺序的，则无法执行此操作。 (NPR-37435).

* 在翻译自适应表单时，HTML标记是翻译的一部分。 如果用户出错，并且标记无效，则整个文本不会显示在记录文档中。 (NPR-37499)

* 以基本语言创建和完成自适应表单时，翻译由外部团队完成并导入。 即使文本有微小变化，如加号或缺少点(.) 在中对基本语言完成，则所有其他语言的完整翻译都将丢失。 (NPR-37189)
...
* 从AEM 6.5.7.0 Forms升级到AEM 6.5.10.0 Forms后，使用PDF生成器服务时出现Office文档问题(NPR-37758)。
...
* 对于AEM 6.5.7.0 Forms，多线程PDF生成器服务在经过一段随机时间后出现死锁(NPR-38053)。

* 在AEM Workbench版本6.5.0.20210518.1.338459中，当您使用电子邮件起点并编辑用户名和密码时，不会保存配置(NPR-37967、CQ-4336081)。

* 保存日志会导致CPU使用率较高，需要重新启动服务器(NPR-37868)。
...
* Experience Manager6.5.12.0需要Experience Manager6.5。参见 [升级文档](/help/sites-deploying/upgrade.md) 以获取详细说明。
...
>[!NOTE]
>
>Adobe不建议删除或卸载 [!DNL Adobe Experience Manager] 6.5.12.0程序包。
...
>[!NOTE]
>
>如果您未在 JEE 上使用 AEM Forms，请跳过。JEE上的Adobe Experience Manager Forms中的修补程序通过单独的安装程序提供。

有关在JEE上安装Experience Manager Forms的累积安装程序和部署后配置的信息，请参阅 [发行说明](/help/release-notes/jee-patch-installer-65.md).
...
* 当您安装带有Service Pack 12的AEM 6.5并尝试下载状态ZIP文件时，Experience Manager会下载损坏的文件。 下载并安装 [AEM Sites SEO索引包](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq650/featurepack/sites-seo-index-content-1.0.0.zip) AEM ，然后再下载ZIP文件来解决问题。
