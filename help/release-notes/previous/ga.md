...
| 类型 | 主要版本 |
| 正式发布日期 | 2019年4月8日 |
| 推荐更新 | 请参阅 [AEM近期更新](https://experienceleague.adobe.com/docs/experience-manager-release-information/aem-release-updates/aem-releases-updates.html?lang=zh-Hans). |
...
[!DNL Adobe Experience Manager] 6.5是的升级版本 [!DNL Adobe Experience Manager] 6.4代码库。 它提供新功能和增强功能、关键客户修复、高优先级客户增强功能以及面向产品稳定性的常规错误修复。它还包括 [!DNL Adobe Experience Manager] 6.4 Service Pack最多发布SP4。
...
* 为获得最佳性能，请使用其他值覆盖默认GC值。 有关更多信息，请参阅 [安装和更新](/help/sites-deploying/custom-standalone-install.md) 中。
...
* 的 [全选](/help/sites-authoring/basic-handling.md#select-all) 操作是一种快速方法，可以对同一文件夹中的所有页面/资产执行一项操作。
* 的 [全选](/help/sites-authoring/basic-handling.md#select-all) 操作会尝试对所有页面/资产执行操作，而不仅仅是对已加载的页面/资产执行操作。 如果未升级操作以处理批量操作，则会显示警告对话框。

>[!CAUTION]
>
>Adobe不打算进一步增强经典UI。 AEM 6.5包含经典UI，从早期版本升级的客户可以继续按原样使用它。 经典UI在弃用时仍完全受支持。 [了解更多](/help/sites-deploying/ui-recommendations.md).

#### 搜索和索引 {#indexing-and-search}

* 在Oak中搜索现在支持动态Facet。 例如，资产搜索中的过滤器边栏会显示预计的结果数。
* 扩展了QueryBuilder以提供包含动态Facet的结果。
...
>[!CAUTION]
>
>自AEM 6.3起推出的新版Oak Segment Tar需要存储库迁移。 如果您从旧版TarMK升级或希望从其他类型的持久性中切换新的Tar区段，则必须执行此步骤。 有关新区段焦油的优势的更多信息，请参阅 [迁移到Oak区段Tar常见问题解答](/help/sites-deploying/revision-cleanup.md#migrating-to-oak-segment-tar).

#### OSGI {#osgi}

* 添加了OSGi Promise和Converter实用程序库。
...
#### 托管的单页应用程序 {#managed-single-page-apps}

页面编辑器增加了在上下文内编辑内容以及在客户端渲染的体验(也称为 [作为SPA编辑器](/help/sites-developing/spa-architecture.md))。 使用AEM SJ SDK构建的现有单页应用程序可以使用JavaScript框架React或Angular进行扩展，以使实际使用者可以编辑。

SPA支持最初作为AEM 6.4 SP2的一部分提供，随着AEM 6.5的推出，它具有以下功能：

* 使用“模板编辑器”编辑和配置SPA的AEM可编辑部分
* 使用多站点管理创建国家/地区、特许经营或白标的SPA体验

#### 无外设内容管理 {#headless-content-management}

AEM可以以各种格式和堆栈的不同级别提供内容。 自2008年以来，一些公司就开始使用 [SlingGET](https://sling.apache.org/documentation/bundles/rendering-content-default-get-servlets.html) 和 [POSTServlet](https://sling.apache.org/documentation/bundles/manipulating-content-the-slingpostservlet-servlets-post.html). 内容服务([Sling模型导出程序](https://experienceleague.adobe.com/docs/experience-manager-learn/foundation/development/develop-sling-model-exporter.html?lang=en))在AEM 6.3中引入，是AEM SJ SDK用于水合物单页应用程序的方法。 的 [适用于资产的HTTP API](/help/assets/mac-api-assets.md) 是一个CRUD API，已针对AEM 6.5进行扩展。

新的HTTP API功能：

* 添加了 [对资产HTTP API的内容片段支持](/help/assets/assets-api-content-fragments.md) 创建、更新、读取和删除片段。
* 通过Content Services和 [内容片段列表核心组件](https://www.aemcomponents.dev).
...
* 通过改进内容重复利用，统一数字和存储中的体验和内容
* 通过支持启动项简化了创作和批准/发布工作流程
* 使用SPA Editor编辑和提供丰富的交互式体验
* 使用启动项规划标牌内容的未来内容更改
* 序列渠道中的按量计费播放
* 使用源文件（如Excel工作表）自动创建项目结构
* 扩展的媒体播放器支持，具有强大的在线和离线操作（智能同步），甚至可以扩展到最大的标牌网络。
* 使用动态占位符，按数据触发内容的位置或配置进行个性化。
* 通过将Adobe Analytics集成到AEM Screens Player来实现统一分析

有关对AEM Screens所做的更改的更多详细信息 — 请参阅 [AEM Screens用户指南](https://experienceleague.adobe.com/docs/experience-manager-screens/user-guide/aem-screens-introduction.html?lang=zh-Hans).
...
* 的 [参考](/help/sites-authoring/author-environment-tools.md#references) 边栏新增了一个部分，用于列出指向所选页面的内部链接。 当计划使页面脱机或删除时，此功能非常有用 — 可了解在离线之前需要调整哪些页面。
* 的 [列表视图](/help/sites-authoring/basic-handling.md#list-view) 新增了一个工作流列，用于显示页面处于工作流中时的状态。
* 在 [页面属性](/help/sites-authoring/editing-page-properties.md)，则现在可以在为页面分配缩略图（缩略图选项卡）时浏览现有资产。

#### 页面编辑器 {#page-editor}

* 允许对使用JS SDK的React和Angular客户端组件(也称为SPA编辑器)构建的单页应用程序体验进行上下文内编辑和组合
* 仅当页面配置了基架页面时，才显示基架模式。
...
* 添加 [注释/批注](/help/assets/content-fragments/content-fragments-variations.md#annotating-a-content-fragment) 通过在RTE中选择文本（全屏视图）
* [比较版本](/help/assets/content-fragments/content-fragments-managing.md#comparing-fragment-versions) 通过引用边栏并排显示内容片段的
* 资产下载报表现在会相应地显示内容片段
* 添加 [对资产HTTP API的内容片段支持](/help/assets/assets-api-content-fragments.md) 通过/api.json。 有用于创建、更新、读取和删除内容片段的API。

#### 体验片段 {#experience-fragments}

* 改进了 [体验片段](/help/sites-authoring/experience-fragments.md)，以便在搜索正在使用其内容的页面时找到这些内容。
* 的 [导出到Target](/help/sites-administering/experience-fragments-target.md) 选项现在允许您以JSON格式(默认为HTML)发送体验片段，或者同时将两者发送。

#### 翻译 {#translation}

* 使用项目主页简化翻译项目的创建过程。
* 默认情况下，将翻译作业设置为已批准状态，从而简化翻译项目的执行。
* 允许使用第三方翻译内存中的更改来更新已翻译页面。
* 允许以JSON格式导出翻译作业。
* 更新Microsoft®翻译集成以使用V3 API。

#### 多站点管理(MSM) {#multi-site-management-msm}

* 对于使用PushOnModify的转出配置，可更好地处理页面移动操作以避免状态不一致。
* 在Live Copy结构中创建页面时，默认情况下会创建一个独立页面。
* 在单页应用程序中使用JS SDK的MSM功能(也称为SPA编辑器)

#### 启动项 {#launches}

* 新增了启动项的审核和批准工作流，并且能够仅提升已批准的启动项页面
* 添加了 [选项，以选择在提升步骤之后删除启动项](/help/sites-authoring/launches-promoting.md#promoting-launch-pages)

#### 内容定位和模拟 {#content-targeting-simulation}

* ContextHub数据层和客户端规则引擎JavaScript已更新，默认情况下使用jQuery 3。
...
##### 增强社区参与度 {#enhancements-to-community-engagement}

**@Mentions支持**
AEM Communities现在允许注册用户在用户生成的内容中标记（提及）其他注册成员以引起他们的注意。 随后将通知标记（提及）成员，并包含指向相应用户生成内容的深层链接。 但是，用户可以选择禁用/启用Web通知和电子邮件通知。

![提及次数支持](/help/release-notes/assets/at-mentions.png)

社区用户无需搜索其名字、姓氏或用户名，即可查看是否有人联系过他们或需要他们的注意。 此外，它还允许UGC作者寻求能够最好地解决问题并添加投入的特定注册用户的响应。

社区管理员需要 **启用提及** 在社区组件上，允许注册用户在这些组件上使用功能。

**组消息传送**

注册的社区成员现在可以通过单个电子邮件组合将私信批量发送到组，而不是单独向组成员发送相同的邮件。 允许 [组消息](/help/communities/configure-messaging.md)，则启用 [报文传送操作服务](/help/communities/messaging.md#group-messaging).
...
A [示例项目](https://github.com/Adobe-Marketing-Cloud/aem-communities-extensions/tree/master/aem-communities-moderation-filter) 用于演示通过标记进行过滤的 [GitHub](https://github.com/Adobe-Marketing-Cloud/aem-communities-extensions/tree/master/aem-communities-moderation-filter). 此项目可用作开发类似自定义过滤器的基础。

![自定义过滤器](/help/release-notes/assets/custom-tag-filter.png)

**批量审核中的列表视图**

在批量审核中提供了具有改进UI的新列表视图，以显示用户生成的内容条目。
...
从AEM 6.5开始，社区允许对不同社区站点和组/嵌套组进行分散管理（和管理）。 现在，托管多个社区站点和嵌套群组的组织在创建站点（和群组）时，可以在创作端为管理员角色选择成员。

![站点管理员](/help/release-notes/assets/site-admin.png)

站点管理员可以在任何层级创建群组，并成为默认管理员。 这些管理员稍后可被其他组管理员删除。 群组管理员可以管理其群组G1并创建嵌套在G1下的子群组。
...
AEM 6.5引入了以下功能和增强功能，以提高AEM用户、DAM角色以及相关的创意和营销角色的工作效率。

#### 与集成 [!DNL Adobe Creative Cloud] 和创意工作流 {#integration-with-adobe-creative-cloud-and-creative-workflows}

[!DNL Adobe Experience Manager] 提供了多种与集成的方法 [!DNL Adobe Creative Cloud] 和共享资产，以在创意和营销或业务团队密切协作的工作流中使用。 [!DNL Experience Manager] 6.5继续改进集成并进一步简化集成，以提供更多机会并简化现有方法。
...
[!DNL Experience Manager Assets Brand Portal] 帮助您轻松获取、有效控制和安全地将批准的资产跨设备分发给外部供应商/代理和内部业务用户。 它有助于提高资产共享效率，加快资产的上市时间，并消除不合规使用和未授权访问的风险。
...
在大型企业中，可以分发创建网站所需的基础结构。 有时，网站创建功能和所需的数字资产会驻留在不同的孤岛中。

[!DNL Experience Manager Sites] 提供创建网页和 [!DNL Experience Manager Assets] 是数字资产管理(DAM)系统，用于为网站提供所需的资产。 [!DNL Experience Manager] 现在通过集成支持上述用例 [!DNL Sites] 和 [!DNL Assets]. 请参阅 [如何配置和使用“连接的资产”功能](/help/assets/use-assets-across-connected-assets-instances.md).
...
您现在可以使用视频本身或DAM中存储的其他内容中的帧自定义视频资产的缩略图。 有关其他说明，请参阅 [关于视频缩略图](/help/assets/video.md#about-video-thumbnails-in-dynamic-media-scene-mode).
...
[!DNL Experience Manager] 从6.5开始，营销人员可以更快地从搜索结果页面发现所需的资产。 即使在应用搜索过滤器之前，搜索彩块化也会随资产数量进行更新。 通过过滤器查看预期计数有助于用户高效地浏览搜索结果。 有关更多信息，请参阅 [在Experience Manager中搜索资产](/help/assets/search-assets.md).
...
您现在可以一次从文件夹或搜索结果中选择所有加载的资产。 这有助于您快速管理多个资产。 此复选框会选择适合方案（如搜索结果）的所有资产，而不只是选择 [!DNL Experience Manager] 界面。
...
指定级联元数据时，可以在运行时从JSON文件加载所做的选择，例如，不要在表单中手动键入。 有关更多信息，请参阅 [级联元数据](/help/assets/metadata-schemas.md#cascading-metadata).
...
Adobe已制定相应的程序和流程，以便收集、确定以客户为中心的错误解决和增强请求开发的优先级并跟踪这些错误。 的 [Experience Manager支持门户](https://experienceleague.adobe.com/?support-solution=Experience+Manager#support) 与Adobe增强和缺陷跟踪系统集成。 客户问题由客户支持团队在可能的情况下识别并解决。 当呈报至研发部门时，会捕获所有客户信息，并将其用于确定优先级和进行报告。 在开发过程中，优先考虑付费支持、被担保人问题和客户付费增强功能。

这一确定优先顺序的过程产生了750多项以客户为中心的更改，这些更改已在AEM 6.5中修复。
...
* Maven/Gradle依赖关系([下载链接](https://repo1.maven.org/maven2/com/adobe/aem/uber-jar/6.5.0/))
...
* Maven项目原型：
   ...
   * 对于全堆栈站点： [GitHub项目](https://github.com/adobe/aem-project-archetype)
...
* AEM现代化工具套件，例如对话框转换工具。 ([GitHub项目](https://github.com/adobe/aem-modernize-tools))

**Assets**

* 用于添加增强的PDF光栅器([阅读更多](/help/assets/aem-pdf-rasterizer.md))
...
[!DNL Experience Manager] 6.5已通过GB18030-2005 CITS认证，可使用中文编码标准。
...
* 的 **AEM Forms Assets扫描程序** 和 **信件到交互式通信迁移** 选项在 **AEM Forms迁移** 页面。 (CQ-4266572)

* (仅限JBoss® 7)当您从以前的版本升级到AEM 6.5 Forms，并且以前的版本具有创建和使用默认提交或默认呈现过程副本的进程(.lca)时，使用此类进程(.lca)的HTML5 Forms无法执行所需的操作。 (CQ-4243928)
* 在自适应中，当从规则编辑器调用表单数据模型服务以动态更新图像选择组件的值时，图像选择组件的值不会更新。 (CQ-4254754)
...
以下网站仅供客户使用。 如果您是Adobe，并且需要访问，请联系您的客户经理。
