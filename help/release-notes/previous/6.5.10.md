...
* **GraphQL API for[!DNL Content Fragments]**:新的GraphQL API是以JSON格式交付结构化内容的标准方法。 GraphQL查询允许客户仅请求相关内容项来渲染体验。 这种选择消除了需要在客户端解析内容的内容过交付（借助HTTP REST API来实现）。 GraphQL架构源自 [!DNL Content Fragment] 模型和API响应采用JSON格式。

* **GraphQL API for[!DNL Content Fragments]**:要支持GraphQL API，内容片段模型属性字段中不再允许使用连字符。 如果任何“内容片段模型”属性名称中存在连字符，则GraphQL查询可能会返回不需要的结果。
...
* 当自定义Servlet过滤器包等修改网页的链接时，无法呈现网页。 (NPR-36857) &lt;! — 根据CQDOC-19203,2022年4月07日更新的注释 — >
