...
* 当用户尝试将XDP表单从PDF转换为HTML5表单时。 在呈现HTML5格式的柬埔寨字体时出现的一些问题包括：
   * 换行符在HTML5表单中呈现。 (CQ-4322508)
   * 列表在HTML5表单中未正确呈现。 (CQ-4322317)
   * 呈现HTML5表单呈现时，会显示额外的空格。 (CQ-4322509)
...
* 当用户尝试将XDP表单从PDF转换为HTML5表单时，一些已修复的问题包括：
   * 支持Farsi渲染为LTR。
   * 与LTR文本混合时支持RTL。
   * 支持图像旋转。
   * 添加了对柬埔寨语的支持。 要启用对Combodian的支持，您需要将标记启用为 `CTL=true` 呈现HTML5表单时。
