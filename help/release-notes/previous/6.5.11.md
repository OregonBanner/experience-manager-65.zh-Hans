...
[!DNL Adobe Experience Manager] 6.5.11.0包括自2019年4月发布6.5版以来发布的新功能、客户请求的关键增强以及性能、稳定性和安全性改进。 服务包安装在 [!DNL Adobe Experience Manager] 6.5.

中引入的主要特性和增强功能 [!DNL Adobe Experience Manager] 6.5.11.0包括：

* 为多行文本数据类型添加了多字段支持。

* 增强功能，使用户了解当前在后台运行的异步作业，以防他们触发同一路径上的多个异步操作。
...
* 业务配置文件支持AdobeAsset Link。

* 您现在可以使用 [!DNL Dynamic Media] 以配置“常规设置”，而不必浏览 [!DNL Dynamic Media Classic] 桌面应用程序。 参见 [配置Dynamic Media常规设置](/help/assets/dm-general-settings.md).

   ![DM常规设置](/help/assets/assets-dm/dm-general-settings.png)

* 您现在可以使用 [!DNL Dynamic Media] 以配置发布设置，而不必通过 [!DNL Dynamic Media Classic] 桌面应用程序。 参见 [配置Dynamic Media发布设置](/help/assets/dm-publish-settings.md).
...
* 配置添加在 `/apps/system` 节点而不是 `/libs` 安装以前的Service Pack时(SITES-3203)。

* 使用内容片段的功能在安装以前的服务包(SITES-3151)时无法正常工作。

* 排序在中不起作用 [!UICONTROL 内容片段模型] 控制台(SITES-2722)。

* GraphiQL未加载模型（架构），端点JSON (SITES-2428)遇到错误。

* 已添加到的枚举字段类型 [!UICONTROL 内容片段模型] 在中不可见 [!UICONTROL 内容片段模型编辑器] (SITES-2391)。
...
* 计划激活多个页面会导致打开一个针对的新JCR会话 `wcm-workflow-service` 用户(NPR-37417)。

* 对站点控制台中的文件夹执行移动操作失败，并出现错误消息“无法检索选定项目的启动信息”(NPR-37340)。

* 移动时内容片段标题未更新(NPR-37257)。

* 为Blueprint生成缩略图并转出到活动副本时，活动副本中的缩略图之后的选项卡继承会损坏(NPR-37190)。

* 用于显示Live Copy的过滤器谓词不显示所有活动副本(NPR-37126)。

* 对作者调用复制事件处理程序时，复制事件不会返回标记为删除的所有父页面和子页面的列表(NPR-37123)。

* 使用批量编辑器保存多值属性时，逗号分隔的字符串存储为数组的第一个元素(NPR-37089)。

* 组件布局调整大小在移动布局中不起作用(NPR-37086)。

* 添加转出配置后，在保存页面属性时，在Live Copy级别上错误地创建了新节点(NPR-37084)。

* 用户无法为新的主控页面创建活动副本或使用页面属性转出(SITES-3442)。

* 标记显示标记名称而不是标题和关闭选项不会完全删除标记，因为在属性级别取消继承时，标记属性工作不正确(NPR-36831)。

* 用于取消选择所有项目的选项不起作用，并且标题与显示活动副本列表的页面的表中的第一行重叠(NPR-37070)。

* 在工作流中使用的自定义对话框中，尝试验证对话框时，Experience Manager失败，并在浏览器控制台中出现错误(GRANITE-35049)。

以下辅助功能增强功能在以下位置提供： [!DNL Adobe Experience Manager Sites]：

* 屏幕阅读器现在宣布了 [!UICONTROL 站点引用] 和 [!UICONTROL 语言副本] 选项(SITES-1791)。

* 现在，浏览器模式焦点的顺序在用户界面上的各种选项上按顺序移动(SITES-1791)。

* 屏幕阅读器现在可讲述所选树项目是否处于选定状态，并向用户宣布显示操作区域(SITES-2109)。

* 屏幕阅读器现在会在选择过滤器或搜索页面时显示加载指示器时通知(SITES-1790)。

* 屏幕阅读器现在可以讲述以下情况 [!UICONTROL 筛选条件] 选项不会返回左边栏中的任何搜索结果(SITES-1599)。

* 在浏览模式下导航时，屏幕阅读器讲述内容页面的角色以及按Enter键时页面的选定状态(SITES-1579)。

* 屏幕阅读器现在会在以下情况下提供旁白： [!UICONTROL 注释添加] 已选中选项(SITES-1573)。

* 现在，表单字段除了占位符之外，还具有可视标签，这样在输入字段值时，屏幕阅读器用户将得到适当的指导(SITES-1258)。

## [!DNL Assets] {#assets-65110}

中提供了以下辅助功能增强功能 [!DNL Assets]：

* 在的卡片视图中 [!DNL Assets] 存储库，使用时 `Tab` 将焦点移动到第一个打开快速操作聚焦的项目时，屏幕阅读器会朗读聚焦项目的名称。
* In [!DNL Dynamic Media] [!UICONTROL 查看器预设编辑器]时，如果阴影颜色和边框颜色不存在，则使用disabled属性禁用输入。 键盘用户无法集中输入内容，屏幕阅读器不会将控件的状态公告为已禁用。
* In [!DNL Dynamic Media]，在界面中创建新视频编码配置文件， [!UICONTROL 智能裁剪比率] 选项标有辅助功能，以便屏幕阅读器恰当地宣布该选项。

* 您现在可以访问中的引用列表控件 [!DNL Experience Manager Assets] 使用键盘。

以下问题已在中修复 [!DNL Assets]：

* 当参与者组的用户导航到DAM资产存储库时，例外情况 `POST` 创建收藏集时会触发请求。 此 `POST` 请求失败并在日志中反映错误(NPR-37171)。

* 创建具有嵌套文件夹结构的Blueprint的Live Copy时，源文件夹的已修改属性未在Live Copy文件夹中更新(NPR-37449)。

* 选择多个资源并修改元数据字段值时，保存资源不会保留这些值。 此外，不会应用元数据更改(NPR-37341)。

* 选择多个资产并修改属性时，自定义属性（下拉列表）值会被默认值覆盖(NPR-36437)。

* 为宣传册、传单和InDesignPDF生成了错误的模板演绎版(NPR-36433)。

* 保存 [!DNL Adobe Target] 活动 [!DNL Experience Manager] 定位模式在出现 [!DNL Adobe Analytics] 引用的报表量度(NPR-37167)。
...
* 将包含上传时生成的自定义元数据的视频添加到页面时，会显示有关未知命名空间的错误，即使命名空间已注册也是如此(CQ-4331471)。

* In [!DNL Assets]，如果 [!DNL Launcher] 禁用，则元数据写回在手动触发时不起作用(CQ-4329082)。

## [!DNL Dynamic Media] {#dynamic-media-65110}

以下错误修复可在中找到 [!DNL Dynamic Media]：

* 资产未更新于 [!DNL Dynamic Media] 在中恢复资源版本时 [!DNL Experience Manager] (NPR-37421)。

* 发布PDF文件时不会发布ECatalog (CQ-4329886)。

* 如果组件使用现成预设，则在打开已发布的页面时，不会加载3D资产(CQ-4329205)。

* 大型存储库的PDF资源处理问题(CQ-4328711)。

* PDF处理错误未传播到 [!DNL Experience Manager] 如果在以下位置出现故障 [!DNL Scene7] (CQ-4331145)。

* 用户无法查看.MOV资源的默认元数据属性(CQ-4332546)。

* 无法将.MXF视频文件上传到 [!DNL Dynamic Media] 使用 [!DNL Experience Manager] (CQ-4329709)。

* 设置自定义公司根目录时出现上传问题(CQ-4332800)。

* In [!DNL Experience Manager] 包含自定义启动器的设置 `ActivationModel` 在工作流中，由于上传PDF文件时出现内存问题，Experience Manager崩溃。 (CQ-4330512).

* 中的性能问题 `DamEventRecorder` (CQ-4334072)。

* 如果购物视频超链接（链接的URL）包含特殊字符，则查看器会对目标URL进行编码，并会生成不正确的产品页面(CQ-4331639)。

* 在视频配置文件页面中，如果用户在页面加载时立即选择视频配置文件，则工具栏选项消失(CQ-4308521)。

* 由于JCR并发写入，DM资产处理失败(CQ-4333489)。

* 如果用户的视频配置文件根目录具有在视频配置文件根节点上定义的自定义访问策略，则访问“视频配置文件”页面失败(CQ-4332941)。

* 在可缩放的图像中，使用快捷键(“+”、“ — ”)或“Esc”可陷印屏幕阅读器焦点(CQ-4290719)。

* 当用户单击表单模式快捷键(“F”)时，屏幕阅读器不会映射 [!UICONTROL 嵌入大小] 菜单按钮位于 [!UICONTROL 获取嵌入内容] 代码对话框(CQ-4290929)。

* 使用键盘导航打开电子邮件链接弹出窗口时，用户界面上显示的“收件人”和“发件人”字段的错误建议不具描述性(CQ-4290930)。

* 导航到电子邮件链接对话框时，屏幕阅读器不会讲述使用向下箭头和表单模式快捷键(“F”)上新添加的编辑字段的标签信息(CQ-4290934)。

* 导航到电子邮件链接对话框时，屏幕阅读器不会反映“收件人”和“发件人”必填字段的视觉星号(*)(CQ-4290935)。
...
* 使用时 [!UICONTROL 稍后发布] 选项，则用户界面不会将状态反映为 [!UICONTROL 等待发布] (CQ-4334229)。
...
* 从升级时 [!DNL Experience Manager] 从6.5.8.0到版本6.5.9.0，安装程序会覆盖上的属性 `/content/dam/projects`. 它会将分配的元数据架构和文件夹的属性重置为默认值(NPR-37124)。
...
* 从翻译项目转出父页面时，特定语言的子页面会被删除(NPR-36998)。

* 创建翻译项目时，页面的循环引用会触发启动项，从而导致错误(CQ-4332982)。

* 已翻译体验片段和页面中的体验片段链接包含启动引用(NPR-37649)。
...
* 辅助功能 — 设置 `Wizard` 自适应表单中的面板布局，导航按钮没有Aria标签和角色(NPR-37613)。

* 自适应表单中日期字段的验证无法按预期工作(NPR-37556)。

* 复选框和单选按钮组件的标签文本较长，文本无法正确放置(NPR-37294)。

* 将样式更改应用于AEM Forms容器组件的感谢消息时，更改不会复制到源自适应表单中(NPR-37284)。

* 的公允价值的差异 `Switch` 用户界面和后端中的组件(NPR-37268)。

* 当您使用键盘按键导航到 `Submit` 选项并按 `Enter` 键，您可以多次提交自适应表单(CQ-4333993)。

* 文件附件组件的删除操作无法按预期进行(NPR-37376)。

* 当字段的标签在可翻译成各种语言的自适应表单中超过1000个字符时，词典无法检索标签的翻译(CQ-4329290)。
...
* 将文档附件传递到Assembler服务时，将显示以下例外(NPR-37582)：
...
* 当用户尝试将XDP表单从PDF转换为HTML5表单时。 在呈现柬埔寨字体的HTML5表单时出现的一些问题包括：
   ...
   * 呈现HTML5表单演绎版时显示额外空格。 (CQ-4322509)
...
>[!NOTE]
>
>如果您尚未升级到AEM 6.5.11.0 Forms，请直接安装AEM Forms 6.5.11.1附加组件包。 如果已安装AEM 6.5.11.0 Forms，Adobe建议升级到AEM 6.5.11.1 Forms。

* 安装Forms 6.5.11.0附加组件包后，提交操作、发送电子邮件和调用AEM Workflow停止工作。
...
* 当用户尝试将XDP表单从PDF转换为HTML5表单时，一些修复的问题包括：
   ...
   * 与LTR文本混合时支持RTL。
   * 支持图像旋转。
   * 添加了对柬埔寨语言的支持。 要启用支持Combodian支持，您需要启用标记为 `CTL=true` 渲染HTML5表单时。
...
* Experience Manager6.5.11.0需要Experience Manager6.5。参见 [升级文档](/help/sites-deploying/upgrade.md) 以获取详细说明。
...
>[!NOTE]
>
>Adobe不建议删除或卸载 [!DNL Adobe Experience Manager] 6.5.11.0程序包。

### 安装服务包 {#install-service-pack}

要将Service Pack [!DNL Adobe Experience Manager] 6.5实例，请按照以下步骤操作：
...
A.将包裹放入 `../crx-quickstart/install` 文件夹（服务器联机时）。 软件包会自动安装。
...
| 区域 | 专题 | 替换 |
|---|---|---|
| 集成 | 此 **[!UICONTROL AEM云服务选择加入]** 屏幕已弃用，因为 [!DNL Experience Manager] 和 [!DNL Adobe Target] 集成在Experience Manager6.5中更新。该集成支持Adobe Target Standard API。 API通过Adobe IMS使用身份验证和 [!DNL Adobe I/O] 并支持AdobeLaunch在检测方面发挥越来越大的作用 [!DNL Experience Manager] 页面进行分析和个性化时，选择加入向导在功能上无关紧要。 | 配置系统连接、Adobe IMS身份验证和 [!DNL Adobe I/O] 通过各自的集成 [!DNL Experience Manager] 云服务。 |
| 连接器 | Experience Manager6.5已弃用适用于Microsoft®SharePoint 2010和Microsoft®SharePoint 2013的AdobeJCR连接器。 | 不适用 |
...
* 当您安装带有Service Pack 11的AEM 6.5并尝试下载状态ZIP文件时，Experience Manager会下载损坏的文件。 下载并安装 [AEM Sites SEO索引包](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq650/featurepack/sites-seo-index-content-1.0.0.zip) AEM ，然后再下载ZIP文件来解决问题。
...
* 在尝试移动/删除/发布内容片段或站点/页面时，在获取内容片段引用时出现问题，因为后台查询将失败；即该功能将无法工作。
...
* 参见 [如何联系Adobe客户支持](https://experienceleague.adobe.com/docs/customer-one/using/home.html).

## 以下时间以来的关键版本 [!DNL Adobe Experience Manager] 6.5 SP10{#key-releases-since-last-sp}

在2021年8月26日至2021年11月25日期间，Adobe除了Service Pack之外还发布了以下内容：
