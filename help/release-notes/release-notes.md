...
* DASH是自适应视频流的国际标准协议，在业界得到了广泛的应用。
* 现在北美提供（通过支持票启用），不久将在亚太和欧洲 — 中东 — 非洲提供。
...
* [Headless自适应Forms](https://experienceleague.corp.adobe.com/docs/experience-manager-headless-adaptive-forms/using/overview.html) 使您的开发人员能够创建、发布和管理可通过API（而不是通过传统的图形用户界面）访问和交互的交互式表单。
...
* 使用分配任务**步骤发送分配任务的通知时，会向分配的个人发送两封电子邮件，而不是一封电子邮件。 (NPR-40078)
