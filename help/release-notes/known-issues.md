---
title: 已知问题
description: 特定于Adobe Experience Manager 6.5已知问题的发行说明
...
* 报告了删除CRX快速入门及其内容的问题。
   ...
   有[知识库](https://helpx.adobe.com/experience-manager/kb/avoid-crx-quickstart-deletion-in-aem-6-5.html)文章可供查阅，其中包含更多详细信息以及此问题的解决方法。
...
* **搜索：** 如果搜索字符串包含前导空格，则搜索不会返回任何结果([OAK-4786](https://issues.apache.org/jira/browse/OAK-4786))
...
* AEM Forms Designer安装程序要求使用32位版本的[Visual C++可再发行运行时包2012](https://support.microsoft.com/zh-cn/help/2977003/the-latest-supported-visual-c-downloads)和[Visual C++可再发行运行时包2013](https://support.microsoft.com/zh-cn/help/3179560/update-for-visual-c-2013-and-visual-c-redistributable-package)。 确保在开始安装之前已安装上述可再发行运行时包。(CQ-4265668)

* PDF生成器不支持基于智能卡的身份验证。  当管理员在Windows服务器上启用组策略`Interactive Logon: Require Smart card`时，所有现有的PDF生成器用户都将失效。

* 当自适应表单配置为动态更新组件的值并且通过调度程序访问托管表单的发布实例时，动态更新字段值的功能将停止工作。要解决此问题，请在发布实例上打开CRXDE，导航到`/libs/fd/af/runtime/clientlibs/guideChartReducer`，然后创建下面列出的属性。
...
* 启动AEM Forms时，会显示`SAX Security Manager could not be setup`警告。
* 在运行AEM FormsReader版本20.10.00的Apple iOS或iPadOS上打开受Adobe Acrobat Document Security保护的PDF时。
