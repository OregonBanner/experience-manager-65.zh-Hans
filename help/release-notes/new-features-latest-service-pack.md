...
* 引入了可选配置，以便从[!DNL Adobe Experience Manager]在ACP API响应中对电子邮件ID进行小写。

   ![配置为从AEM响应ACP中使用小写电子邮件ID](assets/email-lowcase-config.png)

* 根据WCAG，不同位置的文本和图标的对比度（含背景）会得到增强，以便视力和颜色感知受限的用户能够访问该对比度。 有关更多信息，请参阅Assets](sp-release-notes.md#assets-accessibility-6590)中的[辅助功能增强。

### Dynamic Media {#assets-dynamic-media}

* [在以下方面，](sp-release-notes.md#assets-accessibility-6590) Dynamic Media更易于访问：

   * 易于使用键盘键。
   * 各种编辑器中文本、占位符文本和控件的对比度（与背景）。
   * 屏幕阅读器的辅助功能和旁白。

* 智能成像DPR（设备像素比）和网络带宽优化使您能够高效地交付最佳质量的图像；在具有高分辨率显示器和网络带宽受限的设备上。 有关更多信息，请参阅[智能成像常见问题解答](/help/assets/imaging-faq.md)。
...
### 能够使用[!UICONTROL 分配任务] {#group-notification-email}向组发送通知电子邮件

您现在可以使用“分配任务”工作流步骤向组电子邮件地址发送通知电子邮件。

### 能够在修改源交互式通信{#retrieve-draft-after-source-modifications}后检索交互式通信草稿
...
### [!UICONTROL 调用表单数据模型服务]工作流步骤{#input-data-enhancements-fdm}的输入数据增强
...
#### [!DNL Assets](6.5.5.0){#assets-accessibility}中的辅助功能增强功能
...
#### 支持[!DNL Dynamic Media](6.5.5.0){#support-for-3d}中的3D资产
...
#### 列表视图(6.5.3.0)中资产创建日期的可排序列{#sortable-date-created-column}
...
#### Dynamic Media视频配置文件中的智能裁剪(6.5.3.0){#smart-crop-video}
...
#### 根据规则(6.5.8.0){#show-hide-captcha}在自适应表单中显示或隐藏CAPTCHA组件
...
#### 在布局模式(6.5.7.0){#reset-option-layout-mode}下，每个组件的重置选项的可用性
...
#### 在具有双向SSL实施(6.5.6.0){#fdm-integration-rest-apis-two-way-ssl}的服务器上，与RESTful API形成数据模型集成

[!DNL Experience Manager Forms] 表单数据模型现 [在可以与服务器上的RESTful API集成，该服务器上已实施双向SSL](../../help/forms/using/configure-data-sources.md)。

#### 在Automated forms conversion服务(6.5.6.0){#sign-integration-acroform-afcs}中添加了对[!DNL Adobe Sign]文本标记的支持
...
#### 增强了已翻译的自适应表单页面缓存(6.5.6.0){#enhanced-caching-translated-adaptive-forms}
...
#### 附加文件附件组件(6.5.6.0){#attach-multiple-files}的多个文件
...
#### 在Experience ManagerForms工作流(6.5.4.0){#generate-printable-output}中生成可打印的输出
...
#### 在布局模式下(6.5.4.0){#multi-column-adaptive-forms}支持自适应表单和交互式通信的多列支持
...
#### HTML5表单(6.5.4.0){#rich-text-support}中的富文本支持
...
#### 为Experience ManagerForms用户(6.5.3.0){#configure-out-of-office}的收件箱项目配置现成设置
...
## 自[!DNL Adobe Experience Manager] 6.5 SP8 {#key-releases-since-last-sp}以来的关键版本
