...
* AEM工作 [流中现在有新的“生成可打印输出](../forms/using/aem-forms-workflow-step-reference.md) ”步骤可用。
...
* [或者] “平台标记”在标记字段中键入以搜索标记时，标记会显示根边界以外的标记，而不尊重标记字段的 `rootPath` 属性(NPR-31895)。

* [平台UI] 路径浏览器在文本字段中添加了无效路径时会断开(NPR-31884)。
...
* 设计人员：如果启用了标记选项，子表单边框将消失在生成的PDF输出中(NPR-32547)。

* 设计人员：如果启用了标记选项，子表单边框将消失在生成的PDF输出中(NPR-31983、NPR-31950)。
