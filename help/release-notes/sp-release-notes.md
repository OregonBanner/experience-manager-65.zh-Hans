...
| 区域 | 功能 | 替换 |
|---|---|---|
| 集成 | **[!UICONTROL AEM云服务选择加入]**&#x200B;屏幕已弃用。 随着AEM和目标集成在AEM 6.5中更新以支持Target Standard API(通过AdobeIMS和I/O使用身份验证)，以及AdobeLaunch在指导页面以进行分析和个性化方面的作用日益增强，选择加入向导的功能已变得无关紧要。 | 通过各个AEM云服务配置系统连接、AdobeIMS身份验证和[!DNL Adobe I/O]集成。 |
