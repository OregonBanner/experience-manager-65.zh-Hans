...
* 要获取Dynamic MediaScene7的发布URL，将向界 `getRemoteAssetPublishURL` 面添加新 `com.day.cq.dam.api.s7dam.scene7.ImageUrlApi` 方法。
...
* [!UICONTROL 如果资产分析同步作业遇到无效条目] (在Analytics端)而不是转到下一个条目(NPR-32674)，则该作业会停止并失败。
...
* 在使用Dynamic MediaScene7运行模式(NPR-32995)时，将资源从一个文 [!DNL Adobe Experience Manager] 件夹移到另一个文件夹时，资源名称会更改为小写。
...
* 如果在与Dynamic MediaScene7运行模式进行 [!UICONTROL Experience Manager时，默认禁] 用Dynamic Media同步模式，则不发布图像预设(CQ-4294200)。
...
* 6.5.5.0中修复的性能问题包括(CQ-4279206):
   ...
   * Experience Manager缩略图生成时间因Dynamic MediaScene7架构而增加。

* Dynamic MediaScene7迁移问题对于拥有大量资产的客户失败(CQ-4279206)。
...
Experience Manager6.5.5.0的UberJar可在Adobe公共Maven [存储库中获得](https://repo.adobe.com/nexus/content/groups/public/com/adobe/aem/uber-jar/6.5.5/)。
...
本节列表了在AEM 6.5.5.0中标记为已弃用的特性和功能。计划在将来版本中删除的特性将首先设置为已弃用，并设置一个替代选项。

建议客户检查他们是否在当前部署中使用了该功能，并制定计划更改其实施以使用替代选项。

| 区域 | 功能 | 替换 |
|---|---|---|
| 集成 | 已 **[!UICONTROL 弃用AEM cloud services]** “选择加入”屏幕。 随着AEM和目标集成在AEM 6.5中更新以支持Target StandardAPI(通过AdobeIMS和I/O使用身份验证)，以及AdobeLaunch在指导AEM页面进行分析和个性化方面的作用日益增强，选择加入向导的功能已变得无关紧要。 | 通过各个Adobe配置系统连接、AEM cloud servicesIMS身份验证和AdobeI/O集成。 |
...
* 安装AEM 6.5.5.0时，更新版 [!DNL Chrome] 本83会导致构建包时出现问题。 使用其他可用的浏览器(如 [!DNL Internet Explorer] 和 [!DNL Firefox]或其他AEM标准包安装选项)来解决问题。 在安装AEM 6.5.5.0后，问题得到解决。
