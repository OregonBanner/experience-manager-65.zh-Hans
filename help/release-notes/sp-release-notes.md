...
* 批量编辑多个资源的属性时，选择“保存”选项将关闭批量编辑器视图并重定向到主[!DNL Assets]页。 此行为与[!UICONTROL “保存并关闭”]选项的行为相同，不应该这样做(NPR-34546)。

* 存储后，智能收藏夹不显示正确的用户界面设置。 查询已正确保存，但接口始终显示最后添加的“选项”谓词(NPR-34539)。

* 将资源添加到[!DNL Experience Manager]时，不带命名空间的元数据不会导入(NPR-34530)。

* 在文件夹上拖动资源以移动它时，用户界面还显示选项[!UICONTROL Drop in Lightbox]和[!UICONTROL Drop in Collection]。 即使取消移动操作，用户界面仍继续显示后两个选项(NPR-34526)。

* 符号`%>`显示在收藏夹页面上(NPR-34499)。

* 在列视图中，[!DNL Assets]在显示所有资源之前向上和向下滚动时显示重复的文件夹和资源名称(NPR-34464)。

* 如果在创建公共文件夹后立即创建一个专用文件夹，则该公用文件夹将使用专用文件夹设置(NPR-34415)。

* 在卡片视图中，卡片不按字母顺序列出，并且卡片不能按字母顺序排序(NPR-34234)。

* 重新打开层叠规则时，不会在用户界面上保留选择(CQ-4301452)。

#### [!DNL Dynamic Media] {#dynamic-media-6570}

* 对[!DNL Dynamic Media]中的辅助功能进行了以下增强(CQ-4290306)。 有关详细信息，请参阅 [!DNL Dynamic Media]](/help/assets/accessibility-dm.md)中的[辅助功能。

   * 屏幕阅读器(JAWS、Narrator)在“嵌入大小”菜单选项中为菜单项叙述名称、角色和状态(CQ-4290927)。
   * 用户可以使用`Tab`键导航“电子邮件链接”对话框(CQ-4290926)。
   * 鉴于屏幕阅读器增强功能(CQ-4290623、CQ-4290622)，创建视频编码配置文件的工作流程更加用户友好。
   * 使用`Tab`键导航时，焦点将移到工作流中的相应用户界面元素，以创建交互式视频(CQ-4290621、CQ-4290620、CQ-4290619)。
   * “发布”页面、“编辑资源”页面、“编辑智能裁剪”页面和“图像集编辑器”页面经过改进，符合Web标准。 辅助技术(AT)用户现在可以轻松浏览这些页面并执行裁剪图像等操作(CQ-4290617、CQ-4290616、CQ-4290613、CQ-4290612、CQ-4290610、CQ-4290614)。
   * 查看器经过改进，可让用户使用键盘进行导航(CQ-4290615)。
...
* 在向[!DNL Dynamic Media]图像添加热点后或在编辑带图像的[!DNL Dynamic Media]视频或[!DNL Experience Fragment]后尝试预览输出时，z轴顺序不正确(CQ-4307267)。

* [!DNL Dynamic Media] 在重新处理混合媒体集时同步失败(CQ-4307184)。

* 如果将资源移动到其上配置了自动同步到[!DNL Dynamic Media]的文件夹，则该资源不会同步(CQ-4307122)。

* [!DNL Dynamic Media] iOS设备上没有使用本机HTML5视频控件播放视频(CQ-4306977、CQ-4306727)。
...
* 在[!UICONTROL 查看器预设编辑器]中，当编辑[!UICONTROL PanoramicImage/PanoramicImage_VR]预设时，在`PanoramicView`组件中，`PANORAMICVIEW_AUTOROTATE`修饰符标签不可用(CQ-4302443)。

* 如果视频不是MixedMediaSet中的第一个视频，则不显示视频题注(CQ-4298161)。

* iPhone移动设备上的HTML5 eCatalog Viewer无法翻页或翻页(CQ-4296611)。

* 在移动设备上滚动色板时，色板滚动到可见区域的右侧和外侧几秒钟，然后滚回视图(CQ-4296439)。

* 创建查看器预设主记录时，不会发布CSS和图稿，而只发布查看器预设(CQ-4262205)。

* 尝试链接[!UICONTROL 交互式视频／图像]组件中给定热点的体验片段时，它不显示选定的体验片段路径。 而是从路径字段返回一个空值(NPR-35146、CQ-4298136)。

* 无法在IVV编辑器中预览体验片段(CQ-4308560)。

* 在向图像中添加热点并选择体验片段时，无法选择该体验片段的子文件夹和变体(CQ-4307455)。

* 上传后，非图像资源不显示为已发布(CQ-4306415)。

#### [!DNL Experience Manager] 3D资源  {#three-d-assets-6570}

* `DAM CQ MIME Type` 服务对3D资源应用不正确的MIME类型，导致呈现不正确(NPR-34731)。

### [!DNL Commerce] {#commerce-6570}

* “商务”产品集合用户界面不列出集合中超过15种产品(NPR-34502)。

### 平台 {#platform-6570}

* 通过HTTPS的HTTP会话未失效(NPR-35083)。
* 从用户界面启动每日或每周维护任务时，返回`NullPointerException`(NPR-34953)。
* W3C验证程序报告兼容客户端库JavaScript文件的警告(NPR-34898)。
* `AudienceOmniSearchHandler`函数使用不推荐使用的索引(NPR-34870)。
* 从Experience Manager注销不会清除Cookie(NPR-34743)。
* 如果标签名称包含特殊字符，则`TagManager` API的`findByTitle`函数将不起作用(NPR-34357)。
* 导入用户同步包的过程失败(NPR-34399)。
* 为`Coral.Masonry`组件添加了对`ariaLabel`和`ariaLabelledby`属性的支持(GRANITE-29962)。
* 在安装最新的核心组件包(CQ-4306788)后，不会刷新包含内容片段的页面的调度程序缓存。
* 在用户界面上无法正确显示带引号(`"`)的本地化标签名称(CQ-4305439)。

### 用户界面 {#ui-6570}

* 组件属性中的[!UICONTROL 链接到]字段会显示与指定字符串不匹配的自动完成建议(NPR-34865)。

* AEM在计划在2天之间分发的每日维护窗口时显示以下错误消息(NPR-35280):
...
* 从文件夹导航到[!UICONTROL Audiences]页面时，[!UICONTROL 创建]选项显示在[!UICONTROL Audiences]页面(NPR-35151)。
...
* 在[!DNL Experience Manager]中取消翻译项目时，取消该项目的请求不会发送给翻译提供商(NPR-34433)。
...
* 在[!UICONTROL 列表视图]中选择资源时，用户界面不响应(NPR-34728)。

### [!DNL Forms] {#forms-6570}

>[!NOTE]
>
>[!DNL Experience Manager Forms] 在Service Pack预定发布日期后一周内发 [!DNL Experience Manager] 布加载项包。
...
* 在应用[!DNL Experience Manager] Service Pack 6(NPR-35126)后，无法使用经典UI编辑自适应表单。

* 将PDF转换为自适应表单时，无法在选项卡式布局上使用表单数据模型为嵌套面板设置值。 此外，使用代码编辑器动态地使用静态数组设置单选按钮组值时，也会出现问题(NPR-35062)。

* 在自适应表单的文本字段组件中输入日文字符时，可以指定比35个字符的最大限制(NPR-35039)还要多的字符。

* 自适应表单在提交表单后显示的&#x200B;**[!UICONTROL 感谢]**&#x200B;页面上显示不需要的参数，如`owner`和`status`(NPR-34989)。

* [!UICONTROL Attachment]组件的[!UICONTROL 文件选择]对话框显示不支持的文件类型以及在自适应表单提交期间导致错误的选择(NPR-34970)。

* 在[!DNL Experience Manager Sites]页面中插入一个在表单之前包含文本的自适应表单时，光标焦点将直接移到表单而不是表单之前的文本(NPR-34947)。

* [!UICONTROL 使用] Data选项预览使用6.2数据XML [!DNL Experience Manager] 文件预填自适应表单无法正常工作(NPR-35087)。

* 更新自适应表单的数据字典时，不会转换表单，因为自适应表单返回缓存值(NPR-34845)。

* 由于缓存失效，片段以自适应形式加载需要较长的时间(NPR-34567)。

* Tab键导航不适用于自适应格式的屏幕阅读器(NPR-34544)。

**通信管理**

* 无法将包含浮点类型的XML标签的值保存为草稿(NPR-35050)。

* 从ES3迁移资源时，资源包括两个不可编辑的默认条件(NPR-34972)。

* 编辑字母中的数据字典时，[!UICONTROL “借出内容”]部分显示旋转的矩形，而不是有用信息(NPR-34853)。

**交互式通信**

* 安装[!DNL Forms]加载项包后，Interactive Communication的转出配置名称将重复标准的转出配置名称(NPR-34976)。

**文档安全**

* 保存新文档安全策略时，FormsExperience Manager会显示`Relative validity period is required`错误消息(NPR-34679)。

* 文档安全性无法保护PDF 2.0文档(CQ-4305851)。
...
* Service Pack下载可在Adobe[软件分发](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html)上找到。
...
1. 如果实例处于更新模式，则在安装前重新启动该实例（从早期版本更新该实例时也是如此）。 Adobe还建议在实例的当前正常运行时间较高时重新启动。
...
1. 打开包管理器，然后单击“上传包”**[!UICONTROL 以上传包。]**&#x200B;要了解更多信息，请参阅[包管理器](/help/sites-administering/package-manager.md)。

1. 选择软件包，然后单击&#x200B;**[!UICONTROL 安装]**。

1. 要更新S3连接器，请在安装Service Pack后停止该实例，用安装文件夹中提供的新二进制文件替换现有连接器，然后重新启动该实例。 请参阅[AmazonS3数据存储](/help/sites-deploying/data-store-config.md#upgrading-to-a-new-version-of-the-s-connector)。

>[!NOTE]
>
>包管理器UI上的对话框有时在安装服务包时退出。 Adobe建议您在访问部署之前等待错误日志稳定。 请等待与卸载更新程序捆绑相关的特定日志，然后确保安装成功。 通常，这会发生在[!DNL Safari]上，但可能间歇性地发生在任何浏览器上。

**自动安装**

在工作实例上自动安装Adobe Experience Manager6.5.7.0有两种方法：

A.当服务器联机可用时，将包放入`../crx-quickstart/install`文件夹。 包会自动安装。

B.使用包管理器](https://helpx.adobe.com/cn/experience-manager/aem-previous-versions.html)中的[ HTTP API。 使用`cmd=install&recursive=true`以便安装嵌套包。
...
1. 所有OSGi捆绑包均位于OSGi控制台(使用Web控制台：`/system/console/bundles`)。********

1. OSGi捆绑`org.apache.jackrabbit.oak-core`版本为1.22.3或更高版本(使用Web控制台：`/system/console/bundles`)。
...
>[!NOTE]
>
>AEM 6.5.7.0包含新版本的[AEM Forms兼容性软件包](https://experienceleague.adobe.com/docs/experience-manager-release-information/aem-release-updates/forms-updates/aem-forms-releases.html?lang=en#aem-65-forms-releases)。 如果您使用的是旧版AEM Forms兼容性软件包并更新到AEM 6.5.7.0，请安装安装Forms加载项软件包后安装该软件包的最新版本。

### 在JEE上安装Adobe Experience Manager Forms{#install-aem-forms-jee-installer}

>[!NOTE]
>
>如果您未在 JEE 上使用 AEM Forms，请跳过。在Adobe Experience Manager Forms,JEE上的修复通过单独的安装程序提供。

有关在JEE上安装FormsExperience Manager的累积安装程序和部署后配置的信息，请参阅[发行说明](jee-patch-installer-65.md)。

### UberJar {#uber-jar}

[ Maven Central存储库](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.7/)中提供了用于Experience Manager6.5.7.0的UberJar。

要在Maven项目中使用UberJar，请参阅[如何使用UberJar](/help/sites-developing/ht-projects-maven.md)，并在项目POM中包括以下依赖项：
...
>[!NOTE]
>
>UberJar和其他相关对象可在Maven Central Repository上使用，而不是AdobePublic Maven Repository(`repo.adobe.com`)。 主UberJar文件被重命名为`uber-jar-<version>.jar`。 因此，`dependency`标签没有`classifier`（以`apis`为值）。

## 已弃用功能 {#removed-deprecated-features}

以下是[!DNL Experience Manager] 6.5.7.0中标记为已弃用的功能列表。在将来的版本中，已标记为已弃用的功能，在最初和以后的版本中已删除。 通常提供替代选项。

在部署中使用功能时查看。 此外，计划更改实施以使用替代选项。
...
| 连接器 | AdobeJCR Connector for Microsoft SharePoint 2010和Microsoft SharePoint 2013已在AEM 6.5中弃用。 | 不适用 |

## 已知问题 {#known-issues}

* 在安装6.5.7.0Experience Manager时，请忽略`error.log`文件中的以下错误：
...
* 如果要将[!DNL Experience Manager]实例从6.5版升级到6.5.7.0版，可以在`error.log`文件中查看`RRD4JReporter`异常。 重新启动实例以解决此问题。

* 如果您在[!DNL Experience Manager] 6.5上安装[!DNL Experience Manager] 6.5 Service Pack 5或以前的Service Pack，则会删除资源自定义工作流模型（在`/var/workflow/models/dam`中创建）的运行时副本。
要检索运行时副本，Adobe建议使用HTTP API将自定义工作流模型的设计时副本与其运行时副本同步：
   `<designModelPath>/jcr:content.generate.json`。

* 如果在[!UICONTROL 文件夹元数据架构Forms编辑器]和[!UICONTROL 元数据架构Forms编辑器]中编辑和创建层叠规则时遇到问题，请与Adobe客户关怀联系。 已创建和保存的规则正在按预期工作。

* 如果层次结构中的文件夹在[!DNL Experience Manager Assets]中重命名，并且包含资源的嵌套文件夹发布到[!DNL Brand Portal]，则在[!DNL Brand Portal]中不会更新该文件夹的标题，直到根文件夹再次发布。

* 当用户首次选择以自适应表单配置域时，用于保存配置的选项不会显示在属性浏览器中。 选择在同一编辑器中配置自适应表单的其他字段可解决此问题。

* 如果[!UICONTROL 连接的资源配置向导在安装后返回404错误消息，请使用包管理器手动重新安装`cq-remotedam-client-ui-content`和`cq-remotedam-client-ui-components`包。]

* 在安装AEM 6.5.x.x时，可能会显示以下错误和警告消息：
