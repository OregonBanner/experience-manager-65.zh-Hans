...
创建已启用主题列表的逻辑首先允许允许列表中的所有主题，然后删除阻止列表中的主题。 默认情况下，会启用所有主题(允许列表值为`*`)，并且不禁用任何主题(阻止列表没有值)。
...
|---|---|---|
| 主题允许列表 | job.consumermanager.whitelist | 本地JobManager服务处理的主题列表。 &amp;ast；的默认值会将所有主题发送到注册的TopicConsumer服务。 |
...
1. 在作者上创建&#x200B;**复制代理**。 （请参阅[复制代理文档](/help/sites-deploying/replication.md)）。 指定任意&#x200B;**标题**。 **名称**&#x200B;必须遵循命名约定。
1. 使用以下属性创建代理：
...
1. 在作者上创建&#x200B;**反向复制代理**。 （请参阅[复制代理文档](/help/sites-deploying/replication.md)。） 指定任意&#x200B;**标题**。 **名称**&#x200B;必须遵循命名约定。
1. 使用以下属性创建代理：
...
1. 在工作实例上创建&#x200B;**复制代理**。 （请参阅[复制代理文档](/help/sites-deploying/replication.md)。） 指定任意&#x200B;**标题**。 **名称**&#x200B;必须为`offloading_outbox`。
1. 使用以下属性创建代理。
