...
>
>此外，您还可以查看 [性能调整提示](https://experienceleague.adobe.com/docs/experience-manager-65/deploying/configuring/configuring-performance.html?lang=zh-Hans).

一个关键问题是您的网站响应访客请求所花费的时间。 尽管该值会因每个请求而异，但可以定义平均目标值。 一旦该价值被证明是可实现的且可维护的，便可用于监控网站的性能并指示潜在问题的发展。

您计划在创作和发布环境中响应的时间不同，这反映了目标受众的不同特征：

## 创作环境 {#author-environment}

作者可使用此环境输入和更新内容。 更新内容页面和这些页面上的各个元素时，它必须满足一些用户的需求，这些用户在更新内容页面时都会生成大量需要大量执行操作的请求。

## 发布环境 {#publish-environment}

此环境包含可供用户使用的内容。 在这里，请求数量更多，速度也同样重要。 但由于请求的性质不那么动态，因此可以应用额外的性能增强机制；例如缓存内容或负载平衡。

>[!NOTE]
>
>* 配置以优化性能后，请按照 [艰难的一天](/help/sites-developing/tough-day.md) 来测试环境。
>* 另请参阅 [性能调整提示。](https://experienceleague.adobe.com/docs/experience-manager-65/deploying/configuring/configuring-performance.html?lang=zh-Hans)


## 性能优化方法 {#performance-optimization-methodology}

AEM项目的性能优化方法可以归纳为五个简单的规则，可以遵循这些规则以避免从一开始就出现的性能问题：
...
这些规则通常适用于Web项目，并且与项目经理和系统管理员相关，以确保其项目在启动时不会面临性能挑战。

### 优化计划 {#planning-for-optimization}

![chlimage_1-3](assets/chlimage_1-3.jpeg)

在性能优化阶段计划大约10%的项目工作。 实际的性能优化要求取决于项目的复杂性级别和开发团队的经验。 虽然您的项目（最终）可能不需要分配的时间，但最好始终在建议的区域中规划性能优化。

在可能的情况下，首先应向有限的受众软启动项目，以收集实际体验并执行进一步的优化，而无需在发布完整公告后承受额外压力。

在您处于“实时”状态后，性能优化还未结束。 现在，当您在系统上体验到“真实”负载时。 在启动后计划进一步调整很重要。
...
如果您与网站一起上线，然后在启动后发现您遇到性能问题，这可能是因为您的负载和性能测试没有充分地模拟现实。

模拟现实是困难的，而要投入多少精力来实现“真实”取决于项目的性质。 “真实”不仅表示“真实代码”和“真实流量”，还表示“真实内容”，尤其是有关内容大小和结构的内容。 模板的行为可能会因存储库的大小和结构而异。

### 确立坚实的目标 {#establish-solid-goals}

![chlimage_1-5](assets/chlimage_1-5.jpeg)

正确制定业绩目标的重要性不容低估。 通常，在人们关注特定绩效目标后，即使这些目标是基于假设，也很难在以后改变这些目标。
...
一次优化一个瓶颈很重要。 如果您尝试在不验证某个优化的影响的情况下并行执行某些操作，则可能会丢失对哪些优化措施有帮助的跟踪。

### 敏捷迭代周期 {#agile-iteration-cycles}

![chlimage_1-7](assets/chlimage_1-7.jpeg)

性能调整是一个迭代过程，包括测量、分析、优化和验证，直到达到目标。 要考虑这方面，请在优化阶段实施灵活的验证过程，而不是在每次迭代后执行更重的测试过程。

此焦点意味着实施优化的开发人员应该能够快速判断优化是否已达到目标。 此信息非常有价值，因为当达到目标时，优化就会结束。
...
* 25%的页面请求应在100毫秒 — 300毫秒内收到响应。
* 4%的页面请求应在300毫秒 — 500毫秒内获得响应。
* 1%的页面请求应在500毫秒 — 1000毫秒内收到响应。
* 任何页面的响应速度都不应低于1秒。
...
JVM和操作系统级别的调整通常不会导致性能的显着飞跃，因此应在优化周期的最后阶段执行。

内容存储库的结构方式也会影响性能。 为获得最佳性能，附加到内容存储库中各个节点的子节点数不应超过1,000（作为规则）。
...
* 进行编辑的时间范围（通常是工作日的时长，对于国际操作更多）。
* 上传图像的平均大小（以及每个图像生成的演绎版的大小）(MB)。
* 确定平均数据率：

![chlimage_1-78](assets/chlimage_1-78.png)

* 80%的编辑都是在20%的时间内完成的，因此在峰值时间，您的数据率是平均数据率的四倍。 这样的绩效是您的目标。

## 性能监控 {#performance-monitoring}

性能（或者缺乏性能）是用户首先注意到的事情之一，因此，与任何具有用户界面的应用程序一样，性能至关重要。 要优化AEM安装的性能，请监控实例的各种属性及其行为。
...
基本起点是系统正常运行时对系统的良好了解。 除非您知道环境在正常运行时的“外观”和“行为”如何，否则在性能恶化时很难找到问题。 在系统运行顺利时，请花时间调查系统，并确保收集性能信息是一项持续的任务。 这样，您就可以在性能受到影响时进行比较。
...
>[!NOTE]
>
>请记住，用于衡量绩效的机制通常会影响您尝试衡量的内容。 设法解决这些差异，尽可能消除其影响；特别是，应尽可能取消激活浏览器插件。
...
要提高性能，请限制同时运行的工作流进程数量。 默认情况下，工作流引擎会并行处理任意数量的工作流，只要Java™ VM有可用的处理器。 当工作流步骤需要大量处理资源（RAM或CPU）时，并行运行其中几个工作流可能会对可用的服务器资源提出很高要求。

例如，在上传图像（或通常为DAM资产）时，工作流会自动将图像导入DAM。 图像通常具有高分辨率，并且处理时容易消耗数百MB的堆。 并行处理这些图像会给存储子系统和垃圾收集器带来高负载。
...
* Granite工作流外部进程作业队列：此服务用于特殊的外部工作流步骤，通常用于联系外部系统和轮询结果。 例如，InDesign媒体提取流程步骤将作为外部流程实施。 工作流引擎使用外部队列处理轮询。 (请参阅 [com.day.cq.workflow.exec.WorkflowExternalProcess](https://developer.adobe.com/experience-manager/reference-materials/6-5/javadoc/com/day/cq/workflow/exec/WorkflowExternalProcess.html).)
...
如果要配置服务 [使用sling:OsgiConfig节点](/help/sites-deploying/configuring-osgi.md#adding-a-new-configuration-to-the-repository)，则必须找到现有服务的PID，例如：org.apache.sling.event.jobs.QueueConfiguration.370aad73-d01b-4a0b-abe4-20198d85f705。 您可以使用Web控制台发现PID。
...
的 `AssetSynchronizationService` 用于同步来自已装载存储库(包括LiveLink、Documentum®等)的资产。 默认情况下，此同步每300秒（5分钟）进行一次定期检查，因此如果不使用装载的存储库，则可以禁用此服务。

禁用服务的方法是 [配置OSGi服务](/help/sites-deploying/configuring-osgi.md) **CQ DAM资产同步服务** 设置 **同步期** ( `scheduler.period`)至（最少）一年（以秒为单位定义）。

### 多个DAM实例 {#multiple-dam-instances}

例如，部署多个DAM实例有助于提高性能：

* 由于为创作环境定期上传许多资产，因此负载较高；此处，可以专门为作者提供单独的DAM实例。
* 您在全球各地（例如，美国、欧洲、亚洲）拥有多个团队。

其他注意事项包括：

* 在发布时将作者的“在制品”与“最终”分开
* 在发布时将创作内部用户与外部访客/用户分开（例如，代理、新闻代表、客户和学生）。

## 质量保证最佳实践 {#best-practices-for-quality-assurance}

性能对发布环境至关重要。 因此，在实施项目时，您必须仔细规划并分析针对发布环境进行的性能测试。

本节旨在对定义专门用于您的性能测试的测试概念所涉及的问题进行标准化概述 *发布* 环境。 此信息主要对QA工程师、项目经理和系统管理员感兴趣。

下面介绍了对上的AEM应用程序进行性能测试的标准化方法 *发布* 环境。 此性能测试涉及以下五个阶段：
...
控制是一个额外的、包罗万象的过程，是必要的，但不限于测试。

### 知识验证 {#verification-of-knowledge}

第一步是记录在开始测试之前必须了解的基本信息：

* 测试环境的架构
* 详细描述需要测试的内部元素（隔离和组合）的应用程序图
...
获取一个清晰的概述，您可以从中创建整个应用程序的映射（您可能已经在创作环境的测试中拥有此映射）。

应用程序内部元素的图表表示，可以提供测试要求的概述；通过颜色编码，还可以用作报告的基础。

### 范围定义 {#scope-definition}

应用程序通常有一系列用例。 有些用例很重要，而有些则不太重要。

要将性能测试的范围集中在发布上，Adobe建议您定义以下内容：
...
* 每个组件在与性能相关时都有一个特定的中断点。 也就是说，组件可以显示良好的性能，直到达到特定的点为止，之后性能会迅速下降。
* 要获取应用程序的完整概述，您必须首先验证组件以确定何时到达每个断点。
* 要查找可以执行加载测试的断点，其中，在一段时间内，您需要增加用户数量以创建不断增加的加载。 通过监控此负载和组件的响应，您会在到达组件的断点时遇到特定的性能行为。 该点可以按每秒并发事务数以及并发用户数（如果组件对此KPI敏感）来进行鉴别。
* 然后，此信息可作为改进的基准，指示所使用度量的效率，并帮助定义测试情景。

#### 交易 {#transactions}

* 术语事务用于表示完整网页的请求，包括页面本身和所有后续调用。 即，页面请求、任何AJAX调用、图像和其他对象 **请求向下钻取**.
* 要充分分析每个请求，您可以表示调用堆栈的每个元素，然后合计每个请求的平均处理时间。

### 定义绩效目标 {#defining-the-performance-goals}

定义范围和相关KPI后，会设置特定的绩效目标。 此过程涉及设计测试方案和目标值。

在平均和峰值条件下测试性能。 此外，您还需要上线方案测试，以确保在网站首次可用时能够满足客户对网站日益浓厚的兴趣。
...
在您的网站发布后的最初几天，您可能会有更高的兴趣级别。 此方案甚至比您正在测试的峰值还要大。 Adobe建议您测试上线方案，以确保系统能够满足此情况。
...
测试错误情景，以确保系统能正确、适当地反应。 不仅在于错误本身的处理方式，还在于它可能对性能的影响。 例如：

* 当用户尝试在搜索框中输入无效搜索词时会发生什么情况
* 当搜索词非常一般，以致返回过多的结果时会发生什么情况

在设计这些测试时，应当记住并非所有情景都会定期发生。 但是，它们对整个系统的影响很重要。
...
某些问题只有在系统连续运行一段时间（小时或天）后才会出现。 使用耐久性测试来测试在所需时间段内的恒定平均负载。 然后可以分析任何性能下降。
...
提供了一系列工具，可帮助您进行负载生成、性能监控和结果分析。 其中一些工具包括：
...
持续报告可让每个人都了解状态。 如前面使用颜色编码时所述，架构图可用于此状态。
...
的 [Dispatcher](https://experienceleague.adobe.com/docs/experience-manager-dispatcher/using/dispatcher.html?lang=zh-Hans) 是Adobe的缓存和/或负载平衡工具。 使用Dispatcher时，请考虑优化您的网站以提高缓存性能。
...
Dispatcher提供了多种内置机制，在您的网站利用这些机制时，您可以使用这些机制来优化性能。 此部分介绍了如何设计您的网站以最大限度地提高缓存优势。

>[!NOTE]
>
>Dispatcher 将缓存存储在标准 Web 服务器上，记住这一点可能会对您有所帮助。了解此信息意味着您可以缓存所有可以作为页面存储的内容，并使用URL来请求。 而且，您无法存储其他内容，例如Cookie、会话数据和表单数据。
>
>通常，许多缓存策略都涉及选择良好的URL，而不依赖这些附加数据。
>
>使用Dispatcher版本4.1.11，您还可以缓存响应标头，请参阅 [缓存HTTP响应头](https://experienceleague.adobe.com/docs/experience-manager-dispatcher/using/configuring/dispatcher-configuration.html?lang=en#configuring-the-dispatcher-cache-cache).
...
如果您没有一对一的发布者/调度程序配对，请将所有调度程序和发布者的请求添加到一起，以获得准确的测量。 另请参阅 [推荐的部署](/help/sites-deploying/recommended-deploys.md).
...
使用Dispatcher版本4.1.11，您可以缓存响应标头。 如果不在Dispatcher上缓存响应标头，则在标头中存储页面编码信息时可能会出现问题。 在此情况下，当 Dispatcher 从缓存中提供一个页面时，Web 服务器的默认编码将用于该页面。可通过两种方式避免此问题：

* 如果您只使用一种编码，请确保 Web 服务器上使用的编码与 AEM 网站的默认编码相同。
* 要设置编码，请使用 `<META>` 标记在HTML中 `head` 部分，如以下示例中所示：
...
>[!NOTE]
>
>对于大多数布局方面，还可以使用样式表或客户端脚本，或者同时使用这两者。 这些工具在缓存方面非常有效。
>
>此策略对于打印版本也非常有用，在该版本中，您可以使用URL，例如：
>
...
如果将图片用于导航条目，则方法与标题基本相同，但稍微更为复杂。 将所有导航图像与目标页面一起存储。如果将两张图片用于一般或活动场景，则可以使用以下脚本：
...
对于未修改的页面，尽管页面本身会自动失效，但图片仍保留在缓存中。
...
* 相反，如果您提供十个不同的开始页，则可以缓存每个开始页，从而提高性能。
...
如果通过将用户名称置于标题栏来个性化每个页面（例如），则会影响性能。
...
关于在一个页面上混合使用受限内容和公共内容，请考虑使用Dispatcher中服务器端包含或浏览器中客户端包含的Ajax方式的策略。
...
[粘性连接](https://experienceleague.adobe.com/docs/experience-manager-dispatcher/using/dispatcher.html?lang=en#the-benefits-of-load-balancing)可确保同一个用户的文档全部在同一服务器上撰写。如果用户在退出此文件夹不久后返回，则此连接仍保持粘性。要保存要求网站具有粘性连接的所有文档，请定义一个文件夹。 尽量不要在该文件夹中放入其他文件。如果您使用个性化页面和会话数据，此方案会影响负载平衡。
...
使用Dispatcher版本4.1.11，您可以缓存响应标头。 如果不在Dispatcher上缓存响应标头，则MIME类型是HTTP标头的一部分。 因此，如果AEM应用程序返回的文件没有可识别的文件结尾，而是依赖MIME类型，则这些文件可能会被错误地显示。

要确保正确缓存文件，请遵循以下准则：

* 确保文件始终具有正确的扩展名。
* 避免使用通用文件服务脚本，这些脚本具有URL，例如 `download.jsp?file=2214`. 要使用包含文件规范的URL，请重写脚本。 对于上一个示例，此重写为 `download.2214.pdf`.

## 备份性能 {#backup-performance}

本节介绍一系列用于评估AEM备份性能和备份活动对应用程序性能的影响的基准。 AEM备份在系统运行时对系统造成很大负载，并且Adobe会测量此影响以及尝试调制这些影响的备份延迟设置的影响。 其目标是提供一些参考数据，说明实际配置中备份的预期性能和生产数据的数量，并就如何估算计划系统的备份时间提供指导。

### 参考环境 {#reference-environment}

#### 物理系统 {#physical-system}

本文档中报告的结果是从参考环境中运行的基准获得的，其配置如下。 此配置类似于数据中心中的典型生产环境：
...
此服务器上的磁盘子系统速度很快，代表了可能在生产服务器中使用的高性能RAID配置。 备份性能对磁盘性能很敏感，此环境中的结果反映了快速RAID配置的性能。 VMWare映像配置为在RAID阵列上具有物理驻留在本地磁盘存储中的单个大磁盘卷。

AEM配置将存储库和数据存储放在同一逻辑卷上，与操作系统和AEM软件一起放置。 备份的目标目录也位于此逻辑文件系统上。

#### 数据卷 {#data-volumes}

下表说明了备份基准中使用的数据卷的大小。 首先安装初始基准内容，然后添加额外的已知数据量以增加备份内容的大小。 以特定增量创建备份，以表示内容的大幅增加以及一天内可能产生的内容。 内容（页面、图像、标记）的分发大致基于真实的生产资产组合。 页面、图像和标记最多限制为800个子页面。 每个页面都包含标题、Flash、文本/图像、视频、幻灯片、表单、表格、云和轮播组件。 图像从400个唯一文件大小（37 KB到594 KB）的池中上传。
...
加载包含创建的页面、删除的页面、遍历和查询，其中来自页面遍历和查询的加载最多。 添加和删除过多页面会不断增大工作区大小，并阻止备份完成。 脚本使用的加载分布为75%的页面遍历、24%的查询和1%的页面创建（单个级别没有嵌套子页面）。 在空闲系统上，通过四个并行线程（在负载下测试备份时使用）实现每秒峰值平均事务量。

负载对备份性能的影响可以通过此应用程序负载时和不时的性能差异来估计。 备份对应用程序吞吐量的影响可通过比较每小时事务中的方案吞吐量（包括和不包括持续的并发备份）和备份以不同的“备份延迟”设置运行时的情况吞吐量来发现。

* **延迟设置**  — 对于某些情况，备份延迟设置也会有所不同，使用10毫秒（默认）、1毫秒和0毫秒的值，以了解此设置如何影响备份性能。
* **备份类型**  — 所有备份都是将存储库的外部备份到备份目录而不创建zip文件，但在一个比较中，tar命令直接使用的情况除外。 由于增量备份不能创建到zip文件，或者当以前的完全备份是zip文件时，备份目录方法在生产情况中最常使用。

### 结果摘要 {#summary-of-results}

#### 备份时间和吞吐量 {#backup-time-and-throughput}

这些基准的主要结果是显示备份时间随备份类型和总数据量的变化情况。 下图显示了使用默认备份配置获得的备份时间，该时间与总页数成正比。

![chlimage_1-81](assets/chlimage_1-81.png)

空闲实例的备份时间相当一致，平均每秒0.608 MB，而不考虑完整备份或增量备份（请参阅下图）。 备份时间只是备份数据量的函数。 完成完整备份的时间会随着页面总数的增加而明显增加。 完成增量备份的时间也会随着页面总数而增加，但速度要低得多。 由于备份的数据量相对较小，完成增量备份所花费的时间要短得多。

备份的大小是完成备份所花费的时间的主要决定因素。 下图显示了作为最终备份大小的函数所需的时间。

![chlimage_1-82](assets/chlimage_1-82.png)

此图表说明，增量备份和完整备份都遵循一种简单的大小与时间模式，Adobe可以将此模式作为吞吐量进行衡量。 空闲实例上的备份时间相当一致，无论基准环境中的完整备份或增量备份，平均每秒0.61 MB。

#### 备份延迟 {#backup-delay}

提供备份延迟参数以限制备份可能干扰生产工作负载的程度。 参数指定的等待时间（以毫秒为单位），将逐个文件插入到备份操作中。 整体效果部分取决于受影响文件的大小。 以MB/秒为单位测量备份性能为比较延迟对备份的影响提供了一种合理的方法。

* 与常规应用程序负载同时运行备份会对常规负载的吞吐量产生负面影响。
* 影响可能很小（只有5%）或很大，导致吞吐量下降75%。 这可能主要取决于应用程序。
* 备份在CPU上并不是负载过重，因此与I/O密集型工作负载相比，CPU密集型生产工作负载受备份影响较小。

![chlimage_1-83](assets/chlimage_1-83.png)

为了进行比较，使用文件系统备份(“tar”)备份同一存储库文件时获得的吞吐量。 tar的性能相当，但略高于将延迟设置为零的备份。 即使设置小的延迟也会大大降低备份吞吐量，而默认的延迟为10毫秒，则会大大降低吞吐量。 如果在总体应用程序使用率较低或应用程序可能空闲时可能安排备份，请将延迟减少到默认值以下，以便备份能够更快地进行。

持续备份的应用程序吞吐量的实际影响取决于应用程序和基础架构的详细信息。 延迟值的选择应通过对应用程序的经验分析来完成，但应尽可能选择小的延迟值，以便能够尽快完成备份。 由于选择延迟值与对应用程序吞吐量的影响之间只存在微弱的关联，因此选择延迟应该有利于缩短总体备份时间以最大限度地减少备份的总体影响。 备份需要8小时才能完成，但吞吐量影响为–20%，其整体影响可能比完成需要2小时而吞吐量影响为–30%的备份更大。
