...
>
>此外，您还可以查看知识库文章，网址为 [性能调整提示。](https://helpx.adobe.com/experience-manager/kb/performance-tuning-tips.html)

关键问题是网站响应访客请求所用的时间。 尽管该值会因每个请求而异，但可以定义一个平均目标值。 一旦证明这个值是可实现的和可维护的，它就可用于监控网站的性能并指示潜在问题的发展。

您针对的响应时间在创作环境和发布环境中是不同的，反映了目标受众的不同特征：

## 创作环境 {#author-environment}

输入和更新内容的作者使用此环境。 它必须满足少数用户的需求，这些用户在更新内容页面和这些页面上的单个元素时，会分别生成大量性能密集型请求。

## 发布环境 {#publish-environment}

此环境包含可供用户使用的内容。 在这里，请求的数量甚至更多，速度也同等重要，但由于请求的动态性较低，可以应用额外的性能增强机制；例如缓存内容或负载平衡。

>[!NOTE]
>
>* 配置以进行性能优化后，请按照中的步骤操作 [艰难的一天](/help/sites-developing/tough-day.md) 在重负载下测试环境。
...
AEM项目的性能优化方法可以归结为五个非常简单的规则，这些规则可以遵循，以避免从一开始就出现性能问题：

1. [规划优化](#planning-for-optimization)
1. [模拟现实](#simulate-reality)
1. [制定明确的目标](#establish-solid-goals)
1. [保持相关](#stay-relevant)
1. [敏捷迭代周期](#agile-iteration-cycles)

这些规则在很大程度上适用于Web项目，并且与项目经理和系统管理员相关，以确保他们的项目在启动时不会面临性能挑战。

### 规划优化 {#planning-for-optimization}

![chlimage_1-3](assets/chlimage_1-3.jpeg)

在性能优化阶段，应规划大约10%的项目工作。 当然，实际性能优化要求将取决于项目的复杂程度以及开发团队的经验。 虽然您的项目可能（最终）不需要所有分配时间，但最好始终在建议的区域规划性能优化。

如有可能，项目应首先面向有限受众软启动，以收集现实生活体验并执行进一步优化，而不会在完整发布后产生额外压力。

一旦您处于“实时”状态，性能优化就不会结束。 这是您体验到系统上的“实际”负载的时间点。 在启动后计划进行其他调整很重要。
...
如果您在启动后访问网站并发现您遇到了性能问题，则只有一个原因：您的负载和性能测试没有足够接近真实情况。

模拟现实是困难的，您合理希望为实现“真实”投入多少努力取决于项目的性质。 “真实”不仅表示“真实代码”和“真实流量”，还表示“真实内容”，尤其是关于内容大小和结构。 请记住，模板的行为可能完全不同，具体取决于存储库的大小和结构。

### 制定明确的目标 {#establish-solid-goals}

![chlimage_1-5](assets/chlimage_1-5.jpeg)

正确确定业绩目标的重要性不容低估。 通常，一旦人们专注于特定的绩效目标，以后就很难改变这些目标，即使这些目标基于疯狂的假设。
...
一次优化一个瓶颈是很重要的。 如果您尝试并行执行一些操作而没有验证一次优化的影响，则将无法跟踪哪个优化措施真正发挥了作用。

### 敏捷迭代周期 {#agile-iteration-cycles}

![chlimage_1-7](assets/chlimage_1-7.jpeg)

性能调整是一个迭代过程，包括测量、分析、优化和验证，直到达到目标。 为了适当地考虑这一点，在优化阶段实施敏捷验证过程，而不是在每次迭代后实施更重量的测试过程。

这在很大程度上意味着实施优化的开发人员应该可以快速判断优化是否已实现目标。 这是非常宝贵的信息，因为当达到目标时，优化即告结束。
...
* 25%的页面请求应在100毫秒至300毫秒内获得响应。
* 4%的页面请求应在300毫秒至500毫秒内获得响应。
* 1%的页面请求应在500毫秒至1000毫秒内获得响应。
* 任何页面的响应速度都不应小于1秒。
...
有一些问题经常会导致性能问题。 这主要围绕：

* Dispatcher缓存效率低下
* 在一般显示模板中使用查询。

JVM和OS级别调整通常不会导致性能大幅提升，因此应在优化周期的最后阶段执行。
...
* 进行编辑的时间范围（通常为工作日的长度，对国际业务更重要）。
* 已上传图像的平均大小（以及每个图像生成的演绎版的大小）（以MB为单位）。
* 确定平均数据速率：

![chlimage_1-78](assets/chlimage_1-78.png)

* 所有编辑中的80%将在20%的时间内完成，因此在高峰期，您的数据速率将是平均数据速率的4倍。 这是你的绩效目标。

## 性能监控 {#performance-monitoring}

性能（或缺缺性能）是用户首先注意到的事项之一，因此对于任何具有用户界面的应用程序而言，性能至关重要。 要优化AEM安装的性能，您需要监视实例的各种属性及其行为。
...
一个基本出发点是对系统正常工作时的良好了解。 除非您知道环境在正确执行时“外观”和“行为”如何，否则当性能下降时，可能很难找到问题。 这意味着您应该花一些时间来调查您的系统是否运行顺畅，并确保收集性能信息是一项持续的工作。 这将为您在性能下降时提供比较的基础。
...
>[!NOTE]
>
>请记住，您用于测量性能的机制通常会准确影响您尝试测量的内容。 您应始终尝试解决这些差异，并尽可能消除其影响；特别是应尽可能停用浏览器插件。
...
>[!NOTE]
>
>欲了解更多信息，请参见 [知识库文章](https://helpx.adobe.com/experience-manager/kb/performance-tuning-tips.html).
...
限制并发运行的工作流进程数以提高性能。 默认情况下，工作流引擎并行处理的工作流数量与Java VM可用的处理器数量相同。 当工作流步骤需要大量处理资源（RAM或CPU）时，并行运行多个这些工作流可能会对可用的服务器资源提出很高的要求。
...
* Granite工作流外部进程作业队列：此服务用于特殊外部工作流步骤，通常用于联系外部系统和轮询结果。 例如，“InDesign媒体提取流程”步骤作为外部流程实施。 工作流引擎使用外部队列处理轮询。 (请参阅 [com.day.cq.workflow.exec.WorkflowExternalProcess](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/workflow/exec/WorkflowExternalProcess.html).)

配置这些服务以限制并发运行的工作流进程的最大数量。

>[!NOTE]
>
>除非您为特定工作流模型创建了作业队列，否则配置这些作业队列会影响所有工作流(请参阅 [为特定工作流模型配置队列](/help/sites-deploying/configuring-performance.md#configure-the-queue-for-a-specific-workflow) （如下所示）。

#### 存储库中的配置 {#configuration-in-the-repo}

如果您正在配置服务 [使用sling：OsgiConfig节点](/help/sites-deploying/configuring-osgi.md#adding-a-new-configuration-to-the-repository)，您需要找到现有服务的PID，例如：org.apache.sling.event.jobs.QueueConfiguration.370aad73-d01b-4a0b-abe4-20198d85f705。 您可以使用Web控制台发现PID。

您需要配置名为的属性 `queue.maxparallel`.

#### Web控制台中的配置 {#configuration-in-the-web-console}

配置这些服务 [使用Web控制台](/help/sites-deploying/configuring-osgi.md#osgi-configuration-with-the-web-console)，找到Apache Sling作业队列配置服务工厂下的现有配置项目。

您需要配置名为Maximum Parallel Jobs的属性。
...
1. 为每个主题创建一个作业队列。 要创建作业队列，请为Apache Sling作业队列工厂服务创建工厂配置。
   ...
   工厂配置类似于中所述的Granite工作流队列 [并发工作流处理](/help/sites-deploying/configuring-performance.md#concurrent-workflow-processing)，但Topics属性与工作流作业的主题相匹配。

### AEM DAM资产同步服务 {#cq-dam-asset-synchronization-service}

此 `AssetSynchronizationService` 用于同步已装载的存储库（包括LiveLink 、 Documentum等）中的资产。 默认情况下，每300秒（5分钟）进行一次常规检查，因此，如果您不使用已装载的存储库，则可以禁用此服务。

完成日期 [配置OSGi服务](/help/sites-deploying/configuring-osgi.md) **CQ DAM资产同步服务** 以设置 **同步周期** ( `scheduler.period`)到（最少）1年（以秒为单位）。

### 多个DAM实例 {#multiple-dam-instances}

部署多个DAM实例可帮助提高性能，例如：

* 由于为创作环境定期上传大量资产，因此您的负载较高；此处有一个单独的DAM实例专门用于为创作提供服务。
* 您在全球各地拥有多个团队（例如美国、欧洲、亚洲）。

其他注意事项包括：

* 将作者的“进行中的工作”与发布上的“最终工作”区分开
* 将作者上的内部用户与外部访客/发布上的用户（例如，代理、新闻代表、客户、学生等）分隔开。

## 质量保证最佳实践 {#best-practices-for-quality-assurance}

性能对于您的发布环境至关重要。 因此，您需要仔细规划和分析在实施项目时为发布环境进行的性能测试。

本节旨在以标准化方式概述定义测试概念时遇到的问题，专门针对您电脑上的性能测试， *发布* 环境。 这主要与QA工程师、项目经理和系统管理员有关。

AEM以下内容介绍了在EMC的 *Publish* 环境。 这涉及以下5个阶段：
...
控制是一个额外的、涵盖所有内容的过程 — 它是测试所必需的，但不限于测试。

### 知识验证 {#verification-of-knowledge}

第一步是记录开始测试之前需要了解的基本信息：
...
您需要复制已计划的生产发布环境，以及Dispatcher和负载平衡器。

#### 应用程序映射 {#application-map}

要获得清晰的概述，您可以创建整个应用程序的映射（您可能从创作环境上的测试中获得该映射）。

应用程序的内部元素的图表示形式，可以概述测试要求；通过颜色编码，它还可以用作报告的基础。

### 范围定义 {#scope-definition}

应用程序通常有多种使用案例。 有些会非常重要，有些则没那么重要。

要将性能测试范围集中到发布上，我们建议您定义：

* 最重要的业务用例
* 最关键的技术用例

用例的数量取决于您，但应将其限制为易于管理的数量（例如5到10个）。
...
* 每个组件在与性能相关时都有一个特定的断点。 这意味着，组件在到达特定点之前可以表现出良好的性能，之后性能将迅速降低。
* 要获取应用程序的完整概述，您必须首先验证组件，以确定何时达到每个组件的断点。
* 要查找断点，您可以执行负载测试，其中您可以在一段时间内增加用户数量以创建不断增加的负载。 通过监控此负载和组件的响应，您将在组件到达断点时遇到特定的性能行为。 可以通过每秒并发事务数以及并发用户数（如果组件对此KPI敏感）来限定该点。
* 然后，此信息可作为改进基准，指示所用措施的效率，并帮助定义测试场景。

#### 交易 {#transactions}

* 术语事务用于表示完整网页的请求，包括页面本身和所有后续调用；即页面请求、任何AJAX调用、图像和其他对象。**请求深入分析**
* 要全面分析每个请求，您可以表示调用栈栈的每个元素，然后合计每个请求的平均处理时间。

### 定义绩效目标 {#defining-the-performance-goals}

定义范围和相关KPI后，即可设置特定的性能目标。 这涉及设计测试场景以及目标值。

您将需要在平均和峰值条件下测试性能。 此外，您还需要进行上线情景测试，以确保在网站首次发布时能够满足日益增长的对网站的兴趣需求。

您可能从现有网站收集的任何体验或统计数据在确定未来目标时也很有用；例如，来自实时网站的热门流量。
...
在网站发布后的前几天，您可能会更感兴趣。 这可能比您测试过的峰值还要高。 强烈建议测试上线场景，以确保系统能够满足此情况。
...
此外，还必须测试错误情况以确保系统正确且正确地反应。 不仅在错误本身的处理方式上，而且可能在性能上产生影响。 例如：

* 当用户尝试在搜索框中输入无效的搜索词时会发生什么情况
* 如果搜索词过于宽泛，以至于返回的结果数量过多，会出现什么情况

在设计这些测试时，应该记住，并非所有情况都会定期发生。 但是，它们对整个系统的影响非常重要。
...
只有在系统连续运行一段时间后，才会遇到某些问题；可以是几小时，也可以是几天。 持久性测试用于测试一段时间内恒定平均负载。 然后可以分析任何性能下降。
...
提供了一系列工具来帮助您进行负载生成、性能监控和/或结果分析：

* [JMet](https://jakarta.apache.org/jmeter/)
* [加载运行程序](https://www.microfocus.com/en-us/products/loadrunner-load-testing/overview)
* [德泰尔敏](https://www.determyne.com/) InsideApp
...
需要持续报告，以便让每个人都了解此状态，如前面提到的带有颜色编码的架构图可用于此目的。
...
此 [调度程序](https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher.html) 是Adobe的缓存和/或负载平衡工具。 在使用Dispatcher时，您应该考虑优化网站缓存性能。
...
Dispatcher提供了许多内置机制，如果您的网站利用这些机制可以优化性能。 此部分介绍了如何设计您的网站以最大限度地提高缓存优势。

>[!NOTE]
>
>Dispatcher 将缓存存储在标准 Web 服务器上，记住这一点可能会对您有所帮助。这意味着您：
>
>* 可以缓存可以存储为页面并使用URL请求的所有内容
>* 无法存储其他内容，例如Cookie、会话数据和表单数据。
>
...
缓存比率公式估算缓存处理的请求占进入系统的请求总数的百分比。 要计算缓存率，您需要满足以下条件：

* 请求总数。 此信息可在Apache中获取 `access.log`. 欲知更多详情，请参见 [Apache官方文档](https://httpd.apache.org/docs/2.4/logs.html#accesslog).
...
如果您没有一对一的发布者/Dispatcher配对，则需要将所有Dispatcher和发布者的请求添加到一起，以获得准确的测量。 另请参阅 [建议的部署](/help/sites-deploying/recommended-deploys.md).
...
使用Dispatcher版本4.1.11，您可以缓存响应标头。 如果您没有在Dispatcher上缓存响应标头，那么如果您在标头中存储页面编码信息，则可能会出现问题。 在此情况下，当 Dispatcher 从缓存中提供一个页面时，Web 服务器的默认编码将用于该页面。可通过两种方式避免此问题：
...
如果您对每个页面进行个性化（例如，将用户名放入标题栏），则可能会影响性能。
...
关于在单个页面上混合受限内容和公共内容，您可能需要考虑一种策略，该策略利用Dispatcher中的服务器端包含，或浏览器中通过Ajax包含的客户端。
...
使用Dispatcher版本4.1.11，您可以缓存响应标头。 如果您没有在Dispatcher上缓存响应标头，请注意MIME类型是HTTP标头的一部分。 因此，如果您的AEM应用程序返回的文件没有可识别的文件结尾，而是依赖MIME类型，则这些文件可能会显示不正确。

要确保正确缓存文件，请遵循以下准则：

* 确保文件始终具有正确的扩展名。
* 避免使用通用文件服务脚本，这些脚本的URL包括 `download.jsp?file=2214`. 重写脚本以使用包含文件规范的URL。对于上一个示例，这将是 `download.2214.pdf`.

## 备份性能 {#backup-performance}

本节介绍一系列用于评估AEM备份性能和备份活动对应用程序性能影响的基准。 AEM备份会在系统运行时给系统带来巨大负载，我们会测量此负载以及尝试调节这些影响的备份延迟设置的影响。 目的是提供有关备份在真实配置和大量生产数据中的预期性能的一些参考数据，并就如何估计计划系统的备份时间提供指导。

### 参考环境 {#reference-environment}

#### 物理系统 {#physical-system}

本文档中报告的结果是从在参考环境中运行的基准测试中获得的，该环境具有以下配置。 此配置旨在类似于数据中心中的典型生产环境：

* H-P ProLiant DL380 G6,8个CPU x 2.533 GHz
* 串行连接SCSI 300GB 10,000RPM驱动器
* 硬件RAID控制器；RAID0+5阵列中的8个驱动器
* VMware映像CPU x 2 Intel Xeon E5540,2.53 GHz
* RedHat Linux 2.6.18-194.el5；Java 1.6.0_29
* 单个作者实例

此服务器上的磁盘子系统速度非常快，代表可用于生产服务器的高性能RAID配置。 备份性能对磁盘性能非常敏感，而且此环境中的结果反映了非常快速的RAID配置的性能。 VMWare映像配置为在RAID阵列上有一个物理驻留在本地磁盘存储中的大型磁盘卷。

AEM配置将存储库和数据存储区与所有操作系统和AEM软件放在同一逻辑卷上。 备份的目标目录也驻留在此逻辑文件系统上。

#### 数据卷 {#data-volumes}

下表说明了备份基准测试中使用的数据卷的大小。 首先安装初始基线内容，然后添加其他已知数量的数据以增加备份内容的大小。 将以特定的增量创建备份，以表示内容的大幅增加以及一天内可能会产生的内容。 内容（页面、图像、标记）的分发将大致基于实际的生产资产组合。 页面、图像和标记将限制为最多800个子页面。 每个页面都将包含标题、Flash、文本/图像、视频、幻灯片放映、表单、表格、云和轮盘组件。 图像将上传自包含400个唯一文件的池，文件大小从37 kB到594 kB。
...
加载将包含页面创建/删除、遍历和查询，大多数加载来自页面遍历和查询。 添加和删除太多页面会不断增加工作区的大小，并阻止备份完成。 脚本将使用的负载分布是75%的页面遍历、24%的查询和1%的页面创建（单级别不含嵌套子页面）。 空闲系统上的平均每秒事务峰值通过4个并发线程实现，这是在负载下测试备份时使用的线程。
...
* **延迟设置**  — 对于几种情况，我们还更改了备份延迟设置，使用了10毫秒（默认）、1毫秒和0毫秒的值，以探索此设置如何影响备份性能。
...
空闲实例上的备份时间相当一致，平均为0.608 MB/s，无论完全备份还是增量备份（参见下图）。 备份时间只是要备份的数据量的函数。 完成完整备份的时间会随着总页数而明显增加。 完成增量备份的时间也会随着总页数而增加，但速度要低得多。 由于备份的数据量相对较少，完成增量备份所需的时间要短得多。

生成的备份大小是完成备份所花费时间的主要决定因素。 下图显示了作为最终备份大小的函数所花费的时间。

![chlimage_1-82](assets/chlimage_1-82.png)

此图表说明了增量备份和完全备份都遵循简单的大小与时间模式，我们可以测量其吞吐量。 空闲实例上的备份时间相当一致，平均为0.61 MB/秒，无论基准环境上的完整备份或增量备份如何。

#### 备份延迟 {#backup-delay}

提供备份延迟参数以限制备份可能干扰生产工作负载的程度。 参数指定等待时间（以毫秒为单位），该等待时间会逐个文件地散播到备份操作中。 总体效果部分取决于受影响的文件大小。 以MB/秒为单位测量备份性能为比较延迟对备份的影响提供了一种合理的方法。

* 在常规应用程序加载的同时运行备份将对常规加载的吞吐量产生负面影响。
* 影响可能是轻微的（只有5%），也可能是非常显着的，从而导致吞吐量下降多达75%，而这可能更多地取决于应用程序。
* 备份不是CPU上的重负载，因此，与I/O密集型工作负载相比，备份对CPU密集型生产工作负载的影响较小。

![chlimage_1-83](assets/chlimage_1-83.png)

比较使用文件系统备份（使用“tar”）备份相同的存储库文件获得的吞吐量。 tar的性能相当，但略高于延迟设置为零的备份。 即使设置一个小的延迟，也会大大降低备份吞吐量，而默认延迟10ms会导致吞吐量大大降低。 在备份可能安排在应用程序总体使用率非常低或应用程序完全空闲的情况下，最好将延迟降低到默认值以下，以便允许备份更快地进行。

进行中的备份对应用程序吞吐量的实际影响取决于应用程序和基础架构的详细信息。 延迟值的选择应通过对应用程序的经验分析来确定，但应尽量选择较小的值，以便能够尽快完成备份。 由于延迟值的选择与应用程序吞吐量影响之间只有弱的相关性，因此延迟的选择应该有利于缩短整体备份时间，以最大限度地减少备份带来的整体影响。 需要8个小时才能完成备份，但吞吐量影响为–20%的备份，其总体影响可能比需要2个小时才能完成但吞吐量影响为–30%的备份更大。
