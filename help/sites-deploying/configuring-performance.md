...
[粘性连接](https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher.html#the-benefits-of-load-balancing) ，确保一个用户的文档都在同一台服务器上完成。 如果用户离开此文件夹，稍后又返回到该文件夹，则连接仍然保持。 定义一个文件夹，以保存所有需要网站粘性连接的文档。 请尽量不要在其中包含其他文档。 如果使用个性化的页面和会话数据，这会影响负载平衡。
