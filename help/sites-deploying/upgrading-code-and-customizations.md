...
1. **模式检测器**[](/help/sites-deploying/pattern-detector.md) -按照升级计划中的说明运行模式检测器（如本页中所述），以获取模式检测器报告，该报告包含除AEM的Target版本中不可用的API/包外，还需要解决的区域的更多详细信息。 “模式检测”报告应指示代码中的任何不兼容性，如果没有，则您的部署已经兼容6.5，您仍然可以选择使用6.5功能进行新开发，但您不需要它来保持兼容性。 如果报告了不兼容性，则您可以选择a)以兼容性模式运行并推迟开发以获得新的6.5功能或兼容性，b)在升级后决定进行开发，然后转到步骤2。 请参阅AEM 6.5 [中的向后兼容性](/help/sites-deploying/backward-compatibility.md) ，了解更多详细信息。
...
经典UI创作在AEM 6.5中仍可用，但已弃用。 此处可找到更多 [信息](/help/release-notes/deprecated-removed-features.md#pre-announcement-for-next-release)。 如果应用程序当前在经典UI创作环境中运行，则建议升级到AEM 6.5并继续使用经典UI。 然后，可以将迁移到触屏UI计划为单独的项目，以在多个开发周期中完成。 要在AEM 6.5中使用经典UI，需要将多个OSGi配置提交到代码库。 有关如何配置此组件的更多详细信息，请在此 [处找到](/help/sites-administering/enable-classic-ui.md)。
...
ContextHub配置受升级影响。 有关如何恢复现有ContextHub配置的说明，请 [单击此处](/help/sites-administering/contexthub-config.md#recovering contexthub configurations after upgrading)。
