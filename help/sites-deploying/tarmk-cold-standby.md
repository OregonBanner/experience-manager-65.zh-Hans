...
1. 通过转到JMX控制台并使用**org.apache.jackrabbit.oak来停止冷备用实例上的同步过程：状态（“备用”）**bean。 有关如何执行此操作的更多信息，请参阅 [监控](#monitoring).
1. 停止冷备用实例。
...
1. 在维护过程完成且实例重新启动后：
   ...
   * 在待机时，数据存储垃圾收集仅可通过 **BlobGarbageCollection** MBean - `startBlobGC()`. 备用时**RepositoryManagement **MBean不可用。
