...
为了向后兼容，中的内容和配置 **/etc** 和 **/content** 从Adobe Experience Manager (AEM) 6.3开始，升级后不会立即更改或改变。 这样做是为了确保客户应用程序对这些结构的依赖关系保持不变。 即使开箱即用的AEM 6.5中的内容将托管在另一个位置，与这些内容结构相关的功能仍然相同。
...
虽然目标是高效执行，但此升级过程是同步的，因此会根据必须处理的内容量产生停机。 Adobe建议在生产系统之前评估暂存环境中的运行时间，以计划相应的维护时段。

由于这通常还需要调整应用程序，因此此活动应与相应的应用程序部署一起执行。

以下是以下内容的完整列表 `CodeUpgradeTasks` 在6.5中引入：

| **名称** | **相关** **适用于以前的AEM版本** | **迁移** **类型** | **详细信息** |
|---|---|---|---|
...
| `Cq62FormsContentUpgrade` | &lt; 6.2 | 即时 | 由于merge_preserve处理过程，默认的安全拒绝规则会覆盖给定的权限，因此需要在升级时重新排序 |
...
| `CQ63ProcessingProfileConfigUpdate` | &lt; 6.3 | 即时 | 转换Assets中的processingProfile属性（使用到6.1）以匹配6.3及更高版本结构。 还可以将配置文件的相对路径调整为 **/conf** 取代 **/apps**. |
| `CQ63ToolsMenuEntriesContentUpgrade` | &lt; 6.3 | 即时 | 升级任务，用于在升级时删除过时的CRXDE Lite和Web控制台菜单项。 |
| `CQ64CommunitiesConfigsCleanupTask` | &lt; 6.3 | 延迟 | 移动SRP云配置、社区标语配置、清理 **/etc/social** 和 **/etc/enablement** （在运行延迟迁移时，必须调整任何引用和数据 — 任何应用程序部分都不应再依赖此结构）。 |
