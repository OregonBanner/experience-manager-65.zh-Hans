...
这样可以轻松地管理捆绑包，因为它们可以单独停止、安装和启动。 系统会自动处理相互依赖关系。 每个OSGi组件(请参见 [OSGi规范](https://www.osgi.org/Specifications/HomePage))包含在多个捆绑包中的一个。 使用AEM时，可通过多种方法管理此类捆绑包的配置设置；请参阅 [配置OSGi](/help/sites-deploying/configuring-osgi.md) 了解更多详细信息和建议的做法。
...
>所需的配置因项目而异。
>
>有关使用的值和有关参数的详细信息，请参阅Web控制台。

>[!NOTE]
>
>OSGi配置比较工具，属于 [AEM工具](https://helpx.adobe.com/experience-manager/kb/tools/aem-tools.html)，可用于列出默认的OSGi配置。
...
* 此 **运行模式**，其中复制事件将分发给侦听器。 例如，如果定义为作者，则这是将“启动”复制的系统。

* 运行模式 **发布** 如果项目代码在发布环境中处理复制事件（反向复制），则需要添加。 例如，使用Dispatcher从发布环境刷新时，或发生到其他发布实例的标准复制时。
...
* **记录器名称** 和 **日志格式** 配置请求和访问日志记录的位置和格式(默认： `request.log`)。 在分析与Web链相关的性能或调试功能时，此日志文件至关重要。
此项与 [Apache Sling请求记录器](#apacheslingrequestlogger).
...
* **队列大小**，池耗尽时线程队列的最大大小。
推荐值为 `-1` 因为这将队列设置为无限制；如果设置了限制，则超过该限制时可能会发生丢失。

* 更改这些设置有助于在具有大量事件的场景中提高性能；例如，重型AEM DAM或工作流使用。
* 应使用测试建立特定于您的方案的值。
...
**Apache Sling Java脚本处理程序** 将设置配置为将.java文件编译为脚本(servlet)。

某些设置可能会影响性能，应尽可能禁用这些设置，特别是对于生产实例。

* **源VM** 和 **目标VM**，定义JDK版本作为运行时JVM

* 对于生产实例：

   * disable **生成调试信息**

**Apache Sling JCR安装程序** 这些参数可能不需要配置，但在开发或调试时可用于了解这些参数。 例如，安装文件夹对于签入/签出或创建包可能很有用。

* **安装文件夹名称regexp** 和 **安装文件夹的最大层次结构深度**  — 指定在存储库文件夹中搜索要安装的资源的位置和深度。 当使用通配符时（如中的）。&#42;/install)将搜索所有合适的匹配项，例如， `/libs/sling/install` 和 `/libs/cq/core/install`.
...
* **MIME类型** 以将项目所需的那些添加到系统中。 这允许 `GET` 请求对文件设置正确的content-type标头，以链接文件类型和应用程序。

**Apache Sling引用过滤器** 要解决CRX WebDAV和Apache Sling中的跨站点请求伪造(CSRF)的已知安全问题，您需要配置反向链接过滤器。
...
此项与 [Apache Sling可自定义请求数据记录器](#apacheslingcustomizablerequestdatalogger).

有关详细信息，请参阅 [AEM日志记录](/help/sites-deploying/configure-logging.md) 和 [Sling日志记录](https://sling.apache.org/site/logging.html).

**Apache Sling资源解析程序工厂** 配置Sling资源解析的中心方面：

* **资源搜索路径**(s)，添加任何项目特定的路径(但不删除 `/libs` 或 `/apps`)。

* **虚拟URL** 以定义虚URL映射。

* **URL映射** 定义任何别名；例如，从 `/content` 到 `/`.
...
>特别是，必须在存储库中配置这些选项。
>
>否则，对所做的更改 **URL映射** 下次启动时，使用Felix控制台可能会被AEM覆盖。
...
* **执行路径** 列出搜索可执行脚本的路径；通过配置特定路径，您可以限制可以执行的脚本。 如果未配置任何路径，则使用默认值( `/` = root)，这将允许执行所有脚本。
如果配置的路径值以斜杠结尾，则搜索整个子树。 如果没有此类尾随斜杠，则只有在脚本完全匹配时，才会执行脚本。

* **脚本用户**  — 此可选属性可以指定用于读取脚本的存储库用户帐户。 如果未指定帐户，则 `admin` 默认使用用户。

* **默认扩展** 将使用默认行为的扩展列表。 这意味着资源类型的最后一个路径段可用作脚本名称。

**Apache HTTP组件代理配置** 使用Apache HTTP客户端的所有代码的代理配置，在生成HTTP时使用；例如，在复制时。

创建新配置时，不要更改工厂配置，而是使用此处提供的配置管理器为此组件创建新的工厂配置： **https://localhost:4502/system/console/configMgr/**. 代理配置位于 **org.apache.http.proxyconfigurator.**

>[!NOTE]
>
>在AEM 6.0及更早版本中，代理是在Day Commons HTTP Client中配置的。 自AEM 6.1及更高版本起，代理配置已移至“Apache HTTP组件代理配置”而不是“Day Commons HTTP Client”配置。
...
**AdobeGraniteHTML库管理器** 配置此项以控制对客户端库（css或js）的处理；包括例如如何查看基础结构。
...
* 对于JS开发（尤其是在firebugging/debugging时）：
   ...
   * disable **Minify**
   * 启用 **调试** 分隔文件进行调试并与firebug一起使用。
   * 启用 **计时** 如果对时机感兴趣。
   * 启用 **调试** 控制台以查看JS控制台日志消息。

>[!CAUTION]
>
>更改以下任一项的设置时： **Minify** 或 **Gzip** 您还需要删除clientlibs缓存的内容。 请参阅此 [知识库文章](https://helpx.adobe.com/ca/experience-manager/kb/How-to-force-a-recompilation-of-all-Sling-scripts-jsps-java-sightly-on-AEM-6-4.html) 了解详细信息。
...
使用时 [封闭用户组](/help/sites-administering/cug.md) 您可以配置（及其他）：
...
* 要将触屏UI作为默认UI，请 **目标路径** 应指向：

   ```shell
      /projects.html
   ```

* 要将经典UI作为默认UI，请 **目标路径** 应指向：
...
**AdobeGranite SSO身份验证处理程序** 配置单点登录(SSO)详细信息；企业创作设置通常需要这些详细信息，通常与LDAP结合使用。
...
`admin` 将忽略映射。 请注意，字符“=”必须使用前导“\”进行转义。
...
**Day CQ WCM调试过滤器** 这在开发时很有用，因为它允许在访问页面时使用后缀，例如？debug=layout。 例如，https://localhost:4502/cf#/content/geometrixx/en/support.html?debug=layout将提供开发人员可能感兴趣的布局信息。

* 在生产实例上禁用此设置以确保性能和安全性。
...
* 在发布实例上，必须将此值设置为 `disabled` 以确保没有其他模式可访问。
...
* **重写配置列表** 为基于内容的linkchecker配置指定位置列表。 这些配置可以基于运行模式；这对于区分创作环境和发布环境非常重要，因为linkchecker设置可能不同。
...
* **保留驼峰式大小写**  — 默认情况下，HTML解析器将驼峰式大小写（例如eBay）中的属性转换为小写（例如ebay）。 您可以关闭此选项以保留驼峰式大小写属性。 当使用前端框架(如Angular2)时，这将很有用。

**Day Commons JDBC连接池** 配置对用作内容源的外部数据库的访问权限。

这是工厂配置，因此可以配置多个实例。

**CDN重写器** 必须确保AEM和CDN之间的通信，以便以安全的方式向最终用户交付资产/二进制文件。 这涉及两项任务：

* 第一次通过CDN从AEM访问资源（或在缓存中资源过期后）。
* 安全地访问CDN中缓存的资源，因为一旦在CDN中缓存了资源，请求将不会发送到AEM，并且所有有权访问该资源的用户都应从CDN提供服务。
