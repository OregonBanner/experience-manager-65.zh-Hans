...
利用此功能，可轻松管理包，因为可以单独停止、安装和启动这些包。 互依关系会自动处理。 每个OSGi组件(请参阅 [OSGi规范](https://www.osgi.org/Specifications/HomePage))包含在各种包中的一个包中。 使用AEM时，可通过多种方法来管理此类包的配置设置；请参阅 [配置OSGi](/help/sites-deploying/configuring-osgi.md) 以了解更多详细信息和建议的实践。
...
>[!NOTE]
>
>OSGi配置差异工具， [AEM工具](https://experienceleague.adobe.com/docs/experience-cloud-kcs/kbarticles/KA-17488.html?lang=zh-Hans)，可用于列出默认OSGi配置。
...
* 的 **运行模式**，其中复制事件被分发给侦听器。 例如，如果定义为作者，则系统将“启动”复制。

* 添加运行模式 **发布** 如果项目代码在发布环境中处理复制事件（反向复制）。 例如，当使用Dispatcher从发布环境刷新或发生到其他发布实例的标准复制时。
...
* **记录器名称** 和 **日志格式** 要配置请求和访问日志记录的位置和格式(默认值： `request.log`)。 在分析与Web链相关的性能或调试功能时，此日志文件至关重要。 与 [Apache Sling请求记录器](#apacheslingrequestlogger).
...
* **队列大小**，池耗尽时线程队列的最大大小。
建议的值为 `-1` 因为它将队列设置为“无限制”。 如果设置了限制，则在超出限制时可能会发生损失。

* 更改这些设置有助于在发生大量事件的情况下提高性能。 例如，大量AEM DAM或工作流的使用情况。
* 应使用测试建立特定于您方案的值。
...
**Apache Sling JavaScript处理程序** 配置将.java文件编译为脚本(servlet)的设置。

某些设置可能会影响性能。 尽可能禁用这些设置，尤其是对于生产实例。

* **源虚拟机** 和 **目标虚拟机**，定义用作运行时JVM的JDK版本

* 对于生产实例：

   * 禁用 **生成调试信息**

**Apache Sling JCR安装程序** 这些参数可能不需要配置，但在开发或调试时可能有助于了解这些参数。 例如，安装文件夹可用于签入或签出或创建包。

* **安装文件夹名称正则表达式** 和 **安装文件夹的最大层次结构深度**  — 指定在何处和到哪个深度搜索要安装的资源库文件夹。 使用通配符时（如中所示）。&#42;/install)会搜索所有适当的匹配项，例如 `/libs/sling/install` 和 `/libs/cq/core/install`.
...
* **MIME类型** 向系统中添加项目所需的类型。 这样做允许 `GET` 请求为链接文件类型和应用程序设置正确的content-type标头。

**Apache Sling反向链接过滤器** 要解决CRX WebDAV和Apache Sling中跨站点请求伪造(CSRF)的已知安全问题，您必须配置反向链接过滤器。
...
>[!NOTE]
>
>Apache Sling反向链接过滤器取决于快速修补程序包的安装。
...
与 [Apache Sling可自定义的请求数据记录器](#apacheslingcustomizablerequestdatalogger).
...
>在存储库中配置这些选项。
>
>否则，对 **URL映射** 下次启动时，AEM可能会覆盖使用Felix控制台的情况。
...
* **执行路径**  — 列出搜索可执行脚本的路径。 通过配置特定路径，您可以限制可以运行的脚本。 如果未配置路径，则使用默认路径( `/` = root)，允许运行所有脚本。
如果配置的路径值以斜杠结尾，则会搜索整个子树。 如果没有此类尾随斜杠，则脚本仅在完全匹配时才运行。

* **脚本用户**  — 此可选属性可指定用于读取脚本的存储库用户帐户。 如果未指定帐户，则 `admin` 默认使用用户。

* **默认扩展**  — 使用默认行为的扩展列表。 资源类型的最后一个路径段可用作脚本名称。

**Apache HTTP组件代理配置**  — 使用Apache HTTP客户端的所有代码的代理配置，在进行HTTP时使用。 例如，在复制时。

创建配置时，请勿更改工厂配置。 请改为使用此处提供的配置管理器为此组件创建工厂配置： **https://localhost:4502/system/console/configMgr/**. 代理配置在 **org.apache.http.proxyconfigurator。**

>[!NOTE]
>
>在AEM 6.0及更早版本中，代理在Day Commons HTTP客户端中进行了配置。 自AEM 6.1及更高版本起，代理配置已移至“Apache HTTP组件代理配置”，而不是“Day Commons HTTP Client”配置。
...
**AdobeGraniteHTML库管理器** 配置以控制客户端库（css或js）的处理，例如，包括基础结构的显示方式。
...
* 对于JS开发（尤其是在进行消息调试时）：
   ...
   * 禁用 **缩小**
   * 启用 **调试** 以分隔文件进行调试，并与触发错误一起使用。
   * 启用 **计时** 如果对时间感兴趣的话。
   * 启用 **调试** 控制台以查看JS控制台日志消息。

>[!CAUTION]
>
>更改 **缩小** 或 **Gzip**，删除clientlibs缓存的内容。 请参阅 [知识库文章](https://experienceleague.adobe.com/docs/experience-cloud-kcs/kbarticles/KA-16543.html?lang=zh-Hans) 以了解详细信息。
...
使用 [封闭用户组](/help/sites-administering/cug.md)，您可以配置以下各项：
...
* **路径**
此身份验证处理程序处于活动状态的路径。 如果此参数留空，则会禁用身份验证处理程序。 例如，路径/会导致将身份验证处理程序用于整个存储库。

* **服务排名**
OSGi框架服务排名值用于指示调用此服务的顺序。 此值是 
`int` 值越高，则指定的优先级越高。
默认值为 `0`.
...
`admin` 显示在映射的任一侧，将忽略映射。 字符“=”必须使用前导“\”进行转义。
...
**Day CQ WCM调试过滤器** 当进行开发时，这非常有用，因为它允许在访问页面时使用后缀，如？debug=layout。 例如，https://localhost:4502/cf#/content/geometrixx/en/support.html?debug=layout提供了开发人员可能感兴趣的布局信息。
...
* **WCM模式** 定义默认模式。
* 在创作实例上，此模式可能为 `edit`, `disable,preview`或 `analytics`.
其他模式可以从Sidekick或后缀访问 `?wcmmode=disabled` 可用于模拟生产环境。

* 在发布实例上，必须将此模式设置为 `disabled` 以确保无其他模式可访问。
...
* **重写配置列表** 为基于内容的链接检查器配置指定位置列表。 配置可以基于运行模式。 由于链接检查器设置可能有所不同，因此，在区分创作环境和发布环境时，这一事实非常重要。
...
* **用于发送数据的URL** 配置用于跟踪页面统计信息的URL（如果跟踪器请求通过Dispatcher，则此URL至关重要）；例如，默认值为 `https://localhost:4502/libs/wcm/stats/tracker`.
...
* **保留驼峰**  — 默认情况下，HTML解析器会以驼峰式大小写转换属性(例如， `eBay`)更改为小写(例如， `ebay`)。 您可以关闭此设置以保留驼峰式大小写属性。 在使用前端框架(如Angular2)时，此设置非常有用。

**Day Commons JDBC连接池** 配置对用作内容源的外部数据库的访问。

工厂配置，以便可以配置多个实例。

**CDN重写程序** 必须确保AEM与CDN之间的通信，以便以安全的方式将资产/二进制文件交付给最终用户。 此过程涉及以下两项任务：

* 首次（或在缓存中过期后）通过CDN从AEM访问资源。
* 安全地访问缓存在CDN中的资源。 资源缓存到CDN中后，该请求不会转到AEM，所有有权访问该资源的用户都应从CDN提供。
