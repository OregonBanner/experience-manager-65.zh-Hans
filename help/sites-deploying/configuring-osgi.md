...
这样可以轻松地管理捆绑包，因为它们可以单独停止、安装和启动。 系统会自动处理相互依赖关系。 每个OSGi组件(请参见 [OSGi规范](https://www.osgi.org/Specifications/HomePage))包含在多个捆绑包中的一个。
...
虽然存在细微差异，但可以使用任一方法，主要与 [运行模式](/help/sites-deploying/configure-runmodes.md)：
...
* [存储库中的content-nodes (sling：osgiConfig)](#osgi-configuration-in-the-repository)
   ...
   * 这需要使用CRXDE Lite手动配置。
   * 由于的命名惯例 `sling:OsgiConfig` 节点，您可以将配置关联到特定 [运行模式](/help/sites-deploying/configure-runmodes.md). 您甚至可以在同一存储库中保存多个运行模式的配置。
   * 任何适当的配置都将立即应用（取决于运行模式）。
...
此 [Web控制台](/help/sites-deploying/web-console.md) AEM中提供了用于配置捆绑包的标准化界面。 此 **配置** tab用于配置OSGi包，因此是用于配置AEM系统参数的底层机制。

所做的任何更改都将立即应用于相关的OSGi配置，无需重新启动。

>[!NOTE]
>
>在Web控制台中所做的更改在存储库中保存为 [配置文件](#osgi-configuration-with-configuration-files). 这些组件可以包含在内容包中，以供在后续安装中重复使用。

>[!NOTE]
>
>在Web控制台上，任何提及默认设置的说明都与Sling默认设置相关。
>
>Adobe Experience Manager有自己的默认值，因此默认值集可能与控制台中记录的默认值不同。

要使用Web控制台更新配置，请执行以下操作：

1. 访问 **配置** Web Console的选项卡，方法是：
...
1. 此时将打开一个对话框。 在此，您可以根据需要进行编辑；例如，设置 **日志级别** 到 `INFO`：

   ![chlimage_1-140](assets/chlimage_1-140.png)

   >[!NOTE]
   >
   >更新在存储库中保存为 [配置文件](#osgi-configuration-with-configuration-files). 要在之后找到这些标识（例如，要包含在内容包中以供其他实例使用），您应该记下永久标识( `PID`)。
...
## 带有配置文件的OSGi配置 {#osgi-configuration-with-configuration-files}

使用Web控制台所做的配置更改将作为配置文件保留在存储库中( `.config`)，位于：

`/apps`

这些组件可以包含在内容包中，并在其他实例中重复使用。

>[!NOTE]
>
>配置文件的格式非常具体 — 请参阅 [Sling Apache文档](https://sling.apache.org/documentation/development/slingstart.html#default-configuration-format) 以了解完整的详细信息。
>
>因此，建议通过在Web控制台中进行实际更改来创建和维护配置文件。

Web控制台不显示更改保存在存储库中的什么位置，但可以轻松找到它们：
...
除了使用Web控制台之外，您还可以在存储库中定义配置详细信息。 这允许您轻松配置不同的运行模式。

这些配置是通过创建 `sling:OsgiConfig` 供系统引用的存储库中的节点。 这些节点镜像了OSGi配置，并形成指向这些节点的用户界面。 要更新配置数据，请更新节点属性。

如果您修改存储库中的配置数据，则所做的更改会立即应用于相关的OSGi配置，就像已使用Web控制台进行了更改一样，并进行适当的验证和一致性检查。 这也适用于从复制配置的操作 `/libs/` 到 `/apps/`.

由于同一个配置参数可以位于多个位置，所以系统：
...
1. 是否特定 [运行模式](/help/sites-deploying/configure-runmodes.md) 为必填项。 创建文件夹：
...
1. 配置是否已存在于中 `/libs`？ 要列出实例中的所有配置，请使用 **查询** CRXDE Lite中用于提交以下SQL查询的工具：

   `select * from sling:OsgiConfig`

   如果是这样，此配置可以复制到 ` /apps/<yourProject>/`，然后在新位置进行自定义。
...
1. 如果尚不存在，请创建 `config` 文件夹( `sling:Folder`)：
...
1. 对于要配置的每个参数，请在此节点上创建一个属性：

   * 名称： Web控制台中显示的参数名称；该名称显示在字段描述末尾的括号中。 例如，对于 `Create Version on Activation` 使用 `versionmanager.createVersionOnActivation`
   * 类型：根据需要。
   * 值：按要求。
...
1. 保存所有更改。
   ...
   一旦通过重新启动服务更新了节点，就会应用更改（与Web控制台中所做的更改一样）。
...
1. 下的存储库节点 `/apps/*/config...`使用类型执行.ach `sling:OsgiConfig` 或属性文件。

1. 具有类型的存储库节点 `sling:OsgiConfig` 下 `/libs/*/config...`. （开箱即用的定义）。

1. 任意 `.config` 文件来源 `<*cq-installation-dir*>/crx-quickstart/launchpad/config/...`. 本地文件系统上的文件。

这意味着中的常规配置 `/libs` 可以被中的特定项目配置掩盖 `/apps`.

### 运行时分辨率顺序 {#resolution-order-at-runtime}
...
1. 在中修改配置 `/apps` 将会立即生效
1. 在中修改配置 `/libs` 将立即生效，除非被中的配置掩盖 `/apps`.
...
如果所有运行模式与启动时定义的运行模式匹配，则将应用此类文件夹中的配置。

例如，如果使用运行模式启动实例 `author,dev,emea`，配置中的节点 `/apps/*/config.emea`， `/apps/*/config.author.dev/` 和 `/apps/*/config.author.emea.dev/` 将应用，而配置节点位于 `/apps/*/config.author.asean/` 和 `/config/author.dev.emea.noldap/` 将不应用。

如果同一PID有多个配置可用，则应用匹配运行模式数最多的配置。

例如，如果使用运行模式启动实例 `author,dev,emea`，并且两者 `/apps/*/config.author/` 和 `/apps/*/config.emea.author/` 定义配置
`com.day.cq.wcm.core.impl.VersionManagerImpl`，中的配置 `/apps/*/config.emea.author/` 将被应用。

此规则的粒度处于PID级别。
不能在中为同一PID定义某些属性 `/apps/*/config.author/` 以及中更具体的 `/apps/*/config.emea.author/` 相同PID的。
匹配运行模式数量最多的配置将对整个PID有效。
...
* 如果通过Web控制台更改配置，则它通常（通常）会写入存储库的以下位置：
   ...
   * 默认情况下 `{somewhere}` 是 `system/config` 因此该配置将写入
...
* 由更改的设置 `admin` 保存在 `*.config` 文件位于：
   ...
   * 这是OSGi配置管理员的专用数据区域，包含指定的所有配置详细信息 `admin`，无论他们如何进入系统。
   * 这是实施详细信息，切勿直接编辑此目录。
   * 但是，了解这些配置文件的位置非常有用，以便可以获取副本进行备份和/或多个安装：
