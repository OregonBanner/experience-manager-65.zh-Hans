...
1. Oak-run提供了带外重新索引功能，支持以下情况：生产必须可用，不能容忍维护或重新索引所需的停机。
...
可以通过检查损坏的索引 `--index-consistency-check` 操作：

```shell
java -jar oak-run*.jar index --fds-path=/path/to/datastore  /path/to/segmentstore/ --index-consistency-check
```

这将在以下位置生成报告 `indexing-result/index-consistency-check-report.txt`. 有关示例报表，请参阅下文：
...
为了诊断查询性能Adobe的一些情况，通常需要现有的索引定义，即来自客户设置的索引相关统计信息。 到目前为止，这些信息分散在多个资源中。 为了更便于进行故障排除，Adobe创建了工具，该工具将：

1. 将系统上存在的所有索引定义转储到单个JSON文件中；
...
此外，通过Web控制台提供相同的详细信息，这些信息将包含在配置转储zip中。 可在以下位置访问它们：
...
根据 [方案](https://jackrabbit.apache.org/oak/docs/query/indexing.html#reindexing)，在某些情况下，需要执行重新索引。 目前，重新索引是通过设置 `reindex` 标记到 `true` 在索引定义节点中通过CRXDE或通过Index Manager用户界面。 设置标记后，将异步完成重新索引。

关于重新索引需要注意以下几点：

* 重新索引在以下日期要慢得多： `DocumentNodeStore` 设置对比 `SegmentNodeStore` 设置所有内容都是本地的；

* 在当前设计中，当重新索引时，异步索引器会被阻止，并且所有其他异步索引会变得陈旧，并且在索引期间不会更新。 因此，如果系统正在使用中，用户可能无法看到最新的结果；
* 重新索引涉及遍历整个存储库，这会给AEM设置带来高负载，从而影响最终用户体验；
* 对于 `DocumentNodeStore` 在其中重新索引可能需要相当长的时间的安装，如果与Mongo数据库的连接在操作过程中失败，则必须从头开始重新索引；

* 在某些情况下，由于文本提取，重新索引可能需要较长时间。 这主要适用于具有大量PDF文件的设置，其中花费在文本提取上的时间可能会影响索引时间。

为了实现这些目标，oak-run索引工具支持根据需要使用不同的重新索引模式。 oak-run index命令具有以下优点：

* **带外重新索引** - oak-run重新索引可以与正在运行的AEM设置分开完成，因此，它将对正在使用的AEM实例的影响降至最低；

* **非同道重新索引**  — 重新索引不影响索引索引操作。 这意味着异步索引器可以继续索引其他索引；
...
对象 `DocumentNodeStore` 安装可以通过单个oak-run命令完成重新索引：
...
* 对运行AEM实例的影响最小。 大多数读取可以从辅助服务器完成，并且运行AEM缓存不会因重新索引所需的所有遍历而受到不利影响；
* 用户还可以通过提供新索引或更新索引的JSON `--index-definitions-file` 选项。
...
按照既定方式，通过设置完成重新索引 `reindex` 标志。

#### 联机重新索引 — SegmentNodeStore - AEM实例正在运行 {#onlinereindexsegmentnodestoretheaeminstanceisrunning}

对象 `SegmentNodeStore` 安装中只有一个进程能够以读写模式访问区段文件。 因此，Oak-run索引中的一些操作需要执行额外的手动步骤。

这将涉及以下内容：

1. 步骤文本
1. 连接 `oak-run` 到AEM在只读模式下使用的同一存储库，并执行索引。 有关如何实现此目标的示例：
...
对象 `SegmentNodeStore` 安装可以通过单个oak-run命令完成重新索引。 但是，需要关闭AEM实例。
...
这种方法与上面解释的方法的区别在于，检查点的创建和索引导入是自动完成的。 不利的一面是，AEM需要在此过程中停机。
...
在此使用案例中，假定数据存储可在其他实例上访问，但如果 `FileDataStore` 放在基于云的存储解决方案（如EBS）上。 这不包括以下场景 `FileDataStore` 也进行了克隆。 如果索引定义不执行全文索引，则访问 `DataStore` 不是必需的。

## 用例4 — 更新索引定义 {#usecase4updatingindexdefinitions}

目前，您可以通过以下方式发送索引定义更改 [ACS确保索引](https://adobe-consulting-services.github.io/acs-aem-commons/features/ensure-oak-index/index.html) 包。 这允许通过内容包来传送索引定义，之后需要通过设置 `reindex` 标记到 `true`.

对于重新索引时间不长的较小安装，此方法是有效的。 但是，对于非常大的存储库，重新索引将在相当大的时间内完成。 对于这种情况，我们现在可以使用oak-run索引工具。
