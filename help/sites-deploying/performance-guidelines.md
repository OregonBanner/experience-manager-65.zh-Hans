...
微内核在AEM中充当持久性管理器。 有三种类型的微内核与AEM一起使用：TarMK、MongoDB和关系数据库（受限制支持）。 请根据您的实例目的和所考虑的部署类型，选择符合您需求的部署。有关微内核的其他信息，请参阅 [推荐的部署](/help/sites-deploying/recommended-deploys.md) 页面。
