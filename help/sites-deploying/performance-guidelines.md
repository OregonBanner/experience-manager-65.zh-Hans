...
本页提供了有关如何优化AEM部署性能的一般准则。 如果您是AEM的新用户，请先浏览以下页面，然后再开始阅读性能指南：
...
* **首次部署**：在计划首次部署AEM Sites或Assets时，请务必了解在配置微内核、节点存储和数据存储时可用的选项（与默认设置相比）。 例如，将TarMK的“数据存储”的默认设置更改为“文件数据存储”。
* **升级到新版本**：在升级到新版本时，请务必了解与运行环境相比的性能差异。 例如，从AEM 6.1升级到6.2，或从AEM 6.0 CRX2升级到6.2 OAK。
* **响应时间较慢**：当所选的Nodestore架构不满足您的要求时，了解与其他拓扑选项相比的性能差异很重要。 例如，部署TarMK而不是MongoMK，或使用文件数据存储而不是Amazon S3或Microsoft Azure数据存储。
* **添加更多作者**：当推荐的TarMK拓扑不满足性能要求并且扩展创作节点的大小已达到可用的最大容量时，了解与使用具有三个或更多创作节点的MongoMK相比的性能差异很重要。 例如，部署MongoMK而不是TarMK。
* **添加更多内容**：当推荐的数据存储架构不满足您的要求时，了解与其他数据存储选项相比的性能差异很重要。 示例：使用Amazon S3或Microsoft Azure数据存储区，而不是文件数据存储区。
...
AEM部署有三个重要的构建块。 此 **创作实例** 内容作者、编辑者和批准者用于创建和审阅内容。 内容获得批准后，将发布到名为的第二个实例类型。 **发布实例** 从最终用户访问的位置。 第三个构建基块是 **调度程序** 这是一个处理缓存和URL过滤的模块，安装在Web服务器上。 有关AEM架构的其他信息，请参阅 [典型部署方案](/help/sites-deploying/deploy.md#typical-deployment-scenarios).

![chlimage_1-1](assets/chlimage_1-1a.png)

### 微内核 {#micro-kernels}

微内核在AEM中充当持久性管理器。 AEM使用三种类型的微内核：TarMK、MongoDB和关系数据库（受限制支持）。 请根据您的实例目的和所考虑的部署类型，选择符合您需求的部署。有关微型内核的其他信息，请参见 [建议的部署](/help/sites-deploying/recommended-deploys.md) 页面。
...
>[!CAUTION]
>
>关系数据库微内核的支持受到限制。 联系人 [Adobe客户关怀](https://helpx.adobe.com/cn/marketing-cloud/contact-support.html) 使用此类微内核之前。

![chlimage_1-3](assets/chlimage_1-3a.png)

### 数据存储 {#data-store}

在处理大量二进制文件时，建议使用外部数据存储而不是默认节点存储，以便最大化性能。 例如，如果您的项目需要大量媒体资产，将它们存储在File或Azure/S3 Data Store下将使访问这些资产的速度比直接将它们存储在MongoDB中更快。

有关可用配置选项的更多详细信息，请参阅 [配置节点和数据存储](/help/sites-deploying/data-store-config.md).

>[!NOTE]
>
>Adobe建议选择使用Adobe Managed Services在Azure或Amazon Web Services (AWS)上部署AEM的选项，这样客户将受益于具有在这些云计算环境中部署和操作AEM的经验和技能的团队。 请查看我们的 [有关Adobe Managed Services的其他文档](https://www.adobe.com/marketing-cloud/enterprise-content-management/managed-services-cloud-platform.html?aemClk=t).
>
>有关如何在Adobe Managed Services之外的Azure或AWS上部署AEM的建议，我们强烈建议直接与云提供商或我们的合作伙伴之一合作，支持在您选择的云环境中部署AEM。 选定的云提供商或合作伙伴负责其支持的架构的规模规格、设计和实施，以满足您的特定性能、负载、可扩展性和安全性要求。
>
>欲知更多详情，另请参阅 [技术要求](/help/sites-deploying/technical-requirements.md#supported-platforms) 页面。
...
>[!NOTE]
>
>对于大多数部署，Adobe建议使用Lucene索引。 您应仅将Solr用于专业化和复杂部署中的可扩展性。

![chlimage_1-4](assets/chlimage_1-4a.png)

### 开发准则 {#development-guidelines}

您应该针对AEM的目标进行开发 **性能和可扩展性**. 以下是您可以遵循的一些最佳实践：
...
* 请勿更改/libs，而是使用叠加
* 尽量不要使用查询
* 请勿使用Sling绑定在Java代码中获取OSGi服务，而是使用：
...
下面详述的测试场景用于TarMK、MongoMk和TarMK与MongoMk章节的基准部分。 要查看哪个场景用于特定基准测试，请阅读中的场景字段 [技术规范](/help/sites-deploying/performance-guidelines.md#tarmk-performance-benchmark) 表格。
...
* 阅读文章页面(27.4%)、阅读页面(10.9%)、创建会话(2.6%)、激活内容页面(1.7%)、创建内容页面(0.4%)、创建段落(4.3%)、编辑段落(0.9%)、图像组件(0.9%)、浏览资产(20%)、读取资产元数据(8.5%)、下载资产(4.2%)、搜索资产(0.2%)、更新资产元数据(2.4%)、上传资产(1.2%)、浏览项目(4.9%)、读取项目(6.6%)、项目添加资产(1.2%)、项目添加站点(1.2%)、创建项目(0.1%)、作者搜索(0.4%)
* 执行模式：并发用户，每个用户的混合交互
...
>[!NOTE]
>
>下面介绍的最低架构准则适用于生产环境和高流量站点。 这些是 **非** 此 [最低规范](/help/sites-deploying/technical-requirements.md#prerequisites) 运行AEM所需。
...
要获得良好的性能，您应该遵循下面提供的设置准则。 有关如何更改设置的说明， [查看此页面](https://helpx.adobe.com/experience-manager/kb/performance-tuning-tips.html).
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td><p>1048576 (1MB)或更小</p> <p>最大栈大小的2-10%</p> </td>
...
| 服务器 | 裸机硬件(HP) |
| 操作系统 | RedHat Linux |
| CPU/内核 | 英特尔(R)至强(R) CPU E5-2407 @2.40GHz，8核 |
| RAM | 32GB |
| 磁盘 | 磁性 |
| Java | oracleJRE版本8 |
| JVM栈 | 16GB |
...
>[!NOTE]
>
>在生产环境中， MongoDB将始终用作具有主副本集和两个辅助副本集的副本集。 读取和写入操作会转到主数据库，而读取操作则可以转到辅助数据库。 如果存储不可用，可以使用仲裁器替换其中一个辅助实例，但MongoDB副本集必须始终由奇数实例组成。
...
在两者之间进行选择时需要考虑的基本规则是，TarMK是针对性能而设计的，而MongoMK用于可扩展性。 Adobe建议将TarMK作为客户在所有部署场景中使用的默认持久性技术，适用于AEM创作实例和发布实例。
...
* 提供故障切换机制 — 请参见 [冷备用](/help/sites-deploying/tarmk-cold-standby.md) 了解更多详细信息
* 以最低的操作开销提供高性能、可靠的数据存储
* 降低TCO（总体拥有成本）

**选择MongoMK的标准**

* 一天中连接的指定用户数：以千或更多的数量为单位
* 并发用户数：以百计或更多
* 每天的资产摄取量：数十万或更多
* 每天页面编辑量：数十万或更多
* 每天的搜索量：以万或更多

### TarMK与MongoMK性能指标评测 {#tarmk-vs-mongomk-benchmarks}

>[!NOTE]
>
>下面显示的数字已标准化为1作为基线，而不是实际的吞吐量数字。

### 场景1技术规范 {#scenario-technical-specifications}
...
### 场景2技术规范 {#scenario-technical-specifications-1}

>[!NOTE]
>
>要使用MongoDB启用与使用一个TarMK系统相同数量的Author，您需要一个具有两个AEM节点的群集。 与一个TarMK实例相比，四个节点的MongoDB群集可以处理1.8倍的“作者”数量。 8节点MongoDB群集可以处理2.3倍于一个TarMK实例的作者数。
...
* **带有文件数据存储的TarMK** 是推荐给大多数客户的体系结构：

   * 最小拓扑：一个创作实例、两个发布实例、两个Dispatcher
   * 如果文件数据存储是共享的，则打开无二进制复制

* **具有文件数据存储的MongoMK** 是推荐的创作层的水平可扩展性体系结构：
...
* 使用时 **Amazon S3**：
   ...
   * 必须打开无二进制复制
   * 数据存储垃圾收集要求首先在所有创作和发布节点上运行一次，然后在创作节点上运行第二次
...
欲知更多详情，另请阅读 [建议的部署](/help/sites-deploying/recommended-deploys.md) 页面。
