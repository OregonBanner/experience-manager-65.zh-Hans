...
提供从和软件包运行的所有升级操作 `com.day.compat.codeupgrade` 的日 `com.adobe.cq.upgradesexecutor` 志。
   ...
   * `<*cq-installation-dir*>/crx-quickstart/repository`
`revision.log`
修订日志信息。`com.adobe.cq.upgradesexecutor`
...
不要将调试日志级别的日志保留得比必需的更长，因为它会生成大量日志条目，从而占用资源。`org.apache.sling.commons.log.level``/libs/sling/config/org.apache.sling.commons.log.LogManager`[](/help/sites-deploying/osgi-configuration-settings.md#apacheslingloggingconfiguration)

>[!CAUTION]调试文件中的一行通常与DEBUG开始，然后提供日志级别、安装程序操作和日志消息。 例如：
日志级别如下：
...
| 操作失败，安装程序无法继续。 | 1 | 错误 |
|---|---|---|
| 操作失败。 安装将继续进行，但AEM WCM的某一部分安装不正确，将无法工作。 | 2 | 警告 |
| 操作已成功，但遇到问题。 AEM WCM可能正常工作，也可能无法正常工作。 | 3 | 信息 |
| 操作已成功。 | 创建自定义日志文件 {#create-a-custom-log-file} | [!NOTE] |
...
>[!NOTE]在某些情况下，您可能希望创建具有不同日志级别的自定义日志文件。 您可以通过以下方式在存储库中执行此操作：
如果项目尚未存在，请为项目创建新 `sling:Folder`的配置文件夹( `/apps/<*project-name*>/config`)。

在下 `/apps/<*project-name*>/config`面，为新的Apache Sling日志记 [录程序配置创建节点](/help/sites-deploying/osgi-configuration-settings.md#apacheslingloggingloggerconfigurationfactoryconfiguration):

1. 名称： `org.apache.sling.commons.log.LogManager.factory.config-<*identifier*>` （因为这是记录器）`/apps/<*project-name*>/config`
1. 其中 `<*identifier*>` 将替换为您（必须）输入以标识实例的自由文本（您不能忽略此信息）。[](/help/sites-deploying/osgi-configuration-settings.md#apacheslingloggingloggerconfigurationfactoryconfiguration)
...
1. 类型： 字符串
   ...
   * 值： 指定记录器要为其记录消息的OSGi服务； 例如，以下所有内容：`org.apache.sling.commons.log.names`
   ...
   * 值： 指定所需的日志级 `debug`别( `info`、 `warn` 或 `error`); 示例 `debug`
      ...
      名称: `org.apache.sling.commons.log.pattern``info``warn``error``debug`

   * 类型: `String`

      * 值： 根据需要指定日志消息的模式； 例如，`org.apache.sling.commons.log.pattern`
   ...
   >org.apache.sling.commons.log.names必须具有值。`Throwable`
...
&quot;(即，在&lt; ` `cq-*installation-dir*>/旁)`crx-quickstart/`
   ...
   >仅当需要新的写入程序时（即配置与默认写入程序不同时），此步骤才是必需的。
   [!CAUTION]` `**`crx-quickstart/`

1. 仅当现有默认值不合适时，才需要新的日志记录写入程序配置。

   >[!CAUTION]如果未配置显式编写器，则系统将根据默认值自动生成隐式编写器。
   ...
   与记录器一样， `<*identifier*>` 它将替换为您（必须）输入用来标识实例的自由文本（您无法忽略此信息）。 例如，`org.apache.sling.commons.log.LogManager.factory.writer-MINE`](/help/sites-deploying/osgi-configuration-settings.md#apacheslingloggingwriterconfigurationfactoryconfiguration)
   ...
   * 值： 指定日志文件，使其与记录器中指定的文件匹配；`org.apache.sling.commons.log.file`
   ...
   * 类型: `Long`
   ...
   >[!NOTE]最大文件大小
   `org.apache.sling.commons.log.file.size`时间／日期计划
   ...
   * 默认为 &#39;.&#39;yyyy-MM-dd（用于日志旋转）。`KB``MB``GB`
   * 例如，在2010年1月20日午夜（或者，准确地说，发生这种情况后的第一条日志消息）,../logs/error.log将更名为../logs/error.log.2010-01-20。 1月21日的日志将输出到../logs/error.log（新的和空的），直到在下一天的更改将其滚过。`java.util.SimpleDateFormat`
   ...
   | `'.'yyyy-MM-dd-HH-mm`注意： 指定时间／日期时： | 应在一对单引号(“ ”)中“转义”文本； 这是为了避免某些字符被解释为模式字母。 |
   在选项的任意位置仅使用有效文件名所允许的字符。
   1. 使用所选工具阅读新日志文件。
   1. 此示例创建的日志文件将为 `../crx-quickstart/logs/myLogFile.log`。


1. Felix Console还提供有关Sling日志支持的信息，网址为 `../system/console/slinglog`: 例如 `https://localhost:4502/system/console/slinglog`,

   查找审计记录 {#finding-the-audit-records}

保留审计记录，以便提供由谁执行什么和何时执行的记录。 为AEM WCM和OSGi事件生成不同的审核记录。`../system/console/slinglog``https://localhost:4502/system/console/slinglog`

### 页面创作时显示的AEM WCM审核记录 {#aem-wcm-audit-records-shown-when-page-authoring}

打开页面。

#### 在Sidekick中，您可以选择带有锁图标的选项卡，然后多次单击“ **审核日志……”**

1. 将打开一个新窗口，显示当前页面的审计记录列表。
1. ![screen_shot_2012-02-02at43601pm](assets/screen_shot_2012-02-02at43601pm.png)
1. 要 **关闭** 该窗口时，单击“确定”。

   存储库中的AEM WCM审核记录 {#aem-wcm-auditing-records-within-the-repository}](assets/screen_shot_2012-02-02at43601pm.png)

1. 在该文 `/var/audit` 件夹中，会根据资源保留审核记录。 您可以向下展开，直到您看到单个记录及其包含的信息。**

#### 这些条目包含的信息与编辑页面时显示的信息相同。{#aem-wcm-auditing-records-within-the-repository}
...
## 是否所有必需队列都已启用？{#monitoring-your-replication-agents}

是否仍需要任何禁用的队列？[](/help/sites-deploying/replication.md)

* 所有 `enabled` 队列都应具有状态 `idle` 或， `active`表示正常操作； 不应该有队 `blocked`列，这往往是接收方出现问题的一个迹象。
* 如果队列的大小随着时间的推移而增大，这可能表示队列被阻止。
* 要监视复制代理，请执行以下操作：`enabled``idle``active``blocked`
...
1. 结果窗口显示创作环境的所有复制代理的概述，包括其目标和状态。****
...
1. 在此对话框中，您可以：
   ...
   ![查看代理是否已启用。](assets/chlimage_1.jpeg)
   ...
   * **请勿将“测试连接”链接用于发布实例上的反向复制输出框。**
   ...
   >[!CAUTION]如果对发件箱队列执行复制测试，则所有早于测试复制的项目都将通过每次反向复制进行重新处理。
...
[性能优化](/help/sites-deploying/configuring-performance.md) 是一个交互式流程，在开发过程中受到关注。 部署后，通常在特定时间间隔或事件后对其进行审核。`enabled``disabled``active``idle``blocked`

## 在收集信息以进行优化时使用的方法也可以用于进行监视。{#monitoring-performance}
...
>[!NOTE]以下列表出现的常见性能问题，以及有关如何发现和消除这些问题的建议。
区域[](/help/sites-deploying/configuring-performance.md#configuring-for-performance)

症状

| 要提高容量…… | 要减少音量…… | 客户端 | 客户端CPU使用率高。 |
|---|---|---|---|
| 安装性能更高的客户端CPU。 | 简化(HTML)布局。 | 服务器CPU使用率低。 | 升级到更快的浏览器。 |
|  | 改进客户端缓存。 | 有些客户很快，有些很慢。 | 服务器 |
|  | 网络 |  |  |
| 服务器和客户端的CPU使用率都较低。 |  |  |  |
| 消除所有网络瓶颈。 | 改进／优化客户端缓存的配置。 | （相对而言）在服务器上本地浏览速度较快。 | 增加网络带宽。 |
|  | 减少网页的“权重”（例如减少图像、优化HTML）。 | Web服务器 | Web服务器上的CPU使用率很高。 |
| 群集Web服务器。 | 减少每页点击量（访问）。 | 使用硬件负载平衡器。 | 应用程序 |
|  |  | 服务器CPU使用率较高。 |  |
| 群集AEM实例。 | 搜索和消除CPU和内存存储（使用代码检查、定时输出等）。 | 内存消耗高。 | 改善所有级别的缓存。 |
...
### 其中一些将取决于您的操作系统。{#tools-for-monitoring-and-analyzing-performance}
...
监控网站流量 {#monitoring-traffic-on-your-website}`tail -f``request.log``request.log`[`rlog.jar`](#using-rlog-jar-to-find-requests-with-long-duration-times)

请求日志会注册发出的每个请求，以及发出的响应：`request.log`` [yourkit](https://www.yourkit.com/)`

#### 通过在特定时间段内（例如，在不同的24小时时间段内）汇总所有GET条目，您可以对网站上的平均流量进行声明。{#monitoring-traffic-on-your-website}
...
`<*cq-installation-dir*>/crx-quickstart/logs/request.log`此日志每个请求或响应有一行：
...
#### 因此，如上所述，您可以使用脚本提取相关信息并建立统计信息。{#monitoring-search-response-times-with-the-request-log}
...
#### 必须进行测试，以确定系统可以处理的并发用户数，然后才能看到负面影响。 同样，脚本可用于从日志文件中提取结果：{#monitoring-the-number-and-impact-of-concurrent-users}

监视在特定时间范围内发出的请求数，如1分钟`request.log`
...
以下命令显示可能的参数：`<*cq-installation-dir*>/crx-quickstart/opt/helpers`

例如，您可以运行它将文 `request.log` 件指定为参数并显示持续时间最长的10个前请求：`request.log`
...
为了最大限度地减少特殊情况（如垃圾收集等）的影响，建议使用诸如(如 `apachebench` ab，获取进一步的文 [档[#$tu387] )这样的工具来帮助识别内存泄漏并有选择地分析响应时间。
...
有关请求流量的信息（特定时间段内的请求数）会指示实例的负载。 此信息可从request.log [中提取](#interpreting-the-request-log)，但使用计数器将自动收集数据，以便您查看：`-c`

### 活动方面的显着差异(即区分“多个请求”和“低活动”){#request-counters}

当实例不被使用时[](#interpreting-the-request-log)
...
### 开始您的AEM实例。{#monitoring-performance-using-jconsole}
...
1. 之后，您可以选择其他选项。`Local``com.day.crx.quickstart.Main`

   使用(J)VisualVM监控性能 {#monitoring-performance-using-j-visualvm}](assets/chlimage_1-1.png)

   自JDK 1.6起，工具命令 `jvisualvm` 便可用。 安装JDK 1.6后，您可以：

### 开始您的AEM实例。{#monitoring-performance-using-j-visualvm}

[!NOTE]

1. 如果使用Java 5，则可以 `-Dcom.sun.management.jmxremote` 将参数添加到开始JVM的java命令行。 默认情况下，JMX在Java 6中处于启用状态。

   >[!NOTE]运行以下任一操作：
   `jvisualvm`: 在JDK 1.6 bin文件夹（测试版）中

1. `visualvm`: 可从VisualVM [(最新[#$tu419] 版本)下载

   * 
   * 

1. 之后，您可以选择其他选项，包括监视器：`Local``com.day.crx.quickstart.Main`

   ![chlimage_1-3](assets/chlimage_1-3.png)

   您可以使用此工具生成线程转储和内存头转储。 技术支持团队经常要求提供此信息。

   信息收集 {#information-collection}](assets/chlimage_1-3.png)

尽可能地了解您的安装有助于您了解可能导致性能变化的原因，以及这些更改是否合理。 需要定期收集这些指标，以便您能够轻松看到重大变化。

### 以下信息可能有用：{#information-collection}
...
* 有多少作者在使用该系统？ {#how-many-authors-are-working-with-the-system}](#how-many-requests-per-hour-do-you-have-on-the-author-system-at-peak-time)
* [要查看自安装以来使用系统的作者人数，请使用命令行：](#how-many-requests-per-hour-do-you-have-on-the-publish-system-at-peak-time)

#### 要查看在给定日期工作的作者数：{#how-many-authors-are-working-with-the-system}
...
* 您当前在此系统上维护多少页？ {#how-many-pages-do-you-currently-maintain-on-this-system}**`//element(*, cq:AuditEvent)[@cq:type='Activate']`
...
* **确定自安装以来使用存储库查询的转出总数； 通过CRXDE —— 工具-查询:**`//element(*, cq:Page)`
...
* 每月的平均Live Copy数是多少？ {#what-is-the-average-number-of-live-copies-per-month}**`//element(*, cq:AuditEvent)[@cq:type='PageRolledOut']`
...
* 如果您使用AEM Assets，您当前在资产中维护的资产数量是多少？ {#if-you-use-aem-assets-how-many-assets-do-you-currently-maintain-in-assets}**`//element(*, cq:LiveSyncConfig)`
...
* 资产的平均大小是多少？ {#what-is-the-average-size-of-the-assets}**`/`
* 要确定文件夹的总大 `/var/dam` 小：**`/jcr:root/content/dam//element(*, dam:Asset)`

#### 使用WebDAV将存储库映射到本地文件系统。{#what-is-the-average-size-of-the-assets}

使用命令行：`/var/dam`

1. 要获取平均大小，请将全局大小除以（在上面获取）中 `/var/dam` 的资产总数。

1. 当前使用了多少个模板？ {#how-many-templates-are-currently-used}
   ...
   要查看服务器上当前的模板数，请使用存储库查询; 通过CRXDE —— 工具-查询:`/var/dam`
...
* 当前使用了多少个组件？ {#how-many-components-are-currently-used}**`/`
* **要查看服务器上当前使用存储库查询的组件数； 通过CRXDE —— 工具-查询:**`//element(*, cq:Template)`
...
* 在高峰时间，创作系统每小时有多少个请求？ {#how-many-requests-per-hour-do-you-have-on-the-author-system-at-peak-time}**`/`
* **要确定在高峰时间在创作系统上每小时的请求数：**`//element(*, cq:Component)`

#### 要确定自安装以来请求的总数，请使用命令行：{#how-many-requests-per-hour-do-you-have-on-the-author-system-at-peak-time}

要确定开始和终止日期，请执行以下操作：

1. 使用这些值可计算自安装以来已用的小时数，然后计算每小时的平均请求数。
...
1. 在高峰时间，发布系统每小时有多少个请求？ {#how-many-requests-per-hour-do-you-have-on-the-publish-system-at-peak-time}
...
### 如果系统内存不足，则可以通过各种方式查看此问题，包括性能降低和包含子文本的错误消息：{#out-of-memory}
...
### 您是否已禁用调试信息集合； 可以在各种位置进行配置，包括：{#disk-i-o}
...
* [Apache Sling Java脚本处理程序](/help/sites-deploying/osgi-configuration-settings.md#apacheslingjavascripthandler)
   ...
   * [知识库：](/help/sites-deploying/monitoring-and-maintaining.md#activating-the-debug-log-level)[](/help/sites-deploying/configuring.md#loggersandwritersforindividualservices)
...
### 如果默认设置不合适，则在尝试调整JVM之前，必须建立一种方法来监视和评估GC性能； 这可能涉及到监视因素，包括堆大小、算法等。{#jvm-tuning}
...
` [https://www.ibm.com/developerworks/library/j-ibmtools2/](https://www.ibm.com/developerworks/library/j-ibmtools2/)`这些设置用于“宽开放”JMX连接：
...
* 这将帮助您了解正在使用的内存量、GC算法的使用情况、运行它们需要多长时间，以及这对应用程序性能的影响。 如果没有这种情况，调音就只是“随机摆弄旋钮”。   [!NOTE]
