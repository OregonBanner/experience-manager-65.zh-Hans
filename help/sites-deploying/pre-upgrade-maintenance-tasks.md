...
执行升级时，除了内容和代码升级活动之外，还必须执行存储库迁移。 迁移会以新的区段焦油格式创建存储库的副本。 因此，您需要足够的磁盘空间来保留存储库的第二个版本（可能更大）。
...
升级过程能够很好地维护和合并来自 `/apps` 和 `/libs` 存储库中的路径。 对 `/etc` 路径（包括ContextHub配置）通常需要在升级后重新应用这些更改。 升级会生成任何无法合并的更改的备份副本 `/var`，则Adobe建议您在开始升级之前手动备份这些更改。

## 生成quickstart.properties文件 {#generate-quickstart-properties}

从jar文件启动AEM时， `quickstart.properties` 文件在 `crx-quickstart/conf`. 如果AEM过去只是使用启动脚本启动，则此文件不存在，并且升级失败。 确保检查此文件是否存在，如果不存在，请从jar文件中重新启动AEM。

## 配置工作流和审核日志清除 {#configure-wf-audit-purging}

的 `WorkflowPurgeTask` 和 `com.day.cq.audit.impl.AuditLogMaintenanceTask` 任务需要单独的OSGi配置，如果没有这些配置，就无法工作。 如果在升级前任务执行期间失败，则最可能的原因是缺少配置。 因此，如果您不希望运行这些任务，请确保为这些任务添加OSGi配置，或从升级前优化任务列表中完全删除它们。 有关配置工作流清除任务的文档，请参阅 [管理工作流实例](/help/sites-administering/workflows-administering.md) 和审核日志维护任务配置可在 [AEM 6中的审核日志维护](/help/sites-administering/operations-audit-log.md).
...
由于AEM允许的自定义级别，环境通常不遵循统一的升级方式。 因此，创建标准化升级程序是一个困难的过程。

在以前的版本中，也很难对已停止或无法安全恢复的AEM升级进行升级。 出现此问题时，需要重新启动完整升级过程，或者执行了有缺陷的升级而未触发任何警告。

为了解决这些问题，Adobe在升级过程中添加了多项增强功能，使其更具弹性和用户友好性。 必须手动执行之前的升级前维护任务正在优化和自动执行。 此外，还增加了升级后报告，以便对该过程进行充分审查，希望更容易找到任何问题。

升级前维护任务当前分散在部分或完全手动执行的各种界面上。 AEM 6.3中引入的升级前维护优化使能够以统一的方式触发这些任务并能够按需检查其结果。
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>crx2</td>
   <td>运行标记和扫描。 对于共享数据存储，请删除此步骤并运行<br /> 在执行之前，请手动或正确准备实例。</td>
  </tr>
  <tr>
...
>[!CAUTION]
>
>的 `DataStoreGarbageCollectionTask` 使用标记和扫描阶段调用数据存储垃圾收集操作（如果使用）。 对于使用共享数据存储的部署，请确保正确重新配置该数据存储，或准备实例以避免删除其他实例引用的项目。 此过程可能需要在触发此升级前任务之前，在所有实例上手动运行标记阶段。
...
* **预升级**  — 可添加到所有运行状况检查的自定义标记，您可以设置该标记在升级前运行
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>ACTION_INFO</td>
   <td>检查是否正在运行任何升级前维护任务，并<br /> 返回包含当前正在运行任务名称的数组。</td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>动作</td>
   <td><p>运行所有升级前运行状况检查，并将其状态保存在名为 <code>preUpgradeHCStatus.properties</code> 在sling主路上。 如果 <code>shutDownOnSuccess</code> 参数设置为 <code>true</code>，则会关闭AEM实例，但前提是所有预升级运行状况检查均处于“正常”状态。</p> <p>属性文件将用作将来任何升级的先决条件<br /> 升级前运行状况检查时，升级过程将停止<br /> 执行失败。 如果要忽略预升级的结果<br /> 运行状况检查并启动升级，您可以删除文件。</p> </td>
  </tr>
  <tr>
...
禁用JAAS配置中定义的自定义模块 `repository.xml`，则必须编辑配置才能使用默认 `LoginModule`，如下例所示：
...
>[!NOTE]
>
>在关闭AEM实例后，仅从crx-quickstart/install目录中删除包。 此步骤是启动就地升级过程之前的最后一个步骤之一。

删除通过 `crx-quickstart/install` 目录。 这样做可防止在更新完成后，在新AEM版本之上意外安装旧修补程序和Service Pack。

## 停止任何冷备用实例 {#stop-tarmk-coldstandby-instance}

如果使用TarMK冷备用，请停止任何冷备用实例。 如果升级中存在问题，这样做可保证高效地重新联机。 成功完成升级后，必须从已升级的主实例重建冷备用实例。
...
如果使用TarMK，则应在升级前运行脱机修订版清理。 这样做可以大大提高存储库迁移步骤和后续升级任务的执行速度，并有助于确保在升级完成后能够成功执行在线修订清理。 有关运行脱机修订版清理的信息，请参阅 [执行脱机修订版清理](/help/sites-deploying/storage-elements-in-aem-6.md#performing-offline-revision-cleanup).
...
通常，AEM用于持久性的基础Apache Oak堆栈会根据需要负责升级数据库架构。

但是，当架构无法自动升级时，可能会出现这些情况。 此类情况主要是高安全性环境，在这些环境中，数据库以具有有限权限的用户运行。 如果出现这种情况，AEM将继续使用旧模式。

要防止发生这种情况，请通过执行以下操作来升级架构：

1. 关闭必须升级的AEM实例。
1. 升级数据库模式。 请查阅有关数据库类型的文档，以了解实现结果所需的工具。
...
1. 将实例与生产流量分离
1. 创建一个或多个导致问题的用户的备份。 您可以通过包管理器来执行此任务。 有关更多信息，请参阅 [如何使用包。](/help/sites-administering/package-manager.md)
1. 删除导致问题的一个或多个用户。 以下是可能属于此类别的用户列表：
...
Adobe建议在开始升级之前存档您当前的日志文件。 这样，在升级期间和升级后便可以更轻松地监视和扫描日志文件，以识别和解决可能出现的任何问题。
