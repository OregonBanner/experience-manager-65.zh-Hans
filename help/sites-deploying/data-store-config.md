...
* `db`:Mongo数据库的名称。缺省值为&#x200B;**Oak** ``. However, new AEM 6 installations use **aem-author** ``作为缺省数据库名称。
...
1. 如果AEM已配置为与Tar或MongoDB存储一起使用，请在继续操作之前，从***&lt;aem-install>**/*crx-quickstart*/*install*&#x200B;文件夹删除任何现有配置文件。 需要删除的文件包括：
...
缓存支持异步上传到DataStore。 文件将在本地、缓存中（在文件系统上）和异步作业开始上传文件。 异步上传的数量受暂存缓存大小的限制。 使用`stagingSplitPercentage`参数配置暂存缓存的大小。 此参数定义用于暂存缓存的缓存大小百分比。 此外，可下载的缓存百分比也计算为&#x200B;**(100 - `stagingSplitPercentage`)*`cacheSize`**。
...
>[!NOTE]
>
>将AEM与TarMK一起使用时，二进制文件默认存储在FileDataStore中。 要将TarMK与Azure DataStore一起使用，您需要使用`crx3tar-nofds`运行模式开始AEM，例如：
