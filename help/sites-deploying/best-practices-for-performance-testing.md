...
* 在测试移动或响应式网站时，需要使用一组单独的工具。 它们通过限制网络带宽来工作，模拟较慢的移动连接（如3G或EDGE）。 使用范围更广的工具包括：
   ...
   * [**Charles**](https://www.charlesproxy.com/)  — 除其他几种用途外，还提供网络限制的web调试代理应用程序。 提供了适用于Windows、OS X和Linux的版本。
...
AEM 6触屏UI中的新增功能之一是开发人员模式。 正如作者可以在编辑模式和预览模式之间切换一样，开发人员也可以在创作UI中切换到开发人员模式，以查看页面上每个组件的渲染时间以及查看任何错误的堆栈跟踪。 有关开发人员模式的更多信息，请参阅此 [CQ Gems演示](https://experienceleague.adobe.com/docs/experience-manager-gems-events/gems/gems2014/aem-developer-mode.html?lang=en).
...
的 **ObservationQueueMaxLength** 部分将显示Oak观察队列中过去几小时、几分钟、几秒和几周的事件数。 在“每小时”部分中查找最大事件数。 此数字需要与 `oak.observation.queue-length` 设置。 如果观察队列显示的最高数超过 `queue-length` 设置：
...
>[!NOTE]
>另请参阅 [AEM 6.x |性能调整提示](https://helpx.adobe.com/experience-manager/kb/performance-tuning-tips.html)

默认设置为10,000，但大多数部署通常需要将其提高到20,000或50,000。
