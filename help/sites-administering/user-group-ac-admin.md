...
**用户帐户** CRX会根据用户帐户中保留的详细信息识别并验证用户（由该人员或其他应用程序），从而验证访问权限。
...
* 您可以指定用户帐户的访问权限。
  ...
  但是，为了简化管理Adobe，建议（在大多数情况下）为组帐户分配访问权限。 快速为每个用户分配访问权限变得非常困难（当仅存在一两个实例时，某些系统用户例外）。
...
* 对于给定主体，给定节点上最多有1个拒绝条目和1个允许条目。 该实施始终会清除冗余条目，并确保允许条目和拒绝条目中未列出相同的权限。
...
<table>
 <tbody>
  ...
  <tr>
   <td><i>保持简单</i></td>
   <td><p>在配置新安装时投入一些时间和思考，将得到很好的回报。</p> <p>应用清晰的结构将简化持续的维护和管理，确保您当前的同事和/或未来的继任者能够轻松了解正在实施的内容。</p> </td>
  </tr>
  <tr>
   <td><i>测试</i></td>
...
这意味着用户帐户可以指定可以与其帐户一起操作的其他帐户（用户或组）。 换言之，如果允许用户B模拟用户A，则用户B可以使用用户A的完整帐户详细信息（包括ID、名称和访问权限）执行操作。

这允许模拟者帐户完成任务，就像他们使用自己所模拟的帐户一样；例如，在缺勤期间或短期共享过多负载。
...
例如，对于 **当前路径** 在左窗格中选择所需的资源，然后在右下窗格中选择访问控制选项卡：
...
* **主体**
  ...
  这允许您 **Search** 对于 **用户** 或 **组**. 从结果列表中选择所需的主体，然后单击 **确定** 以将值带回上一个对话框。
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>这是jcr：write和jcr：nodeTypeManagement的特定于jackrabbit的聚合特权。<br /> </td>
  </tr>
  <tr>
