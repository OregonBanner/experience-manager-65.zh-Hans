...
1. 导航到 **Apache Felix OSGi管理控制台** 然后改变 **用户名** 和 **密码**.
...
>[!NOTE]
>
>如果此实例在中运行，则We.Retail示例应用程序将被删除 [生产就绪模式](/help/sites-administering/production-ready.md). 如果由于任何原因，情况并非如此，您可以转到包管理器以卸载示例内容，然后搜索并卸载所有We.Retail包。 有关更多信息，请参阅 [使用包](package-manager.md).
...
1. 检查默认渲染器的配置，这可能是一个问题区域。
   ...
   * 作为一种预防性措施，可禁用其他默认渲染器(HTML、纯文本、XML)。 通过配置 [Apache SlingGETServlet](/help/sites-deploying/osgi-configuration-settings.md#apache-sling-get-servlet).
   >[!CAUTION]
   >
