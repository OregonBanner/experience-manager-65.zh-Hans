...
description: 适用于Microsoft SharePoint 2010和Microsoft SharePoint 2013的Day JCR连接器，版本4.0。
...
本文包括有关Microsoft SharePoint 2010和Microsoft SharePoint 2013版本4.0的AdobeJCR连接器的详细信息。

SharePoint连接器支持以下基本功能：

* 从SharePoint读取内容和元数据。
* 通过应用本机SharePoint身份验证和授权，确认访问内容的SharePoint安全设置
* 使用Content Finder进行内容集成
* 使用AEM组件（如外部资源）显示SharePoint图像和视频
* 将SharePoint与AEM Assets同步

所有功能都是使用本机SharePoint Web服务作为SharePoint内容和服务的界面实现的。
...
连接器是便于安装的内容封装。 使用包管理器安装包，然后设置SharePoint服务器URL和其他配置选项。 AEM存储库中提供了SharePoint内容。
...
>[!NOTE]
>
>该连接器仅支持Microsoft正式支持的配置。 参见 [MOSS 2010](https://technet.microsoft.com/en-us/library/cc262485(office.14).aspx) 和 [MOSS 2013](https://technet.microsoft.com/en-us/library/cc262485.aspx) 系统要求。
...
安装SharePoint连接器后，配置该连接器的应用程序和SharePoint层。

设置SharePoint服务器URL以使您的SharePoint存储库符合JCR标准。 您可以设置额外的参数来配置与SharePoint服务器的连接。 此外，还可以使用SharePoint连接器配置身份验证。
...
默认情况下，连接器会公开一个JCR工作区。 此工作区公开的SharePoint Server是通过“Sharepoint Server URL”配置参数设置的。

连接器也可以配置为用于多个工作区。 在这种情况下，每个工作区都与通过工作区公开的各个SharePoint服务器的URL相关联。 要添加工作区，请将工作区定义添加到“工作区”参数中。 工作区定义的格式如下：
`<name>`= `<url>` 位置
`<name>` 是JCR工作区的名称，并且
`<url>` 是该工作区的SharePoint服务器的URL。

在AEM中，除了上述配置步骤外，请执行其他步骤。 允许列表&#39;**com.day.cq.dam.cq-dam-jcr-connectors**&#39;包。
...
* SharePoint用户凭据有效，且用户拥有必要的SharePoint权限
* 已正确安装和配置连接器
...
1. 编辑配置值。
1. 设置有权访问SharePoint网站的用户名和相应的密码。
1. 单击“保存”。
...
适用于Microsoft SharePoint 2010和Microsoft SharePoint 2013的AEM JCR连接器版本4.0。支持基于声明的身份验证(由Microsoft建议)，该身份验证以下列模式运行：

* **基本/NTLM身份验证**：连接器首先尝试使用基本身份验证连接。 如果不可用，它将切换到基于NTLM的身份验证。
* **基于Forms的身份验证**：Sharepoint根据用户在登录表单（通常为网页）中键入的凭据来验证用户。 系统为经过身份验证的请求发出一个令牌，该令牌包含用于为后续请求重新建立身份的密钥。
...
1. 安装 `SharepointConnectionFactory` 作为片段捆绑包与片段主机的实施 `com.day.crx.spi.crx2sharepoint-bundle`.

   使用Maven时，调整以下配置 `maven-bundle-plugin` 符合项目要求：
