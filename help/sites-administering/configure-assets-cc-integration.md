...
1. 使用此配置可将AEM Assets内的资产与Creative Cloud同步。

在后端，AEM服务器使用网关验证您的用户档案，然后在AEM Assets和Experience Cloud之间同步数据。

>[!CAUTION]
>
>AEM到Creative Cloud文件夹共享功能在AEM Assets中已弃用。 在AEM和Creative Cloud集成最佳实践 [中了解更多信息并查找替换项](/help/assets/aem-cc-integration-best-practices.md)。

![集成AEM Assets和Creative Cloud时的数据流](assets/chlimage_1-48.png)

集成AEM Assets和Creative Cloud时的数据流
...
1. 单击本地AEM Assets实例的用户界面上的AEM徽标，然后导航到“工 **[!UICONTROL 具]** ”> **[!UICONTROL Cloud Service]** > **[!UICONTROL “旧]**&#x200B;版Cloud Service”。

1. 找到 **[!UICONTROL Adobe Marketing Cloud]** 服务。 如果不存在配置，请单击“ **[!UICONTROL 立即配置]**”。 如果存在配置，请单 **[!UICONTROL 击“显示配]** 置”, `+` 然后单击以添加新配置。
...
1. 在“租 **[!UICONTROL 户URL]** ”字段中，指定AEM Assets的URL。
   ...
   >[!CAUTION]
   >
   >由于重新设定品牌，如果您根据需 `https://<tenant_id>.marketing.adobe.com` 要输入租户URL, `https://<tenant_id>.experiencecloud.adobe.com.` 请将其更改为。为此，请执行以下步骤：
   >
   >1. 导航到&#x200B;**工具 > 云服务 > 旧版云服务**。
   1. 在“Adobe Marketing Cloud”下，单 **击“显示配置**”。
...
1. 在“客 **[!UICONTROL 户端ID]** ”字段中 [，将您复制的应用程序 ID粘贴到“创建应](/help/sites-administering/configure-assets-cc-integration.md#create-an-application)用程序”过程末尾。
   ...
   ![提供集成应用程序 ID和Creative Cloud所需的AEM Assets值](assets/cloudservices_tenant_info.png)
...
1. Click **[!UICONTROL Update]**. 立即将资产与AEM Assets实例同步。

## 测试同步 {#test-the-synchronization}

1. 单击本地AEM Assets实例的用户界面上的AEM徽标，然 **[!UICONTROL 后导航到]**&#x200B;工具 **[!UICONTROL >部署]**> **[!UICONTROL Replication]**，找到为同步创建的复制用户档案。
...
1. 在边栏中，转到“管 **[!UICONTROL 理]** ”，然后单击／点 **[!UICONTROL 按启动企业仪表板]**。
...
1. 从菜单中，选择 **[!UICONTROL Adobe Marketing Cloud]** ，然后单击 **[!UICONTROL 共享]**。 系统会显示一条消息，通知已与Marketing Cloud共享文件夹。
   ...
   >[!NOTE]
   共享类型的“资产” `sling:OrderedFolder`文件夹在Adobe Marketing Cloud中不受支持。 如果要共享文件夹，则在AEM Assets中创建文件夹时，不要选择“已订 **[!UICONTROL 购]** ”选项。

1. 刷新AEM Assets用户界面。 您在本地AEM Assets实例的“资产”控制台中创建的文件夹将复制到Marketing CloudUI。 在AEM服务器处理后，您以AEM Assets形式上传到文件夹的资产会显示在Marketing Cloud中文件夹的副本中。
1. 您还可以在Marketing Cloud中上传文件夹复制副本中的资产。 处理资产后，资产会以AEM Assets形式显示在共享文件夹中。

## 在AEM Assets和Creative Cloud之间交换资产 {#exchange-assets-between-aem-assets-and-creative-cloud}

>[!CAUTION]
已弃用AEM到Creative Cloud文件夹共享功能。 强烈建议客户使用较新的功能，如 [Adobe资产链接](https://helpx.adobe.com/cn/enterprise/using/adobe-asset-link.html) 或AEM [桌面应用程序](https://helpx.adobe.com/experience-manager/desktop-app/aem-desktop-app.html)。 了解AEM和Creative Cloud [集成最佳实践的更多信息](/help/assets/aem-cc-integration-best-practices.md)。
...
例如，如果资产是从AEM Assets（本地）实例上传的，则对资产所做的更改会从该实例传播到Marketing Cloud实例。 但是，从Marketing Cloud实例对同一资产所做的更改不会传播到AEM实例，从Marketing Cloud上传的资产也会传播到实例，反之亦然。
