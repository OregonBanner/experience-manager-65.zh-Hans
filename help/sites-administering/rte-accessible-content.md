...
要配置和自定义这些功能，请 [配置RTE插件](#configure-the-plugin-features) 的子菜单。 例如， `paraformat` 插件允许您添加其他块级语义元素，包括扩展基本的标题级别以外支持的标题级别数 `H1`, `H2`和 `H3` 默认提供。

RTE可在支持触屏操作的用户界面和经典用户界面的各种组件中使用。 但是，使用RTE的主组件是 **文本** 组件。 下图显示了启用了一系列插件的RTE，包括 `paraformat`:
...
通过在相应的 `rtePlugins` CRXDE Lite的子分支中，您可以激活该插件的所有功能或特定功能。
...
在 **表**，则显式设置或从DOM元素中删除属性。 该值由 `config` 对象。 请注意，应使用相应的 `CQ.form.rte.Common` 方法( `com` 是 `CQ.form.rte.Common`)，以避免浏览器实施存在的常见缺陷。
...
RTE可以使用 `strong` 和 `em` 标记替代 `b` 和 `i`. 将以下节点作为同级节点添加到 `uiSettings` 和 `rtePlugins` 中的节点。
...
>[!NOTE]
>
>纯文本字段不是唯一允许为标题元素值输入的类型。 您可以使用任何ExtJS小组件，该小组件通过 `getValue()` 方法。
>
