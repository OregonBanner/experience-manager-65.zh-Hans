...
>[!NOTE]
>
>升级到6.4后，建议将文件从/etc移动。 参见 [AEM 6.5中的常见存储库重组](/help/sites-deploying/all-repository-restructuring-in-aem-6-5.md#translation-rules) 了解更多详细信息。
...
例如，您可以创建一个规则，以翻译作者添加到页面上所有AEM Foundation文本组件的内容。 此规则可以标识 `foundation/components/text` 组件的 `/content` 节点和 `text` 属性。
...
以下示例规则导致为 `/content` 节点下的所有页面翻译所有 `text` 属性的内容。该规则适用于将内容存储在中的任何组件。 `text` 属性，如foundation文本组件和foundation图像组件。
...
以下示例翻译所有 `text` 属性，并且还会翻译foundation图像组件的其他属性。 如果其他组件具有同名属性，则该规则不适用于它们。
...
有4个属性可以通过UI更改： `isDeep`， `inherit`， `translate` 和 `updateDestinationLanguage`.
...
在编辑器中，您可以选中/取消选中 **深** 在 **筛选器** 选项卡。

![chlimage_1-59](assets/chlimage_1-59.jpeg)

以下是以下情况下生成的xml示例 **深** 未在UI中选中：
...
**inherit** 这适用于属性。 默认情况下，每个属性都会被继承，但如果您希望某个属性不会在子项上继承，则可以将该属性标记为false，以便仅将该属性应用于该特定节点。
...
在UI中，您可以选中/取消选中 **转换** 在 **属性** 选项卡，但适用于将语言代码作为值的特定属性。
...
如果编辑translation_rules.xml文件，请在内容包中保留备份副本。 安装AEM服务包或重新安装某些AEM包可将当前translation_rules.xml文件替换为原始文件。 要在此情况下恢复您的规则，您可以安装包含备份副本的包。
