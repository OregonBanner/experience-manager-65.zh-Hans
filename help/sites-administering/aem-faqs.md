...
例如，在创建名为(和标题 `/content/geometrixx` 为法语（卢森堡） `fr_LU` 的语言根目录。 随后，从引用面板中创建页面的语言副本，并导航到中 `Create structure only` 的选项 `Create & Translate`。 最后，创建翻译项目，然后将语言副本添加到翻译作业。
...
请参阅了 [解升级AEM的理由](https://helpx.adobe.com/experience-manager/kt/platform-repository/using/upgrade-aem-article-understand.html) ，其中描述了考虑升级到最新版Adobe Experience manager的客户的主要功能的高级细分。
...
Adobe Experience Manager(AEM)6.5当前允许您一次上传最多2 GB的资产。

有关可以使用AEM 6.5操作的资产最大数量的其他信息，请参阅资产规 [模调整指南](/help/assets/assets-sizing-guide.md)。
...
AEM提供了各种机制，使您能够自定义创作实例的控制台和页面创作功能。 要了解如何创建自定义控制台和自定义控制台的默认视图，请参阅自 [定义控制台](/help/sites-developing/customizing-consoles-touch.md)。
...
要详细了解，请参阅基于CoralUI 3 [的迁移指南](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/granite-ui/api/jcr_root/libs/granite/ui/components/legacy/coral2/migration.html)。

#### 如何在AEM资产中自定义搜索组件？ {#how-to-customize-the-search-component-in-aem-assets}

要了解搜索提升／排名和进一步实施信息，请参阅简 [单搜索实施指南](https://helpx.adobe.com/experience-manager/kt/sites/using/search-tutorial-develop.html)。

简单搜索实施是2017年峰会实验室AEM Search Demystifed的材料。

#### AEM Assets与AEM MediaLibrary之间有何区别？ {#what-is-the-difference-between-aem-assets-and-aem-medialibrary}

AEM资产是AEM Platform上的一个应用程序，它允许我们的客户在基于Web的存储库中管理其数字资产（图像、视频、文档和音频剪辑），而AEM媒体库是AEM WCM内容存储库中存储图像和其他共享资源的指定部分。

有关更 [多信息，请参阅AEM资产与AEM media库](/help/assets/medialibrary.md) 。

#### 能否为WordPress构建插件，让客户能够访问Adobe Asset Picker来选择图像？ {#is-it-possible-to-build-plugin-for-wordpress-that-allows-a-customer-to-access-adobe-asset-picker-to-select-images}

是的，使用WordPress的客户可以使用Adobe Asset Picker从其AEM Assets服务器中选择图像，以将其添加到其WordPress站点上的帖子。

有关详细 [信息，请参阅资产](../assets/search-assets.md#assetselector) 选择器。

#### 是否可以扩展AEM资产中的搜索彩块化以添加其他谓词？ {#is-it-possible-to-extend-the-search-facets-in-aem-assets-to-add-additional-predicates}

Adobe Experience Manager(AEM)资产的企业级部署能够存储许多资产。 您可以向默认表单添加谓词，或使用包含您选择的facet的自定义表单。
