...
AEM WCM会安装多个用户和组。 在安装后首次访问Security Console时，可以看到这些内容。
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>用户</td>
   <td><p>具有完全访问权限的系统管理帐户。</p> <p>此帐户用于AEM WCM和CRX之间的连接。</p> <p>如果意外删除此帐户，则会在存储库重新启动时重新创建此帐户（在默认设置中）。</p> <p>管理帐户是AEM平台的一项要求。 因此，无法删除此帐户。</p> </td>
   <td><p>Adobe强烈建议更改此用户帐户的密码，使其不再为默认设置。</p> <p>最好在安装时进行，不过之后可以进行。</p> <p>注意：不应将此帐户与CQ Servlet引擎的管理员帐户混淆。</p> </td>
  </tr>
  <tr>
   <td><p>匿名</p> <p> </p> </td>
   <td>用户</td>
   <td><p>拥有对实例进行未经身份验证访问的默认权限。 根据默认，这将保留最低访问权限。</p> <p>如果您意外删除此帐户，则会在启动时重新创建此帐户。 无法永久删除它，但可以禁用它。</p> </td>
   <td>请避免删除或禁用此帐户，因为它将对作者实例的运行产生负面影响。 如果安全要求要求您删除它，请确保首先正确测试它对您的系统产生的影响。</td>
  </tr>
  <tr>
   <td><p>作者</p> <p>默认密码：作者</p> </td>
   <td>用户</td>
   <td><p>允许写入/content的作者帐户。 包含投稿人和浏览者权限。</p> <p>可用作网站管理员，因为它有权访问整个/content树。</p> <p>这不是一个内置用户，而是另一个geometrixx演示用户</p> </td>
   <td><p>Adobe建议彻底删除帐户，或者更改密码。</p> <p>最好在安装时进行，不过之后可以进行。</p> </td>
  </tr>
  <tr>
   <td>管理员</td>
   <td>组</td>
   <td><p>为其所有成员授予管理员权限的组。 仅允许管理员编辑此组。</p> <p>具有完全访问权限。</p> </td>
   <td>即使您在节点上设置了“deny-everyone”，管理员仍可以访问节点</td>
  </tr>
  <tr>
   <td>content-author</td>
   <td>组</td>
   <td><p>负责内容编辑的组。 需要读取、修改、创建和删除权限。</p> </td>
   <td>您可以创建自己的具有项目特定访问权限的内容创作组，前提是您添加读取、修改、创建和删除权限。</td>
  </tr>
  <tr>
   <td>contributor</td>
   <td>组</td>
   <td><p>允许用户写入内容的基本权限（仅在功能中）。</p> <p>不向/content树分配任何权限 — 这些权限必须专门分配给各个组或用户。</p> </td>
   <td> </td>
  </tr>
  <tr>
   <td>dam-users</td>
  ...
  <tr>
   ...
   <td>组</td>
   <td><p>参与工作流的用户必须是组workflow-users的成员。 这将授予他或她对/etc/workflow/instances的完全访问权限，以便他或她可以更新工作流实例。</p> <p>该组包含在标准安装中，但您必须手动将用户添加到该组。</p> </td>
  </tr>
 </tbody>
</table>
...
权限定义允许对资源执行哪些操作的人员。 权限是以下各项的结果 [访问控制](#access-control-lists-and-how-they-are-evaluated) 评估。

通过选中或清除单个AEM的复选框，您可以更改向给定用户授予/拒绝的权限 [操作](security.md#actions). 复选标记表示允许某个操作。 无复选标记表示操作被拒绝。

复选标记在网格中的位置还指示用户在AEM中的哪些位置（即哪些路径）具有哪些权限。
 ...
 <tbody>
  ...
  <tr>
   ...
   <td><p>用户可以：</p>
    ...
    </ul> <p>在JCR级别，用户可以通过修改资源的属性、锁定、版本控制和nt-modifications来修改资源，并且他们对定义jcr：content子节点的节点具有完整的写入权限，例如cq：Page、nt：file、cq：Asset。</p> </td>
  </tr>
  <tr>
   <td>创建</td>
   <td><p>用户可以：</p>
    <ul>
     <li>创建新页面或子页面。</li>
    </ul> <p>如果 <strong>修改</strong> 被拒绝jcr：content下的子树被明确排除，因为创建jcr：content及其子节点被视为页面修改。 这仅适用于定义jcr：content子节点的节点。</p> </td>
  </tr>
  <tr>
   <td>删除</td>
   <td><p>用户可以：</p>
    <ul>
     ...
     <li>删除页面或子页面。</li>
    </ul> <p>如果 <strong>修改</strong> 被拒绝jcr：content下的任何子树都因移除jcr：content而被特别排除，其子节点被视为页面修改。 这仅适用于定义jcr：content子节点的节点。</p> </td>
  </tr>
  <tr>
   <td>读取 ACL</td>
...
访问控制列表由各个权限组成，用于确定这些权限的实际应用顺序。 该列表根据所考虑页面的层次结构形成。 然后自下而上扫描此列表，直到找到应用于页面的第一个相应权限。
...
>
>当您创建特定于社区的ACL时，可能会向加入这些社区的成员授予附加权限。 例如，当用户在以下位置加入社区时，可能会出现这种情况： `/content/we-retail/us/en/community`
...
>对于CQ 5.3用户：
>
>与以前的CQ版本相比， **创建** 和 **delete** 如果用户只需要修改页面，则不应再被授予。 相反，授予 **修改** 操作。
>
...
<table>
 <tbody>
  ...
  <tr>
   <td>下部</td>
   <td>列出在树的其他位置可能具有效果的非有效条目（如特殊属性所指示，相应的ACE限制条目的范围）。 或者，这是一个条目，其效果已被在给定路径或祖先节点上定义的另一个条目所撤销。</td>
  </tr>
 </tbody>
</table>
...
* 不要将权限直接分配给用户。 仅将它们分配给组。
   ...
   这将简化维护工作，因为组的数量比用户数量少得多，而且波动性也小。

* 如果您希望组/用户只能修改页面，请不要授予他们创建或拒绝权限。 仅授予他们修改和读取权限。
* 请谨慎使用“拒绝”。 尽可能仅使用“允许”。

   如果以与预期顺序不同的顺序应用权限，则使用deny可能会导致意外效果。 如果用户是多个组的成员，则来自一个组的Deny语句可以取消来自另一个组的Allow语句，反之亦然。 发生此情况时，很难保留概览，并且容易导致不可预见的结果，而允许分配不会导致此类冲突。

   Adobe建议您使用“允许”而不是“拒绝”，请参见 [最佳实践](#best-practices).

在修改任一权限之前，请确保您了解它们的工作方式和相互关系。 请参阅CRX文档以说明AEM WCM的工作原理 [评估访问权限](/help/sites-administering/user-group-ac-admin.md#how-access-rights-are-evaluated) 以及有关设置访问控制列表的示例。
...
<table>
 <tbody>
  <tr>
   <td>上半部分</td>
   <td><p>重复您在树状网格中看到的信息。 对于每个操作，都有一个图标显示是允许还是拒绝该操作：</p>
    <ul>
     <li>无图标=无声明条目</li>
     <li>（勾号）=声明的操作（允许）</li>
     <li>(-) =声明的操作（拒绝）</li>
...
这意味着用户帐户可以指定可以与其帐户一起操作的其他帐户。 换句话说，如果允许用户B模拟用户A，则用户B可以使用用户A的完整帐户详细信息执行操作。

这允许模拟者帐户完成任务，就像他们使用自己所模拟的帐户一样；例如，在短期缺勤或共享过多负载时。

>[!NOTE]
>
>为了模拟非管理员用户的工作环境，模拟者（在上述示例中是用户B）需要在以下位置具有“读取”权限： `/home/users` 路径。
>
>有关如何实现此目标的更多信息，请参阅 [AEM中的权限](/help/sites-administering/security.md#permissions-in-aem).

>[!CAUTION]
>
>如果一个帐户模拟另一个帐户，则很难查看。 模拟开始和结束时在审核日志中创建一个条目，但其他日志文件（如访问日志）不包含有关事件上已发生模拟的事实的信息。 因此，如果用户B模拟用户A，则所有事件看起来都像由用户A个人执行。
...
| 规则 | 原因 |
|--- |--- |
| *使用组* | 避免逐个用户分配访问权限。 这有几个原因：<ul><li>用户数多于组数，因此组简化了结构。</li><li>组可帮助提供所有帐户的概述。</li> <li>对组进行继承更简单。</li><li>用户来了又走。 组是长期的。</li></ul> |
| *积极的* | 始终使用Allow语句指定组的权限（如果可能）。 避免使用Deny语句。 按顺序评估组，并且可能按用户定义不同的顺序。 换言之：您可能无法控制语句的实施和评估顺序。 如果您只使用Allow语句，则顺序无关紧要。 |
| *保持简单* | 在配置新安装时投入一些时间和精力，将得到很好的回报。 应用清晰的结构将简化持续的维护和管理，确保您当前的同事和/或未来的继任者能够轻松了解正在实施的内容。 |
...
| 制表符 | 描述 |
|--- |--- |
| 筛选框 | 用于筛选列出的用户和/或组的机制。 参见 [筛选用户和组](#filtering-users-and-groups). |
| 隐藏用户 | 将隐藏列出的所有用户，仅保留组的切换开关。 参见 [隐藏用户和组](#hiding-users-and-groups). |
| 隐藏组 | 将隐藏列出的所有组，仅保留用户的切换开关。 参见 [隐藏用户和组](#hiding-users-and-groups). |
...
>
>如果您在JEE上使用AEM Forms，请不要使用以下说明更改密码，而应使用JEE Admin Console (/adminui)上的AEM Forms更改密码。
...
1. 输入新密码两次；由于它们未以明文显示，因此用于确认 — 如果它们不匹配，则系统显示错误。
1. 单击 **设置** 以激活帐户的新密码。
...
1. 选中 **会员** 列来指定您希望拥有该路径权限的成员。 清除要删除其权限的成员复选框。 对进行更改的单元格中会出现一个红色三角形。
1. 单击&#x200B;**确定**&#x200B;以保存您的更改。
...
当部署是 [发布场](/help/sites-deploying/recommended-deploys.md#tarmk-farm)，用户和组需要在所有发布节点之间同步。
...
您可以指定一个或多个允许模拟当前用户的用户。 这意味着他们可以将其帐户设置切换到当前用户的帐户设置，并代表此用户执行操作。
...
1. 单击 **模拟者** 选项卡。
1. 单击您希望能够模拟所选用户的用户。 将用户（将进行模拟）从列表拖到模拟窗格。 该名称将显示在列表中。
...
1. 将您要授予管理其他用户的权限的用户添加到管理员组，并保存更改。
...
1. 在用户的 **权限** 选项卡，导航到“/”，在复制列中，选中允许在“/”处复制的复选框，然后单击 **保存**.
   ...
   现在，选定的用户能够停用、激活、删除和创建用户。

### 扩展项目级别的权限 {#extending-privileges-on-a-project-level}

如果您计划实施特定于应用程序的权限，以下信息将描述实施自定义权限需要了解的信息以及如何在CQ中实施自定义权限：

jcr-privileges的组合覆盖了层次结构修改权限。 已命名复制权限 **crx：replicate** 该权限将与其他权限一起在jcr存储库中存储/评估。 但是，在jcr级别上并未强制执行。

自定义权限的定义和注册正式属于 [Jackrabbit API](https://jackrabbit.apache.org/api/2.8/org/apache/jackrabbit/api/security/authorization/PrivilegeManager.html) 从版本2.4开始(另请参阅 [JCR-2887](https://issues.apache.org/jira/browse/JCR-2887))。 JCR访问控制管理涵盖了其他用法，例如由定义的 [JSR 283](https://jcp.org/en/jsr/detail?id=283) （第16节）。 此外，Jackrabbit API还定义了两个扩展。

权限注册机制反映在下的用户界面中 **存储库配置**.

新（自定义）权限的注册本身受必须在存储库级别授予的内置权限保护（在JCR中：将“null”作为ac mgt api中的“absPath”参数进行传递，有关详细信息，请参阅jsr 333）。 默认情况下， **管理员** 所有管理员成员都拥有该权限。
