...
1. 从包管理器中，安装下载的包。 请参阅 [如何使用包](/help/sites-administering/package-manager.md) 有关在AEM中使用Software Distribution和包的更多信息
...
1. 选择要在其中创建新Livefyre配置的配置文件夹，然后单击 **创建**.
   ...
   >[!NOTE]
   >
   >文件夹的属性中必须启用云配置，然后才能向其添加Livefyre配置。 配置文件夹在 [配置浏览器。](/help/sites-administering/configurations.md)
   >
...
为页面启用Livefyre后，必须将容器配置为允许使用Livefyre组件。 请参阅 [在设计模式下配置组件](https://experienceleague.adobe.com/docs/experience-manager-65/authoring/siteandpage/default-components-designmode.html) 以了解有关如何启用不同组件的说明。
...
1. 选择是创建新的Livefyre应用程序还是嵌入现有应用程序。
   ...
   有关插入组件的更多信息，请参阅 [编辑页面内容](https://experienceleague.adobe.com/docs/experience-manager-65/authoring/authoring/editing-content.html).
