...
本节介绍如何在中使用批量编辑器 **工具** 控制台。 通常，管理员使用批量编辑器来搜索和编辑多个项目。 这是通过使用GQL查询填充表，然后选择要处理的内容项来完成的。 作者通常使用批量编辑器作为自定义批量编辑器应用程序的一部分，该应用程序可通过访问 [产品列表](/help/sites-authoring/default-components.md#productlist) 组件。
...
<table>
 <tbody>
  ...
  <tr>
   <td>属性/列</td>
   <td>选中希望Bulk Editor返回的属性的复选框。 您选择的属性是结果窗格中的列标题。 默认情况下，节点路径显示在结果中。</td>
  </tr>
  <tr>
   <td>自定义属性/列</td>
...
* **订购：** 按给定的属性对结果进行排序。 您可以指定多个以逗号分隔的属性名称。 要以降序排序结果，只需在属性名称的前面加一个减号即可。 例如，order：-name。 使用加号会按升序返回结果，这也是默认设置。
* **限制：** 使用间隔限制结果的数量。 例如，limit：10..20间隔从零开始，开始为包含开始，结束为包含结束。 您还可以指定开放间隔:limit:10.. 或限制：...20如果忽略这些点并且只指定一个值，则GQL最多将返回此数量的结果。 例如，limit：10（返回前十个结果）。
...
默认情况下，打开“批量编辑器”时导入功能处于隐藏状态。 只需添加参数 `hib=false` 到URL时显示 **导入** 按钮。 您可以从任何制表符分隔的格式( `.tsv`)文件。 要使导入正常工作，列标题（单元格的第一行）必须与要导入到的表的列标题匹配。
