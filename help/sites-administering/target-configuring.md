...
此 [选择加入向导](/help/sites-administering/opt-in.md) 该 [将AEM与Adobe Target集成](/help/sites-administering/target.md) 自动创建名为已设置Target配置的Target云配置。 该向导还会为名为“已设置的Target框架”的云配置创建一个Target框架。 如有必要，您可以修改云配置和框架的属性。
...
为此，您需要指定要将您的Adobe Target云配置连接到的A4T云配置：
...
您可以配置云配置以从 Adobe Target 同步片段。如果启用同步，则在保存云配置时，将从后台的Target导入区段。
...
1. 在对话框中，提供这些属性的值。
   ...
   * **使用准确定位：**&#x200B;默认情况下，此复选框处于选中状态。如果选择，则云服务配置将等待上下文加载后再加载内容。 请参阅以下注释。
...
* 在Adobe Target中编辑活动时，URL指向本地主机。 通过将AEM外部化器设置为正确的URL来解决此问题。
