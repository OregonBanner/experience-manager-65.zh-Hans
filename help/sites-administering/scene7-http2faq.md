...
Adobe很高兴地宣布推出HTTP/2内容交付功能。 当您使用HTTP/2时，会注意到整体性能有所提高。
...
性能提高会因各种因素而有很大的差异，例如您网站的代码、您如何使用Dynamic Media、客户的设备、屏幕和位置。
...
* 使用专用域(即， `images.company.com` 或 `mycompany.scene7.com`)，而不是通用的Dynamic Media域(即， `s7d1.scene7.com`， `s7d2.scene7.com`，或 `s7d13.scene7.com`)。
   ...
   要查找您的域，请打开 [Dynamic Media Classic桌面应用程序](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/getting-started/signing-out.html#getting-started)，然后登录到您的公司帐户或帐户。 然后导航到 **[!UICONTROL 设置]** > **[!UICONTROL 应用程序设置]** > **[!UICONTROL 常规设置]**. 查找标记为的字段 **已发布的服务器名称**. 如果您当前使用的是通用Dynamic Media域，则可以请求在此过渡中转移到您自己的自定义域。

## 为我的Dynamic Media帐户启用HTTP/2的过程是怎样的？ {#what-is-the-process-for-enabling-http-for-my-scene-account}

1. [使用Admin Console创建支持案例](https://helpx.adobe.com/enterprise/admin-guide.html/enterprise/using/support-for-experience-cloud.ug.html) 和请求切换到HTTP/2；不会自动为您完成此操作。
1. 在您的支持案例中提供以下信息：
   ...
   * 验证您是否使用专用域。 那就是， `images.company.com` 或 `mycompany.scene7.com`，不是通用Dynamic Media域，例如 `s7d1.scene7.com`， `s7d2.scene7.com`， `s7d13.scene7.com`.
      ...
      要查找您的域，请打开 [Dynamic Media Classic桌面应用程序](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/getting-started/signing-out.html#getting-started)，然后登录到您的公司帐户或帐户。 然后导航到 **[!UICONTROL 设置]** > **[!UICONTROL 应用程序设置]** > **[!UICONTROL 常规设置]**. 查找标记为的字段 **[!UICONTROL 已发布的服务器名称]**. 如果您当前使用的是通用Dynamic Media域，则可以请求在此过渡中转移到您自己的自定义域。

1. Adobe客户支持根据提交请求的顺序将您添加到HTTP/2客户轮候表中。
1. 当Adobe准备好处理您的请求时，“支持中心”会联系您以协调过渡并设置目标日期。
1. 完成后，您将会收到通知，并且可以验证是否成功过渡到HTTP2。

## 我何时可以过渡到HTTP/2？ {#when-can-i-expect-to-be-transitioned-over-to-http}

请求将按照Adobe客户支持部门接收它们的顺序进行处理。
...
非缓存内容直接点击Adobe的原始服务器，直到再次重建缓存。 由于此操作，Adobe计划一次处理几个客户过渡，以便在从Adobe源拉取请求时保持可接受的性能。

## 如何验证URL或网站是否通过HTTP/2激活？ {#how-can-you-verify-whether-a-url-or-website-is-activated-with-http}

下载可与Web浏览器一起使用的扩展。 对于Firefox和Chrome，扩展名为 **[!UICONTROL HTTP/2和SPDY指示器]**. 浏览器仅安全地支持HTTP/2，因此有必要使用HTTPS调用URL以进行验证。 如果支持HTTP/2，则扩展将以蓝色Flash符号和标头“X-Firefox-Spdy”的形式表示：“h2”。
