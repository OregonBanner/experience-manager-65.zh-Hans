...
为了帮助您监视和分析实例的状态，AEM提供了一系列默认报表，您可以根据自己的要求对这些报表进行配置：
...
所有报告都可从以下位置访问： **工具** 控制台。 选择 **报表** 在左侧窗格中，双击右侧窗格中的所需报告以将其打开以供查看和/或配置。

还可以从创建新报表实例 **工具** 控制台。 选择 **报表** 在左侧窗格中，然后 **新建……** 工具栏中。 定义 **标题** 和 **名称**，选择所需的报表类型，然后单击 **创建**. 您的新报告实例将显示在列表中。 双击以打开，然后从Sidekick中拖动组件以创建第一列并启动报表定义。

>[!NOTE]
>
>除了开箱即用的标准AEM报告之外，您还可以 [开发您自己的（全新）报告](/help/sites-developing/dev-reports.md).
...
* 要添加新列，请将所需的组件从sidekick拖放到所需的位置
   ...
   * 绿色勾号将指示位置何时有效，一对箭头将准确指示位置位置
   * 位置无效时，将显示一个红色的no-go符号

* 要移动列，请单击标题并按住并拖动到新位置
* 要删除列，请单击列标题，按住并向上拖动到报表标题区域（红色减号表示该位置无效）；释放鼠标按钮，“删除组件”对话框将请求确认您确实要删除该列。

### 列下拉菜单 {#column-drop-down-menu}

报表中的每一列都有一个下拉菜单。 当鼠标光标在列标题单元格上移动时，该图标将变得可见。

标题单元格的最右侧将显示一个箭头头(不要与标题文本右侧的箭头头混淆，标题文本右侧指示 [当前排序机制](#sorting-the-data))。

![reportcolumnsort](assets/reportcolumnsort.png)

菜单上的可用选项将取决于列的配置（如在项目开发期间所做的），任何无效选项都将灰显。

### 对数据进行排序 {#sorting-the-data}

数据可根据特定列按以下任一方式排序：

* 单击相应的列标题；排序将在升序和降序之间切换，标题文本旁边有一个箭头指示此顺序
* 使用 [列的下拉菜单](#column-drop-down-menu) 以具体选择 **升序排序** 或 **降序排序**；再次在标题文本旁边有一个箭头指示此名称
...
在至少将一列分组后，将饼图分为 **当前数据** 将基于此分组生成。 如果对多个列进行分组，这也会在图表中指示。

![报告用户](assets/reportuser.png)

将光标移动到饼图上将显示相应区段的聚合值。 这会使用当前为列定义的聚合；例如count、minimum、average等。
...
* 使用 **完成……** 以重新初始化集合。
  ...
  **完成** “冻结”报表的结构（即分配给报表的列，这些列按分组、排序、过滤等） 开始拍摄快照。
...
>[!NOTE]
>
>预配置的报告不会对性能造成很大影响，但仍建议在生产环境中使用每日快照。 如果可能，请在网站上没有太多活动时运行这些每日快照；这可以使用进行定义 `Daily snapshots (repconf.hourofday)` 参数 **Day CQ报表配置**；请参见 [OSGI配置](/help/sites-deploying/configuring-osgi.md) 以了解有关如何配置的详细信息。
...
* **报表处理**
   ...
   * **手动刷新数据**
     ...
     选择此项后， **[加载数据](#load-data)** 按钮将会显示（在旁边） **编辑** （在报告中）。 **加载数据** 将加载数据，并刷新显示的报表数据。
...
* 报告定义 *截至该时间点* 将用于生成快照（之后，您可以继续处理报告定义，因为它与快照是分开的）。
* 将删除任何现有的快照。
...
所有组件都包含在产品标准和特定于项目的情况下。 使用 **编辑** 对话框，用户也可以设置 **根路径** 定义报告的起点 — 该根下的所有组件都考虑用于报告。
...
报告从存储库的根( / )开始；通过单击特定分支，您可以在存储库内向下展开（当前路径将反映在报告标题中）。
...
>[!CAUTION]
>
>如果您愿意 **历史数据** （对于发布环境），然后 **完成** 在激活页面之前有关创作环境的报表。
