...
>
>可以在项目级别上实施电子邮件组件，已弃用的AEM电子邮件组件说明了如何实现这一点。 但是，这些项目不应使用这些已弃用的组件。

本文档介绍了有关电子邮件设计的一些最佳实践，从而生成了完善的电子邮件活动模板。

AEM中提供的演示活动遵循了所有这些最佳实践。 介绍了如何在演示活动中实施最佳实践，以适合每个最佳实践。

在创建您自己的新闻稿时，请使用这些最佳实践。

>[!NOTE]
>
>所有活动内容都应在类型页 `master` 面下创建 `cq/personalization/components/ambitpage`。
>
>例如，如果您计划的活动结构类似于
>
...
>[!NOTE]
>
>在创建用于Adobe Campaign的邮件时，必须在模板的 **jcr:content** ( **jcr:content** )节点中包含具有值 **mapRecipient** 的属性acMapping **** ，或者您无法在EM的“页面属性”（禁用）字段中选择Adobe Campaign模板。
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td><p>可通过设计更改 <i>“/etc/designs/default/jcr:content/活动_newsletterpage”中的cq:doctype</i><i>属性进行配置</i></p> <p>默认值为“XHTML”:</p> <p>&lt;!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "https://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"&gt;</p> <p>可更改为“HTML_5”:</p> <p>&lt;!DOCTYPE HTML&gt;</p> </td>
  </tr>
  <tr>
...
AEM附带了多个模板和组件，供您开箱即用以创建活动新闻稿。 您可以使用这些模板和组件创建自定义新闻稿。

### 模板 {#templates}

为了优惠坚实的基础并扩大内容流的各种可能性，现有的模板类型有三种略有不同。 您可以轻松使用这些组件来构建自定义Newsletter。
...
| Textimage | /libs/mcm/活动/components/textimage |
...
这些组件在Adobe Campaign组件中有详细 [的介绍](/help/sites-authoring/adobe-campaign-components.md)。