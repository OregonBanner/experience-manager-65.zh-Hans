...
列表中包含每个发布实例的URL，该实例将运行该实例的诊断。 url参数 `syncUser` 附加到诊断URL中，其值设置为 *授权同步用户* 创建于 [步骤2](#createauthuser).
...
作为 `administrators` 组中，授权用户应对所有发布实例具有以下权限：
