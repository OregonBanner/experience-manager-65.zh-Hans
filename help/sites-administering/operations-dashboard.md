...
这将触发下载包含系统状态和配置有用信息的zip文件。 存档包含实例配置、包列表、OSGi、Sling量度和统计信息，这可能会生成大文件。 您可以使用 **下载状态ZIP**&#x200B;窗口。 窗口可从以下位置访问：**AEM >工具>操作>诊断>下载状态ZIP。**
...
如果维护任务不应在所有安装上运行（例如，只在发布实例上运行），则可以通过添加 `@Component(policy=ConfigurationPolicy.REQUIRE)`. 然后，您可以根据存储库中的运行模式，将相应的配置标记为。 有关更多信息，请参阅 [配置OSGi](/help/sites-deploying/configuring-osgi.md#creating-the-configuration-in-the-repository).
