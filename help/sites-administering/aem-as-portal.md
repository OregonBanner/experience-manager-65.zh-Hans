...
假定门户服务器在主机localhost上运行，端口8080，并且AEM portlet Web应用程序在Web应用程序上下文中装载 *cqportlet*，则web控制台的url为 `https://localhost:8080/cqportlet/cqbridge/system/console`. 默认的用户和密码为 **管理员**.
...
1. 假定门户服务器在主机上运行 **localhost**、 **端口8080 **和AEM portlet web应用程序已装载到上下文中 **cqportlet**，则用于刷新缓存的url为 `https://localhost:8080/cqportlet/cqbridge/cqpcache?Path=$(path)`. 使用GET作为方法。
...
1. 单击 **编辑。**
1. 在 **设置** 选项卡，选择 **已启用** 复选框，选择 **调度程序刷新** ，并输入重试超时(例如，60000)。
...
发布视图只有一个按钮，可将工具栏切换到管理视图。 发布视图由 [上一包](/help/sites-deploying/configuring-osgi.md#bundles). 在HTML中，您可以使用以下占位符，在呈现时，这些占位符将由portlet替换为相应的内容：

#### 发布视图占位符 {#publish-view-placeholders}

| 占位符字符串 | 描述 |
|---|---|
| {buttonManage} | 占位符替换为 **管理** 按钮，将portlet状态切换为管理状态。 |

#### 管理视图 {#manage-view}

“管理”视图包含四个按钮：“编辑”、“网站”选项卡、“刷新”和“返回”。 管理视图由 [上一包](/help/sites-deploying/configuring-osgi.md#bundles). 在HTML中，您可以使用以下占位符，在呈现时，这些占位符将由portlet替换为相应的内容：

#### 管理视图占位符 {#manage-view-placeholders}

| 占位符字符串 | 描述 |
|---|---|
| {buttonEdit} | 占位符替换为 **编辑** 按钮，以在AEM编辑模式下使用当前页面打开一个新窗口。 |
...
当从AEM请求内容时，Portlet使用当前显示模式（发布或创作）和当前路径来组合完整的URL。 如果使用默认值，则第一个url为 `https://localhost:4503/content/geometrixx/en.portlet.html`. 的值 `htmlSelector` 会在扩展之前自动添加到URL中。

如果Portlet切换到帮助模式，则 `appendHelpViewModeAsSelector` ，则 `help` 此外，还附加了选择器，例如， `https://localhost:4503/content/geometrixx/en.portlet.html.help`. 如果Portlet窗口已最大化，并且 `appendMaxWindowStateAsSelector` ，则也会附加选择器，例如， `https://localhost:4503/content/geometrixx/en.portlet.max.help`.
