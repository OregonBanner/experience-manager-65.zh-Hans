...
可以在中手动创建用户和组 [!DNL Admin Console] UI。 如果可供管理的用户不多，则可以使用此方法。 例如，少于50个AEM用户。
...
在本例中，用户还被分配到了 [!DNL Admin Console]. (可以使用用户同步工具从LDAP同步用户和组，也可以在本地创建。 参见 **将用户载入[!DNL Admin Console]** （更早）。
