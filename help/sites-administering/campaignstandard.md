...
通过将AEM与Adobe Campaign集成，您可以直接在AEM中管理电子邮件投放、内容和表单。 需要同时执行Adobe Campaign Standard和AEM中的配置步骤才能在解决方案之间启用双向通信。

此集成允许AEM和Adobe Campaign Standard单独使用。 营销人员可以在Adobe Campaign中创建活动并使用定位，而内容创建者则可以同时在AEM中进行内容设计。 通过集成，Adobe Campaign可以定位和交付在AEM中创建的营销活动的内容和设计。
...
1. 点按或单击菜单栏左上角的Adobe Campaign徽标以打开全局导航，然后选择 **管理** > **用户和安全性** > **用户** 从导航菜单中。

1. 点按或单击 `aemserver` 用户控制台中的用户。

1. 确保 `aemserver` 至少将用户分配给具有角色的安全组 `deliveryPrepare` 分派给它的。 默认情况下，组 `Standard Users` 具有此角色。

   ![Adobe Campaign中的aemserver用户](assets/acs-aemserver-user.png)

1. 点击或单击 **保存** 以保存更改。

您的 `aemserver` 用户现在拥有必要的权限，因此AEM可以使用该权限与Adobe Campaign通信。
...
1. 点按或单击菜单栏左上角的Adobe Campaign徽标以打开全局导航，然后选择 **管理** > **应用程序设置** > **选项** 从导航菜单中。

1. 点按或单击 `AEMResourceTypeFilter` 选项控制台中的。
...
1. 点按或单击菜单栏左上角的Adobe Campaign徽标以打开全局导航，然后选择 **资源** > **模板** > **投放模板** 从导航菜单中。

1. 在投放模板控制台中，找到默认的电子邮件模板 **通过电子邮件发送（邮件）** 并将鼠标悬停在表示该卡的卡（或线）上以显示选项。 单击 **重复元素**.

   ![重复元素](assets/acs-copy-template.png)

1. 在 **确认** 对话框，单击 **确认** 以复制模板。

   ![确认对话框](assets/acs-confirmation.png)

1. 此时将打开模板编辑器，其中包含您的 **通过电子邮件发送（邮件）** 模板。 单击 **编辑属性** 图标。

   ![模板编辑器](assets/acs-template-editor.png)

1. 在属性窗口中，更改 **标签** 用于描述新AEM模板的字段。
...
AEMAdobe Campaign使用内置集成和 `aemserver` 您在Adobe Campaign中配置的用户。 按照以下步骤配置此集成。
...
1. 会打开新窗口和对话框会，用以编辑配置。 提供必要的信息。
   ...
   * **用户名**  — 这是 [此 `aemserver` 您在上一步中配置的Adobe Campaign中的用户。](#aemserver-user)默认情况下，这是 `aemserver`。
   * **密码**  — 这是的密码 [此 `aemserver` 您在上一步中向Adobe客户关怀部门请求的Adobe Campaign中的用户。](#aemserver-user)
...
当您 [创建了特定于AEM的电子邮件投放模板，](#aem-email-delivery-template) 您指定模板应使用 `aemInstance` 用于与AEM通信的外部帐户。 为了在这两个解决方案之间启用双向通信，您需要在Adobe Campaign中配置此帐户。

1. 以管理员身份登录Adobe Campaign。

1. 点按或单击菜单栏左上角的Adobe Campaign徽标以打开全局导航，然后选择 **管理** > **应用程序设置** > **外部帐户** 从导航菜单中。
