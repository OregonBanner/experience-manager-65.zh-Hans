...
description: 了解如何将AEM与Adobe Campaign Standard集成。
...
通过将AEM与Adobe Campaign集成，您可以直接在AEM中管理电子邮件投放、内容和表单。 需要在Adobe Campaign Standard和AEM中执行配置步骤，才能在解决方案之间实现双向通信。

此集成允许单独使用AEM和Adobe Campaign Standard。 营销人员可以在Adobe Campaign中创建营销活动和使用定位，而内容创建者可以并行处理AEM中的内容设计。 通过使用该集成，Adobe Campaign可以定位和交付在AEM中创建的营销活动的内容和设计。

## 集成步骤 {#integration-steps}

配置AEM和Adobe Campaign Standard之间的集成需要在这两个解决方案中执行多个步骤。
...
* 管理员对Adobe Campaign Standard的访问权限
...
Adobe Campaign Standard默认附带 `aemserver` AEM用来连接到Adobe Campaign的用户。 您必须为此用户分配适当的安全组并设置其密码。

1. 以管理员身份登录Adobe Campaign。

1. 点按或单击菜单栏左上角的Adobe Campaign徽标以打开全局导航，然后选择 **管理** > **用户和安全** > **用户** 中。

1. 点按或单击 `aemserver` 用户。

1. 确保 `aemserver` 用户至少被分配给具有角色的安全组 `deliveryPrepare` 分配给它。 默认情况下，组 `Standard Users` 有这个角色。

   ![Adobe Campaign中的aemserver用户](assets/acs-aemserver-user.png)

1. 点按或单击 **保存** 以保存更改。

您的 `aemserver` 用户现在拥有必要的权限，以便AEM能够使用它与Adobe Campaign通信。

但是，在AEM使用 `aemserver` 用户，必须设置其密码。 这不能通过Adobe Campaign。 必须由Adobe支持工程师执行。 [请向Adobe客户关怀部门提交票证](https://experienceleague.adobe.com/?support-tab=home#support) 请求重置 `aemserver` 密码。 获得Adobe客户关怀部门的密码后，请将其保留在安全位置。

## 在Campaign中验证AEMResourceTypeFilter {#resource-type-filter}

的 `AEMResourceTypeFilter` 是Adobe Campaign中的一个选项，用于筛选可在Adobe Campaign中使用的AEM资源。 由于AEM包含大量内容，因此此选项充当过滤器，允许Adobe Campaign仅检索专门设计用于Adobe Campaign的类型的AEM内容。

此选项已预配置。 但是，如果您已自定义AEM的Campaign组件，则可能需要更新它。 验证 `AEMResourceTypeFilter` 选项，请执行以下步骤。

1. 以管理员身份登录Adobe Campaign。

1. 点按或单击菜单栏左上角的Adobe Campaign徽标以打开全局导航，然后选择 **管理** > **应用程序设置** > **选项** 中。

1. 点按或单击 `AEMResourceTypeFilter` （在选项控制台中）。

1. 确认 `AEMResourceTypeFilter`. 路径以逗号分隔，默认包含：
...
您的 `AEMResourceTypeFilter` 现在已配置为从AEM检索正确的内容。

## 在Campaign中创建特定于AEM的电子邮件投放模板 {#aem-email-delivery-template}

默认情况下，Adobe Campaign的电子邮件模板中未启用AEM。 您必须配置新的电子邮件投放模板，以用于使用AEM内容创建电子邮件。 要创建特定于AEM的电子邮件投放模板，请执行以下步骤。

1. 以管理员身份登录Adobe Campaign。

1. 点按或单击菜单栏左上角的Adobe Campaign徽标以打开全局导航，然后选择 **资源** > **模板** > **投放模板** 中。

1. 在投放模板控制台中，找到默认的电子邮件模板 **通过电子邮件（邮件）发送** 并将鼠标悬停在代表该卡片（或折线）上以显示选项。 单击 **复制元素**.

   ![复制元素](assets/acs-copy-template.png)

1. 在 **确认** 对话框，单击 **确认** 复制模板。

   ![确认对话框](assets/acs-confirmation.png)

1. 此时会打开模板编辑器，其中包含的 **通过电子邮件（邮件）发送** 模板。 单击 **编辑属性** 图标。

   ![模板编辑器](assets/acs-template-editor.png)

1. 在属性窗口中，更改 **标签** 字段来描述新的AEM模板。

1. 单击 **内容** 标题，然后选择 **Adobe Experience Manager** 在 **内容源** 下拉菜单。
...
1. 在模板编辑器中，单击 **保存** 以保存修改后的电子邮件模板副本，以便与AEM一起使用。

现在，您有一个可以使用AEM内容的电子邮件模板。

## 在AEM中配置Campaign集成 {#campaign-integration}

AEM使用内置集成和 `aemserver` 用户。 请按照以下步骤配置此集成。
...
1. 会打开新窗口和对话框会，用以编辑配置。 提供必要的信息。
   ...
   * **用户名**  — 这是 [the `aemserver` 您在上一步中配置的Adobe Campaign中的用户。](#aemserver-user)默认情况下，这是 `aemserver`。
   * **密码**  — 这是的密码 [the `aemserver` Adobe Campaign中的用户。](#aemserver-user)
...
[外部化器是 AEM 中的一个 OSGi 服务，它可将资源路径转换为外部和绝对 URL，这是 AEM 提供 Campaign 可以使用的内容所必需的。](/help/sites-developing/externalizer.md)您必须将其配置为使Campaign集成正常工作。
...
## 在AEM中配置campaign-remote用户 {#campaign-remote-user}

正如您需要Adobe Campaign中可供AEM用于与Adobe Campaign通信的用户，Adobe Campaign也需要AEM中的用户才能与AEM通信。 默认情况下，Campaign集成会创建 `campaign-remote` 用户。 请按照以下步骤配置此用户。
...
当您 [创建了特定于AEM的电子邮件投放模板，](#aem-email-delivery-template) 您指定模板应使用 `aemInstance` 与AEM通信的外部帐户。 要在两个解决方案之间实现双向通信，您需要在Adobe Campaign中配置此帐户。

1. 以管理员身份登录Adobe Campaign。

1. 点按或单击菜单栏左上角的Adobe Campaign徽标以打开全局导航，然后选择 **管理** > **应用程序设置** > **外部帐户** 中。

1. 点按或单击 **Adobe Experience Manager实例(aemInstance)** 用户。

1. 确保用户已 **Adobe Experience Manager** 作为 **类型**.

1. 在 **连接** 定义以下字段：

   1. 服务器：这是您的AEM创作服务器的URL。 不应以斜杠结尾。
   1. 帐户：这是 `campaign-remote` 用户 [之前在AEM中配置。](#campaign-remote-user)
   1. 密码：这是 `campaign-remote`用户 [之前在AEM中配置。](#campaign-remote-user)
