...
根页面存储在创建应用程序时指定的应用程序的“目标路径”属性下方（默认路径为/content/phonegap/apps）。 页面名称是应用程序的Name属性。 例如，站点的根页面的默认URL命名为 `myphonegapapp` 是 `http://localhost:4502/content/phonegap/apps/myphonegapapp.html`.
...
* 页面内容是应用程序语言变体的垃圾页面。
* 页面属性控制应用程序的多个设计方面，例如用于请求内容更新的URL，以及有关连接到Cloud Build和Adobe Analytics Services集成的信息。
...
移动应用程序基于Blueprint，可定义页面结构和属性。 您可以配置以下应用程序属性：

* **标题：** 应用程序标题。
* **目标路径：** 存储库中存储应用程序的位置。 保留默认值以根据应用程序名称创建路径。

* **名称：** 默认值为删除了空格字符的Title属性的值。 该名称在CQ中用于引用应用程序，例如表示该应用程序的存储库节点。
* **描述：** 应用程序的描述。
...
1. （可选）如果使用SiteCatalyst跟踪应用程序活动，请在Analytics选项卡上选择要使用的配置。
...
要使更新的内容可用于应用程序，需要更新内容同步缓存。 第一次更新缓存时，将添加所有已发布的内容。 后续更新仅添加自上次更新以来发生更改的已发布内容。

内容同步还跟踪更新的时间。 利用此信息，内容同步可以确定要发送到移动应用程序的缓存更新。
...
此功能随Apps 6.1 Feature Pack 2提供，为利用现有应用程序模板在AEM中创建新应用程序提供了一种简单的方法。
...
最新的AEM Apps 6.1示例包中包含带有应用程序模板的Geometrixx应用程序的更新版本。 或者，您可以安装也提供模板的StarterKit。
...
1. 提供应用程序ID和标题，但您可能还希望包含名称和描述。
   ...
   1. 请记住，在“管理应用程序”图块中创建应用程序后，您可以编辑所有这些字段。 应用程序ID除外，设置应用程序ID后，您将无法对其进行更改。

![chlimage_1-150](assets/chlimage_1-150.png)

1. 单击“创建”按钮，将显示2个选项：完成（返回到“应用程序”目录视图）或“管理应用程序”（打开应用程序仪表板）。
1. 创建应用程序后，您应该会在应用程序目录中看到新应用程序列出：

![chlimage_1-3](assets/chlimage_1-3.jpeg)

1. 单击应用程序以将其打开，表示您已基于现有应用程序的模板成功创建了新应用程序。

>[!NOTE]
>
>如果您从AEM卸载Geometrixx Outdoors引用应用程序包，并根据其模板创建应用程序，则该应用程序将无法再正常运行。 可以删除Geometrixx Outdoors应用程序，但是，如果其他移动设备应用程序使用模板，则必须保留该应用程序模板。

## 探索示例Geometrixx Outdoors应用程序 {#exploring-the-sample-geometrixx-outdoors-app}

Geometrixx Outdoors应用程序是一个PhoneGap示例应用程序，用于演示默认PhoneGap应用程序Blueprint和示例移动组件的功能。

要打开应用程序，请在边栏中单击移动设备应用程序，然后选择Geometrixx Outdoors应用程序。

### “常用”页面功能 — Geometrixx移动应用程序 {#common-page-features-geometrixx-mobile-app}
...
* 菜单列表组件，提供指向“齿轮”、“评论”、“新闻”和“关于我们”子页面的链接。
* 可显示子页面的轻扫轮盘组件。
...
每个类别页面使用与“齿轮”页面相同的内容结构。 通过轮盘可以访问作为产品子类别的子页面。 子类别页面包含产品列表，提供指向产品页面的链接。
...
轻扫轮盘组件是一个用于展示和导航站点页面的工具。 该组件包含一个轮播，轮播可在页面链接列表上方的页面上循环浏览图像。 编辑组件以指定要显示的页面和轮播的行为。
...
* 播放速度：显示下一个图像之前每个图像显示的时间（以毫秒为单位）。
...
* 生成列表使用：用于指定要包含在轮盘中的页面的方法。 请参阅生成页面列表。
* 排序依据：选择用于排序页面列表的页面属性。 例如，选择jcr：title可按标题的字母顺序对页面进行排序。
* 限制：要包含的最大页数。 此属性适用于构建页面列表的基于搜索的方法。

#### 构建页面列表 {#building-the-page-list}

滑动轮盘组件为“生成列表使用”属性提供以下值。 “编辑”对话框会根据您选择的值而发生更改：

**子页面**

该组件列出特定页面的所有子页面。 选择此值后，请在“子页面”选项卡中选择该页面，或者不指定任何值来列出当前页面的子页面。
...
使用关键词搜索的结果填充列表。 搜索在您指定的页面的子页面中执行：

1. 要指定搜索的根页面，请使用“起始位置”属性选择页面路径。 在当前页面下方指定无搜索路径。
1. 在“搜索查询”属性中输入搜索关键词。
...
“商店位置”组件为用户提供了用于查找业务网点的工具：

* 搜索
* 距离设备GPS坐标近或远的位置列表。

该组件要求存储库包含每个存储的位置信息。 示例位置安装在/etc/commerce/locations/adobe节点上。 ![chlimage_1-152](assets/chlimage_1-152.png)
